...
def block_expand(arr,shape_i,shape_o=None,**kwargs):
	...
	# Extend data
	if shape_o is None: shape_o = round_up_ratio(shape_tot,shape_i)
...
def block_neighbors(shape_i,top=True):
	...
	"""
	shape_i = np.array(shape_i); vdim=len(shape_i); size_i = np.prod(shape_i)
	aX = [np.arange(s+1) if top else np.arange(s-1,2*s) for s in shape_i]
	x_e = np.array(np.meshgrid(*aX)).astype(int)
	x_e = np.moveaxis(x_e.reshape((vdim,-1)),0,-1)
	x_e = np.array([x for x in x_e if np.any(x==(shape_i if top else shape_i-1))])
	ind = np.arange(2**vdim*size_i).reshape((2,)*vdim+tuple(shape_i))
	ind = block_squeeze(ind,tuple(2*shape_i))
	ind_x = np.array([ind[tuple(x)] for x in x_e])
	x_e = x_e[np.argsort(ind_x)]
	#print(np.array([ind[tuple(x)] for x in x_e]))
	if not top: x_e-=shape_i
	return x_e
