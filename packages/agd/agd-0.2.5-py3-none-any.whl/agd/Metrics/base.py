...
# ---- Proximal methods ----
	def make_proj_dual(self,**kwargs):
		...
		"""
		raise NotImplementedError("Sorry, make_proj_dual is not implemented for this metric")
	...
	def make_proj(self,**kwargs):
		...
		"""
		return self.dual().make_proj_dual(**kwargs)
	...
	def make_prox(self,**kwargs):
		...
		- **kwargs : passed to self.make_proj_dual
		"""
		# Implementation is based on the Moreau formula x = prox_{τ f}(x) + τ prox_{f^*/τ} (x/τ)
		# noting that f^* is the characteristic function of the dual unit ball, hence 
		# prox_{f^*/τ} is the orthogonal projection onto the dual unit ball. 
		proj = self.make_proj_dual(**kwargs)
		return lambda x,τ=1.: x - τ*proj(x/τ)
