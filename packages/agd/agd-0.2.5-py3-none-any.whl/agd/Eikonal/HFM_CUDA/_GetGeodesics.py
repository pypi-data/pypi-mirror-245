...
def GetGeodesics(self):
	...
	SetCst('geodesicStep',geodesic.policy.step,self.float_t) # ODE timestep
	...
	typical_len = self.GetValue('geodesic_typical_length',default=min(max_len,typical_len),
	...
	if self.geodesic_mixed_state: 
		eye = cp.eye(self.nstates,dtype=self.float_t)
		tips = cp.array([np.concatenate((tip[:-1],eye[int(np.round(tip[-1]))])) for tip in tips])
	...
	if self.geodesic_mixed_state: argnames.append(('flow_state','flow'))
	...
	while len(corresp)>0:
		...
		geoIt+=1
		nGeo = len(corresp)
		x_s = cp.full( (nGeo,typical_len,tips.shape[1]), np.nan, self.float_t)
	...
	if self.geodesic_mixed_state:
		pdim = self.ndim-1 # Physical dimension (no state)
		self.hfmOut['geodesic_states'] = [geo[:,pdim:].T for geo in geodesics_cat]
		geodesics_cat = [np.concatenate((geo[:,:pdim],
			np.argmax(geo[:,pdim:],axis=1)[:,np.newaxis]),axis=1) for geo in geodesics_cat]
