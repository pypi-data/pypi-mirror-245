...
class GitLabAPI(RepoAPI):
    ...
    def create_commit_and_push(self, name: str, branch: str, nameZip: str = "" , is_zip: bool = False, type: str = ""):
        try:
            if is_zip:
                ...
                if type == 'BACKEND':
                    ...
                    id_from_repo.append(project.id)
                changes = []
                for root, dirs, files in os.walk(f"/tmp/{type}"):
                    for file in files:
                        file_path = os.path.join(root, file)
                        relative_path = os.path.relpath(file_path, f"/tmp/{type}")
