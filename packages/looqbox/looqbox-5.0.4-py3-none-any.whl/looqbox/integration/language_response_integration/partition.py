...
def extract_partition_values(grouping, only_value, as_dict, default=None):
    ...
    if only_value:
        return grouping.values if grouping is not None else default
    if as_dict:
        return [as_dict_ignore_nones(grouping.to_token())] if grouping else None
    return grouping


@multimethod
def get_partition(partition: str, par_json, partition_default=None, only_value=False, as_dict=True) -> GroupBy:
    """
