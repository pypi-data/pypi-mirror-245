...
def format_single_date(header, date_str, language):
    ...
    return header + language_vars["var_day"] + formatted_date + date_range_name([date_as_object, date_as_object], language)
...
def format_date_range(header, start_date, end_date, language):
    ...
    return header + language_vars["var_from"] + formatted_start_date + language_vars["var_to"] + formatted_end_date
...
def title_with_date(header: str = None, date_int: list = None, language: str = GlobalCalling.looq.language):
    ...
    # TODO i18n
    if not date_int[0]:
        return header + " até " + format(date_int[1], determine_format_type(date_int[1]).lower(), language)
    if not date_int[1]:
        return header + " a partir de " + format(date_int[0], determine_format_type(date_int[0]).lower(), language)

    if date_int[0] == date_int[1]:
        return format_single_date(header, date_int[0], language)
    else:
        return format_date_range(header, date_int[0], date_int[1], language)
...
def title_with_date(header: str, date_int: TemporalRelation, language: str = GlobalCalling.looq.language):
    ...
    if len(distinct_date) == 1:
        # TODO i18n
        return f"{header} em {distinct_date[0]}"

    elif len(distinct_date) > 1:
        return format_date_range(header, distinct_date[0], distinct_date[-1], language)
