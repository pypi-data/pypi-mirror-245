...
class TypeInspector:
    ...
    def _recorded_type(self, idx: int) -> typing.Tuple[Any, int]:
        ...
        if tag == "literal":
            ...
            return (
                Literal.__getitem__(Literal, *param),
                idx,
        ...
        if tag == "tuple":
            ...
            if not item_ts:
                item_ts = [tuple()]
            t = (
                pending_type.__class_getitem__(tuple(item_ts))
                if pending_type is not None
                else typing.Tuple.__getitem__(tuple(item_ts))
            )
            return t, idx
    ...
    def _append_constructor_args(self, args: TypeConstructorArgs) -> None:
        ...
        if sys.version_info[1] >= 9:
            self._recorded_constructors.append(
                typing.cast(
                    TypeConstructorArgs, ("type", (pending_param, args_tag, args_param))
                )
            )
        else:
            self._recorded_constructors.append(
...
def _type_error(
    val: Any, t: Any, *causes: TypeError, is_union: bool = False
) -> TypeError:
    """
    ...
    validation_failure = ValidationFailure(
        val, t, *_causes, is_union=is_union, type_aliases=_validation_aliases
    )
...
def _validate_collection(val: Any, t: Any) -> None:
    ...
    assert isinstance(t.__args__, tuple) and len(t.__args__) == 1, _wrong_args_num_msg(
        t, 1
    )
    item_t = t.__args__[0]
    if isinstance(val, TypeInspector):
...
def _validate_mapping(val: Any, t: Any) -> None:
    ...
    assert hasattr(t, "__args__"), _missing_args_msg(t)
    assert isinstance(t.__args__, tuple) and len(t.__args__) == 2, _wrong_args_num_msg(
        t, 2
    )
