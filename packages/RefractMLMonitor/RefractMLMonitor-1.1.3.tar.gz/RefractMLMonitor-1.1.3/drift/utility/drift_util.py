...
def get_classification_metrics_info(reference_feature_drift_data,current_feature_drift_data,alert_info):
    ...
    message += f'Accuracy has improved by {round(accuracy_change_perc*100,2)}% from {re_accuracy_value} to {accuracy_value}, ' if accuracy_change > 0 else "No change found in Accuracy, " if accuracy_change==0 else f'Accuracy has decreased by {round(accuracy_change_perc*100,2)}% from {re_accuracy_value} to {accuracy_value}, '


    precision_change = precision_value - re_precision_value
    precision_change_perc = abs(precision_change)/re_precision_value
    message += f'Precision has improved by {round(precision_change_perc*100,2)}% from {re_precision_value} to {precision_value}, ' if precision_change > 0 else "No change found in Precision, " if precision_change==0 else f'Precision has decreased by {round(precision_change_perc*100,2)}% from {re_precision_value} to {precision_value}, '

    recall_change = recall_value - re_recall_value
    recall_change_perc = abs(recall_change)/re_recall_value
    message += f'Recall has improved by {round(recall_change_perc*100,2)}% from {re_recall_value} to {recall_value}, ' if recall_change > 0 else "No change found in Recall, " if recall_change==0 else f'Recall has decreased by {round(recall_change_perc*100,2)}% from {re_recall_value} to {recall_value}, '

    f1_change = f1_value -re_f1_value
    f1_change_perc = abs(f1_change)/re_f1_value
    message += f'F1 Score has improved by {round(f1_change_perc*100,2)}% from {re_f1_value} to {f1_value}.' if f1_change > 0 else "No change found in F1 Score." if f1_change==0 else f'F1 Score has decreased by {round(f1_change_perc*100,2)}% from {re_f1_value} to {f1_value}.'
...
def get_regression_metrics_info(reference_feature_drift_data,current_feature_drift_data,alert_info):
    ...
    message += f'Mean Error(ME) has increased by {round(me_change_perc*100,3)}% from {re_me} to {me}.' if me_change > 0 else "No change found in Mean Error(ME), " if me_change==0 else f'Mean Error(ME) has decreased by {round(me_change_perc*100,3)}% from {re_me} to {me}, '

    mae_change =  mae - re_mae
    mae_change_perc = abs(mae_change)/re_mae
    message += f'Mean Absolute Error (MAE) has increased by {round(mae_change_perc*100,3)}% from {re_mae} to {mae}.' if mae_change > 0 else "No change found in Mean Absolute Error(MAE), " if mae_change==0 else f'Mean Absolute Error(MAE) has decreased by {round(mae_change_perc*100,3)}% from {re_mae} to {mae}, '

    mape_change =  mape - re_mape
    mape_change_perc = abs(mape_change)/re_mape
    message += f'Mean Absolute Percentage Error(MAPE) has increased by {round(mape_change_perc*100,3)}% from {re_mape} to {mape}.' if mape_change > 0 else "No change found in Mean Absolute Percentage Error(MAPE), " if mape_change==0 else f'Mean Absolute Percentage Error(MAPE) has decreased by {round(mape_change_perc*100,3)}% from {re_mape} to {mape}, '
