...
from .oauth import OAuth2Manager, OAuth2Storage
from .readable import GoogleReadableFile
from .writable import create_writable
from .network import Network, create_network
from .hasher import create_hasher
from .extra import fetch_child_by_name, fetch_node_by_id


@asynccontextmanager
async def create_service(*, client_secret: str, oauth_token: str):
    storage = OAuth2Storage(client_secret=Path(client_secret), oauth_token=Path(oauth_token))
...
class GoogleDriveFileService(FileService):
    ...
    async def create_directory(
        ...
        *,
        ...
        from .api.files import create_folder
    ...
    async def purge_trash(self) -> None:
        from .api.files import empty_trash
    ...
    async def move(
        ...
        *,
        ...
        node = await fetch_node_by_id(self._network, node.id)
        return node
