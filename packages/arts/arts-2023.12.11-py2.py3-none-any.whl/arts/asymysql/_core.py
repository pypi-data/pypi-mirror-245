...
class ToolPool():
    ...
    async def get(self, db_name=''):
        ...
        if db_name and obj['db_name'] != db_name:
            await obj['cursor'].execute(f'use {db_name}')
            obj['db_name'] = db_name
        return obj
...
class acORM():
    async def asy_init(self, mkconn):
        ...
        self.conn_pool = ToolPool()
        await self.conn_pool.asy_init(mktool=_, minlen=1, maxlen=1)
        # 当增删改查报错时, conn不再放回连接池, 以避免含有残留数据
    
    async def get_dbs(self):
        obj = await self.conn_pool.get()
        await obj['cursor'].execute("show databases")  # 非本地操作, 需要连接到数据库
        r = await obj['cursor'].fetchall() or []
        await self.conn_pool.put(obj)
        return r
    
    async def get_db_names(self): return [x['Database'] for x in await self.get_dbs()]

    async def len(self): return len(await self.get_dbs())
...
class db_orm():
    ...
    async def get_sheet_names(self):
        obj = await self.conn_pool.get(self.db_name)
        ...
        r = [x['TableName'] for x in await obj['cursor'].fetchall()]
        await self.conn_pool.put(obj)
        return r
...
class sheet_orm():
        ...
        if isinstance(key, slice):
            ...
            if S in [None,1]:
                if (L in [None,1] and R in [None,-1]) or (L == -1 and R == 1):
                    rdata, cursor = await self.execute(f"delete from {self.sheet_name}{self.where}")
                    return cursor
        # [1]且无排序
        if key == 1 and not self._ParseOrder():
            rdata, cursor = await self.execute(f"delete from {self.sheet_name}{self.where} limit 1")
            return cursor
        # 其它情况
        pk = await self.get_pk()
        try:
            ...
            rdata, cursor = await self.execute(f"delete from {self.sheet_name} where 1 = 2 limit 1")
        else:
            if isinstance(pks, list):
                if pks:
                    pks = [x[pk] for x in pks]
                    rdata, cursor = await self.execute(f"delete from {self.sheet_name}{mc[pk].isin(*pks)}")
                else:
                    rdata, cursor = await self.execute(f"delete from {self.sheet_name} where 1 = 2")
            else:
                rdata, cursor = await self.execute(f"delete from {self.sheet_name}{mc[pk] == pks[pk]} limit 1")
        return cursor
    ...
    async def _updateBase(self, key, data:dict):
        ...
        if isinstance(key, slice):
            ...
            if S in [None,1]:
                if (L in [None,1] and R in [None,-1]) or (L == -1 and R == 1):
                    rdata, cursor = await self.execute(f"update {self.sheet_name} set {data}{self.where}")
                    return cursor
        # [1]且无排序
        if key == 1 and not self._ParseOrder():
            rdata, cursor = await self.execute(f"update {self.sheet_name} set {data}{self.where} limit 1")
            return cursor
        # 其它情况
        pk = await self.get_pk()
        try:
            ...
            rdata, cursor = await self.execute(f"update {self.sheet_name} set {data} where 1 = 2 limit 1")
        else:
            if isinstance(pks, list):
                if pks:
                    pks = [x[pk] for x in pks]
                    rdata, cursor = await self.execute(f"update {self.sheet_name} set {data}{mc[pk].isin(*pks)}")
                else:
                    rdata, cursor = await self.execute(f"update {self.sheet_name} set {data} where 1 = 2")
            else:
                rdata, cursor = await self.execute(f"update {self.sheet_name} set {data}{mc[pk] == pks[pk]} limit 1")
        return cursor
    
    async def execute(self, sql:str, data=None, commit=True):
        mobj:dict = await self.conn_pool.get(self.db_name)
        conn = mobj['conn']
    ...
    async def get_columns(self, comment=True, type=True):
    ...
    async def len(self):
        rdata, cursor = await self.execute(f"select count(1) as tatal from {self.sheet_name}{self.where}", commit=False)
