...
class trace_plot:
    def __init__(self, time: np.ndarray, intenstity: np.ndarray, threshold: float):
        self.time = time
        ...
        for i, peak in enumerate(peaks):
            if len(probabilities) > 0:
                self.fig.add_annotation(
                    ...
                    text=f"frame: {peak}, height: {self.intenstity[peak]}, probability: {round(probabilities[i]*100,2)}%",
                    showarrow=True,
                )
            else:
                self.fig.add_annotation(
