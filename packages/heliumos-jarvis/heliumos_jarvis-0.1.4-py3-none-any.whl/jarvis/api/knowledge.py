...
async def _summary(summary_id: str, docs: List[Document]) -> Tuple:
    ...
    result = await chain.arun(input_documents=docs, token_max=2048)
    # result = ""
...
async def delete_knowledge_source_by_id(source_id: str):
    ...
    if record is not None:
        ...
        collection = Collection(name=record.knowledge_id)
        res_pks = collection.query(f"source_id == \"{record.id}\" ")
