...
def read_energy_and_save_to_csv(counter, output_model_dir, lig_name, csv_path, input_file):    
    ...
    if energy is not None:
        with open(csv_path, 'a') as csv_file:
            csv_file.write(f"{counter['value']}, {lig_name.replace('output_', '').replace('_minimized', '').upper()}, {smiles}, {energy:.3f}\n")
    else:
...
def sort_and_rewrite_csv(csv_path):
    try:
        ...
        sorted_data = sorted(data, key=lambda x: float(x[3]))  # Menggunakan lambda untuk mengakses kolom kedua (indeks 3) sebagai kunci pengurutan
