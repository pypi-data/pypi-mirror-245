...
def read_energy_and_save_to_csv(counter, output_file, csv_path, lig_name, input_file):
    ...
    if energy is not None:
        with open(csv_path, 'a') as csv_file:
            csv_file.write(f"{counter['value']}, {lig_name.replace('_minimized', '').upper()}, {smiles}, {energy:.3f}\n")
...
def process_docking_ligand_native(prepare_gpf, prepare_dpf, counter, spacing, npts, max_workers, num_iterations, csv_path, prepare_receptor, receptor_name, prepare_ligand, lig_name, directory, ensamble, output_model_dir, parameter):
    ...
    # Save to csv   
    read_energy_and_save_to_csv(counter, output_file, csv_path, lig_name, input_file)
...
def process_docking_ligand(directory, prepare_gpf, prepare_dpf, pdbqt_file, counter, prepare_receptor, receptor_name, lig_name, parameter, ensamble, output_model_dir, csv_path):
    try:
        ...
        read_energy_and_save_to_csv(counter, output_file, csv_path, filename, input_file)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        for pdbqt_file in ligand_files:
            futures.append(executor.submit(
                process_docking_ligand, directory, prepare_gpf, prepare_dpf, pdbqt_file, counter, prepare_receptor, receptor_name, lig_name, parameter, ensamble, output_model_dir, csv_path))
