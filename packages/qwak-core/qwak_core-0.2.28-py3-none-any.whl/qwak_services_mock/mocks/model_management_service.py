...
class ModelsManagementServiceMock(ModelsManagementServiceServicer):
    ...
    def ListModels(self, request, context):
        models = [model for _, model in self.models.items() if model.project_id == request.project_id]
        return ListModelsResponse(models=models)

    def ListModelsMetadata(self, request, context):
        models = [ModelMetadata(model=model) for _, model in self.models.items() if model.project_id == request.project_id]
