...
class ModelCardReader:
    ...
    def read_from_server(cls, model_card_id, override_dic=None):
        ...
        key = model_card_id+datestr
        key = key.encode()

        params = {}
        params['bot_id'] = model_card_id
        params['auth'] = hmac.new(Config.SERVER_KEY.encode(), key, hashlib.sha256 ).hexdigest()
        r = requests.post('https://chat.ailike.me/character_json.php', data=params)
        dic = r.json()
        logging.info("서버에서 캐릭터 정보 수신")

        if type(dic) is not dict:
            raise Exception("잘못된 서버 응답")

        if dic['result'] =="error":
            raise Exception("캐릭터 조회 에러")

        character_dic = json.loads(base64.b64decode(dic['response']))
