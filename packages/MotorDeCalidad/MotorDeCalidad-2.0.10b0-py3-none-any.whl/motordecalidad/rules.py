...
def validateCatalog(object:DataFrame,
    ...
    print(listValuesEncoded)
    dataRequirement = f"El atributo {entity}.{columnName}, debe tomar solo los valores {fieldsString}."
    print(object[columnName])
    object[columnName].show()
    evaluatedColumn = utf8_to_latin1_udf(object[columnName].cast("string"))
    evaluatedColumn.show()
    object = object.withColumn(columnName,evaluatedColumn)
    object.show()
    errorDf = object.filter(~col(columnName).isin(listValuesEncoded))
    errorDf.show()
    errorCount = errorDf.count()
    ratio = (One - errorCount/registerAmount) * OneHundred
