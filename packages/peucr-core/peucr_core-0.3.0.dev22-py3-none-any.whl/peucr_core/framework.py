...
class TestFramework:
    ...
    def exec_test_suite(self, specs):
        print(self.config.get("parallel"))
        print(isinstance(self.config.get("parallel"), int))
        print(self.config["parallel"] is not True)
        if isinstance(self.config.get("parallel"), int) and self.config["parallel"] is not True:
            workers = self.config["parallel"]
            print("here1")
        else:
            workers = 5 if self.config.get("parallel") is True else 1
            print("here2")
