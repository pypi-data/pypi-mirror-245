...
class diaBan(object):
    def __init__(self, thangSinhAmLich, gioSinhAmLich, thienCanNam):
        ...
        if not listOfCanDiaBan:
            listOfCanDiaBan = [].append(thienCan[0]).append(listOfCanDiaBan)
            self.thapNhiCung = [cungDiaBan(i, safeListGetElement(listOfCanDiaBan, i)) for i in range(13)]
        else:
            self.thapNhiCung = [cungDiaBan(i) for i in range(13)]
        self.nhapCungChu()
        self.nhapCungThan()
        
    def safeListGetElement(list, index):
    ...
    def canDiaBan(self, thienCanNam):
        ...
        if thienCanNam["chuCaiDau"] == "G" and thienCanNam["chuCaiDau"] == "K":
            firstThienCan = tmpThienCan[0:1] or []
            secondThienCan = tmpThienCan[2:] or []
            return secondThienCan.append(firstThienCan) or []
        elif thienCanNam["chuCaiDau"] == "A" and thienCanNam["chuCaiDau"] == "C":
            firstThienCan = tmpThienCan[0:3] or []
            secondThienCan = tmpThienCan[4:] or []
            return secondThienCan.append(firstThienCan) or []
        elif thienCanNam["chuCaiDau"] == "B" and thienCanNam["chuCaiDau"] == "T":
            firstThienCan = tmpThienCan[0:5] or []
            secondThienCan = tmpThienCan[6:] or []
            return secondThienCan.append(firstThienCan) or []
        elif thienCanNam["chuCaiDau"] == "D" and thienCanNam["chuCaiDau"] == "N":
            firstThienCan = tmpThienCan[0:7] or []
            secondThienCan = tmpThienCan[8:] or []
            return secondThienCan.append(firstThienCan) or []
        elif thienCanNam["chuCaiDau"] == "M" and thienCanNam["chuCaiDau"] == "Q":
            return tmpThienCan or []
