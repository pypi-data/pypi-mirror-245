...
def set_myip_on_spark_conf(spark=None, host_network=True):
    if host_network:
        return spark
    ...
    return spark.config("spark.driver.host", MYIP).config(
        "spark.driver.bindAddress", MYIP
    )


def get_spark_beta(scale=0, queue=None, extra_jars=None, host_network=True):
    import os
    import uuid
    import tempfile
    from pyspark.sql import SparkSession
    ...
    if scale in [1, 2, 3, 4]:
        spark = (
            spark.config("spark.driver.memory", f"{scale * 8}g")
            .config("spark.executor.memory", f"{scale * 3}g")
            .config("spark.executor.instances", f"{scale * 8}")
            .config("spark.driver.maxResultSize", f"{scale * 4}g")
            .config("spark.yarn.queue", queue)
        ...
        spark = (
            spark.config("spark.driver.memory", "6g")
            .config("spark.executor.memory", "8g")
            .config("spark.shuffle.service.enabled", "true")
            .config("spark.dynamicAllocation.enabled", "true")
            .config("spark.dynamicAllocation.maxExecutors", "200")
    ...
    spark = set_myip_on_spark_conf(spark, host_network).getOrCreate()
    spark.conf.set("spark.sql.execution.arrow.pyspark.enabled", "true")
    return spark
