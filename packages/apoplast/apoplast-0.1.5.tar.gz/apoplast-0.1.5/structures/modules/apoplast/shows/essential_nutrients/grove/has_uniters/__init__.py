...
import apoplast.shows.essential_nutrients.grove.seek_uniter as seek_uniter
...
def check (grove, return_problem = False):
	...
	The uniter '{ uniter ["essential"] ["names"] }' is comprised of 
	'{ len (uniter ["natures"]) }' natures.
	
	It unites '{ entry ["essential"] ["names"] }' which has 
	'{ len (entry ["natures"]) }' natures.				
					...
					entry_name = entry ["essential"] ["names"]
