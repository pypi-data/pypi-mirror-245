...
async def get_user_nick_name(secUid: str, db: AsyncUserDB) -> str:
    ...
    user_data = await db.get_user_info(secUid)
    if not user_data:
        user_data = await handler_user_profile(secUid)
        await db.add_user_info(**user_data)
    return user_data.get("nickname", "")


async def get_or_add_user_data(kwargs: dict, secUid: str, db: AsyncUserDB) -> Path:
    """
    获取或创建用户数据同时创建用户目录
    ...
    # 从服务器获取当前用户最新数据
    current_user_data = await handler_user_profile(secUid)
...
async def select_playlist(playlists: dict) -> int:
    ...
    for i, mix_id in enumerate(playlists.get("mixId", [])):
        ...
        rich_console.print(f"[cyan]{i + 1}[/cyan]: {mix_name} ({video_count} videos)")
...
@mode_handler("one")
async def handler_one_video(kwargs):
    """
    用于获取指定视频的信息
    ...
    async with AsyncUserDB("tiktok_users.db") as udb:
        user_path = await get_or_add_user_data(
            kwargs, str(aweme_data.get("secUid")), udb
        )

    async with AsyncVideoDB("tiktok_videos.db") as vdb:
        await get_or_add_video_data(aweme_data, vdb)
...
async def fetch_one_video(itemId: str) -> dict:
    """
    用于获取指定视频的详细信息
...
@mode_handler("post")
async def handler_user_post(kwargs):
    """
    用于获取指定用户的视频信息
    ...
    page_counts = kwargs.get("page_counts", 35)
    max_counts = kwargs.get("max_counts")

    secUid = await SecUserIdFetcher.get_secuid(kwargs.get("url"))
...
async def fetch_user_post_videos(
    secUid: str, cursor: int, page_counts: int, max_counts: float
) -> AsyncGenerator:
    """
    用于获取指定用户发布的视频列表
...
@mode_handler("like")
async def handler_user_like(kwargs):
    """
    用于获取指定用户的点赞视频信息
    ...
    cursor = kwargs.get("cursor", 0)
    page_counts = kwargs.get("page_counts", 30)
    max_counts = kwargs.get("max_counts")
...
async def handler_user_collect(kwargs):
    """
    用于获取指定用户的收藏视频信息
...
@mode_handler("mix")
async def handler_user_mix(kwargs):
    """
    用于获取指定用户的合集视频信息
    ...
    playlist = await fetch_play_list(secUid, cursor, page_counts)
        ...
        for mixId in playlist.get("mixId", []):
            ...
            async for aweme_data_list in fetch_user_mix_videos(
                ...
                await downloader.create_download_tasks(
                    kwargs, aweme_data_list, user_path
                )
    else:
...
async def fetch_user_mix_videos(
    mixId: str, cursor: int, page_counts: int, max_counts: float
) -> AsyncGenerator:
    """
    用于获取指定用户合集的视频列表
