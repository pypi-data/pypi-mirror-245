...
from ogmios.datatypes import Utxo, Lovelace
...
class QueryProjectedRewards:
    ...
    def execute(
        ...
        :return: Rewards that can be expected assuming a pool is fully saturated. Such rewards are
            ...
            Keys of the map can be either Lovelace amounts or account credentials depending on the
    ...
    def send(
        ...
        """
        params = om.Params4(
            stake=[{"lovelace": int(amt.lovelace)} for amt in stake], scripts=scripts, keys=keys
        )
