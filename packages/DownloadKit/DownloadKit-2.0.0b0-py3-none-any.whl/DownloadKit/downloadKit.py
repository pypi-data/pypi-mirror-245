...
class DownloadKit(object):
    ...
    def add(self, file_url, goal_path=None, rename=None, suffix=None, file_exists=None, split=None, **kwargs):
        ...
        mission = Mission(self._missions_num, self, file_url, str(goal_path or self.goal_path),
                          rename, suffix, file_exists or self.file_exists,
                          self.split if split is None else split, kwargs)
    ...
    def _connect(self, url, session, _headers, method, **kwargs):
        """生成response对象
        :param url: 目标url
    ...
    def _download(self, mission_or_task, thread_id):
        ...
        if isinstance(mission_or_task, Task):
            ...
            r, inf = self._connect(file_url, task.mission.session, task.mission.headers, task.mission.method, **kwargs)
        ...
        r, inf = self._connect(file_url, mission.session, mission.headers, mission.method, **kwargs)
        ...
        # -------------------获取文件信息-------------------
        file_info = get_file_info(r, goal_path, rename, suffix, file_exists, self._lock)
