...
class Connection:
    ...
    def __init__(self, client):
        ...
        self._connection = httpx.AsyncClient(
            proxies=self._client._proxy,
            headers={'user-agent': self._client._user_agent,
                     'origin': 'https://web.rubika.ir',
                     'referer': 'https://web.rubika.ir/'},
            timeout=httpx.Timeout(timeout=self._client._timeout)
        )
    ...
    async def execute(self, request: dict):
        ...
        if not method_urls:
            raise exceptions.UrlNotFound(
                'It seems that the client could not get the list of Rubika api\'s. Please wait and try again.',
                request=request
            )
        ...
        if self._client._auth is None:
            ...
            self._client._logger.info('create auth secret', extra={'data': self._client._auth})

        if self._client._key is None:
            self._client._key = Crypto.passphrase(self._client._auth)
            self._client._logger.info('create key passphrase', extra={'data': self._client._key})
        ...
        if request['api_version'] == '6' and not tmp_session:
        ...
        while True:
            for url in method_urls:
                try:
                    result = await self._connection.post(url, json=request)
                    ...
                    self._client._logger.warning(
                        'request status ' + capitalize(status_det), extra={'data': request})
                    ...
                    print(f'Internet connection problem... ({method}, {try_count})')
                    await asyncio.sleep(try_count)
                    try_count += 1

        raise exceptions.InternalProblem('rubika server has an internal problem', request=request)
    ...
    async def receive_updates(self):
        ...
        for url in default_sockets:
            async with websockets.connect(url) as wss:
                ...
                self._client._logger.info('start receiving updates', extra={'data': url})

                async for message in wss:
                    try:
                        message = json.loads(message)
                        data_enc = message.get('data_enc')

                        if not data_enc:
                            continue

                        result = Crypto.decrypt(data_enc, key=self._client._key)
