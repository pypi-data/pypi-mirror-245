...
def _test_validate_docker(docker_repository, input_path, result_path, regenerate, verbose) -> int:
    with log_context():
        test_runner = FlitsTestRunner(input_path, result_path, regenerate, verbose)
...
def _main():
    ...
    if not test_standalone and not test_docker and not test_manual:  # and not test_manual:
        raise FriendlyError('No test option requested. Try "test_plugin -h" to show all options for running tests.')
    ...
    if test_docker:
        _ensure_docker()
        sys.exit(_test_validate_docker(test_docker, input_path, result_path, regenerate, verbose))
    if test_manual:
