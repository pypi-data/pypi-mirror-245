...
class Aquilify:
    ...
    async def _urlI8N(self, file_path: str) -> str:
        try:
    ...
    async def handle_request(
        ...
        try:
            ...
            if not self.routes:
                if self.debug:
                    _package_name = "aquilify"
                    _package_path = "aquilify/views/urlI8N.html"
                    package = pkg_resources.get_distribution(_package_name)
                    package_path = package.location

                    combined_path = os.path.join(package_path, *_package_path.split('/'))

                    full_file_path = os.path.join(sys.prefix, combined_path)
                    html_content = await self._urlI8N(full_file_path)

                    if html_content:
                        response = HTMLResponse(content=html_content, status=200)
                    else:
