...
class TemplateConfigSettings:
    ...
    def _extract_template_data(self, template: dict, csrf_setting) -> dict:
        ...
        return {
            ...
            'csrf': csrf,
            'options': {
                'autoscape': options.get('autoscape', False),
                'cache_size': options.get('cache_size', 0),
                'extensions': options.get('extensions', []),
                'enable_template_cache': options.get('enable_template_cache', False),
                'context_processors': context_processors,
                'filters': options.get('filters', {}),
                'globals': options.get('globals', {}) 
            }
        }
