...
class S3(AWS):
    ...
    async def generate_cloud_credentials(self, request: Request, oidc_client: str, arn: str) -> AWSCredentials | None:
...
async def generate_cloud_credentials(request: Request, oidc_client: str, arn: str) -> AWSCredentials | None:
    """
    :param request: the HTTP request (required).
    ...
    roles_url = await client.get_resource_url(request.app, Role)
    logger.debug('Checking if user %s has role %s...', sub, arn)
    assumed_role_object = None
    try:
        if await client.has(request.app, URL(roles_url) / 'byname' / encode_role(arn), headers={hdrs.AUTHORIZATION: auth, SUB: sub}):
            ...
            with closing(await loop.run_in_executor(None, boto3.client, 'sts')) as sts_client:
                assumed_role_object = await loop.run_in_executor(None, partial(sts_client.assume_role_with_web_identity,
                    WebIdentityToken=auth_token[1], RoleArn=arn,
