...
class scStateDynamics:
    ...
    def plotFlowCostDistr(self, run_i = -1, threshold = False, bins = None, color = "#598cbe", 
        ...
        flow_info = self.getAlign_results('flow_info', run_i = run_i)
        if isinstance(threshold, bool):
            if threshold:
                threshold = self.getAlign_results('error_flow_thres', run_i = run_i)
    ...
    def plotFlowSankey(self, run_i = -1, title = True, saveFig = True, saveName = 'Flow-Sankey.png'):
        init_flow_info = self.getAlign_results('flow_info', run_i = 0)
        final_flow_info = self.getAlign_results('flow_info', run_i = run_i)
    ...
    #                 ax.fill_between(xs, ys_dw, ys_up, alpha=0.7, color=self.pre_colors[si])
                    ...
                    ot_clq = self.getAlign_results('ot_clq', run_i = run_i)
    ...
    def getAlign_results(self, variable, run_i = -1):
    ...
    def addCellFlow(self):
        flow_info = self.getAlign_results('flow_info', run_i = -1)
        ot_clq = self.getAlign_results('ot_clq', run_i = -1)
    ...
    def plotGrowRate(self, run_i = -1, saveFig = True, saveName = 'GrowthRate-bar.png'):
        grow_rates = self.getAlign_results('grow_rates', run_i = run_i)
    ...
    def plotFlowHeat(self, run_i = -1, title = None, saveFig = True, saveName = 'Flow-heatmap.png'):
        ot_cls = self.getAlign_results('ot_cls', run_i = run_i)
    ...
    def plotFlowNormalizedHeat(self, run_i = -1, saveFig = True, saveName = 'Flow-NormalizedHeatmap.png'):
        ot_cls = self.getAlign_results('ot_cls', run_i = run_i)
...
def createScStateDynamicsObj(sc_obj_comb, key, pre_name, pos_name, run_label, pre_colors=None, pos_colors=None, 
    cls_prefixes = ['S', 'T'], savePath = "", saveFigFormat = "png"):
    '''
