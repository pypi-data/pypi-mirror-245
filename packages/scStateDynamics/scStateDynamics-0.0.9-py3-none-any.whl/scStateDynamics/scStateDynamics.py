...
def runScanpyQC(scobj, runLabel, savePath):
    cur_savePath = savePath + "/scanpy/"
    if not os.path.exists(cur_savePath):
        os.makedirs(cur_savePath)
    ...
    scobj.var['mt'] = scobj.var_names.str.startswith('MT-')
    ...
    sc.pl.violin(scobj, 'n_genes_by_counts', jitter=0.4, alpha = 0.3, multi_panel=False, ax=ax1)
    ax2 = fig.add_subplot(1,3,2)
    sc.pl.violin(scobj, 'total_counts', jitter=0.4, alpha = 0.3, multi_panel=False, ax=ax2)
    ax3 = fig.add_subplot(1,3,3)
    sc.pl.violin(scobj, 'pct_counts_mt', jitter=0.4, alpha = 0.3, multi_panel=False, ax=ax3)
    fig.tight_layout()
    fig.savefig(cur_savePath + runLabel + '_qc-violin.png', dpi = 300)
    
    return(scobj)


def runScanpyFilter(scobj, sc_name, nGene_thres = None, nCount_thres = None, mito_thres = None, filter_gene = True):
    if nGene_thres is not None:
        scobj = scobj[scobj.obs.n_genes_by_counts < nGene_thres, :]
    if nCount_thres is not None:
        scobj = scobj[scobj.obs.total_counts < nCount_thres, :]
    if mito_thres is not None:
        scobj = scobj[scobj.obs.pct_counts_mt < mito_thres, :]
    
    if filter_gene:
        scobj = scobj[:,~scobj.var_names.str.startswith('MT-')]
        scobj = scobj[:,~scobj.var_names.str.startswith('RPL')]
        scobj = scobj[:,~scobj.var_names.str.startswith('RPS')]
        scobj = scobj[:,~scobj.var_names.str.startswith('MRPL')]
        scobj = scobj[:,~scobj.var_names.str.startswith('MRPS')]
    return(scobj)


def preProcess(scobj):
    sc.pp.normalize_total(scobj, target_sum=1e4)
    sc.pp.log1p(scobj)
    sc.pp.highly_variable_genes(scobj, min_mean=0.0125, max_mean=3, min_disp=0.5)
    return(scobj)



def runScanpy(scobj, n_neighbors = 10, n_pcs = 40, reg_vars = ['total_counts', 'pct_counts_mt']):
    scobj.raw = scobj
    scobj = scobj[:, scobj.var.highly_variable]
    
    sc.pp.regress_out(scobj, reg_vars)
...
def getScObjects(scobj, item, values, n_neighbors = 10, n_pcs = 40):
    ...
    scobj = preProcess(scobj)
    scobj = runScanpy(scobj)
    
    for di in range(len(data_split)):
        data_split[di].var.highly_variable = [gene in comm_hvgs for gene in data_split[di].var.index]
        data_split[di] = runScanpy(data_split[di], n_neighbors, n_pcs)
        
        data_split[di].obsm['X_umap_comb'] = scobj[data_split[di].obs.index, :].obsm['X_umap']
        data_split[di].obsm['X_pca_comb'] = scobj[data_split[di].obs.index, :].obsm['X_pca']
...
def getHeatAnnoColor(clq_cls, colors = None):
    if not colors:
        colors = list(sns.color_palette().as_hex())
    color_dict = pd.Series(clq_cls['Cluster_id']).unique()
    color_dict.sort()
    color_dict = dict(zip(color_dict, colors))
    anno_colors = pd.Series(clq_cls['Cluster_id']).map(color_dict).to_numpy()
    return(anno_colors)
...
# def _assignFlowType(flow_info, method = 'Outlier', criterion = "AIC", up_thres = None):
#     if method == 'Outlier':
#         up_thres = getBoxOutlierThres(flow_info['dist'])
#         flow_info['type'] = "keep"
#         flow_info.loc[flow_info.dist > up_thres, 'type'] = "error"
#     elif method == 'GMM':
...
#         up_thres =  min(flow_info.loc[flow_info['type'] == 'error', 'dist'])

#     elif method == 'Manual':
#         if up_thres is None:
#             print("Error: please set the parameter 'up_thres' when 'method' is 'Outlier'.")
#         flow_info['type'] = "keep"
#         flow_info.loc[flow_info.dist > up_thres, 'type'] = "error"
    
#     return(flow_info, up_thres)
...
def saveGrowthForRSankey(ot_cls, n_clus, final_grow_rates, saveName = "growth_for_Rsankey.txt"):
    out_growth = {'Type':np.hstack((np.repeat('Pre', n_clus[0]), np.repeat('Proliferation', n_clus[0]))),
                  'Cluster':np.hstack((np.arange(n_clus[0]), np.arange(n_clus[0]))),
                  'Fraction':np.hstack((ot_cls.sum(axis = 1), ot_cls.sum(axis = 1) * final_grow_rates))}
    out_growth = pd.DataFrame(out_growth)
    out_growth.to_csv(saveName, sep='\t', header=True, index=False)
    return(out_growth)
...
class scStateDynamics:
    ...
    def __init__(self, data_pre, data_pos, pre_name, pos_name, run_label, pre_colors=None, pos_colors=None, 
        ...
        self.p_DistanceHistogram = None
        self.p_DistanceHeatmap = None
        self.p_growRateBar = None
        self.p_flowSankey = None
        self.p_flowHeatmap = None
        self.p_flowNormalizeHeatmap = None
    ...
    def __repr__(self):
        ...
        all_str = all_str + "\n  - Pos-data dimensions: " + str(self.data_pos.shape)

        if self.bool_Clustering:
            all_str = all_str + "\n+ 1. Cell state clustering is finished."
            all_str = all_str + "\n  - Number of clusters: " + str(self.n_clus)
            all_str = all_str + "\n  - Number of cliques: " + str(self.n_clqs)

        if self.bool_Distance:
            all_str = all_str + "\n+ 2. Distance diffusion and measurement is finished."

        if self.bool_Align:
            all_str = all_str + "\n+ 3. Cell state alignment is finished."

        if self.bool_Correct:
            all_str = all_str + "\n+ 4. Cell growth and death correcting is finished."

        if self.bool_FA:
            all_str = all_str + "\n+ 5. Cell-level differential effect analysis is finished."
    ...
    def runClustering(self, cls_resolutions, clq_resolutions, savePath = None):
        ...
        clq_resolutions: 'list'
            A list with length 2, containing the resolution parameters used in the `sc.tl.leiden` function of `scanpy` to identify meta-cells (cliques) at pre- and post-timepoint. Generally, these resolutions can be set as a large number, such as 30 or 50.
    ...
    ### 2. Distance diffusion and measurement ---------------------------
    def plotDistHistogram(self, color = "#69b3a2", saveFig = True, saveName = 'Distance-histogram.png'):
        fig = plt.figure(figsize = (2.8, 2.5), tight_layout = True)
        ax = sns.histplot(self.pair_dist.reshape(1,-1)[0], kde = True, edgecolor = None, color=color)
    ...
    def plotDistHeatmap(self, saveFig = True, saveName = 'Distance-heatmap.png'):
        row_colors = getHeatAnnoColor(self.clq_cls_pre, self.pre_colors)
        col_colors = getHeatAnnoColor(self.clq_cls_pos, self.pos_colors)
        
        g = sns.clustermap(self.pair_dist, cmap="YlGnBu", row_colors=row_colors, col_colors=col_colors, figsize=(4, 4))
        g.ax_col_dendrogram.set_title("Distance between pre- and post-cliques")
        g.ax_heatmap.set_xlabel(self.pos_name + '-clique')
        g.ax_heatmap.set_ylabel(self.pre_name + '-clique')
        if saveFig:
            g.fig.savefig(saveName, dpi=300, bbox_inches="tight")
            print('| - Saving figure:', saveName)
        return(g.fig)



    def calcGlobalDist(self, k, alpha, step, savePath = None):
        '''
        ...
        self.p_DistanceHistogram = self.plotDistHistogram(saveName = saveName)
        saveName = savePath + self.run_label + '_Distance-heatmap.' + self.saveFigFormat
        self.p_DistanceHeatmap = self.plotDistHeatmap(saveName = saveName)
    ...
    def plotFlowCostDistr(self, run_i = -1, threshold = False, bins = None, color = "#598cbe", 
                          title = True, saveFig = True, saveName = "FlowCost-hist.png"):
        flow_info = self.getOT_results('flow_info', run_i = run_i)
        if isinstance(threshold, bool):
            if threshold:
                threshold = self.getOT_results('error_flow_thres', run_i = run_i)
        ...
        if isinstance(title, bool):
            if title:
                if run_i == -1:
                    run_i = len(self.OT_results)
                ax.set_title('Round ' + str(run_i - 1))
    ...
    def plotFlowSankey(self, run_i = -1, title = True, saveFig = True, saveName = 'Flow-Sankey.png'):
        init_flow_info = self.getOT_results('flow_info', run_i = 0)
        final_flow_info = self.getOT_results('flow_info', run_i = run_i)
    ...
    #                 ax.fill_between(xs, ys_dw, ys_up, alpha=0.7, color=self.pre_colors[si])
                    ...
                    ot_clq = self.getOT_results('ot_clq', run_i = run_i)
    ...
    #             ax.fill_between(xs, ys_dw, ys_up, alpha=0.7, color=self.pre_colors[si])
        ...
        if isinstance(title, bool):
            if title:
                if run_i == -1:
                    run_i = len(self.OT_results)
                ax.set_title(' Round ' + str(run_i - 1), loc = 'left')
        elif isinstance(title, str):
    ...
    def alignCellStates(self, savePath = None):
        ...
        self.OT_results[0]['p_flowCostHist'] = p_flowCostHist
        self.OT_results[0]['p_flowSankey'] = p_flowSankey

        self.bool_Align = True



    def getOT_results(self, variable, run_i = -1):
        return(self.OT_results[run_i][variable])




    def assignFlowType(self, run_i = -1, method = 'Outlier', criterion = "AIC", threshold = None, savePath = None):
        '''
        Identify the types of cell subcluster flows.
        The scStateDynamics object will record the identified cell subcluster flow types at the current iteration round (`run_i`).
        ...
        method: 'str'
            The method name to identify the flow types. Possible values:
        ...
        if isinstance(run_i, int) & (run_i > 0):
            if run_i > len(self.OT_results):
                run_i = len(self.OT_results)

            self.OT_results = self.OT_results[0:run_i]

            flow_type, error_thres = _assignFlowType(self.getOT_results('flow_info')['dist'], method = method, 
                                                     criterion = criterion, threshold = threshold)
            self.OT_results[-1]['flow_info']['type'] = flow_type
            self.OT_results[-1]['error_flow_thres'] = error_thres
    
            saveName = savePath + self.run_label + '_FlowCost-hist-' + str(self.OT_results[-1]['run_i']) + '.' + self.saveFigFormat
            p_flowCostHist = self.plotFlowCostDistr(threshold = True, bins = None, color = "#7ccd7c", 
                                                    saveFig = False, saveName = saveName)
    
            self.OT_results[-1]['p_flowCostHist'] = p_flowCostHist
    ...
    def correctGrowthDeath(self, run_i = -1, savePath = None):
        '''
        Correct the unreasonable cell flows and estimate proliferation or inhibition rates of clusters.
        The scStateDynamics object will record the estimated cluster proliferation or inhibition rates and the corrected cell alignments at the current iteration round (`run_i`).
        ...
        if isinstance(run_i, int) & (run_i > 0):
            ...
            if run_i == 1:
                print("\n## 4. Cell growth and death correcting ---------------------------")

            self.OT_results = self.OT_results[0:run_i]

            flow_info = self.OT_results[-1]['flow_info']
            grow_rates = self.OT_results[-1]['grow_rates']

            print('| Repreforming OT: round ', run_i, '---------')
            ...
            self.OT_results.append({'run_i': run_i,  'flow_info': flow_info,
                                    'ot_clq': ot_clq,  'ot_cls': ot_cls,
                                    'grow_rates': grow_rates})

            saveName = savePath + self.run_label + '_FlowCost-hist-', str(run_i), '.' + self.saveFigFormat
            p_flowCostHist = self.plotFlowCostDistr(threshold = False, bins = None, color = "#7ccd7c", saveFig = False, saveName = saveName)
            saveName = savePath + self.run_label + '_Flow-Sankey-' + str(run_i), '.' + self.saveFigFormat
            p_flowSankey = self.plotFlowSankey(saveFig = False, saveName = saveName)
            self.OT_results[-1]['p_flowCostHist'] = p_flowCostHist
            self.OT_results[-1]['p_flowSankey'] = p_flowSankey

        else:
            print("Error: the parameter 'run_i' should be positive integer or -1.")
    ...
    def addCellFlow(self):
        flow_info = self.getOT_results('flow_info', run_i = -1)
        ot_clq = self.getOT_results('ot_clq', run_i = -1)
    ...
    def calcOT_clq_sp(self):
        ot_clq = self.getOT_results('ot_clq')
        ...
        self.OT_results[-1]['ot_clq_sparse'] = ot_clq_sp



    def plotGrowRate(self, run_i = -1, saveFig = True, saveName = 'GrowthRate-bar.png'):
        grow_rates = self.getOT_results('grow_rates', run_i = run_i)
    ...
    def plotFlowHeat(self, run_i = -1, title = None, saveFig = True, saveName = 'Flow-heatmap.png'):
        ot_cls = self.getOT_results('ot_cls', run_i = run_i)
    ...
    def plotFlowNormalizeHeat(self, run_i = -1, saveFig = True, saveName = 'Flow-NormalizeHeatmap.png'):
        ot_cls = self.getOT_results('ot_cls', run_i = run_i)
    ...
    def plotFlowLandscape(self, scale = 100, saveFig = True, saveName = 'Flow-Landscape.png'):
        embedding = MDS(n_components=2, dissimilarity='precomputed')
        mds_df = embedding.fit_transform(self.global_dist)
        mds_df = pd.DataFrame(mds_df, columns=['MDS1', 'MDS2'])
        
        mds_df['Sample'] = np.hstack((np.repeat(self.pre_name, self.n_clqs[0]), np.repeat(self.pos_name, self.n_clqs[1])))
        mds_df['Cluster'] = np.hstack((self.clq_cls_pre['Cluster'], self.clq_cls_pos['Cluster']))
        a = np.array([sum(self.data_pre.obs['clique'] == str(x)) for x in range(self.n_clqs[0])])
        b = np.array([sum(self.data_pos.obs['clique'] == str(x)) for x in range(self.n_clqs[1])])
        a = a / sum(a)
        b = b / sum(b)
        mds_df['n_Cells'] = np.concatenate([a,b],axis=0)
        
        
        def rbf(x, y, sigma):
            return np.exp((-(x**2)-(y**2)) / sigma)
        
        def getClsMDS(mds_df, sample):
            cur_df = mds_df[mds_df['Sample'] == sample]
            sam_cls = cur_df['Cluster'].unique()
            sam_cls.sort()
            mds_cls_1 = [np.median(cur_df['MDS1'][cur_df['Cluster'] == cls]) for cls in sam_cls]
            mds_cls_2 = [np.median(cur_df['MDS2'][cur_df['Cluster'] == cls]) for cls in sam_cls]
            mds_cls_df = pd.DataFrame({'MDS1': mds_cls_1, 'MDS2': mds_cls_2, 'Cluster': sam_cls, 'Sample': sample}, index = sam_cls)
            return(mds_cls_df)
        ...
        y1 = max(mds_df['MDS2'])
        pad = np.sqrt(scale)
        X, Y = np.mgrid[(x0-pad):(x1+pad):complex(0, N), 
                        (y0-pad):(y1+pad):complex(0, N)]
        Z = np.zeros(X.shape)
    
        for ii in range(mds_df.shape[0]):
            if mds_df['Sample'][ii] == self.pre_name:
                Z = Z + mds_df['n_Cells'][ii] * rbf(X - mds_df['MDS1'][ii], Y - mds_df['MDS2'][ii], scale)
            elif mds_df['Sample'][ii] == self.pos_name:
                Z = Z - mds_df['n_Cells'][ii] * rbf(X - mds_df['MDS1'][ii], Y - mds_df['MDS2'][ii], scale)
        maxValue = max(abs(Z.min()), abs(Z.max()))
        
        dens_map = cm.get_cmap('PRGn')
        dens_map = dens_map.reversed()
        shadeopts = {'cmap': dens_map, 'shading': 'gouraud'}
        
        colors_dict = dict(zip([self.cls_prefixes[0]+str(i) for i in range(len(self.pre_colors))], self.pre_colors))
        colors_dict.update(dict(zip([self.cls_prefixes[1]+str(i) for i in range(len(self.pos_colors))], self.pos_colors)))
        
        fig, ax = plt.subplots(figsize=(10, 3), tight_layout = True)
        ax.pcolormesh(X, Y, Z, **shadeopts, vmin=-maxValue, vmax=maxValue)
        
        
        pre_cls_mds = getClsMDS(mds_df, self.pre_name)
        pos_cls_mds = getClsMDS(mds_df, self.pos_name)
        ax.scatter(pre_cls_mds['MDS1'], pre_cls_mds['MDS2'], 
                   c = [colors_dict[zi] for zi in pre_cls_mds['Cluster']],
                   marker = 'o', s = 80, alpha = 0.7, label='Pre-cluster')
        ax.scatter(pos_cls_mds['MDS1'], pos_cls_mds['MDS2'], 
                   c = [colors_dict[zi] for zi in pos_cls_mds['Cluster']],
                   marker = '^', s = 80, alpha = 0.7, label='Post-cluster')
        ...
        for i in range(flow_info.shape[0]):
            s = flow_info['s'].iloc[i]
            t = flow_info['t'].iloc[i]
            x_source, y_source = np.array(pre_cls_mds.loc[self.cls_prefixes[0] + str(s), ['MDS1', 'MDS2']]).tolist()
            x_target, y_target = np.array(pos_cls_mds.loc[self.cls_prefixes[1] + str(t), ['MDS1', 'MDS2']]).tolist()
            x_source = x_source + 0.1 * (x_target - x_source)
            y_source = y_source + 0.1 * (y_target - y_source)
            x_target = x_target - 0.1 * (x_target - x_source)
            y_target = y_target - 0.1 * (y_target - y_source)
            width = flow_info['s_pm_perc'].iloc[i] * 3
            
            ax.arrow(x_source, y_source, x_target-x_source, y_target-y_source, 
                     width=width, head_width=2.5*width, head_length=2*width, 
                     length_includes_head=True, color = 'black', alpha = 0.3)
        
        
        cur_col_names = pre_cls_mds['Cluster'].unique()
        cur_col_names.sort()
        handles1 = [plt.plot([],[], color=colors_dict[cat], marker="o", ls="", alpha = 0.5, label=cat)[0] for cat in cur_col_names]
        cur_col_names = pos_cls_mds['Cluster'].unique()
        cur_col_names.sort()
        handles2 = [plt.plot([],[], color=colors_dict[cat], marker="^", ls="", alpha = 0.5, label=cat)[0] for cat in cur_col_names]
        handles1.extend(handles2)
        legend = ax.legend(handles=handles1, bbox_to_anchor=(1.5, 0.5), loc='center left', title="Cluster", 
                           fontsize = 9, handletextpad = 0, framealpha = 0.1)
        legend.get_title().set_fontsize(10)
        ax.tick_params(axis='both', length=0, width=0)
        ax.set_xticklabels([])
        ax.set_yticklabels([])
        ax.set_aspect((X.max() - X.min()) / (Y.max() - Y.min()))
        ax.set_xlabel('MDS_1', fontsize = 10)
        ax.set_ylabel('MDS_2', fontsize = 10)
        
        smap = cm.ScalarMappable(cmap=dens_map)
        smap.set_array([-maxValue, maxValue])
        colorbar = fig.colorbar(smap, ax = ax, aspect=20, fraction=0.013, location = 'right', label = 'Density of cells')
        colorbar.ax.tick_params(labelsize=8, length=0)
        colorbar.ax.title.set_fontsize(9)
        
        fig.subplots_adjust(left=0.1, right = 0.8)
        
        ax.text(x=1.18, y=0.93, s=self.pre_name, transform=ax.transAxes, ha='center', va='center', fontsize = 10)
        ax.text(x=1.18, y=0.07, s=self.pos_name, transform=ax.transAxes, ha='center', va='center', fontsize = 10)
    ...
    def saveResults(self, savePath = None, save_label = None):
        '''
        Correctthe unreasonable cell flows and estimate proliferation or inhibition rates of clusters.
        Parameters
        ----------
        savePath: 'path-like str'
            A path to save the analysis results. The default is to save at current directory. If 'None', the default savePath in scStateDynamics object will be used.
        ...
        flow_info = self.getOT_results('flow_info')
        grow_rates = self.getOT_results('grow_rates')
        ot_cls = self.getOT_results('ot_cls')
        ot_clq = self.getOT_results('ot_clq')
        ...
        self.p_flowNormalizeHeatmap = self.plotFlowNormalizeHeat(saveName = savePrefix + 'Flow-NormalizeHeatmap.' + self.saveFigFormat)
        ...
        ot_clq_sp = self.getOT_results('ot_clq_sparse')
        ...
        grow_rates.to_csv(savePath + self.run_label + '_GrowRates.txt', sep='\t', header=True, index=True)
    ...
    def model(self):
        ...
        ot_clq_sparse = self.getOT_results('ot_clq_sparse')
    ...
    def factorAnalysis(self, n_factor, learningRate = 0.02, steps = 500, savePath = None):
        print("\n## 5. Cell-level differential effect analysis ---------------------------")
        if savePath is None:
            savePath = self.savePath
...
def createScStateDynamicsObj(data_pre, data_pos, pre_name, pos_name, run_label, pre_colors=None, pos_colors=None, 
    cls_prefixes = ['S', 'T'], savePath = "", saveFigFormat = "png"):
    '''
    ...
    ----------
    data_pre: 'AnnData'
        A scanpy object of pre-timepoint scRNA-seq data.
    data_pos: 'AnnData'
        A scanpy object of post-timepoint scRNA-seq data.
    pre_name: 'str'
    ...
    cls_prefixes:'list'
        A list with length 2, containing the prefixes of the clusters at pre-timepoint and post-timepoint. The default is 'S' and 'T'.
    savePath: 'path-like str'
