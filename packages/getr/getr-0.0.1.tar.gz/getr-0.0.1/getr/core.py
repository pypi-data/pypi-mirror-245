...
def getr(
    ...
    default = None, otype: Optional[Callable] = None,
):
    ...
    try: val = getattr(obj, attr, default) or default
    except: val = default    
...
def make(
    attr: str, check: bool = True, guard: Optional[Callable] = notnone, default = None, otype: Optional[Callable] = None
) -> Callable[[R], R]:
    ...
    def deco(fn: Callable[..., TypeGuard[object]]):
        ...
        def wrap(
            ...
            return getr(obj, attr, check=check, guard=guard, default=default)
        wrap.__doc__ = doc
        wrap = name(wrap, attr)        
        return wrap
    return deco
