...
from .scan import scan_creation_stream, ScanState
...
class DataStore:
    def __init__(
        ...
        default_scan_cls=Scan,
        init_db=False,
    ):
        self._url = url
        self._redis = redis.Redis.from_url(url)
        self._default_scan_cls = default_scan_cls
...
redis.register_function('seal_stream', seal_stream)"""
    ...
    def load_scan(self, key, scan_cls=None):
        if scan_cls is None:
            scan_cls = self._default_scan_cls
        return scan_cls._load(self, key)

    def create_scan(self, identity, info={}, scan_cls=None):
        if scan_cls is None:
            scan_cls = self._default_scan_cls
        return scan_cls._create(self, identity, info)
