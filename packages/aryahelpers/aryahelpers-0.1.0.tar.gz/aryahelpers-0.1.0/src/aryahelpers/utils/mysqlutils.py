...
class MySQLHelpers(object):
    ...
    def _form_clientinfo_dict(user_info: dict, client_info: dict):
        ...
        if user_info and client_info:
            create_date = client_info["CreatedDate"].strftime("%d-%b-%Y, %H:%M:%S IST") if client_info["CreatedDate"] else ""
            update_date = client_info["UpdateDate"].strftime("%d-%b-%Y, %H:%M:%S IST") if client_info["UpdateDate"] else ""
            clientinfo_dict = {
    ...
    def fetch_or_add_client_for_user(db_key: str, company_name: str, id_key=None, id_type='user_id'):
        ...
        select_query = f"SELECT CompanyID, CompanyName, ATSClientID, Organization, IsActive, CONVERT_TZ(\
            CreatedDate,'+00:00','+05:30') AS CreatedDate, CONVERT_TZ(LastUpdateTimeStamp,'+00:00','+05:30') AS \
                UpdateDate FROM `mstcompany` WHERE `Organization`={org_id} AND LOWER(`CompanyName`)=LOWER('{company_name}');"
    ...
    def get_clientsmapping_for_user(db_key: str, id_key=None, id_type='user_id', save_path=False):
        ...
        query_str = f"SELECT CompanyID, CompanyName, ATSClientID, Organization, IsActive, CONVERT_TZ(\
            CreatedDate,'+00:00','+05:30') AS CreatedDate, CONVERT_TZ(LastUpdateTimeStamp,'+00:00','+05:30') \
                AS UpdateDate FROM `mstcompany` WHERE Organization={org_id};"
