...
class Parameters:
    ...
    environment_browser = os.getenv('PYBOT_BROWSER')

    # CONFIGURACION DE BROWSER Y PRUEBAS
    if enviroment_confguration == "Server":
        if environment_browser == 'Edge':
            browser = 'EDGE'
        else:
            browser = 'CHROME'
    else:
        if enviroment_confguration is None:
            browser = 'EDGE'
        else:
            browser = 'CHROME'
    ...
    record = False if environment_record is None else False if environment_record.lower() == "false" else bool(environment_record)
