...
def test_logisticPriorMAP():
    ...
    match_list = read_match_list_from_adj_matrix(
        "tests/test_case_files/adj_matrix_1.txt"
    )
    ...
    for i in range(n):
        ...
        est_score_error = scores_df.loc[
            scores_df["label"] == player_label, "score_error"
        ].values[0]
...
def test_logisticPrior_probability():
    ...
    # Call function
    prob, prob_error = probability(
        match_list,
        "player_0",
        "player_1",
        force_mode="average",
        model_name="logistic_prior",
    )
...
def test_luckOnly_probability():
    ...
    # Call function
    prob, prob_error = probability(
        match_list, "player_0", "player_1", force_mode="average", model_name="luck_only"
    )
