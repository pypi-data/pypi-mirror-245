...
class CmdAction(Cmd):
    ...
    def add_parser_args(parser: ArgumentParser) -> None:
        ...
        parser.add_argument('--doc-db', dest='doc_db', type=bool, default=False, required=False)
    ...
    def run(self, *args: Any, **kwargs: Any) -> None:
        ...
        if self.doc_db:
            ...
            spec = util.spec_from_file_location("flask_app", flask_app_path)
            flask_app = util.module_from_spec(spec)
            sys.modules["flask_app"] = flask_app
            spec.loader.exec_module(flask_app)
            result = subprocess.run(
                [f"pymongo-migrate {self.cmd} --uri '{flask_app.db_config.uri}' {additional_args} --migrations {migration_dir}"],
                shell=True,
                stderr=sys.stderr,
                stdout=sys.stdout,
            )
