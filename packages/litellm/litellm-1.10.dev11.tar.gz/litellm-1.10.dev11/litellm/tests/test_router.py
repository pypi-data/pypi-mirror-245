...
def test_reading_keys_os_environ():
	try:
		...
		for model in router.model_list:
			assert model["litellm_params"]["api_key"] == os.environ["AZURE_API_KEY"]
			assert model["litellm_params"]["api_base"] == os.environ["AZURE_API_BASE"]
			assert model["litellm_params"]["api_version"] == os.environ["AZURE_API_VERSION"]
			assert model["litellm_params"]["timeout"] == float(os.environ["AZURE_TIMEOUT"])
			assert model["litellm_params"]["stream_timeout"] == float(os.environ["AZURE_STREAM_TIMEOUT"])
			assert model["litellm_params"]["max_retries"] == int(os.environ["AZURE_MAX_RETRIES"])
