...
class MultiCropper(object):
    ...
    gpu: bool
        If True, Use NVIDEA FFMpeg GPU codecs. Deafult: False.
    ...
    def __init__(self,
        ...
        if self.crop_cnt == 0:
            raise CountError(msg='The number of cropped output videos is set to ZERO. The number of crops has the be a value above 0.', source=self.__class__.__name__)
    ...
    def __test_crop_locations(self):
        for idx, row in self.crop_df.iterrows():
            ...
            if all(v == 0 for v in lst):
                raise CountError(msg='SIMBA ERROR: A crop for video {} has all crop coordinates set to zero. Did you click ESC, space or enter before defining the rectangle crop coordinates?!'.format(video_name), source=self.__class__.__name__)
            else:
                pass
    ...
    def __crop_videos(self):
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            if not ret:
                raise InvalidVideoFileError(msg='The first frame of video {} could not be read'.format(video_name), source=self.__class__.__name__)
            ...
            for box_cnt in range(self.crop_cnt):
                ...
                cv2.putText(frame, str('Define crop #{} coordinate boundaries and press ENTER or ESC'.format(str(box_cnt+1))), (10, ((height - height) + space_size * self.add_spacer)), cv2.FONT_HERSHEY_TRIPLEX, font_size, (180, 105, 255), 2)
        ...
        for video_name in self.crop_df['Video'].unique():
            ...
            for cnt, (idx, row) in enumerate(video_crops.iterrows()):
                print(f'Creating {name} crop clip {cnt+1}...')
                ...
                print(f'Video {name} crop {cnt+1} complete (elapsed time: {crop_timer.elapsed_time_str})...')
