...
from cellpose_omni import utils, plot, transforms
...
def get_image_files(folder, mask_filter='_masks', img_filter=None, look_one_level_down=False,
    ...
    if look_one_level_down:
        ...
        folders = natsorted(glob.glob(
            os.path.join(folder, "*", '')))  # forward slash is unix only, this should generalize to windows too
...
def load_train_test_data(train_dir, test_dir=None, image_filter='', mask_filter='_masks',
    ...
    for img_filename in image_names:
        # Obtenir le chemin du répertoire et le nom de fichier à partir du chemin complet
        directory, filename = os.path.split(img_filename)

        # Obtenir le numéro d'index à partir du nom de fichier
        index = filename.split('_')[0]

        # Générer le nom de fichier pour le masque D en utilisant le numéro d'index
        D_filename = os.path.join(directory, f'{index}_D.png')
        S_filename = os.path.join(directory, f'{index}_S.png')

        # Ajouter le nom de fichier du masque D à la liste D_names
        D_names.append(D_filename)
        S_names.append(S_filename)
    ...
    D = [imread(l) for l in D_names]
    S = [imread(l) for l in S_names]
    ...
    return images, labels, D, S, links, image_names, test_images, test_labels, test_links, image_names_test
