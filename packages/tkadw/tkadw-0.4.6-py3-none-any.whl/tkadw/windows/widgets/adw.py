...
class Adw(Tk):
    def __init__(self, *args, master=None, title: str = "adw", config: bool = False,
        ...
        self.dark_with_icon(dark, dark_with_refresh)
        ...
        if platform == "win32" and wincaption is not None:
            ...
            windll.dwmapi.DwmSetWindowAttribute(windll.user32.GetParent(self.winfo_id()), 35,
                                                byref(c_int(RGB(wincaption[0], wincaption[1], wincaption[2]))),
                                                sizeof(c_int))
        self.protocol("WM_DELETE_WINDOW", self.quit)

    def dark_with_icon(self, enable: bool, dark_with_refresh: bool = False):
        self.dark(enable, dark_with_refresh)
        self.icon_dark(enable)

    def dark(self, enable: bool, dark_with_refresh: bool = False):
        from sys import platform
        if platform == "win32":
            if enable:
                try:
