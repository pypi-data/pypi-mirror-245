    ...
    def run_for_model(self, model: str, **kwargs):
        ...
        if kwargs.get('is_check_detail', True):
            if kwargs.get('creation_parameters'):
                ...
                test_instance = model_class.objects.create(**kwargs.get('creation_parameters'))
                content_type = ContentType.objects.get_for_model(model_class)
                response = self.client.get(
                    reverse(f'admin:{content_type.app_label}_{content_type.model}_change', args=[test_instance.pk]),
                    follow=True
                )
