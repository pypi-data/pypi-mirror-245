...
class TestProofByContradiction2(TestCase):
    def test_proof_by_contradiction_2(self):
        ...
        pass
        self.assertTrue(proposition_of_interest.is_formula_syntactically_equivalent_to(phi=o1 | u.c1.eq | o2))
        ...
        f = u.c1.declare(symbol='f', arity=2, signal_proposition=True)
        h2: pu.Hypothesis = t1.pose_hypothesis(hypothesis_formula=f(o1, o2), subtitle='We pose the negation hypothesis')
        with self.assertRaises(pu.PunctiliousException) as error:
            t1.i.proof_by_contradiction_2.infer_formula_statement(h=h2, inc_h=u.c1.inc(h2))
        ...
        h3: pu.Hypothesis = t1.pose_hypothesis(hypothesis_formula=o2 | u.c1.unequal | o3,
            subtitle='We pose the negation hypothesis')
        with self.assertRaises(pu.PunctiliousException) as error:
            t1.i.proof_by_contradiction_2.infer_formula_statement(h=h3, inc_h=u.c1.inc(h3.child_theory))
