...
class TestProofByContradiction1(TestCase):
    def test_proof_by_contradiction_1(self):
        ...
        self.assertTrue(proposition_of_interest.is_formula_syntactically_equivalent_to(phi=f(o1, o3)))
        ...
        with self.assertRaises(pu.PunctiliousException) as error:
            t1.i.proof_by_contradiction_1.infer_formula_statement(h=h2, inc_h=u.c1.inc(h2))
        ...
        self.assertIs(pu.error_codes.error_002_inference_premise_syntax_error, error.exception.error_code)
        # Validity error
        h3: pu.Hypothesis = t1.pose_hypothesis(hypothesis_formula=u.c1.lnot(f(o2, o3)),
            subtitle='We pose the negation hypothesis')
        with self.assertRaises(pu.PunctiliousException) as error:
            t1.i.proof_by_contradiction_1.infer_formula_statement(h=h3, inc_h=u.c1.inc(h3.child_theory))
