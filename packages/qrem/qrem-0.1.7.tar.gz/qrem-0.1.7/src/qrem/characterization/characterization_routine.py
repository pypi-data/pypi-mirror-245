...
def compute_marginals_dictionary(results_dictionary:Dict[str,Dict[str,int]],marginals_list:List,experiment_type:str='DDOT', data_directory:str ='', name_string=''):
    ...
    if experiment_type == 'DDOT':
        ...
        marginals_analyzer.compute_all_marginals(subsets_dictionary=marginals_list,show_progress_bar=True,multiprocessing=False)
...
def compute_reduced_POVMs_and_noise_matrices(results_dictionary:Dict[str,Dict[str,int]],marginals_dictionary:Dict[str,Dict[str,int]] ,subset_of_qubits:List,experiment_type:str='DDOT'):
    ...
    if experiment_type == 'DDOT':
        #compute noise matrices for all  subsets
...
def perform_clustering_routine(results_dictionary:Dict[str,Dict[str,int]],marginals_dictionary:Dict[str,Dict[str,int]], noise_matrices_dictionary:Dict[str,Dict[str,np.array]],correlations_data:np.array, number_of_qubits:int,clustering_functions_parameters: Dict ,perform_neighbors_search:bool = False):
    ...
    for max_cluster_size in sizes_clusters:
        ...
        for alpha_multiplier in alpha_hyperparameters:
            ...
            clusters_tuples_now, score = partition_algorithm_v4_cummulative(correlations_table = correlations_data[distance_type][correlations_type],alpha=alpha_multiplier,C_maxsize=max_cluster_size,N_alg=1000,printing=False,disable_pb=True, drawing=False) 
...
def execute_characterization_workflow(results_dictionary:Dict[str, Dict[str, int]], marginals_dictionary:Dict[str,Dict[Tuple, np.ndarray ]] = None, experiment_type: str = 'DDOT',   data_directory: str ='', return_old_mitigation_data: bool = False, ground_states_list = None, coherence_witnesses_list = None,  name_id ='', perform_noise_model_reconstruction = True, find_neighbors = False):
    ...
    if marginals_dictionary == None:
        ...
        # NEW
        # results_dictionary should be in NEW format
        # normalized_marginals = characterization.compute_all_marginals(results_dictionary=results_dictionary,
        #                                                              subsets_of_qubits=marginals_to_mitigate,
        #                                                              backup = Path(dictionary_to_save).joinpath(file_name_marginals),
        #                                                              overwrite = True,
        #                                                              verbose_log = True)
    ...
    characterization_data_container.marginals_dictionary = characterization_data_dictionary['characterization_marginals_dictionary']
    
    characterization_data_container.results_dictionary = characterization_data_dictionary['characterization_results_dictionary']

    characterization_data_container.benchmark_results_dictionary = characterization_data_dictionary['benchmarks_results_dictionary']
    
    characterization_data_container.benchmark_marginals_dictionary = characterization_data_dictionary['benchmarks_marginals_dictionary']  
    ...
    #if experiment_type == 'DDOT':
        # # NEW
        # noise_matrices = characterization.compute_noise_matrices_ddot(experiment_results=experiment_results,
        #                                                               noramlized_marginals=normalized_marginals,
        #                                                               subset_of_qubits = marginals_to_mitigate,
        #                                                               backup = False,
        #                                                               verbose_log=True)
        
    characterization_data_container.POVMs_dictionary, characterization_data_container.noise_matrices_dictionary = compute_reduced_POVMs_and_noise_matrices(results_dictionary=characterization_data_container.results_dictionary,marginals_dictionary= characterization_data_container.marginals_dictionary,subset_of_qubits=all_two_qubits_subsets,experiment_type='DDOT')
    ...
    dictionary_to_save = {'POVMs_dictionary':characterization_data_container.POVMs_dictionary}
    ...
    noise_analysis_dictionary =  perform_POVMs_errors_and_correlation_coefficients_computation(results_dictionary=characterization_data_container.results_dictionary,marginals_dictionary= characterization_data_container.marginals_dictionary,POVMs_dictionary=characterization_data_container.POVMs_dictionary) 

    
    
    
    characterization_data_container.correlation_coefficients_dictionary = noise_analysis_dictionary['correlations_data_dictionary']

    

    characterization_data_container.POMVs_errors_dictionary = noise_analysis_dictionary['POVMs_error_dictionary']
    ...
    if coherence_witnesses_list != None:
        ...
        characterization_data_container.coherence_bound_dictionary = fca.compute_coherence_indicator(marginals_dictionary=characterization_data_dictionary['coherence_witnesses_marginals_dictionary'],subset_list=[(i, j) for i in range(number_of_qubits) for j in range(i + 1, number_of_qubits)],settings_list=settings_list)
    ...
    #dictionary_to_save = {'correlations_data':characterization_data_container.correlation_coefficients_dictionary,
    ...
    characterization_data_container.clusters_neighbors_sets_dictionary = perform_clustering_routine(results_dictionary = characterization_data_container.results_dictionary,marginals_dictionary =  characterization_data_container.marginals_dictionary, noise_matrices_dictionary = characterization_data_container.noise_matrices_dictionary ,correlations_data = characterization_data_container.correlation_coefficients_dictionary, number_of_qubits=number_of_qubits,clustering_functions_parameters = clustering_functions_parameters)
    ...
    if perform_noise_model_reconstruction:
        ...
        characterization_data_container.noise_model_list = perform_noise_model_reconstruction_routine(results_dictionary=characterization_data_container.results_dictionary,number_of_qubits=number_of_qubits,all_clusters_sets_neighbors_dictionary=characterization_data_container.clusters_neighbors_sets_dictionary)

        characterization_data_dictionary['noise_models_list'] = characterization_data_container.noise_model_list
    
    characterization_data_dictionary['benchmarks_results_dictionary'] = characterization_data_container.benchmark_results_dictionary
    characterization_data_dictionary['benchmarks_marginals_dictionary']= characterization_data_container.benchmark_marginals_dictionary
    characterization_data_dictionary['POVMs_dictionary'] = characterization_data_container.POVMs_dictionary
    characterization_data_dictionary['correlations_data'] = characterization_data_container.correlation_coefficients_dictionary
    ...
    if return_old_mitigation_data:
        ...
        characterization_data_dictionary['correlation_coefficients_dictionary'] = characterization_data_container.correlation_coefficients_dictionary
        
        characterization_data_dictionary['POVMs_dictionary']= characterization_data_container.POVMs_dictionary
        
        characterization_data_dictionary['POVMs_distances_dictionary']=characterization_data_container.POMVs_errors_dictionary
        ...
        characterization_data_dictionary['all_clusters_sets_neighbors_dictionary'] = characterization_data_container.clusters_neighbors_sets_dictionary

        characterization_data_dictionary['coherence_bound_dictionary']  = characterization_data_container.coherence_bound_dictionary
