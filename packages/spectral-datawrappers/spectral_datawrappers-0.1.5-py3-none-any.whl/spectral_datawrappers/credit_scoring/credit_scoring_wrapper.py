...
class CreditScoringWrapper(DataWrapperInterface):
    ...
    def request_batch(self, input: list, workers: int = 100) -> list:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for wallet_address in input:
                futures[executor.submit(self.request, {"wallet_address": wallet_address})] = wallet_address
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    data = future.result()
                    if data:
                        # move wallet_address to the first key
                        items = list(data.items())
                        items.insert(0, items.pop())
                        data = dict(items)
                        features.append(data)
                    else:
                        failed_wallet_addresses.append(wallet_address)
                except Exception as exc:
                    logging.debug(f"An unexpected error occurred for {wallet_address}: {exc}")
                    failed_wallet_addresses.append(wallet_address)

        if len(failed_wallet_addresses) > 0:
            for wallet_address in failed_wallet_addresses:
                try:
                    data = self.request(wallet_address)
                    if data:
                        # move wallet_address to the first key
                        items = list(data.items())
                        items.insert(0, items.pop())
                        ...
                        logging.debug(f"Failed to fetch data for {wallet_address}.")
                except Exception as exc:
                    logging.debug(f"An unexpected error occurred for {wallet_address}: {exc}")
                    continue
