    ...
    def build_extension(self, ext: Extension):
        if isinstance(ext, CMakeExtension):
            ...
            if os.name == 'nt':  # windows
                status = 0
                for gen in ("-GVisual Studio 17 2022", "-GVisual Studio 16 2019", "-GVisual Studio 15 2017",
                            "-GVisual Studio 14 2015", "-GMinGW Makefiles"):
                    try:
                        subprocess.check_call(cmake_args + [gen], cwd=BUILD_DIR, env=env)
                        self.logger.info('%s is used for building Windows distribution.', gen[2: ])
                        status = 1
                        break
