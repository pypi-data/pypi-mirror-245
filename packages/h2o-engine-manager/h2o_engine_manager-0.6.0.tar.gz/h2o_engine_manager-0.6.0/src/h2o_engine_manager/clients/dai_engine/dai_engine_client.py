    ...
    def create_engine(
        ...
        Args:
            ...
            max_running_duration (str, optional):  Maximum time na engine can be running. When exceeded, the engine will pause. Must be specified as a number with `s` suffix. Example `36000s`.
                If not specified, a default value will be set by server.
            profile_id (str, optional): The ID of the profile to use for the engine. Profile will be adjusted to fit the workspace contraints.
        ...
        if profile_id is not None:
            profile = self.adjusted_profile_client.get_adjusted_profile(workspace_id=workspace_id,
            ...
            if memory_bytes is None:
                memory_bytes = quantity_convertor.quantity_to_number_str(profile.memory_bytes)
            if storage_bytes is None:
                storage_bytes = quantity_convertor.quantity_to_number_str(profile.storage_bytes)
        ...
        try:
            api_engine = (
                self.client_info.api_instance.d_ai_engine_service_get_dai_engine(
                    name_2=self.__build_api_engine_name(
