...
Import transect and intersection shapefiles into a GeoPandas dataframe. Note - If proj_lib error is shown set proj library path manually using os.environ['PROJ_LIB'] = 'local path to the proj lib library'
```
from SCA import SCA
from DOD import DOD
...
```
Implement the transect definition - decide whether the transect locator one or two is need by assessing the shape and coords of the transects. *Note - if only DEM functions are desired set all other class arguments as 0 except for the 'path =' argument and then run Datacleaning.results() which will create the results for and provide the 'save_to_path' needed in DEM class functions.

```
#Georeferencing and measurement errors of each shoreline in meters. 
...
#Add georeferencing and measurement errors for each shoreline to dataframe and save the newly configured intersection file to variable. 
...
figheight = 10, save_to_path = results, path = paths, MaskingCRS = 'EPSG:4326', measurementerror = 0.15)

#Mask the DEM's using 'VolumePoly.shp' file present in the data directory. 
SauntonDOD.Masking()
 
#DEM of Difference function. 
SauntonDEMoDResults = Saunton.DEMofDifference()
