...
class ViewerHelperRRT:
    ...
    def __init__(self, viewer, urdf, srdf, start, goal):
        self.viewer = viewer
        ...
        _loadViewerModel(
            self.viz_start, "start", color=[0.0, 1.0, 0.0, 0.5])

        self.viz_goal = MeshcatVisualizer(
            self.robot.model, self.robot.collision_model, self.robot.visual_model)
        self.viz_goal.initViewer(self.viz.viewer)
        _loadViewerModel(
            self.viz_goal, "goal", color=[1.0, 0.0, 0.0, 0.5])
