...
def page_ditector(symbol):
    url = "https://search.codal.ir/api/search/v2/q?&Audited=true&AuditorRef=-1&Category=3&Childs=false&CompanyState=-1&CompanyType=-1&Consolidatable=true&IsNotAudited=false&Length=-1&LetterType=-1&Mains=true&NotAudited=true&NotConsolidatable=true&PageNumber=1&Publisher=false&Symbol={}&TracingNo=-1&search=true".format(symbol)
...
def codal_crwaler(symbol,page):
    url = "https://search.codal.ir/api/search/v2/q?&Audited=true&AuditorRef=-1&Category=3&Childs=false&CompanyState=-1&CompanyType=-1&Consolidatable=true&IsNotAudited=false&Length=-1&LetterType=-1&Mains=true&NotAudited=true&NotConsolidatable=true&PageNumber={}&Publisher=false&Symbol={}&TracingNo=-1&search=true".format(page,symbol)
    r = requests.get(url, timeout=15, headers = {
        'User-Agent' : 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36'
        } )
...
def codal_all_page_crwaler_symbol(symbol):
    page = page_ditector(symbol)
    if page >0:
        ls1 = []
        for i in range(1,page+1):
            a = codal_crwaler(symbol,i)
            ls1.append(a)
...
########################################################################################
symbols = []
def codal_speed_crwaler_symbol(codal_all_page_crwaler_symbol ,symbols):
    Stock_url = speed_loader(codal_all_page_crwaler_symbol,symbols,20)
