...
def format_output(obj, level="debug", depth=2, caller_info: dict = None):
    def __output_response(__obj: requests.Response):
        ...
        if __obj.request.body:
            if __obj.request.headers.get(
                    "Content-Type"
            ) and "application/json" in __obj.request.headers.get("Content-Type"):
        ...
        if __obj.text.strip():
            if __obj.headers.get(
                    "Content-Type"
            ) and "application/json" in __obj.headers.get("Content-Type"):
