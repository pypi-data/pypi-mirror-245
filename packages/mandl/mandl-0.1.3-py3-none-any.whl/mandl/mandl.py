...
class Mandl:
    ...
    def log(self, metrics: Dict[str, float]):
        # Log to Loki
        try:
            self._logger.info(json.dumps({**metrics, "counter": self._counter}), extra={"message_type", "user_metric"})
            self._counter += 1
    ...
    def finish(self):
        ...
        try:
            self._logger.info({"used_metrics:": list(self.seen_user_metrics)}, extra={"message_type", "finish"})
            result = subprocess.run(
