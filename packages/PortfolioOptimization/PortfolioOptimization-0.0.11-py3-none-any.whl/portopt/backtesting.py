...
def optimal_strategy(weights_df_optimal: pd.DataFrame,
...
def benchmark_strategy(strategy_name: str,
...
class WeighRandomlyWithBounds(bt.Algo):
    ...
    Methods:
        generate_random_weights(selected: list) -> pd.Series: Generates random weights for selected assets within
            specified bounds
        __call__(target) -> bool: Calls the algorithm with the current target (bt.Target)
    """
...
def generate_random_strategy_with_bounds(weight_bounds: Union[list, np.ndarray],
...
def run_backtest_with_benchmark(strategy_backtest: bt.Backtest,
    ...
    Args:
        ...
        benchmark_backtest (bt.Backtest): Backtest object for the benchmark
...
def run_backtest_with_random_benchmarks(strategy_backtest: bt.Backtest,
...
def get_series_from_object(results_object: bt.backtest.Result,
