...
@click.option("--num-karts", type=int, help="Number of karts (must be greater than number of zip files)")
@click.argument("zip_files", type=click.Path(exists=True, dir_okay=False), nargs=-1)
@rld.command("stk-race")
def rld_stk_race(hide, num_karts, zip_files):
    """Race"""
    check_last_masterdac(original_args)
