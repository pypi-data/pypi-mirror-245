...
def print_valuation(df,indicator='PE',base=''):
    ...
    df3['均值对比']=df3[col_mean_rel].apply(lambda x: '<---基准' if x == 1 else '+'+str(round((x-1)*100,2))+'%' if x >1 else '-'+str(round((1-x)*100,2))+'%')
    df3['对比@'+col_latest_date]=df3[col_latest_rel].apply(lambda x: '<---基准' if x == 1 else '+'+str(round((x-1)*100,2))+'%' if x >1 else '-'+str(round((1-x)*100,2))+'%')

    #df4=df3[['序号','证券名称',col_mean,col_mean_rel,'均值对比',col_latest_date,col_latest_rel,'对比@'+col_latest_date]]
    df4=df3[['序号','证券名称',col_mean,col_latest_date,col_mean_rel,col_latest_rel,'均值对比','对比@'+col_latest_date]]
