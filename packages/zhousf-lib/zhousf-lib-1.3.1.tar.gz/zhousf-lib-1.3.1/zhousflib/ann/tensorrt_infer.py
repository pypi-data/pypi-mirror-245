...
class RTInfer(object):
    ...
    def __init__(self, trt_file_path: Path, device_id: int = -1):
        check_device_id(device_id)
        os.environ['CUDA_VISIBLE_DEVICES'] = '{0}'.format(device_id)
        ...
        for binding in self.engine:
            ...
            if self.engine.binding_is_input(binding):
                if input_arr is not None:
                    input_memory = cuda.mem_alloc(input_arr.nbytes)
                    # 转换为内存连续存储的数组，提高运行效率
                    input_buffer = np.ascontiguousarray(input_arr)
