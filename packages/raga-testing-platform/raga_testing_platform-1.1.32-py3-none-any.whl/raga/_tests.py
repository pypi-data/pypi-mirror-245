...
def semantic_similarity(test_session:TestSession,
                    ...
                    output_type: str,
                    embed_col_name: str,
                    generated_embed_col_name:str,
                    rules: LQRules):

    dataset_id = semantic_similarity_validation(test_session, dataset_name, test_name, type, output_type, embed_col_name, generated_embed_col_name, rules)
    return {
        "datasetId": dataset_id,
        "experimentId": test_session.experiment_id,
        "name": test_name,
        ...
        "outputType": output_type,
        "embeddingColName": embed_col_name,
        "generatedEmbeddingColName": generated_embed_col_name,
        "rules": rules.get(),
        "test_type":"semantic_similarity"
    }

def semantic_similarity_validation(test_session:TestSession, dataset_name:str, test_name:str, type:str, output_type:str, embed_col_name:str, generated_embed_col_name:str, rules: LQRules):
    from raga.constants import INVALID_RESPONSE, INVALID_RESPONSE_DATA, REQUIRED_ARG_V2
    ...
    assert isinstance(output_type, str) and output_type, f"{REQUIRED_ARG_V2.format('output_type', 'str')}"
    assert isinstance(embed_col_name, str) and embed_col_name, f"{REQUIRED_ARG_V2.format('embed_col_name', 'str')}"
    assert isinstance(generated_embed_col_name, str) and generated_embed_col_name, f"{REQUIRED_ARG_V2.format('generated_embed_col_name', 'str')}"
    assert isinstance(rules, LQRules) and rules, f"{REQUIRED_ARG_V2.format('rules', 'str')}"
    return dataset_id
