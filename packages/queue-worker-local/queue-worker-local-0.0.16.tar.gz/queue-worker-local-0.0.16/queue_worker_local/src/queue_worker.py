...
class QueueWorker(DatabaseQueue):
    ...
    def execute(self, action_ids: tuple,
                ...
                total_missions: int = sys.maxsize,
        ...
        try:
            ...
            for mission in range(total_missions):
                ...
                print(f"{queue_item['action_id']}, {os.getpid()}, {current_thread_id}, {result.returncode}")

                stderr = self.save_execution_result(result, queue_item[self.id_column_name])
    ...
    def save_execution_result(self, result: subprocess.CompletedProcess, queue_item_id: int) -> str:
        ...
        params = (stdout, stderr, return_code, return_message, returned_value, queue_item_id)
...
def main():
    ...
    parser.add_argument('-raise_on_error', type=bool, default=True, help='Whether to raise an exception on error')
