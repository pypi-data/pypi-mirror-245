...
class _TurboQueueBase:
    def __init__(self):
        # self._age = 0
        self.version = "0.7.5.0"
        logging.info(f'Turbo Queue init version:{self.version}')
...
class Enqueue(_TurboQueueBase):
    ...
    #####
    # testing methods for clearing slow batches:
    ## WIP
    ...
    ## WIP
    async def check_slow_batch(self):
        """check_slow_batch()

        Call from the parent task/process/loop

        Example:
            # in PARENT/CALLING process:
            def parent_check_slow_batch():
                await asyncio.sleep(10)
                my_turbo_queue_enqueue.check_slow_batch()
                self.loop.create_task(self.parent_check_slow_batch())
        """
        if (
            self._create_epoch + int(self._max_batch_age)
        ) < self.get_current_epoch_int():
...
class Dequeue(_TurboQueueBase):
    ...
    def roll_back(self):
        """
        Similar to calling clean_up, except where cleanup removed the DB file from disk, roll_back puts the file back into ready state, to be picked up again.
        This is useful when auto_cleanup is False, and you want to manually control that process.  For example, you are unable to delivery the events of a batch and have reached a timeout.
        You can simply roll_back, and ensure that sending the batch will be attempted again.

        This will first check that there is a _db_path before proceeding.  It may be common when calling that a yielded get request found no available batches to pickup.  This ensures that a rollback 
        is not attempted, since there is nothing to rollback.
        """
        self._close_db()
        rename(
            self._db_path, f"{self.root_path}/{self.queue_name}/ready_{self._db_name}"
