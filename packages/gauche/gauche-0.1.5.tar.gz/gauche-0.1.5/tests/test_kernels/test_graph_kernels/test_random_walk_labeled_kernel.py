"""
Unit tests for the random walk (labelled) graph kernel.
"""
...
from gauche.kernels.graph_kernels import RandomWalkLabeledKernel
...
@pytest.mark.parametrize(
    "node_label",
    ["total_degree", "XYZ", None, 1],
)
def test_random_walk_labeled_kernel_node_label(node_label):
    """
    Test if random walk (labelled) kernel works as intended
    ...
    class GraphGP(SIGP):
        def __init__(self, train_x, train_y, likelihood):
            ...
            self.covariance = RandomWalkLabeledKernel(node_label=node_label)
    ...
    X, y = loader.features[:50], loader.labels[:50]
...
def test_random_walk_labeled_kernel_edge_label(edge_label):
    """
    Test if random walk (labelled) kernel fails consistently
    ...
    class GraphGP(SIGP):
        def __init__(self, train_x, train_y, likelihood):
            super().__init__(train_x, train_y, likelihood)
            self.mean = gpytorch.means.ConstantMean()
            self.covariance = RandomWalkLabeledKernel(
