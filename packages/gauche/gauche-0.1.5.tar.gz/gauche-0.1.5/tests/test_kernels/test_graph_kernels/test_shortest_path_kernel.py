"""
Unit tests for the shortest path graph kernel.
"""
...
from gauche.kernels.graph_kernels import ShortestPathKernel
...
def test_shortest_path_kernel():
    """
    Test if shortest path kernel works as intended
    ...
    class GraphGP(SIGP):
        def __init__(self, train_x, train_y, likelihood):
            ...
            self.covariance = ShortestPathKernel()
...
def test_shortest_path_kernel_node_label(node_label):
    """
    Test if shortest path kernel fails consistently
    ...
    class GraphGP(SIGP):
        def __init__(self, train_x, train_y, likelihood):
            super().__init__(train_x, train_y, likelihood)
            self.mean = gpytorch.means.ConstantMean()
            self.covariance = ShortestPathKernel(node_label=node_label)
...
def test_shortest_path_kernel_edge_label(edge_label):
    """
    Test if shortest path kernel fails consistently
    ...
    class GraphGP(SIGP):
        def __init__(self, train_x, train_y, likelihood):
            super().__init__(train_x, train_y, likelihood)
            self.mean = gpytorch.means.ConstantMean()
            self.covariance = ShortestPathKernel(edge_label=edge_label)
