...
def test_shortest_path_labeled_kernel_node_label(node_label):
    """
    Test if shortest path (labelled) kernel works as intended
    ...
    class GraphGP(SIGP):
        def __init__(self, train_x, train_y, likelihood):
            ...
            self.covariance = ShortestPathLabeledKernel(node_label=node_label)
...
def test_shortest_path_labeled_kernel_edge_label(edge_label):
    """
    Test if shortest path (labelled) kernel fails consistently
    ...
    class GraphGP(SIGP):
        def __init__(self, train_x, train_y, likelihood):
            super().__init__(train_x, train_y, likelihood)
            self.mean = gpytorch.means.ConstantMean()
            self.covariance = ShortestPathLabeledKernel(
