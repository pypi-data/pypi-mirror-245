...
@memory.cache
def sort_hashmap(test_index, similarities):
...
def sample_similar_examples(test_index, similarities, df_training, few_shot_size):
    ...
    items_indices = sort_hashmap(test_index, similarities)
    ...
    df_pro_instances.sort_values(by="id", key=lambda column: column.map(lambda v:items_indices.index(v)), inplace=True)
    df_con_instances.sort_values(by="id", key=lambda column: column.map(lambda v:items_indices.index(v)), inplace=True)
        #df_training.sort_values(by="id", key=lambda column: column.map(lambda v:print(v)), inplace=True)

    df_sampled_pro_instances = df_pro_instances.head(half_few_shot_size)
    df_sampled_con_instances = df_con_instances.head(half_few_shot_size)
    df_few_shots = pd.concat([df_sampled_pro_instances, df_sampled_con_instances])
