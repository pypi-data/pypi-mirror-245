...
def run_instructiona_fine_tuning_similar(config, params, experiment, validate, logger, topic_count, similarity, fine_tune=True):
    all_test_preds = []
    all_test_labels = []
    cutoff_len = 256
...
def run_instructional_fine_tuning(config, params, experiment, validate, logger, topic_count=None, sampling_strategy=None, similarity=None, fine_tune=True):

    if sampling_strategy =="similar":
        return run_instructiona_fine_tuning_similar(config, params, experiment, validate, logger, topic_count, similarity, fine_tune=fine_tune)
    else:
        cutoff_len= 256
        path_template = get_template_path()
        model_name = config["model-name"]
