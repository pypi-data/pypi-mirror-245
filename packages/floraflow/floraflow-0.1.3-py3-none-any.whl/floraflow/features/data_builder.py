...
import numpy as np
import floraflow.models as lm
import cv2 

from floraflow.features import segmentation
# data_dir= 'sources/nonsegmentedv2/'
data_dir:str = '/kaggle/input/v2-plant-seedlings-dataset'
...
def create_dataset_from_directory(data_dir: str, train_size: float = 0.9, shuffle: bool = True, excluded_folder: str = 'nonsegmentedv2') -> ImageDataWrapper:
    """
    ...
    Args:
        ...
        excluded_folder (str): Folder to exclude from the dataset. Defaults to 'nonsegmentedv2'.
    Returns:
        ImageDataWrapper: The dataset.
    """
    dataset_path = Path(data_dir)
    # Exclude the directory during the globbing itself
    images = list(dataset_path.glob(r'**/*.png'))
    images = [img for img in images if excluded_folder not in str(img)]

    labels = [img.parents[0].stem for img in images]
    ...
    train_df, test_df = train_test_split(data, train_size=train_size, shuffle=shuffle)
