            ...
            if graph.segments:
                for segment_name, segment_datas in graph.segments.items():
                    gfa_writer.write("S\t"+f"{segment_name}\t{segment_datas['seq'] if 'seq' in segment_datas else 'N'*segment_datas['length']}\t" + '\t'.join(
                        [f"""{key}:{GFAParser.get_python_type(value)}:{str(value).replace("'",'"')}""" if not key.startswith('ARG') else str(value) for key, value in segment_datas.items() if key not in ['length', 'seq']])+"\n")
            if graph.lines:
