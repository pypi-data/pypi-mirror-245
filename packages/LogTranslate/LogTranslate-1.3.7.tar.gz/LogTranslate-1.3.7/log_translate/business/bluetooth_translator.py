...
def bt_stack(msg):
    ...
    if "bta_jv_rfcomm_connect: sec_id=0" in msg:
        ...
        return Log(translated=f">>>>>>>>>> 系统蓝牙问题,可能是因为蓝牙协议栈资源不足无法分配Security ID（安全标识符）导致无法为通道[ch={result.group(1)}]创建RFCOMM连接，建议重新关闭开启系统蓝牙再试试 <<<<<<<< ",level=Level.e)
    if "is zero or BTM_SetSecurityLevel failed" in msg:
        result = re.search("remote_scn:(\d+)",msg)
        return Log(translated=f">>>>>>>>>> 系统蓝牙问题,SM通过BTM_SetSecurityLevel函数设置安全级别失败，导致无法为通道[ch={result.group(1)}]创建RFCOMM连接，建议重新关闭开启系统蓝牙再试试 <<<<<<<< ",level=Level.e)
    if "RFCOMM_CreateConnectionWithSecurity: no resources" in msg:
        return Log(translated=">>>>>>>>>> 系统蓝牙问题,蓝牙协议栈资源不足SM无法设置连接的安全级别导致无法创建rfcomm连接，建议重新关闭开启系统蓝牙再试试 <<<<<<<< ",level=Level.e)
...
def BluetoothQualityReportNativeInterface(msg):
    ...
    if "mNoRxCount" in msg:
        ...
        return Log(translated=f' xxxxxxxxxxxxxxxx 手机没收到设备的心跳包回复，导致连接超时断开({result.group()}) norx值比较大就有问题 xxxxxxxxxxxxxxxx ')
    return None
