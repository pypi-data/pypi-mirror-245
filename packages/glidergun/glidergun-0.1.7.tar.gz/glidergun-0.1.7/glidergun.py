...
from typing import Callable, Iterable, List, Literal, Tuple, Union, overload
...
def focal(grid, func: Callable[[ndarray], Union[float, int]], buffer: int = 1):
    blocks = grid._blocks(buffer)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        return grid._create(np.array([list(map(func, block)) for block in blocks]))


def focal2(
    grid1: Grid,
    grid2: Grid,
    f: Callable[[ndarray, ndarray], Union[float, int]],
    buffer: int,
):
    blocks1 = grid1._blocks(buffer)
    blocks2 = grid2._blocks(buffer)

    rows = []

    for y in range(grid1.height):
        row = list(zip(blocks1[y], blocks2[y]))
        rows.append([f(*row[x]) for x in range(grid1.width)])
