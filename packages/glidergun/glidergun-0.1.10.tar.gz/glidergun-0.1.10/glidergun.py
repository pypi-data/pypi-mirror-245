...
class Grid:
    ...
    def _focal(self, func: Callable, buffer: int = 1):
        blocks = self._blocks(buffer)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=RuntimeWarning)
            with futures.ProcessPoolExecutor() as executor:
                return self._create(np.array(list(executor.map(func, blocks))))

    def focal(self, func: Callable[[ndarray], Any], buffer: int = 1) -> "Grid":
        return focal(lambda t: func(t[0]), buffer, self)[0]
    ...
    def to_points(self) -> Iterable[Tuple[float, float, Union[float, int]]]:
        n = self.cell_size / 2
...
def focal(func: Callable, buffer: int, *grids: Grid) -> Tuple[Grid, ...]:
    grids_adjusted = standardize(True, *grids)
    block_stacks = np.stack([g._blocks(buffer) for g in grids_adjusted], axis=2)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        rows = [[func(row) for row in block_stack] for block_stack in block_stacks]
        count = len(rows[0][0]) if isinstance(rows[0][0], tuple) else 1
        arrays = [
            [
                [value[i] if isinstance(value, tuple) else value for value in row]
                for row in rows
            ]
...
def focal_pearson(grid1: Grid, grid2: Grid, buffer: int = 1) -> Grid:
    ...
    return focal(pearson, buffer, grid1, grid2)[0]
...
def focal_ttest(grid1: Grid, grid2: Grid, buffer: int = 1) -> TtestResult:
    ...
    result = focal(ttest, buffer, grid1, grid2)
    return TtestResult(result[0], result[1])
