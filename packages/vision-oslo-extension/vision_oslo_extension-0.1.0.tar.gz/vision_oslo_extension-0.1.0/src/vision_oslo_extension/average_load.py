    ...
    if option == "3":
        ...
        for supply in supply_list:
            ...
            if not SharedMethods.check_existing_file(filename):
                print("d4 file {} do not exist. Select option 2 to proceed.".format(supply))
                return False
        filename = simname + ".osop.mxn"
        if not SharedMethods.check_existing_file(filename):
            print("File {} do not exist. Select option 2 to proceed.".format(filename))
            return False
...
def get_result_range(start,total,space,fsnumber):
    ...
    # 1
    data_range = [f"M{start + 2}:W{start + total + 1}",f"M{start + total + space + 1}:Z{start + total*2 + space + 1}", \
...
def condtional_formating(sheet,range_list):
    ...
    red_fill = PatternFill(start_color="FF7F7F", end_color="FF7F7F", fill_type="solid")  # Red (50% S)
    yellow_fill = PatternFill(start_color="FFFF00", end_color="FFFF00", fill_type="solid")  # 255,152,51 (80% S)
    green_fill = PatternFill(start_color="80FF80", end_color="80FF80", fill_type="solid")  # Green (50% S)
