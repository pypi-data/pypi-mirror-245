...
# host options
######################################################################
register(options.ListOption('host.free.ports',
                            default='',
                            help_string='''\
...
register(options.Option('vm.manage_etc_hosts', default='localhost',
                        help_string='''\
...
# MISSINGTESTS: vila 2023-10-11 Test the ../scripts/first-use/*lxd scripts
# using the following `lxd.install.*` options. Separating the uninstall was not
# enough to inerate safely, this needs to happen in a proper lxd container or
...
and its content, one package by line, is inserted in the list.
'''))
register(options.Option('lxd.install.storage.backend',
                        default=None,
                        help_string='''The storage driver for lxd.'''))
register(options.Option('lxd.install.storage.pool',
                        default=None,
                        help_string='''The pool name for lxd.'''))
register(options.Option('lxd.install.storage.device',
                        default=None,
...
'byovm setup' command).
'''))
# FIXME: Provide a lxd.id_map option as a file containing the `raw.idmap` lxc
# config key -- vila 2023-12-08
...
register(options.Option('docker.base',
...
byov will append --file {docker.file} and the right working directory.'''))
register(options.ListOption(
...
register(options.ListOption(
    ...
    default='docker, container, stop, --time, {docker.container.stop.timeout},'
    ' {vm.name}',
    help_string='''docker command to stop the container.'''))
register(options.Option('docker.container.stop.timeout',
                        default='10',
                        help_string='''\
