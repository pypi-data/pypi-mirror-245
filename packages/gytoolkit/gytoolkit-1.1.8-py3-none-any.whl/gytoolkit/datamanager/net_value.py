...
class NetValueLoader(BaseDataLoader):
    ...
    def filter(self, nvdf:pd.DataFrame, reg_ids=None, start_date=None, end_date=None):
        if nvdf.empty:
            return nvdf
    ...
    def load_ppw(self, reg_ids=None, start_date=None, end_date=None):
        ...
        if reg_ids:
            ...
            ppwnv = ppwnvformatter(
                api.get_netvalue(fund_info.index, start_date, end_date), df=True
            )
        else:
            ppwnv = ppwnvformatter(
                api.get_netvalue(start_date=start_date, end_date=end_date), df=True
            )
        return ppwnv

    def load(
        self, reg_ids=None, start_date=None, end_date=None, df=True
    ) -> Union[List[NetValueData], pd.DataFrame]:
        return super().load(
            reg_ids=reg_ids, start_date=start_date, end_date=end_date, df=df
        )
