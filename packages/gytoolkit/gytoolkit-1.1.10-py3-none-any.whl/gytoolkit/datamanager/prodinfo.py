    ...
    def load_local(self) -> Union[List[ProdInfoData], pd.DataFrame]:
        ...
        for file in file_paths:
            local_prodinfo_list.append(
                pd.read_excel(file).drop_duplicates(keep="last")
            )
        ...
        local_prodinfo.产品上线时间 = pd.to_datetime(local_prodinfo.产品上线时间,format="%Y%m%d")
    ...
    def filter(
        ...
        if reg_ids:
            if isinstance(reg_ids, str):
                reg_ids = [reg_ids]
            data = data[data["prodcode"].isin(reg_ids)]
