...
def tangent_space_embedding(
    ...
    if method == 'umap':    
        ...
        # T1 = time.time()
        import umap
        umap_embedding = umap.UMAP(n_neighbors=30,random_state=random_state, metric='euclidean',min_dist=min_dist).fit_transform(X=X_pca)
...
def variation_embedding(
    ...
    gene_var_norm_two = np.linalg.norm(gauge_vh[:, :2, :], axis=1)
    gene_var_norm_two[gene_var_norm_two==0] = 1
    pc_loadings_scale = gauge_vh[:, :2, :] /\
        gene_var_norm_two[:,np.newaxis,:] *\
    ...
    emb_variation = umap.UMAP(random_state=random_state, n_neighbors=30,metric=metric,min_dist=min_dist, spread=1, n_components=2).fit(emb_svd)
