...
class NieuwbouwspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
            ...
            #response.css('a.card.card-project.card-project-list::attr(href)') this includes all the projects on this webpage including the ones outside amsterdam found by the partersites
    ...
    def parse_project_page(self, response, projects = None, project = None, project_list = list, project_info = None):
        ...
        if house_types:
            ...
            for house_type_link in house_type_links:
                ...
                yield response.follow(house_type_link, callback=self.parse_house_type_page,cb_kwargs={'project_list':project_list,'project_info':project_info,'house_type_info_list':house_type_info_list,'expected_house_types':expected_house_types,'house_types':house_types,'house_type_info':house_type_info})
...
#set up a check point for the situation if there is no sepicfic information for house type collections
    ...
    def parse_house_type_page(self,response, project = None, projects = None, project_list = list, project_info = None, house_type_info_list = list,expected_house_types = int, house_types = list,house_type_info= dict):
        ...
        #schema = {
        #    "street_name": {"type": "string", lennght: 10, "default": "Unknown"}
        #}
...
"""
    def closed(self, reason):
        ...
        self.log("All possible project keys:")
        for key in self.all_possible_project_keys:
            self.log(key)

        # Print all possible house type keys
        self.log("All possible house type keys:")
        for key in self.all_possible_house_type_keys:
            self.log(key)
"""