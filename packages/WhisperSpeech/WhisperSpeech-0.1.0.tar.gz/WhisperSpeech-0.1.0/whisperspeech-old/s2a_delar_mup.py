# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb.
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 1
import io
import time
import random
import dataclasses

# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 2
import torch
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 3
from pathlib import Path
import json
from fastprogress import progress_bar, master_bar
import pandas as pd

# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 4
from .train import *
from .modules import *

# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 8
def rand(start, end):
    return random.random() * (end - start) + start

# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 9
import torch.nn.functional as F
...
class SADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Ptoks = torch.load(self.basedir/row['pitch'], map_location='cpu')[j]
        ...
        Ptoks = Ptoks[:int(Atoks.shape[-1]//3*2 + .99)]
        return (
            F.pad(Stoks, (0, 1500 - Stoks.shape[-1]), value=1024),
            F.pad(Ptoks, (0, 1500 - Ptoks.shape[-1]), value=64),
            F.pad(Atoks, (0, 2250 - Atoks.shape[-1]), value=-100),
            torch.tensor(self.speakers[row['speaker']]),
        )

# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 10
import math
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 20
import pylab as plt
import fastprogress
import IPython
import numpy as np
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 21
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 22
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 23
class DelSumDecoder(nn.Module):
    def __init__(self, depth=6, n_head=6, head_width=64, qk_scale=1, ffn_mult=4, length=2250, codes=1024, quantizers=8, linear_heads=True, rope=False, pos_embs=None):
        ...
        self.layers = nn.ModuleList([
            ResidualAttentionBlock(width, n_head, qk_scale=qk_scale, ffn_mult=ffn_mult, rope=rope) for _ in range(math.floor(depth))
        ])
...
class Tunables:
    ...
    # FIXME: what about old serialized models
...
class SADelARTransformer(nn.Module):
    def __init__(self, depth=3, ctx_n=2250, n_head=3, head_width=64, ffn_mult=4,
        ...
        self.quantizers = quantizers
        width = n_head * head_width
        store_attr("depth,ctx_n,n_head,head_width,ffn_mult,quantizers,speaker_map")
        self.width = width
        ...
        self.pitch_embedding = nn.Embedding(64+1, width)
        ...
        self.pitch_ln_post = LayerNorm(width)
    ...
    def forward(self, Stoks, qpitch, Atoks, speakers, noloss=False):
        semb = self.embed_stoks(Stoks)
        if self.tunables.use_pitch:
            b,n = qpitch.shape
            x = qpitch.reshape(b,n//2,2)
            x = x.repeat_interleave(2, -1)[:,:,:3]
            x[:,:,1] = 64
            x = x.reshape(b,n//2*3)
            pemb = self.pitch_embedding(x.to(torch.long))
        with record_function("encoder"):
            if self.positional_embeddings is not None: semb = semb + self.positional_embeddings
            xenc = self.ln_post(self.encoder(semb))
            if self.tunables.use_pitch:
                xenc = xenc + self.pitch_ln_post(self.pitch_encoder(pemb))
    ...
    @torch.no_grad()
    def generate(self, stoks, ptoks, speakers, N=None, T=0.7, top_k=None):
        dev = self.device
        N = N or len(stoks) * 3 // 2
        stoks = F.pad(stoks.to(dev), (0, self.ctx_n * 2 // 3 - len(stoks)), value=1024).unsqueeze(0)
        ptoks = F.pad(ptoks.to(dev), (0, self.ctx_n * 2 // 3 - len(ptoks)), value=64).unsqueeze(0)
        ...
        for i in progress_bar(range(N)):
            p = self(stoks, ptoks, toks[:,:,:i], speakers, noloss=True)
            last_p = p[0,:,-1]
            if top_k:
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT Pitch.ipynb 24
def make_model(size:str, quantizers:int=4, tunables:Tunables=Tunables(), dataset:torch.utils.data.Dataset=None):
    assert(dataset is not None)
    kwargs = dict(speaker_map=dataset.speakers, quantizers=quantizers, tunables=tunables)
    if size == 'micro':
