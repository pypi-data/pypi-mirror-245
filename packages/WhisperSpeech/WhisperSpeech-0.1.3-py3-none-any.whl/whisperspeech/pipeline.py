...
class Pipeline:
    ...
    def generate_atoks(self, text, speaker=None, lang='en'):
        if speaker is None: speaker = self.default_speaker
        text = text.replace("\n", " ")
        stoks = self.t2s.generate(text, cps=14, lang=lang)
    ...
    def generate_to_file(self, fname, text, speaker=None):
        self.vocoder.decode_to_file(fname, self.generate_atoks(text, speaker))
        
    def generate_to_notebook(self, text, speaker=None, lang='en'):
        self.vocoder.decode_to_notebook(self.generate_atoks(text, speaker, lang=lang))
