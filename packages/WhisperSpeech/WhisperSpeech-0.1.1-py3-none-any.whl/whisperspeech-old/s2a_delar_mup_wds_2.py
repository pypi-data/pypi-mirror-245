# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb.
...
# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 1
import io
import time
import math
import random
...
# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 2
import torch
...
# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 3
from pathlib import Path
import json
from fastprogress import progress_bar, master_bar
import webdataset as wds

# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 4
from .train import *
from .modules import *
from . import vq_stoks

# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 8
def rand(start, end):
    return random.random() * (end - start) + start

# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 9
...
# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 10
...
# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 15
def load_datasets(
...
# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 29
import pylab as plt
import fastprogress
import IPython
import numpy as np
...
# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 30
...
# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 31
...
# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 32
...
#         self.register_buffer('val_start_true', torch.zeros(self.quantizers).cuda())
#         self.register_buffer('val_start_total', torch.zeros(self.quantizers).cuda())
...
#                 valid_Atoks[:,10:] = False
#                 self.val_start_true[i] += (logits[:,i,i:].argmax(-1)[valid_Atoks] == Atoks_i[valid_Atoks]).float().sum()
#                 self.val_start_total[i] += valid_Atoks.float().sum()
...
#         metrics.update({
#             f'acc_start_{i}':x.item() for i,x in enumerate(self.val_start_true / self.val_start_total)
#         })
        self.val_true[:] = 0
        self.val_total[:] = 0
#         self.val_start_true[:] = 0
#         self.val_start_total[:] = 0
        return metrics
    ...
    @torch.no_grad()
    def generate(self, stoks, speakers, initial_atoks=None, N=None, T=0.7, top_k=None, show_progress_bar=True):
        dev = self.device
        if self.stoks_len == 1500:
        ...
        if initial_atoks is not None:
            toks[0,:,:initial_atoks.shape[-1]] = initial_atoks
            it = range(initial_atoks.shape[-1],N)
        else:
            it = range(0,N)
...
# %% ../nbs/3C. Semantic to acoustic modeling 2-Copy1.ipynb 33
