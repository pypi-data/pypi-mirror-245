# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb.
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 1
import io
import time
import random
import dataclasses

# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 2
import torch
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 3
from pathlib import Path
import json
from fastprogress import progress_bar, master_bar
import pandas as pd

# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 4
from .train import *
from .modules import *

# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 8
def rand(start, end):
    return random.random() * (end - start) + start

# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 9
import torch.nn.functional as F
...
class SADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        jA = j * 2250
        Stoks = torch.load(self.basedir/row['semantic'], map_location='cpu')[j,::2]
        ...
        Stoks = Stoks[:int(Atoks.shape[-1]//3 + .99)]
        return (
            F.pad(Stoks, (0, 750 - Stoks.shape[-1]), value=1024),
            F.pad(Atoks, (0, 2250 - Atoks.shape[-1]), value=-100),
            torch.tensor(self.speakers[row['speaker']]),
        )

# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 10
import math
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 13
import pylab as plt
import fastprogress
import IPython
import numpy as np
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 14
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 15
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 16
...
#         x = Stoks.reshape(b,n//2,2)
        x = Stoks[:,::3].repeat_interleave(9, -1)
#         x[:,:,1] = 1024
#         x = x.reshape(b,n//2*3)
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT cut-semantic.ipynb 17
