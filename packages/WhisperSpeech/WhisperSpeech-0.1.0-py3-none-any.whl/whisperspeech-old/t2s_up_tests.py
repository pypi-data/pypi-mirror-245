# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/5. Text to semantic token modeling μP-Copy1.ipynb.

# %% auto 0
__all__ = ['load_datasets', 'rand', 'Tunables', 'Encoder', 'Decoder', 'TSARTransformer', 'make_model']

# %% ../nbs/5. Text to semantic token modeling μP-Copy1.ipynb 1
import dataclasses
import random
import math
import torch
...
# %% ../nbs/5. Text to semantic token modeling μP-Copy1.ipynb 2
from pathlib import Path
import pylab as plt
import pandas as pd
import numpy as np

# %% ../nbs/5. Text to semantic token modeling μP-Copy1.ipynb 3
import whisper
from whisperspeech.train import *
from whisperspeech.modules import *

# %% ../nbs/5. Text to semantic token modeling μP-Copy1.ipynb 30
import torch.nn.functional as F

class SADataset(torch.utils.data.Dataset):
    def __init__(self, data, tokenizer, stoks_config):
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        Stoks = row['stoks']#[:math.ceil(t/30*self.stoks_per_second)]
        ...
        return F.pad(torch.tensor(Ttoks), (0, self.ttoks_len - len(Ttoks)), value=self.tokenizer.eot).to(torch.long), \
               F.pad(torch.tensor(Stoks), (0, self.stoks_len - len(Stoks)), value=self.stoks_codes).to(torch.long)

# %% ../nbs/5. Text to semantic token modeling μP-Copy1.ipynb 33
import re
...
# %% ../nbs/5. Text to semantic token modeling μP-Copy1.ipynb 40
def rand(start, end):
...
# %% ../nbs/5. Text to semantic token modeling μP-Copy1.ipynb 41
...
# %% ../nbs/5. Text to semantic token modeling μP-Copy1.ipynb 42
class Decoder(nn.Module):
    def __init__(self, depth=6, stoks_width=384, width=384, n_head=6, length=1500, codes=1024, ffn_mult=4, pos_embs=None, tunables=Tunables()):
        ...
        self.embedding = nn.Embedding(codes+1, stoks_width)
        if self.emb_factor:
...
# %% ../nbs/5. Text to semantic token modeling μP-Copy1.ipynb 43
class TSARTransformer(nn.Module):
    def __init__(self, depth=6, n_head=6, head_width=64, ffn_mult=4, language='en',
        ...
        super().__init__()
        store_attr("depth,n_head,head_width,stoks_width,ttoks_len,stoks_len,ttoks_codes,stoks_codes,language")
    ...
    def generate(self, txt, N=None, T=0.7, top_k=None, show_progress_bar=True):
        ...
        for i in it:
            ...
            toks[0,i] = tok
            if toks[0,i] == self.stoks_codes: return toks[0,:i]
        return toks[0]

# %% ../nbs/5. Text to semantic token modeling μP-Copy1.ipynb 44
def make_model(size:str, tunables:Tunables=Tunables(), dataset:SADataset=None):
    kwargs = dict(stoks_len = dataset.stoks_len, stoks_codes=dataset.stoks_codes, ttoks_len = dataset.ttoks_len, tunables=tunables)
    if size == 'micro':
