 	...
 	"""
        ...
        if self.event in self.section_navigation_keys+self.sub_navigation_keys:
            ...
            if nav_data != None:
                ...
                if len(refference_object)<=sub_section_number:
            ...
            if self.event == '-CLEAR_CHUNKS-':
                ...
                data = self.add_to_request(self.window_mgr.get_values()[self.script_event_js['-FILE_TEXT-']])
                self.update_request_data(data)
                request_data=True
            if not request_data:
