...
def utilities():
    ...
    json_to_string=make_component("Checkbox",'Json to String', key=text_to_key("json to string",section='responses'), enable_events=True)
...
def abstract_browser_layout(section=None,extra_component=None):
    ...
    if extra_component != None:
        extra_frame = make_component("Frame",'output',layout=ensure_nested_list(extra_component))
...
def get_tab_layout(title,layout=None):
    if not layout:
        layout = [get_nav_buttons(f"{title} section",section_type='query'),make_component("Multiline",key=text_to_key(title), **expandable())]
    return make_component("Tab",title.upper(),ensure_nested_list(layout))
...
def get_chunked_sections():
    return [get_nav_buttons(section='chunk section',section_type='query'),
            get_nav_buttons(section='chunk',section_type='sub'),
...
def get_prompt_data_section():
    return [get_nav_buttons(section='prompt_data section',section_type='query'),
...
def get_request_section():
    return [get_nav_buttons(section='request section',section_type='query'),
            [make_component("Frame",'', layout=[[get_right_click_multi(key=text_to_key('request'),args={"enable_events":True,**expandable()})]],**expandable())]]
def get_nav_buttons(section,section_type='sub'):
    return [make_component("Push"),make_component('Frame',f"{section_type} itteration",ensure_nested_list([make_component("Button",button_text="<-",key=text_to_key(f"{section} back"),enable_events=True),
                                                                                                          make_component("input",default_text='0',key=text_to_key(f"{section}  number"),size=(4,1)),
                                                                                                          make_component("Button",button_text="->",key=text_to_key(f"{section} forward"),enable_events=True)]))
            ,make_component("Push")]
def get_query_section():
    return [get_nav_buttons(section='query section',section_type='query'),
            get_nav_buttons(section='query',section_type='sub'),
...
window = make_component('Window',layout=ensure_nested_list(get_total_layout()))
while True:
    window.read()
