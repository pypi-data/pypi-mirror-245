...
class AbstractBrowser:
    ...
    def __init__(self, window_mgr=None,window_name=None,window=None):
        ...
        self.key_list = ['-BROWSER_LIST-','-CLEAR_LIST-','-MOVE_UP_LIST-','-MOVE_DOWN_LIST-','-ADD_TO_LIST-',"-FILES_BROWSER-","-DIR-","-DIRECTORY_BROWSER-","-FILES_LIST-","-SCAN_MODE_ALL-","-SELECT_HIGHLIGHTED-","-SCAN-", "-SELECT_HIGHLIGHTED-", "-MODE-", "-BROWSE_BACKWARDS-", "-BROWSE_FORWARDS-"]+self.modes
    ...
    def get_scan_browser_layout(section=None, extra_buttons=[]):
        ...
        control_buttons_layout = [
            ...
            sg.Button('^', key=text_to_key(text='move up list',section=section)),
            sg.Button('Add', key=text_to_key(text='add_to_list',section=section)),
            sg.Button('v', key=text_to_key(text='move down list',section=section)),
    ...
    def while_static(self,event_key_js,values,window):
        ...
        if self.event_key_js['found'] == '-MODE-':
            ...
            self.window.Element(self.event_key_js['-MODE-']).update(text=f"{capitalize(self.scan_mode)} Scan Mode")
            self.scan_it(self.return_directory())
        if self.event_key_js['found'] in ['-MOVE_UP_LIST-','-MOVE_DOWN_LIST-']:
            self.list_it()
        ...
        def add_position(list_obj, var, num, direction):
            ...
            for i, obj in enumerate(list_obj):
                if i == num:
                    ...
                    new_list.append(obj)
            return new_list
        list_values = self.window[self.event_key_js['-FILES_LIST-']].Values
        list_value = self.values[self.event_key_js['-FILES_LIST-']]
        if list_value:
            list_value = list_value[0]
            position = get_list_num(list_values, list_value)
            if self.event.split('_')[0].lower() == 'move':
                direction = self.event.split('_')[1]
                if direction.lower() == 'up' and position > 0:
                ...
                self.window[self.event_key_js['-FILES_LIST-']].update(values=list_values)
                self.window[self.event_key_js['-FILES_LIST-']].update(
                    set_to_index=[get_list_num(list_values, list_value)])
            elif event == 'remove':
                list_values.remove(list_value)
                self.window[self.event_key_js['-FILES_LIST-']].update(values=list_values)
        self.window[self.event_key_js['-FILES_LIST-']].update(value=str(list_values))
