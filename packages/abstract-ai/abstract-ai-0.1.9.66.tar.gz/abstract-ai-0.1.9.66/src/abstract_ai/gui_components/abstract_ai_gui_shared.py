...
def abstract_browser_layout(section=None,extra_component=None):
    ...
    key_selection=make_component("Frame",'response key',layout=ensure_nested_list(make_component("combo",[], key=text_to_key("response key selection",section='responses'), enable_events=True)))
    ...
    extra_buttons = [make_component("Button",'CHUNK_DATA',key=text_to_key(text='add file to chunk',section=section),enable_events=True),
                     make_component("Frame",'chunk title',layout=[collate_responses])]
...
browser_mgr = AbstractBrowser()
window= make_component("Window",layout=get_total_layout())
while True:
    event,values=window.read()
    browser_mgr.handle_event(event,values,window)
