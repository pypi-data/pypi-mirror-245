...
class AbstractNavigationManager:  # Replace with your actual class name
    ...
    def get_navigation_data(self, navigation_type):
        ...
        """
        section_number = self.window_mgr.get_from_value(f"-{navigation_type.upper()}_SECTION_NUMBER-")
    ...
    def get_sub_section_number(self, navigation_type):
        ...
        number_key = self.window_mgr.get_from_value(self.number_key)
        if number_key != None:
            number_key = int(number_key)
        return number_key if self.window_mgr.exists(self.number_key) else None
    ...
    def update_navigation_counters(self, nav_data, direction):
        ...
        if 'SECTION' in self.number_key:
            ...
            if len(reference_obj)-1>current_section_number:
                current_section_number = len(reference_obj)-1
