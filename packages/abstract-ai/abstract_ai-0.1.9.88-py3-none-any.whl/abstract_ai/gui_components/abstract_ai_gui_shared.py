...
def abstract_browser_layout(section=None,extra_component=None):
    ...
    chunk_title = make_component('Column',ensure_nested_list(make_component("Frame",'output',layout=ensure_nested_list(make_component("Input",key=text_to_key("chunk title",section=section),size=(15,1)))))),
...
def get_instructions():
    ...
    sub_layout = [make_component("Column", ensure_nested_list(sub_layout), **expandable(size=(800, 1600), scroll_vertical=True))]
...
def get_total_layout():
    prompt_tabs= get_prompt_tabs({"query":get_query_section(),"request":get_request_section(),"prompt data":get_prompt_data_section(),"instructions":get_instructions(),"chunks":get_chunked_sections()},args={**expandable(size=(int(0.3*window_width),int(window_height)))})
    return [
        [get_progress_frame()],
        [get_output_options()],
