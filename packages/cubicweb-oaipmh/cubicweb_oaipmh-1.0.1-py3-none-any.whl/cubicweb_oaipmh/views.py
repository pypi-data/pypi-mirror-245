...
class UTCParseError(Exception):
    ...
    def __init__(self, msg=None):
        super().__init__({"UTCParseError": msg})
...
class IdDoesNotExist(OAIError):
    ...
    def __init__(self, identifier):
        msg = f"no entity with OAI identifier {identifier} in repository"
...
class OAIRequest:
    ...
    def __init__(
        ...
        try:
            ...
            msg = f"'{date_part}' argument must be a UTCdatetime value"
...
class OAIRecord:
    ...
    def header(self, prefix):
        ...
        if prefix not in self.record.metadata_formats:
            raise OAIError(
                {"noRecordsMatch": f'unsupported metadata prefix "{prefix}"'}
            )
...
class OAIView:
    ...
    def __call__(self):
        ...
        assert encoding == "UTF-8", f"unexpected encoding {encoding}"
...
class OAIErrorView(OAIView):
    ...
    def errors(self):
        ...
        if verb:
            return {"badVerb": f'illegal verb "{self.oai_request.verb}"'}
        else:
...
class OAIIdentifyView(OAIView):
    ...
    def errors(self):
        if set(self.request.params) - {"verb", "vid"}:
    ...
    def verb_content(self):
        ...
        yield E(
            "earliestDatestamp",
            datetime_isoformat(oai.earliest_datestamp().astimezone(pytz.utc)),
        )
