...
class ColumnWhereExec:
    ...
    def csv(self, file_name: str, delimiter=',', header=True, encoding='utf-8'):
        ...
        """
        self.where_exec.load(*self.columns).csv(file_name, delimiter, header, encoding)

    def df(self):
        """
        sqlexec.table('person').columns('name', 'age').where(name__eq='李四').df()
        """
        return self.where_exec.load(*self.columns).df()

    def json(self, file_name: str, encoding='utf-8'):
        """
        sqlexec.table('person').columns('name', 'age').where(name__eq='李四').json('test.json')
        """
        self.where_exec.load(*self.columns).json(file_name, encoding)
...
class ColumnExec:
    ...
    def csv(self, file_name: str, delimiter=',', header=True, encoding='utf-8'):
        """
        sqlexec.table('person').columns('name', 'age').csv('test.csv')
        """
        self.table_exec.load(*self.columns).csv(file_name, delimiter, header, encoding)

    def df(self):
        """
        sqlexec.table('person').columns('name', 'age').df()
        """
        return self.table_exec.load(*self.columns).df()

    def json(self, file_name: str, encoding='utf-8'):
        """
        sqlexec.table('person').columns('name', 'age').json('test.json')
        """
        self.table_exec.load(*self.columns).json(file_name, encoding)
