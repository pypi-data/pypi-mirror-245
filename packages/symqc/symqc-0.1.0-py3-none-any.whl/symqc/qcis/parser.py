import tempfile
from typing import Any, Callable, Dict, Iterable, List, Optional, Tuple, Set, cast

import ply.yacc as yacc

from symqc.qcis.instr import QCISOpCode, QCIS_instr
from symqc.qcis.lexer import QCISLexer
...
OP_CODE_CONVERTER = {
    ...
    "TOFFOLI": QCISOpCode.Toffoli,
    "FREDKIN": QCISOpCode.Fredkin,
}
...
class QCISParser(object):
    ...
    def p_instruction_n_qubit(self, p):
        ...
        self.__instructions.append(
            QCIS_instr(OP_CODE_CONVERTER[p[1]], qubits_list=p[2])
        )
    ...
    def p_instruction_3_qubit_0_param(self, p):
        """instruction : TOFFOLI QREG QREG QREG
        | FREDKIN QREG QREG QREG"""
        self.__qubit_names.update({p[2], p[3], p[4]})
        if p[1] == "TOFFOLI":
            self.__instructions.append(
                QCIS_instr(
    ...
    def parse(self, data: str) -> Tuple[bool, List[QCIS_instr], Set[str]]:
        """Parse the input QCIS program and return a list of instructions.

        Parameters
        ----------
        data : str
            The input QCIS program.

        Returns
        -------
        Tuple[bool, List[QCIS_instr], Set[str]]
            The first element is a boolean value indicating whether the parsing is successful.
            The second element is a list of QCIS instructions.
            The third is a list of qubit names used in the QCIS program.
        """
        ...
        self.parser.parse(data.upper(), lexer=self.lexer)
