...
class ThinkTrader(base1x.Singleton):
    ...
    def single_available(self, num: int) -> float:
        ...
        can_use_cash = acc_cash + can_use_amount - self._config.keep_cash
        # 5. 计算预留仓位, 给下一个交易日留position_ratio仓位
        reserve_cash = base1x.float_round(acc_total * self._config.position_ratio)
        ...
        if available > acc_cash:
            logger.warning('!!! 持仓占比[{}%], 已超过可总仓位的[{}%], 必须在收盘前择机降低仓位, 以免影响下一个交易日的买入操作 !!!', base1x.float_round(100*(acc_value / acc_total)),
                           base1x.float_round(100 * (1 - self._config.position_ratio)))
        # 8. 计算单一标的可用资金
        ...
        elif single_funds_available < self._config.buy_amount_min:
            return 0.00
    ...
    def calculate_buy_fee(self, price: float, volume: int) -> float:
        ...
        _stamp_duty_fee = base1x.float_round(volume * price * self._config.stamp_duty_rate_for_buy)
        # 2. 过户费, 按照股票数量, 双向, 0.06%
        _transfer_fee = base1x.float_round(volume * self._config.transfer_rate)
        # 3. 券商佣金, 按照成交金额计算, 双向, 0.025%
        _commission_fee = base1x.float_round(volume * price * self._config.commission_rate)
    ...
    def order(self, code: str, order_type: int, order_volume: int, price_type: int, price: float,
        ...
        """
        order_id = self.xt_trader.order_stock(self.account, code, order_type, order_volume, price_type, price,
    ...
    def total_strategy_orders(self, strategy_code: int) -> int:
        ...
        if len(df) == 0:
            return 0
        # print(df)
        condition = (df['order_remark'] == 'tick') & (df['strategy_name'] == strategy_code)
        df = df[condition]
        return len(df)
