...
struct PlyFile::PlyFileImpl {
  ...
  size_t read_property_binary(const size_t& stride, void* dest, size_t& destOffset, size_t destSize,
                              std::istream& is);
  size_t read_property_ascii(const Type& t, const size_t& stride, void* dest, size_t& destOffset,
  ...
  void write_property_binary(std::ostream& os, const uint8_t* src, size_t& srcOffset,
...
void PlyFile::PlyFileImpl::read_header_text(std::string line, std::vector<std::string>& place,
                                            int erase) {
...
size_t PlyFile::PlyFileImpl::read_property_binary(const size_t& stride, void* dest,
...
size_t PlyFile::PlyFileImpl::read_property_ascii(const Type& t, const size_t& stride, void* dest,
                                                 size_t& destOffset, size_t destSize,
                                                 std::istream& is) {
  if (destOffset + stride > destSize) {
    throw std::runtime_error("unexpected EOF. malformed file?");
...
void PlyFile::PlyFileImpl::write_property_ascii(Type t, std::ostream& os, const uint8_t* src,
                                                size_t& srcOffset) {
  switch (t) {
...
void PlyFile::PlyFileImpl::write_property_binary(std::ostream& os, const uint8_t* src,
...
std::shared_ptr<PlyData> PlyFile::PlyFileImpl::request_properties_from_element(
  ...
  if (elementIndex >= 0) {
    ...
    if (keys_not_found.size()) {
      ...
      throw std::invalid_argument("the following property keys were not found in the header: " +
                                  ss.str());
    }
    ...
    for (const auto& key : propertyKeys) {
      ...
      if (result.second == false) {
        throw std::invalid_argument("element-property key has already been requested: " +
...
void PlyFile::PlyFileImpl::parse_data(std::istream& is, bool firstPass) {
  ...
  if (isBinary) {
    read = [this, &listSize, &dummyCount, &read_list_binary](
               PropertyLookup& f, const PlyProperty& p, uint8_t* dest, size_t& destOffset,
               size_t destSize, std::istream& _is) {
      if (!p.isList) {
    ...
    read = [this, &listSize, &dummyCount](PropertyLookup& f, const PlyProperty& p, uint8_t* dest,
      ...
      if (!p.isList) {
        read_property_ascii(p.propertyType, f.prop_stride, dest + destOffset, destOffset, destSize,
                            _is);
      } else {
        dummyCount = 0;
        ...
        for (size_t i = 0; i < listSize; ++i) {
          read_property_ascii(p.propertyType, f.prop_stride, dest + destOffset, destOffset,
                              destSize, _is);
        }
      }
    };
      ...
      for (auto& property : element.properties) {
        ...
        if (!lookup.skip) {
          ...
          if (firstPass) {
            ...
            read(lookup, property, helper->data->buffer.get(), helper->cursor->byteOffset, destSize,
                 is);
          }
        } else {
...
void PlyFile::add_properties_to_element(const std::string& elementKey,
  ...
  return impl->add_properties_to_element(elementKey, propertyKeys, type, count, data, listType,
                                         listCount);
}
