...
def proper_function_main( order, param, importmodule = None):
    """
    barebone function
    """
    import subprocess as sp
    ...
    if importmodule is None:
        print("X... THIS FUNCTION NEEDS TO IMPORT YOUR MODULE.PY")
        sys.exit(1)
    impome = "singlemod.py"
    impome = importmodule
...
def submit( xcore_function,   parameters, upload = None, server=None):
 ...
 2/LIST of parameters
    ...
    for i in range(nnodes):
        ...
        if upload is not None:
            # here i give one more param for import
            # NO res = client.submit( xcore_function, i , parameters[i] , kwargs={'importmodule': upload})
            res = client.submit( xcore_function, i , parameters[i] , upload)
        else:
