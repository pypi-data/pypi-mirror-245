...
from common import util, config, quota
...
class Image(ResourceBase):
    ...
    async def download_src_file(self, url, work_space):
        ...
        log.info(f"Download {name} from {url}.")
        ...
        for attempt_count in range(1, max_download_attempts + 1):
            try:
                ...
                if attempt_count == max_download_attempts:
                    await util.exec_cmd(f"rm -f {src_file}")
                    return
            except Exception as ex:
                log.error(f"Image download failed. {ex}")
                if attempt_count == max_download_attempts:
                    await util.exec_cmd(f"rm -f {src_file}")
                    return
    ...
    async def update_image_location(self, image_id, location):
        ...
        params = [{"op": "add",
                ...
                "value": {
                    "url": location,
                    "metadata": {"store": "rbd"}}}]
        resp = await OSImage(self.token_pack).patch(image_id, params)
        return resp
    ...
    async def task_add_image_from_res(self, id, req):
        ...
        if image_type == "instance":
            ...
            return
        rbd = RBD()
        pool = util.get_pool_by_volume_type(vol_type)
    ...
    async def task_copy_image(self, id, arg):
        image_id = id
        copy_zone = arg["zone_name"]
        rbd = RBD()
        log.info(f"Task copy image {image_id} to {copy_zone}.")
        ...
        project_id = await Project(token_pack).get_id_by_name(
                arg["project_name"])
        if not project_id:
            log.error(f"Get project {arg['project_name']} failed!")
            return
        ...
        headers = {"x-auth-token": token_pack["token"],
                    "content-type": "application/json"}
        params = {"source_zone": config.config["DEFAULT"]["zone"]}
    ...
    async def rh_get_quota(self):
        ...
        limits = await self.quota.get_quota_all_default_limits("glance")
        reg_quote_result = {}
        quota_result = {}
