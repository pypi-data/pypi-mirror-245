...
from common import util, config, validator, quota
...
class VolumeBackup(ResourceBase):
    ...
    async def import_snapshot_for_backup(self, name, vol_id, ss_id):
    ...
    async def import_snapshot(self, backup_id, inc):
        log.info("Import snapshot to backup.")
    ...
    async def task_add_volume_backup(self, backup, vol, ss_id_rbd):
        ...
        if ss_id_rbd:
            ss_id = await self.import_snapshot_for_backup(
        ...
        if not ss_id:
            ...
            return -1
        image_prop = ""
        pool = util.get_pool_by_volume_type(backup["volume_type"])
        ...
        if not inc:
            log.info(f"Clean up snapshot {ss_id} on volume.")
            await ss_ins.delete(ss_id)
            log.info(f"Clean up snapshot {ss_id} on backup.")
            await rbd.delete_snapshot("backup", backup_id,
                    f"snapshot-{ss_id}")
            ss_id = ""
        await self.import_snapshot(backup_id, inc)
    ...
    async def task_update_volume_backup(self, backup, cur_ss_id):
        ...
        if not obj:
            ...
            return -1
        if cur_ss_id:
            cur_ss_id = await self.import_snapshot_for_backup(
