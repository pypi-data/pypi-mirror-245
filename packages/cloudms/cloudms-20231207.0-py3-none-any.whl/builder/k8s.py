...
class K8sObject(BaseModel):
    ...
    node_subnet_address: IPv4Network = "192.168.200.0/24"
    node_subnet_id: UUID = ""
...
class K8s(ResourceBase):
    ...
    async def rh_post(self, req_data):
        ...
        cluster = {"name": req["name"],
                ...
                "api_addr": str(req["api_address"]),
                "api_addr_auto": True,
                "ingress_addr": str(req["ingress_address"]),
                "ingress_addr_auto": True,
                "pod_addr_block": str(req["pod_address_block"]),
                "service_addr_block": str(req["service_address_block"]),
                "node_subnet_addr": str(req["node_subnet_address"]),
                ...
                "status": "building"}
        if cluster["api_addr"]:
            cluster["api_addr_auto"] = False
        if cluster["ingress_addr"]:
            cluster["ingress_addr_auto"] = False
        if cluster["node_subnet_id"]:
            ...
            cluster["node_subnet_addr"] = obj["cidr"]
    ...
    async def build_data(self, cluster):
        ...
        if cluster["node_subnet_auto"]:
            block = IPv4Network(cluster["node_subnet_addr"])
    ...
    async def create_network(self, cluster):
        ...
        params = {"name": subnet_name,
                ...
                "cidr": str(cluster["node_subnet_addr"])}
    ...
    async def build_vip(self, cluster, type):
        ...
        fip_ins = FloatingIP(self.token_pack)
        fip = cluster[f"{type}_addr"]
        if cluster[f"{type}_addr_auto"]:
    ...
    async def build_node_ign(self, cluster, role, count):
        ...
        vars = {"ssh_key": key,
                ...
                "registry": config["kubernetes"]["registry"],
    ...
    async def provision_node(self, cluster, role):
        ...
        await util.exec_cmd(cmd)
        vars = {"repo_fedora_coreos": config["repo"]["fedora-coreos"],
        ...
        vars = {"api_int_vip": cluster["int_api_addr"],
                ...
                "registry": config["kubernetes"]["registry"],
                "registry_name": \
                    config["kubernetes"]["registry"].split(":")[0],
                "service_proxy": config["DEFAULT"]["service-proxy"]}
        ...
        for host in cluster["hosts"]:
            ...
            cmd = f"{self.svc_path}/k8sadm provision-{role}" \
                    f" {host['mgmt_addr']} {cluster_id}"
    ...
    async def join_node(self, cluster, role, hosts):
        ...
        for host in hosts:
            ...
            with open(f"{self.tmp_path}/join-{role}.yaml", "w") as fd:
                fd.write(t.render(vars))
            cmd = f"{self.svc_path}/k8sadm join-{role}" \
                    f" {host['mgmt_addr']} {cluster_id}"
            with open(self.build_log, "a") as fd:
                rc = await util.exec_cmd(cmd, output_file = fd)
            if rc:
    ...
    async def bootstrap_cluster(self, cluster):
        ...
        vars = {"name": cluster["name"],
                ...
                "api_port_backend": 8443,
                "api_name": "api." + cluster["domain"],
                "service_addr_block": cluster["service_addr_block"],
                "pod_addr_block": cluster["pod_addr_block"],
                "registry": config["kubernetes"]["registry"]}
    ...
    async def deploy_service(self, cluster):
        ...
        vars = {"registry": config["kubernetes"]["registry"]}
    ...
    async def delete_network(self, cluster):
        ...
        fip_ins = FloatingIP(self.token_pack)
        if cluster["api_addr_auto"]:
            if await self.delete_fip(fip_ins, cluster, "api_addr"):
                return -1
        if cluster["ingress_addr_auto"]:
            if await self.delete_fip(fip_ins, cluster, "ingress_addr"):
                return -1
