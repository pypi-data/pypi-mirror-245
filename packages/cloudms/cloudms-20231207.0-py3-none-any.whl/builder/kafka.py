    ...
    async def provision_cluster(self, cluster):
        ...
        for node in nodes:
            cmd = f"{self.svc_path}/kafkaadm deploy" \
                    f" {node['mgmt_address']} {cluster_id} {cookie}"
            ...
            if rc:
                ...
                return rc
        for node in nodes[1:]:
            log.info("Join {} to Kafka cluster {}.".format(
                    node["instance_name"], cluster_id))
            cmd = f"{self.svc_path}/kafkaadm join" \
                    f" {node['mgmt_address']} {cluster_id}" \
                    f" {nodes[0]['instance_name'].replace('_', '-')}"
            with open(self.build_log, "a") as fd:
                rc = await util.exec_cmd(cmd, output_file=fd)
            if rc:
