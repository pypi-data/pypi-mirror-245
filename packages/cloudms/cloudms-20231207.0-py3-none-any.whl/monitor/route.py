...
from monitor.objects import MonitorClusterPost, MonitorClusterPut,\
    MonitorClusterAction, MonitorClusterInterface
...
async def post_monitor_cluster(req: MonitorClusterPost, token_pack=None,
        x_auth_token=Header(None)):
    return await MonitorCluster(token_pack).rh_post(req.dict())
...
async def put_monitor_cluster(cluster_id: str, req: MonitorClusterPut,
        token_pack=None, x_auth_token=Header(None)):
    return await MonitorCluster(token_pack).rh_put(cluster_id, req.dict())
...
@app.post("/v1/monitor/cluster/{cluster_id}/interface")
@route_wrapper
async def post_monitor_cluster_interface(cluster_id: str,
        req: MonitorClusterInterface,
        token_pack=None, x_auth_token=Header(None)):
    return await MonitorCluster(token_pack).rh_post_action(cluster_id,
        req.dict())


@app.delete("/v1/monitor/cluster/{cluster_id}/interface/{interface_id}")
@route_wrapper
async def delete_monitor_cluster_interface(cluster_id: str,
        interface_id: str, token_pack=None, x_auth_token=Header(None)):
    return await MonitorCluster(token_pack).rh_delete_interface(cluster_id,
        interface_id)
