    ...
    async def wait_for_ready(self, res_ins, res_ids, status, count=10,
        ...
        while count > 0:
            ...
            for id in res_ids:
                ...
                if obj:
                    log.debug(f"Wait list: {id} status is {obj['status']}.")
                    if obj["status"] == status:
                        done_list.append(id)
                else:
                    if status == "DELETED":
                        log.debug(f"Wait list: {id} status is {status}.")
                        done_list.append(id)
                    else:
                        log.error(f"Wait list: {id} is not found.")
            if len(done_list) == ids_len:
                return
            count -= 1
        else:
            log.error(f"Wait list: {res_ids} {status} timeout!")
            return -1
