...
class SnapshotPlan(PlanBase):
    ...
    '''
    async def apply_retention(self, plan, res_id):
        plan_id = plan["id"]
        res_type = plan["resource_type"]
        log.info(f"Apply retention for snapshot plan {plan_id}.")
        res_ins = VolumeSnapshot(self.token_pack)
        time_name = "created_at"
        objs = await res_ins.get_obj_list()
        for obj in objs:
            if obj["name"] != f"plan-{plan_id}":
                objs.remove(obj)
        if not objs:
            log.info(f"No snapshot from plan {plan_id}.")
            return
        log.debug(f"Snapshot list {objs}.")
        objs_sorted = sorted(objs, key = lambda x:x[time_name],
                reverse = True)
        while len(objs_sorted) > plan["retention"]:
            obj = objs_sorted[-1]
            log.info(f"Delete expired snapshot {obj['id']}.")
            await res_ins.delete(obj["id"])
            objs_sorted.pop()
    '''
