...
class Plugin(PoetryPlugin):
    def activate(self, poetry: Poetry, io: IO):
        ...
        if not gitroot:
            io.write_error_line(
                "poetry-dateversioning plugin is enabled but this is not a git repository"
            )
            return
...
def _enabled(poetry: Poetry) -> bool:
    """looks up [tool.poetry-dataversioning] enable = true|false"""
    return (
        poetry.pyproject.data.get("tool", {})
        .get("poetry-dateversioning", {})
        .get("enable", False)
    )
