...
class RecordU(PyApiB):
    ...
    def m3u8(self,
             ...
             startTime=None,
             endTime=None):
        nowTime = pyuc.datetimeU('recordU').dataStr('%H:%M')
        ...
        savePath = savePath.format(
            time=pyuc.datetimeU('recordU').dataStr('%Y-%m-%d_%H_%M_%S'))
        import requests
        import os
        ...
        try:
            ...
            pyuc.videoU('recordU').formatTo(savePath + '.ts', savePath,
                                               True)
            self.isRecoding = False
        ...
        with open(savePath, 'ab') as f:
            for ts_file in box:
                with open(f"{tslist}/{ts_file}", 'rb') as ts:
                    f.write(ts.read())
        if rmTslist:
            pyuc.fileU('recordU').remove(tslist)
