...
class ScrapyprojectnamePipeline(object):
    ...
    def __init__(self):
        host = pyuc.envU().get('mongo_host')
        port = int(pyuc.envU().get('mongo_port','1'))
        user = pyuc.envU().get('mongo_user')
        pswd = pyuc.envU().get('mongo_pswd')
        # 创建MONGODB数据库链接
        self.client = pyuc.mongoDBU().init(host, port, user, pswd)
        self.saveSignList = {}
                ...
                if _saveType == 'begin':
                    ...
                    if len(_saves) > 0:
                        ...
                        if _optionType == 'insert':
                            ...
                            if len(ops) > 1:
                                savePath = ops[1]
                            pyuc.fileU().write_json(savePath,_saveDatas)
