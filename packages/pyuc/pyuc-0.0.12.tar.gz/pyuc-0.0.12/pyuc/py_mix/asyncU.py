...
import random
if PyApiB.tryImportModule("inspect",installName="inspect"):
    import inspect
if PyApiB.tryImportModule("ctypes",installName="ctypes"):
    import ctypes
...
class AsyncU(PyApiB):
    ...
    def asyncRun(self, target, args=None, asyncKey=None, isProcess=False):
        if asyncKey == None:
            asyncKey = pyuc.algorithmU("cmdU").md5(str(random.randint(0,9999999999999)))
        if isProcess:
