...
class SortU(PyApiB):
    ...
    def __compare(self, p, r, byDict=None):
        if isinstance(p, dict) or isinstance(p, list) and byDict != None:
            for byKey in byDict:
                if isinstance(byKey, int) or (isinstance(byKey, str) and byKey.isdecimal()):
                    keyIndex = int(byKey)
                    if p[keyIndex] == r[keyIndex]:
                        continue
                    else:
                        return -1 if (p[keyIndex] < r[keyIndex]) == (byDict[byKey] < 0) else 1
                elif byKey in p and byKey in r:
                    if p[byKey] == r[byKey]:
                        continue
                    else:
