...
class ProxyU(PyApiB):
    ...
    def __init__(self):
        self.redis: pyuc.RedisDBU = pyuc.redisDBU()
        self.mongoDB: pyuc.MongoDBU = pyuc.mongoDBU()
    ...
    def addProxyCrawlers(self, crawlers):
        cs = pyuc.moduleU().importClsByPcls(crawlers,ProxyCrawlerU)
        for c in cs:
            crawlerName = c
            if "." in c:
    ...
    def crawlProxysAndSave(self):
        """ 采集所有爬虫，并保存所获得的代理 """
        asyncU:pyuc.AsyncU = pyuc.asyncU("ProxyU")
    ...
    def loopCrawl(self, crawlDuring):
        """ 循环采集所有爬虫，并保存所获得的代理 """
        ctrlCU:pyuc.CtrlCU = pyuc.ctrlCU("proxyU")
        ctrlCU.on()
    ...
    def checkOneProxy(self, proxyMeta=None, timeout=60, checkUrl=CHECK_BAIDU):
        ...
        if proxyMeta:
            ...
            if float(expireTime) > nowTime:
                ...
                # 多实例pyuc.HttpU() 不能用pyuc.httpU()
                res = pyuc.HttpU().get(checkUrl,headers=headers,proxyMeta=_proxyMeta,timeout=timeout)
    ...
    def loopCheckOneProxy(self, timeout=20, checkUrl=CHECK_BAIDU):
        ...
        while not pyuc.ctrlCU("proxyU").isExit():
            res = False
            try:
