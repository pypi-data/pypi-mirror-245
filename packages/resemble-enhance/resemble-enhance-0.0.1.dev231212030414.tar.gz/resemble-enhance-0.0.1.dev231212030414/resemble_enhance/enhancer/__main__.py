...
def main():
    ...
    parser.add_argument(
        "--run_dir",
        type=Path,
        default=Path("runs/enhancer_stage2"),
        help="Path to the enhancer run folder",
    )
    parser.add_argument(
        "--suffix",
        type=str,
    ...
    for path in args.in_dir.glob(f"**/*{args.suffix}"):
        ...
        if args.denoise_only:
            hwav, sr = denoise(dwav, sr, args.run_dir, args.device)
        else:
            hwav, sr = enhance(dwav, sr, args.run_dir, args.device, nfe=args.nfe, solver=args.solver, alpha=args.alpha)
