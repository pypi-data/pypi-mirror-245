...
class Enhancer(nn.Module):
    ...
    def configurate_(self, nfe, solver, alpha):
        """
        Args:
    ...
    def forward(self, x: Tensor, y: Tensor | None = None, z: Tensor | None = None):
        ...
        if self.hp.lcfm_training_mode == "cfm":
            ...
            if self.training:
                alpha = torch.rand_like(x_mel_denoised[:, :1, :1])
            else:
                alpha = self._eval_alpha
            x_mel_denoised = alpha * x_mel_denoised + (1 - alpha) * x_mel_original
        else:
