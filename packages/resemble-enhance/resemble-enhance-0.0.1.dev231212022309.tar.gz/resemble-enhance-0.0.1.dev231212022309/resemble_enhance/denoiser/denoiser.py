...
class Denoiser(nn.Module):
    ...
    def stft_cfg(self) -> dict:
        ...
        return dict(
            hop_length=hop_size,
            n_fft=hop_size * 4,
            win_length=hop_size * 4,
            window=torch.hann_window(hop_size * 4, device=self.dummy.device),
        )
    ...
    def _stft(self, x):
        ...
        s = torch.stft(x.float(), **self.stft_cfg, return_complex=True)  # (b f t+1)
        s = s[..., :-1]  # (b f t)
    ...
    def _istft(self, mag: Tensor, cos: Tensor, sin: Tensor):
        ...
        x = torch.istft(s, **self.stft_cfg, return_complex=False)
