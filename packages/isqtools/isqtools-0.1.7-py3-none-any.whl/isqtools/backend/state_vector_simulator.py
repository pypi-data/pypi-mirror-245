...
class StateVectorSimulator(AbstractBackend):
    ...
    def multi_gate(
        ...
        if ctrl < target:
            ...
            if gate in ["CX", "CNOT"]:
                ...
                state = self.stack(
                    [state[:, 0, :, :, :], self.stack([b, a], axis=2)], axis=1
                )
            elif gate == "CY":
                ...
                state = self.stack(
                    [state[:, :, :, 0, :], self.stack([a, b], axis=1)], axis=3
                )
            return self.ravel(state)
        else:
            ...
            if gate in ["CX", "CNOT"]:
                ...
                state = self.stack(
                    [state[:, :, :, 0, :], self.stack([b, a], axis=1)], axis=3
                )
            elif gate == "CY":
    ...
    def swap(
        ...
        state = self.stack(
            [self.stack([a, c], axis=2), self.stack([b, d], axis=2)], axis=1
        )
        return self.ravel(state)
