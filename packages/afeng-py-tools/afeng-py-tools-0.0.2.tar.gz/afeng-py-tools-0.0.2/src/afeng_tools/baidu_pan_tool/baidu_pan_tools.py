...
from afeng_tools.baidu_pan_tool import baidu_pan_config
...
from afeng_tools.baidu_pan_tool.tools import auth_tools, file_path_tools, file_upload_tools
...
def _save_token(token_info: TokenInfo):
    ...
    file_tools.save_file(json.dumps(token_info.model_dump(mode='json')).encode('utf-8'),
                         baidu_pan_config.get_config(BaidupanConfigKeyEnum.token_file), binary_flag=True)
...
def get_access_token() -> str:
    """获取Access Token"""
    if os.path.exists(baidu_pan_config.get_config(BaidupanConfigKeyEnum.token_file)):
        # token文件转换为token
        token_info_bytes = file_tools.read_file(baidu_pan_config.get_config(BaidupanConfigKeyEnum.token_file), binary_flag=True)
        ...
        if _is_need_refresh(token_info):
            ...
            token_info = auth_tools.refresh_token(token_info.refresh_token,
                                                  baidu_pan_config.get_config(BaidupanConfigKeyEnum.app_key), baidu_pan_config.get_config(BaidupanConfigKeyEnum.secret_key))
            # 2、保存刷新后的token
            _save_token(token_info)
        return token_info.access_token
    else:
        # 获取授权码Code
        auth_code = auth_tools.get_authorization_code(app_id=baidu_pan_config.get_config(BaidupanConfigKeyEnum.app_id), app_key=baidu_pan_config.get_config(BaidupanConfigKeyEnum.app_key))
        # 换取AccessToken凭证
        token_info = auth_tools.get_token_authorization_code(auth_code=auth_code,
                                                             app_key=baidu_pan_config.get_config(BaidupanConfigKeyEnum.app_key),
                                                             secret_key=baidu_pan_config.get_config(BaidupanConfigKeyEnum.secret_key))
        # 保存token到本地
        _save_token(token_info)
        return token_info.access_token
