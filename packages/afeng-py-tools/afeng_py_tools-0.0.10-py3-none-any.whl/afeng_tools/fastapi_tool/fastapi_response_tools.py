...
def resp_404(message: str, request: Request = None, context_data: dict = None):
...
def resp_500(message: str, request: Request = None, context_data: dict = None):
    if request and not request_tools.is_json(request.headers):
        is_mobile = request_tools.is_mobile(request.headers.get('user-agent'))
        if context_data is None:
...
def resp_file(file_path: str, file_name: str = None, download_flag: bool = False, context_data: dict = None) -> Response:
    """响应文件"""
