...
def calc_file_size(file_size: int) -> str:
    """计算文件大小"""
    one_kb = 1024
    one_mb = 1024 * 1024
...
def parse_file_size(file_size: str) -> int:
    ...
    if file_size.endswith('B'):
        ...
        return int(file_size.removesuffix('KB') * one_kb)
    elif file_size.endswith('MB'):
        return int(file_size.removesuffix('MB') * one_mb)
    elif file_size.endswith('GB'):
        return int(file_size.removesuffix('GB') * one_gb)
