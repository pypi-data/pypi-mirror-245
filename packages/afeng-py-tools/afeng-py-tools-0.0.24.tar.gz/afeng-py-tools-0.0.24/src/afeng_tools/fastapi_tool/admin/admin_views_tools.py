...
def _create_param_convert_depend_func(module_code: str, orm_model_class: Type[Model]) -> Callable:
    ...
    for tmp_field, tmp_column in params_dict.items():
        ...
        if issubclass(tmp_type, list):
            ...
            params_list.append(f"{tmp_field}: Optional[{tmp_column.type.python_type.__name__}[{tmp_item_type.__name__}]] = Form(title='{tmp_column.comment}', default=None)")
        else:
            params_list.append(f"{tmp_field}: Optional[{tmp_column.type.python_type.__name__}] = Form(title='{tmp_column.comment}', default=None)")
