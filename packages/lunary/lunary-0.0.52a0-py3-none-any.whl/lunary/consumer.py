...
class Consumer(Thread):
    ...
    def send_batch(self):
        ...
        if len(batch) > 0:
            if self.verbose:
                logging.log(f"[Lunary] Sending {len(batch)} events.")

            try:
                if self.verbose:
                    logging.log("[Lunary] Sending events to ", self.api_url)
                ...
                if self.verbose:
                    logging.log("[Lunary] Events sent.", response.status_code)

                if response.status_code != 200:
                    logging.log("[Lunary] Error sending events")
            except Exception as e:
                if self.verbose:
                    logging.log("[Lunary] Error sending events", e)
