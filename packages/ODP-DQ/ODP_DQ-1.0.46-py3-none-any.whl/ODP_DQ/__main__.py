...
class DataQuality:
    ...
    def main(self):
        ...
        if self.choice['DQRules']==1:
            ...
            final_table=f"{self.schema}.BusinessContext_{self.input_table}" 
            ...
            final_table=f"{self.schema}.DQRules_{self.input_table}" 
        ...
        if self.choice['AnomalyDetection']==1:
            ...
            final_table=f"{self.schema}.AnomalyDetectionResult_{self.input_table}"
        ...
        if self.choice['Standardization']==1:
            ...
            final_table=f"{self.schema}.StandardizationResult_{self.input_table}"
            result.write.option("catalog", self.catalog_name).option("name", final_table).mode('overwrite').saveAsTable(final_table)
