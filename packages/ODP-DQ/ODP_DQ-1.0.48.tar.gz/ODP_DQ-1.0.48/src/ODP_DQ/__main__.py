...
class DataQuality:
    ...
    def main(self):
        ...
        if self.choice['DQRules']==1:
            ...
            final_table=f"businesscontext.{self.input_table}_{time.strftime("%Y%m%d%H%M%S")}" 
            ...
            final_table=f"dqrules.{self.input_table}_{time.strftime("%Y%m%d%H%M%S")}" 
        ...
        if self.choice['AnomalyDetection']==1:
            ...
            final_table=f"anomalydetection.{self.input_table}_{time.strftime("%Y%m%d%H%M%S")}"
        ...
        if self.choice['Standardization']==1:
            ...
            final_table=f"standardization.{self.input_table}_{time.strftime("%Y%m%d%H%M%S")}"
            result.write.option("catalog", self.catalog_name).option("name", final_table).mode('overwrite').saveAsTable(final_table)
