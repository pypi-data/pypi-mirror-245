...
class GoogleAPITranslatorService(BaseTranslatorService):
    ...
    def __init__(self, max_segments=128):
        assert (
            googleapiclient
        ), "`GoogleAPITranslatorService` requires `google-api-python-client` package"

        self.developer_key = getattr(settings, "GOOGLE_TRANSLATE_KEY", None)
        assert self.developer_key, (
            "`GOOGLE_TRANSLATE_KEY` is not configured, "
    ...
    def translate_strings(
        ...
        try:
            ...
            assert isinstance(
                strings, collections.abc.MutableSequence
            ), "`strings` should be a sequence containing string_types"
        assert (
            not optimized
        ), "optimized=True is not supported in `GoogleAPITranslatorService`"
        if len(strings) == 0:
            return []
            ...
            response = (
                self.service.translations()
                .list(source=source_language, target=target_language, q=strings)
                .execute()
            )
            ...
            self.translate_strings(
                strings[0 : self.max_segments],
                target_language,
                source_language,
                optimized,
            )
            _translated_strings = self.translate_strings(
                strings[self.max_segments :],
                target_language,
                source_language,
                optimized,
            )
