...
class CRFCheckListCreator():
    ...
    def Creator():
        ...
        CRF_File_Path = FunctionTools.getFileLoc(*CRF_List)
        CRF_Dic = {}
        ...
        for key, value in CRF_Dic.items():
            dic = FunctionTools.CheckListCreator(str(key), str(value))
        ...
        if len(page_List) == 2:
            i = 0
            FunctionTools.OutFile(export_path, "CheckList", page_List[i], CRF_List[i],page_List[i+1], CRF_List[i+1])
