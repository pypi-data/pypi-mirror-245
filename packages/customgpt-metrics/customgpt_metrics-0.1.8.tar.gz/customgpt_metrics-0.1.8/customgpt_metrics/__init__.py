...
class CustomGPTMetrics:
    ...
    def analyze_log(self, chat, metrics=['intent', 'language', 'context', 'emotion']):
        ...
        metrics_result = {}
        metrics_evaluation = {key: value for key, value in self.metrics.items() if key in metrics}

        with ThreadPoolExecutor() as executor:
            # Using executor.map to concurrently evaluate metrics
            metrics_values = executor.map(lambda metric: metric.evaluate(adapted_entry), metrics_evaluation.values())
    ...
    def stream_analyze_logs(self, db_url, table='prompt_histories', query="SELECT * FROM %s LIMIT %s", limit=100, metrics=['intent', 'language', 'context', 'emotion']):
        if self.api_key is None:
            raise ValueError("Metric Calculation require openai api_key to be set.")
        if db_url is None:
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for log_entry in log_entries:
                    results.append(executor.submit(self.evaluate_metrics, log_entry, metrics))

                yield from self.process_results(results)
        except Exception as e:
            raise e

    def analyze_logs(self, db_url, table='prompt_histories', query="SELECT user_query, openai_response, openai_prompt FROM %s LIMIT %s", limit=10, metrics=['intent', 'language', 'context', 'emotion']):
        if self.api_key is None:
            raise ValueError("Metric Calculation require openai api_key to be set.")
        if db_url is None:
            raise ValueError("Database URL is required")
    ...
    def evaluate_metrics(self, adapted_entry, metrics):
        metrics_result = {}
        metrics_evaluation = {key: value for key, value in self.metrics.items() if key in metrics}
        with ThreadPoolExecutor() as executor:
            # Using executor.map to concurrently evaluate metrics
