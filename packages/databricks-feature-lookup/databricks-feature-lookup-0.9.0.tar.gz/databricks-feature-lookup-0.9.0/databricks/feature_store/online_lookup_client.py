...
def generate_lookup_brickstore_http_engine(
    ...
    return LookupBrickstoreHttpEngine(online_feature_table)
...
def load_credentials_from_env(online_ft: OnlineFeatureTable):
    ...
    elif online_ft.online_store.store_type == StoreType.BRICKSTORE:
        if should_use_brickstore_http_gateway(online_ft):
            # Brickstore HTTP gateway lookup engine loads oauth token internally.
            return None
    return (
...
def should_use_brickstore_http_gateway(
...
class OnlineLookupClient:
    ...
    def _generate_lookup_engine(
        ...
        elif first_online_feature_table.online_store.store_type == StoreType.BRICKSTORE:
            if should_use_brickstore_http_gateway(online_feature_table):
