...
def read_xml_ext(annotation_file_path):
    ...
    xml_info.append(filename_text) #>>>>> 1
    ...
    xml_info.append(db_src_text) #>>>>> 3

    height = root.find('size').find('height').text
    xml_info.append(height) #>>>>> 4
    width = root.find('size').find('width').text
    xml_info.append(width) #>>>>> 5
    depth = root.find('size').find('depth').text
    xml_info.append(depth) #>>>>> 6
    segmented = root.find('segmented')
    if segmented is None:
        segmented_text = ''
    else:
        segmented_text = segmented.text
    xml_info.append(segmented_text) #>>>>> 7
    ...
    xml_info.append(frame_no_text) #>>>>> 8
    ...
    xml_info.append(movie_name_text) #>>>>> 9
