...
def validate_source(config):
    ...
    for i, source in enumerate(config.config["sources"]):
        ...
        if "owner_id" not in source:
            logger.error(
                f"Source number [{total_sources}] has no owner_id"
            )
            is_source_valid = False
                    ...
                    if response_data["versions"] != []:
                        if (
                            ...
                            if (
                                ...
                                if "id" in source and source["id"] == response_data["id"]:
                                    pass
                                else:
                                ...
                                if "owner_id" in source and source["owner_id"] == response_data["user_id"]:
                                    pass
                                else:
                                    # also sync the user id
                                    config.config["sources"][i]["owner_id"] = response_data[
                                        "user_id"
                                    ]
                                    source_synced = True

                                if "owner" in source and source["owner"] == response_data["owner"]['login']:
                                    pass
                                else:
                                    # and the user login
                                    config.config["sources"][i]["owner"] = response_data["owner"]['login']
                                    source_synced = True

                                # Save the updated configuration
                                if source_synced:
                                    config.save_source_config()
                                    logger.success(f"Source [{source['name']}] already published with id [{response_data['id']}] synced with local config")
                                else:
                                    logger.info(f"Source [{source['name']}] already published with id [{response_data['id']}], no need to sync local config")
                            else:
                                update_source = True
                        else:
