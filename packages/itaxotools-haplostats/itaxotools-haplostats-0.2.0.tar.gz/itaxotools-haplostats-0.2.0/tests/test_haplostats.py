...
def test_empty():
    ...
    assert stats.format_set_id(0) == "FOR01"
    assert stats.get_haplotypes() == {}
    assert stats.get_haplotypes_per_subset() == {}
    assert stats.get_haplotypes_shared_between_subsets() == []
    assert stats.get_fields_of_recombination() == {}
    assert stats.get_subsets_per_field_of_recombination() == {}
    assert stats.get_fields_of_recombination_per_subset() == {}
    assert stats.get_fields_of_recombination_shared_between_subsets() == []
...
def test_get_fields_of_recombination(stats_example: HaploStats):
    assert stats_example.get_fields_of_recombination() == {
        "FOR01": [
            "Hap01",
            "Hap02",
            "Hap03",
...
def test_get_subsets_per_field_of_recombination(stats_example: HaploStats):
    assert stats_example.get_subsets_per_field_of_recombination() == {
        "FOR01": {
            "total": 8,
            "species": {
                "verrucosa": 6,
...
def test_get_fields_of_recombination_per_subset(stats_example: HaploStats):
    assert stats_example.get_fields_of_recombination_per_subset() == {
        "verrucosa": {
            "total": 6,
            "FORs": {
                "FOR01": 6,
...
def test_get_fields_of_recombination_shared_between_subsets(stats_example: HaploStats):
    assert stats_example.get_fields_of_recombination_shared_between_subsets() == [
        {
            "species_a": "verrucosa",
            "species_b": "enigmatica",
            "common": {
...
def test_get_fields_of_recombination_shared_between_subsets_including_empty(
    stats_example: HaploStats,
):
    assert stats_example.get_fields_of_recombination_shared_between_subsets(
        include_empty=True
