...
class _AbstractTask(Generic[FutureT]):
    def __init__(
        self,
        future: FutureT,
        coro: Coroutine,
...
class ThreadTask(_AbstractTask[Future]):
    def result(self, timeout: Optional[float] = None):
        if (
...
class EventLoop:
    ...
    def create_task(self, coro, ensure_completed=False) -> _AbstractTask:
        task: _AbstractTask
        if self.in_same_thread():
            task = AsyncIOTask(
...
class future_property(Generic[ReturnT]):
    def __init__(
        self, 
        func: Optional[Callable[[Any], Awaitable[ReturnT]]] = None,
        on_demand=False
    ):
        self.func = func
        ...
        self._tasks: Dict[Any, _TaskProp] = {}

    def __call__(self, func: Callable[[Any], Awaitable[ReturnT]]):
        self.func = func
        self.__doc__ = func.__doc__
        return self
