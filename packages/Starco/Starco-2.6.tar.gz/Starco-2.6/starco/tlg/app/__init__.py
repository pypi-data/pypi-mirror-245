...
class TlgApp:
    def __init__(self, number, key='17203958', hash='82cefc4001e057c9d1488ab90e23d54f', loop=None, debug_mode=True,country_diring=False,reset_json=False,auto_connect=True,disable_proxy=False, **kwargs) -> None:
        '''
        relative_path='.'
    ...
    def get_user_info(self, per_users:list,to_dict=False):
        
        async def _get_user_info():
            users = per_users
            if not isinstance(users,list):users=[users]
            try:
                res = await self.client(GetUsersRequest(users))
                if to_dict:
                    out=[]
                    for i in res:
                        out+=[i.to_dict()]
