    # Modified SRCNN model for 3 channel processing in RGB color mode #
    ...
    settings = \
        {
            ...
            'seed': 19,
            'shuffle': True,
            'stride': 11,  # the same with input size, to have no overlap as is in the paper.
            'target_channels': 3,
            'target_cmode': 'RGB',
    ...
    def build_model(self, testmode=False):
        ...
        SRCNN = Conv2D(64,(9,9), kernel_initializer=self.kernel_initialer, dilation_rate=self.dilation_rate,
                       padding='valid', input_shape=input_shape, activation=self.activation)(SRCNN_INPUT)
        SRCNN = Conv2D(32, (1,1), kernel_initializer=self.kernel_initialer, dilation_rate=self.dilation_rate,
                       padding='valid', activation=self.activation)(SRCNN)
        SRCNN = Conv2D(1,(5,5), kernel_initializer=self.kernel_initialer, dilation_rate=self.dilation_rate,
                       padding='valid', activation=self.activation)(SRCNN)
