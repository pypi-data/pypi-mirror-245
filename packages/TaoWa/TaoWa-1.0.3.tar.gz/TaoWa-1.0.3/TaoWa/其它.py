...
skin = None
def 皮肤_加载(皮肤文件名: str='guohe'):
    '''
    :param 皮肤文件名: 模块Skin文件下的文件名
    :return:
    '''
    global skin
    路径 = sys._MEIPASS if getattr(sys, "frozen", False) else os.path.abspath(".\\Skin\\")
    if not skin:
        skin = ctypes.WinDLL(路径+'\\TaoWa\\Skin\\Skin.dll')
    皮肤路径 = 路径 +'\\TaoWa\\Skin\\'+ 皮肤文件名
    skin.SkinH_AttachEx(皮肤路径.encode(), None)
