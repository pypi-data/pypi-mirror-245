...
def data2PIV(data):
    ...
    if (PIV.Inp.FlagWindowing == 0) :
      ...
      numInitIt = PIV.Inp.numInitIt +(PIV.Inp.FlagWindowing - 1)# if equal to 2 then should be one element longer
      FlagWindowingVec=np.array([0 if ii<numInitIt -1 else -PIV.Inp.FlagWindowing for ii in range(numInitIt) ],dtype=np.intc)
    
    flagCalcVelVec=np.array([PIV.Inp.FlagCalcVel],dtype=np.intc)
    semiDimCalcVelVec=np.array([abs(PIV.Inp.SemiDimCalcVel)],dtype=np.intc)
