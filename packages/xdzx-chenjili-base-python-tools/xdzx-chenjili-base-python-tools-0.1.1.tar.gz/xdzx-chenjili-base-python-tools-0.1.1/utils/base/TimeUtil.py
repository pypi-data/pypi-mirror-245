...
def get_date_str(timestamp: float, hours=0) -> str:
    ...
    date_str = beijing_date.strftime('%Y-%m-%d %H:%M:%S.%f')
    return date_str


def get_current_date_str(hours=0) -> str:
    '''
    获取日期时间字符串
    :param hours: 时区便宜，默认东8区
    :return:
    '''
    timestamp = get_timestamp(TimeUnitEnum.SECOND)
    return get_date_str(timestamp, hours)
...
if __name__ == '__main__':
    print(f'MICROSECOND: {get_timestamp(TimeUnitEnum.MICROSECOND)}')
    print(f'MILLISECOND: {get_timestamp(TimeUnitEnum.MILLISECOND)}')
    print(f'SECOND: {get_timestamp(TimeUnitEnum.SECOND)}')
    print(f'DEFAULT: {get_timestamp()}')
    print(f'MINUTE: {get_timestamp(TimeUnitEnum.MINUTE)}')
    print(f'HOUR: {get_timestamp(TimeUnitEnum.HOUR)}')
    print(f'DAY: {get_timestamp(TimeUnitEnum.DAY)}')
    print(f'WEAK: {get_timestamp(TimeUnitEnum.WEAK)}')
