...
from sms_message_aws_sns_local.SendAwsSms import (
    SmsMessageAwsSns, send_sms_using_aws_sms_using_api_getaway)
import pprint
from label_message_local.LabelMessage import LabelsMessageLocal
from label_message_local.LabelConstans import LabelsLocalConstants
...
class MessagesLocal:
    ...
    def send_sync(self, original_body: str,
                  to_recipients: List[dict], cc_recipients: List[dict] = None, bcc_recipients: List[dict] = None,
        ...
        #return  # TODO: remove this line when you implement the method
        logger.start("MessagesLocal.py send_sync()")
        to_recipients = self._get_recipients(to_recipients)
        cc_recipients = self._get_recipients(cc_recipients) if cc_recipients else []
        bcc_recipients = self._get_recipients(bcc_recipients) if bcc_recipients else []

            # TODO fill the required parameters
        message_local = MessageLocal(original_subject=original_subject, original_body=original_body)
        for recipient in to_recipients:
            message_recipient_channel_id = message_local.get_message_channel_id(recipient)
            message_recipient_provider_id = message_local.get_message_provider_id(message_recipient_channel_id,
                                                                                    recipient)
            if message_recipient_channel_id == SMS_MESSAGE_CHANNEL_ID \
                ...
                message_local._init__.SmsMessageAwsSns(boto3.resource("sns"))
            if os.getenv("IS_REALLY_SEND"):
                message_local.send(recipient)
        logger.end()

    def push_message(self, start_timestamp: Union[str, datetime], end_timestamp: Union[str, datetime, None],
                     to_recipients: List[Recipient], cc_recipients: List[Recipient] = None,
                     bcc_recipients: List[Recipient] = None, importance: MessageImportance = MessageImportance.MEDIUM,
                     sender_profile_id: int = None,
                     original_subject: str = None, original_body: str = None,
                     template_id: int = None, user_external_id: int = None) -> None:
        ...
        class_json = {"default_original_body": original_body,
                      "default_subject": original_subject,
                      "default_importance": importance.value
                      }
        ...
        LabelsMessageLocal.add_label(LabelsMessageLocal(), label_id=LabelsLocalConstants.MESSAGE_OUTBOX_LABEL_ID, message_id=queue_id)
    ...
    def _recipients_to_json(recipients: List[Recipient]) -> List[dict]:
        """recipients to json"""
        # TODO: implement to_json in Recipient
        if not recipients:
            return []
