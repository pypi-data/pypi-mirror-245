...
class dataHelper():
    ...
    def build_6city(self):
        ...
        if self.city in NYC:
            self.G = gt.load_graph("/lab/kiran/beogym/nyc.gt")
            self.Gdict = pickle.load(open('/lab/kiran/beogym/nycgraph.pkl', 'rb'))
            self.Greversed={value: key for key, value in self.Gdict.items()}
        elif self.city in Pits:
            self.G = gt.load_graph("/lab/kiran/beogym/pits.gt")
            self.Gdict = pickle.load(open('/lab/kiran/beogym/pitsgraph.pkl', 'rb'))
            self.Greversed={value: key for key, value in self.Gdict.items()}
        elif 'navigation' in self.city:
            self.G = gt.load_graph("/lab/kiran/beogym/tcd.gt")
            self.Gdict = pickle.load(open('/lab/kiran/beogym/tcdgraph.pkl', 'rb'))
            self.Greversed={value: key for key, value in self.Gdict.items()}
