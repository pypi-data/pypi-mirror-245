...
def legacy_to_toml_structure(
    ...
    if (
        ...
        dependencies["apt"] = req_list_to_dict(apt)  # no versions for apt. So we just add the list
...
def load_config(
    ...
    if deps:
        ...
        original_deps = {k: v for k, v in original_deps.items() if (len(v) > 0) and (v is not None)}
...
def parse_requirements(file: str):
    ...
    # remove comments
    requirements_list = [r.split("#")[0] for r in requirements_list]
    # remove empty lines
