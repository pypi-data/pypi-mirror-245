    ...
    with tqdm(total=total_duration, unit='sec', disable=verbose is not False, desc=task.title()) as tqdm_pbar:
        ...
        while seek_sample < audio.shape[-1]:
            ...
            if no_speech_threshold is not None:
                ...
                # no_speech_fallbacks = 2
                # mel_cut = mel_segment.shape[-1] // (no_speech_fallbacks + 1)
                # for _cut in range(mel_cut, mel_segment.shape[-1], mel_cut):
                #     if not should_skip:
                #         break
                #     _tks = torch.tensor(tokenizer.sot_sequence, device=device)[None]
                #     _mel = torch.zeros_like(mel_segment)
                #     _mel[:, :mel_cut] = mel_segment[:, _cut:_cut + mel_cut]
                #     with torch.no_grad():
                #         no_speech_prob = model(_mel[None], _tks)[0, 0].float().softmax(dim=-1)[tokenizer.no_speech].item()
