...
def import_media(
    ...
    """
    Import media from directory <IMPORT_PATH> into media repository
    """
    ...
    try:
        ...
        exiftool_cmd = ["exiftool", "-j", str(Path(import_path).absolute())]

        if recurse:
            exiftool_cmd.insert(1, "-r")

        logging.info(
            f"Searching/parsing media files in <{str(Path(import_path).absolute())}> started"
        )
        logging.debug(f"Running external command: {' '.join(exiftool_cmd)}")

        p = subprocess.run(exiftool_cmd, capture_output=True)
        if p.returncode != 0:
            raise Exception(f"exiftool error: {p.stderr}")
    ...
    logging.info(
        f"Scanned files: {len(scanned_files)}, Media files: {len(media_files)}"
    )
    ...
    for f in media_files:
        ...
        logging.info(f"File <{src_file_path}> successfuly imported")
        import_stats["ok"] += 1
