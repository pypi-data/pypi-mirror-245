...
class GPF:
    ...
    subgraph cluster_Graph{}{{
     node [ fontname = "siyuan", fontsize = 12]; 
     rankdir = LR;
     label=" {}:{}  "
     bgcolor="yellow" '''
            ...
            Script+=ColHead+"\n"
            for U in Col:
                Script+=self.UnitInfo(U)+'[label=" '+U+self.GetUnitKV(U,"Word")+' "]'+"\n"
            ...
            Script+="->".join(ColInfo)+' [dir=none,color="yellow"]'+"\n"
            Script+='}'+"\n"
            ID+=1
        Script+='}'+"\n"
        self.DotBuff(Script, Img)	
