...
def mkdesc():
    return "<p>" + get_paragraph(count=2, sep="</p><p>") + "</p>"
...
class Loader1(object):
    ...
    def objects(self):
        ...
        self.SESSION = named(Product, _("Session"), sales_price=15, storage_management=True)
        yield self.SESSION

        # self.MIN_ASSETS = Cycler(['5', '8', '12'])

        # t5 = babeld(Tariff, _("5 times"), number_of_events=5, min_asset=1, product=SESSION)
        # yield t5
        # t8 = babeld(Tariff, _("8 times"), number_of_events=8, min_asset=2, product=SESSION)
        # yield t8
        # t12 = babeld(
        #     Tariff, _("12 times"), number_of_events=12, min_asset=4, product=SESSION)
        # yield t12
        ...
        p = product("48", self.course_fees, "48€/8 hours", storage_management=True)
        yield p
        yield Component(parent=p, child=self.SESSION, qty=8)
        # yield Tariff(product=p, number_of_events=8, min_asset=2)

        p = product("64", self.course_fees, "64€/12 hours", storage_management=True)
        yield p
        yield Component(parent=p, child=self.SESSION, qty=12)
        # yield Tariff(product=p, number_of_events=12, min_asset=4)

        p = product("50", self.course_fees, "50€/5 hours", storage_management=True)
        yield p
        yield Component(parent=p, child=self.SESSION, qty=5)
        ...
        kw.update(is_public=True)
        self.kurse = event_type(**kw)
        yield self.kurse
        settings.SITE.site_config.update(default_event_type=self.kurse)
        # yield settings.SITE.site_config

        self.seminare = event_type(is_public=True, **dd.str2kw('name', _("Seminars")))
        yield self.seminare

        self.excursions = event_type(
            is_public=True, max_days=10, **dd.str2kw('name', _("Excursions")))
        # de="Ausflüge",
        #  fr="Excursions",
        #  en="Excursions",
        ...
        self.hikes = event_type(
            is_public=True, max_days=60, **dd.str2kw('name', _("Hikes")))
        # de="Wanderungen",
        # fr="Randonnées",
        # en="Hikes",
        yield self.hikes

        yield event_type(is_public=True, **dd.str2kw('name', _("Meetings")))
                                      # de="Versammlungen",
                                      # fr="Réunions",
                                      # en="Meetings",
...
class Loader2(Loader1):
    ...
    def objects(self):
        ...
        def add_course(*args, **kw):
            ...
            tariff = Component.objects.filter(parent=obj.line.fee).first()
            if tariff:
                obj.max_events = None
            return obj
        ...
        def add_journey(*args, **kw):
            kw.update(user=USERS.pop())
            kw.update(teacher=TEACHERS.pop())
            kw.update(every_unit=cal.Recurrences.once)
        ...
        kw.update(every=1)
        kw.update(every_unit=cal.Recurrences.weekly)
    ...
    """
        ...
        kw.update(user=USERS.pop())
        kw.update(teacher=TEACHERS.pop())
        kw.update(every_unit=cal.Recurrences.monthly)
        ...
        for pupil in pupils:
            for i in range(pupil.id % 6):
                ...
                tariff = Component.objects.filter(parent=course.line.fee).first()
                ...
                for rd, sd, ed in story:
                    ...
                    if tariff and tariff.qty:
                        ...
                        # yield Filler(partner=pupil, product=tariff.product, )
                        yield Filler(partner=pupil,
                            provision_product=self.SESSION,
                            # filler_product=FILLERS.pop(),
                            storage_state='purchased',
                            # min_asset=self.MIN_ASSETS.pop(),
                            min_asset=tariff.qty,
                            fill_asset="10")
