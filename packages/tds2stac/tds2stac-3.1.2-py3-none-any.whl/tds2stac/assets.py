            ...
            if (
                ...
                for s in service.findall("{%s}service" % constants.unidata):
                    service_url = utils.references_urls(
                        url, s.get("base")
                    ) + harvesting_vars["dataset"].get("urlPath")
                    ...
                    if s.get("name") in [
                        ...
                        if (
                            ...
                            and asset_properties.get("verify_explore_data")
                            is not True
                        ):
                            item.add_asset(
                                key="explore_data",
                    ...
                    if s.get("name") in [
                        ...
                        "http"
                    ] and "Seventh Scenario" not in str(Recognizer_output):
                    ...
                    if asset_properties is None or (
                        ...
                        and asset_properties.get("assets_list_avoided") is None
                    ):
                        item.add_asset(
                            key=s.get("name"),
                            asset=pystac.Asset(
                        ...
                        asset_properties is not None
                        and asset_properties["assets_list_allowed"] is not None
                        and asset_properties["assets_list_avoided"] is not None
                        and s.get("name")
                        in asset_properties["assets_list_allowed"]
                        and s.get("name")
                        not in asset_properties["assets_list_avoided"]
                    ):
                        item.add_asset(
                            key=s.get("name"),
                            asset=pystac.Asset(
        ...
        if asset_properties is not None:
            if (
                ...
                thumbnail.collection(
                    ...
                    stac_catalog,
                    asset_properties["collection_thumbnail_link"],
                    asset_properties["collection_overview_link"],
                    logger_properties=self.logger_properties,
                )
            if asset_properties.get("collection_custom_asset") is not None:
                collection = stac_catalog.get_child(
                    dict(collection_dict)["collection_id"]
                )
