...
def lighter(base_color, n_color):
    ...
    lighter_lightness = list(range(_min, _max, (_max - _min) // n_color))[
        0:n_color
    ]
    # the below code blocked is generated by CodeWhisperer.
    lighter_colors = [
        hls2hex((_hls[0], lighter_lightness[index]/(_max), _hls[2]))
        for index in range(0, n_color)
    ]
    return lighter_colors
...
def main():
    ...
    palette = Palette(
        base_colors_total, gradations_total, general_max_color, dark_max_color
    )
