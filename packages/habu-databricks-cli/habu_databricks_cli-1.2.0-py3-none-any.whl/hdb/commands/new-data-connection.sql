...
-- MAGIC
-- MAGIC try:
-- MAGIC   db_nm = getArgument("db_nm", "")
-- MAGIC   schema_nm = getArgument("schema_nm", "")
-- MAGIC   table_nm = getArgument("table_nm", "")
-- MAGIC   data_connection_id = getArgument("data_connection_id", "")
-- MAGIC   org_id = getArgument("org_id", "")
-- MAGIC   dataset_type = getArgument("dataset_type", "")
-- MAGIC   
-- MAGIC   sqlContext.sql(f"select 1 from {db_nm}.{schema_nm}.{table_nm} limit 1")
-- MAGIC
-- MAGIC   sqlContext.sql(f"DELETE FROM HABU_CLEAN_ROOM_COMMON.DATA_CONNECTIONS.DATA_CONNECTION_COLUMNS WHERE DATA_CONNECTION_ID = '{data_connection_id}'")
-- MAGIC
-- MAGIC   sqlContext.sql(f"DELETE FROM HABU_CLEAN_ROOM_COMMON.DATA_CONNECTIONS.DATA_CONNECTIONS WHERE ID = '{data_connection_id}'")
-- MAGIC
-- MAGIC   sqlContext.sql(f"""INSERT INTO HABU_CLEAN_ROOM_COMMON.DATA_CONNECTIONS.DATA_CONNECTIONS 
...
-- MAGIC   """)
-- MAGIC
-- MAGIC   sqlContext.sql(f"""INSERT INTO HABU_CLEAN_ROOM_COMMON.DATA_CONNECTIONS.DATA_CONNECTION_COLUMNS (ID, ORGANIZATION_ID, DATA_CONNECTION_ID, COLUMN_NAME, COLUMN_POSITION, DATA_TYPE, NUMERIC_PRECISION, NUMERIC_SCALE)
-- MAGIC   (
