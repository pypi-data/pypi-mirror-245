...
-- MAGIC         else:
-- MAGIC             print(f"running notebook {notebook} with {args} failed", e)
-- MAGIC             message = response.get('message', '')
-- MAGIC             stack_trace = response.get('stack_trace', '')
-- MAGIC             sqlContext.sql(f"insert into habu_clean_room_common.clean_room.clean_room_errors (code, state, message, stack_trace, created_at, request_id, proc_name) select '-100', 'FAILED', '{message}', '{stack_trace}', current_timestamp, '{request_id}', '{notebook}'")
-- MAGIC             return "FAILED"
...
-- MAGIC         sqlContext.sql(f"insert into habu_clean_room_common.clean_room.clean_room_errors (code, state, message, stack_trace, created_at, request_id, proc_name) select '-1', 'FAILED', '{message}', '{stack_trace}', current_timestamp, '{request_id}', '{notebook}'")
-- MAGIC         return "FAILED"
-- MAGIC
...
-- MAGIC     sqlContext.sql(f"insert into habu_clean_room_common.clean_room.clean_room_requests select \'{record['id']}\', \'{record['request_type']}\', (select request_data from {share_db}.clean_room.clean_room_requests where id = \'{record['id']}\'), \'{record['created_at']}\', current_timestamp, '{curr_record_status}'")
