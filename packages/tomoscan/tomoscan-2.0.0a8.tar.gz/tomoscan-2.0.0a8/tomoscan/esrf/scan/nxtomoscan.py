    ...
    def build_icat_metadata(self) -> dict:
        ...
        def get_z_delta_and_z_start():
            if self.z_translation is None or len(self.z_translation) == 0:
                ...
                unique_z = numpy.unique(
                    numpy.array(self.z_translation)[
                        self.image_key_control == ImageKey.PROJECTION.value
                    ]
                )
                if len(unique_z) > 1:
        ...
        return {
            ...
            "zStart": z_start,
            "zDelta": z_delta,
            "refN": self.flat_n if self.flat_n is not None else UNKNOW_VALUE,
            "darkN": self.dark_n if self.dark_n is not None else UNKNOW_VALUE,
            "exposureTime": get_exposure_time(),
            "detectorDistance": self.get_distance() / MetricSystem.MICROMETER.value
            if self.get_distance() is not None
            else UNKNOW_VALUE,
            # "sourceSampleDistance": , # source / sample distance
            "halfAcquisition": str(self.field_of_view) is FOV.HALF
            if self.field_of_view is not None
            else UNKNOW_VALUE,  # requested value is a float.
            "scanRange": self.scan_range
            if self.scan_range is not None
            else UNKNOW_VALUE,
            "pixelSize": get_pixel_size_micron(),
            "start_angle": get_start_angle(),
            "x_pixel_n": self.dim_1 if self.dim_1 is not None else UNKNOW_VALUE,
            "y_pixel_n": self.dim_2 if self.dim_2 is not None else UNKNOW_VALUE,
