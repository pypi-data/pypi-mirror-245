...
def PlayWAV(sound_name: String) -> Action:
    """Play WAV file.
...
def SetCountdownTimer(time_modifier: Modifier, time: Dword) -> Action:
...
def LeaderBoardResources(resource_type: Resource, label: String) -> Action:
...
def KillUnitAt(count: Count, unit: Unit, where: Location, for_player: Player) -> Action:
...
def RemoveUnitAt(count: Count, unit: Unit, where: Location, for_player: Player) -> Action:
    """Remove all units for player.
...
def LeaderBoardGoalResources(goal: Dword, resource_type: Resource, label: String) -> Action:
...
def MoveUnit(
    count: Count, unit_type: Unit, owner: Player, start_location: Location, dest_location: Location
) -> Action:
...
def SetNextScenario(scenario_name: String) -> Action:
...
def CreateUnit(number: Byte, unit: Unit, where: Location, for_player: Player) -> Action:
...
def Order(
    unit: Unit, owner: Player, start_location: Location, order_type: _Order, dest_location: Location
) -> Action:
