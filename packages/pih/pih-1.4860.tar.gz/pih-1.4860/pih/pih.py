...
class PIH:
    ...
    class ERROR:
        ...
        @contextmanager
        def detect(final_action: Callable[[None], None] | None = None) -> bool:
            try:
                yield True
    ...
    class DATA(DataTool, StringTool, ListTool, DateTimeTool, EnumTool, FullNameTool):
        class JOURNAL:
            @staticmethod
            def type_by_any(value: str | int | None) -> JournalType | None:
                if n(value):
                    return None
                is_str: bool = isinstance(value, str)
                try:
                    value = int(value)
                    is_str = False
                ...
                for item in JournalType:
                    ...
                    if (not is_str and item_value[0] == value) or (
                        is_str
                        and (
                            item_value_value.name.lower() == value
                            or item_value_value.caption.lower().find(value) != -1
                        )
                    ):
                        return item
                return None
            ...
            def tag_by_id(value: int) -> Tag | None:
                for item in Tag:
                    item_value: tuple[int, str, str] = EnumTool.get(item)
                    item_value_value: ParamItem = item_value[1]
                    if item_value[0] == value:
                        return item
                return None
        ...
        class MATERIALIZED_RESOURCES:
            ...
            def add_count(type_value: MATERIALIZED_RESOURCES.TYPES, value: int) -> bool:
                return PIH.ACTION.DATA_STORAGE.value(
                    PIH.DATA.MATERIALIZED_RESOURCES.get_count(value) + value,
                    type_value.name,
                    MATERIALIZED_RESOURCES.NAME,
                )
        ...
        class FORMAT:
            ...
            @staticmethod
            def format(value: str, local: dict[str, Any] | None = None) -> str:
                parameters: dict[str, Any] = globals()
                if nn(local):
                    for lkey, lvalue in local.items():
                        parameters[lkey] = lvalue
                    ...
                    try:
                        if ne(re.findall(r"print\(.*?\)", part)):
                            ...
                            with redirect_stdout(io):
                                exec(part[1:-1], globals(), parameters)
    ...
    class RESULT(ResultTool):
        class JOURNALS:
            ...
            def get(
                ...
                def convert(event: EventDS) -> JournalRecord:
                    ...
                    # event.parameters[PARAM_ITEMS.LOGIN.name] = user
                    tag: int = event.parameters[PARAM_ITEMS.TAG.name]
                    for item in Tag:
                        if EnumTool.get(item)[0] == tag:
                            event.parameters[PARAM_ITEMS.TAG.name] = item
                            break
                    type_value: int = event.parameters[PARAM_ITEMS.TYPE.name]
                    for item in JournalType:
                        if EnumTool.get(item)[0] == type_value:
                            event.parameters[PARAM_ITEMS.TYPE.name] = item
                            break
        ...
        class NOTES:
            ...
            @staticmethod
            def files(
                name: str | None = None, cached: bool = False
            ) -> Result[list[Note]]:
                ...
                if n(PIH.RESULT.NOTES.cache):
                    cached = False
                if cached:
                    result = ResultTool.filter(search_function, PIH.RESULT.NOTES.cache)
                if not cached or e(result):
                    PIH.RESULT.NOTES.cache = PIH.RESULT.NOTES.by_label(
                        CONST.NOTES.FILE_SECTION
                    )
                    result = ResultTool.filter(
                        search_function,
                        PIH.RESULT.NOTES.cache,
                    )
                return result
        ...
        class POLIBASE:
            ...
            def persons_by_pin(
                ...
                if ne(value) and ResultTool.is_empty(result):
                    raise PIH.ERROR.POLIBASE.create_not_found_error(
                        "идентификационным номером", value
                    )
                return result
