...
from pih.tools import j, nl, js, jnl, b as mb, while_not_do, esc, e, ne, nn, n, i, one
                        ...
                        self.output.write_line(
                            j(
                                (
                                    ...
                                    self.italic("Свободное пространство"),
                                )
                            )
                        )
        ...
        with self.output.make_indent(2):
            ...
            if CheckableSections.DISKS in checkable_section_list:
                ...
                def label_function(resource: DisksResourceStatus, _: int) -> str:
                    ...
                    def every_action(item: DiskStatistics) -> None:
                        result.append(
                            j(
                                (
                                    js(("", A.CT_V.BULLET, "")),
                                    self.bold(item.name),
                                    ": ",
                                    round(100 * item.free_space / item.size, 2),
                                    js(
                                        (
                                            "%",
                                            A.CT_V.ARROW,
                                            A.D_F.size(item.free_space),
                                            "/",
                                            A.D_F.size(item.size),
                                        )
                                    ),
                                )
                            )
