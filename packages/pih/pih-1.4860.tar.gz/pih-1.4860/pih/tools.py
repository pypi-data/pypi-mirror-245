...
def one(value: T | list[T]) -> T:
    return DataTool.get_first_item(value)
...
class ResultTool:
    ...
    def filter(
        ...
        try:
            result.data = DataTool.filter(filter_function, result.data)
...
e = DataTool.is_empty
ne = lambda item: not DataTool.is_empty(item)
