...
class Download:
    ...
    def DownYouTubeVideo(self):
        url = 'https://onlinedownloader.net/wp-content/plugins/Codehap_YTDl/result.php'

        data = {'codehap_link': self.link}

        req = requests.post(url,data).text

        vl = re.findall('<a class="dl_link" href="([^"]+)',req)[3]
        return {'link':vl}
    ...
    def DownYouTubeAudio(self):
        ...
        al = re.findall('<a class="dl_link" href="([^"]+)',req)[8]
        return {'link':al}
