...
class BatchJob:
    ...
    def get_jobs_status(self, batch_id: str):
        ...
        Returns:
            Response dict
        """
        resp = get(url=self.base_url + f"batchJobs/{batch_id}/complete", headers=self.headers, ssl_verify=self.ssl_verify)
    ...
    def _get_batch_response(self, job_id_list: list, as_of_date: str = None, snapshot_types: list = None):
        ...
        resp = post(url=self.base_url + f"jobs/bulkResults", headers=self.headers, data=body, ssl_verify=self.ssl_verify)
        return resp.json()
    ...
    def cancel_jobs(self, job_ids: list):
        ...
        resp = post(url=self.base_url + f"bulkJobs/cancel", headers=self.headers, data=job_ids, ssl_verify=self.ssl_verify)
        return json.loads(resp.text)
