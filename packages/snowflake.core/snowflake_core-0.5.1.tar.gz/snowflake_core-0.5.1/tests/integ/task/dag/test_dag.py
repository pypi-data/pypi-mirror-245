...
def test_deploy_dag_with_branch(schema, db_parameters):
    ...
    try:
        ...
        try:
            ...
            assert f"SYSTEM$GET_PREDECESSOR_RETURN_VALUE('{normalize_name(task2_branch.full_name)}') = '{task4.name}'" \
                   == task4.condition
            assert task1_branch in task2.predecessors
            assert task1_branch in task3.predecessors
            assert task2_branch in task4.predecessors
        finally:
            op.delete(dag)
    finally:
        schema._connection.execute_string(f"drop stage {test_stage}")
...
def test_add_one_predecessor(schema):
    ...
    with DAG(dag_name, schedule=timedelta(minutes=10)) as dag:
        ...
        assert task1.predecessors == set([task2])
        assert dag.get_task("task2") == task2
