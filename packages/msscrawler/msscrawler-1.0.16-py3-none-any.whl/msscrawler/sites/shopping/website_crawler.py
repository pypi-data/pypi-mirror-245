        ...
        while True:
            if first_time:
                ...
                if date_time_now >= next_time:
                    self.init_schedule()
                    next_time = date_time_now + timedelta(
                        seconds=self.loop_time
                    )
                else:
        ...
        try:
            ...
            for website in collection.find():
                ...
                for job in cron:
                    if job.comment == website["spider_name"]:
                        ...
                        if (
                                ...
                                job.slices.render() == "* * * * *"
                                and website["schedule"].replace("/1", "") != job.slices.render()
                        ):
                ...
                if not found:
                    ...
                    self.get_logger().info(f"[*] Init new job for {website['spider_name']}")
