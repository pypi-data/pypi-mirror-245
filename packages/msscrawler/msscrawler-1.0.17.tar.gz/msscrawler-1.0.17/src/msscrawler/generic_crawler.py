...
class GenericCrawler(BaseCrawler, SpiderCacheMixin):
        ...
        try:
            ...
            with self.database_client.connection.start_session(
                ...
                session.with_transaction(
                    lambda s: self.process_data_callback(
                        s, record, body_message, method.delivery_tag
                    ),
            ...
            with self.database_client.connection.start_session(
                ...
                session.with_transaction(
                    lambda s: self.update_data_callback(
                        s,
                        record["url"],
                        {
    ...
    def process_data_callback(
        self, session, record, body_message, rabbit_method_delivery_tag
    ):
        if record:
            self.data_coll.update_one(
                {"url": body_message["url"]},
            ...
            if self.name == "product":
                ...
                if exist_product:
                    if self.check_crawled_in_previous_time(exist_product):
                        return self.connector_client.send_processed_message_status(
                            is_processed=True,
                            delivery_tag=rabbit_method_delivery_tag,
                        )
                    ...
                    self.data_coll.update_one(
                        {"unique_product_id": exist_product["unique_product_id"]},
                        {"$set": self.build_update_object(body_message)},
                        session=session,
                    )
                else:
    ...
    def update_data_callback(self, session, url, update_obj):
        self.data_coll.update_one(
            {"url": url},
            {
                "$set": update_obj,
