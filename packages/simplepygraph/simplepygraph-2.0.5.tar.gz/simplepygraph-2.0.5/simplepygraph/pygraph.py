...
class Graph:
    ...
    def _repr_mimebundle_(self, *args, **kwargs):
        return self.__view._repr_mimebundle_(*args, **kwargs)
    ...
    def label_on(self, on_side=False):
        for node_id in self.node_ids():
            self.node_view(node_id).label_on()
            if on_side:
                self.node_view(node_id).label_on_side()
            else:
                self.node_view(node_id).label_off_side()


    def label_off(self, off_side=True):
        for node_id in self.node_ids():
            self.node_view(node_id).label_off()
            if off_side:
                self.node_view(node_id).label_off_side()
...
class DiGraph(Graph):
    ...
    def __init__(self, nodes_count=0, random=False, engine='neato', strict=False):
        Graph.__init__(self, nodes_count, random=random, directed=True, strict=strict, engine=engine)
