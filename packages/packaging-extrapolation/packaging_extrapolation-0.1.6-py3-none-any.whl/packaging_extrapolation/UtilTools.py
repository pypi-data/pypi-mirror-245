...
def train_dt_k_fold(model, *, X, y, k, method, init_guess=0.001):
    ...
    for i in range(len(train_index_list)):
        ...
        _, alpha_list = train(model, x_energy_list=x_energy_list_train,
                              ...
                              limit_list=y_train, method=method, level=level, init_guess=init_guess, is_Fitting=True)
...
def train(model, *, method, x_energy_list, y_energy_list, limit_list=None, init_guess=0.01, level='dt', is_Fitting=False, alpha=None):
...
def plot_alpha_value(model, *, mol_list, x_energy_list, y_energy_list, fitting_list,
    ...
    energy_list = train(model, method=method, x_energy_list=x_energy_list,
                        y_energy_list=y_energy_list,
