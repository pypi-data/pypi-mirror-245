...
from arthurai.common.exceptions import UnexpectedValueError, UserValueError, UserTypeError
...
def add_predictions_or_ground_truth(inference_data: List[Dict[str, Any]],
    ...
    if stage == Stage.GroundTruth or stage == Stage.GroundTruthClass:
        ...
        raise UnexpectedValueError(f"Unexpected stage {stage}, should be GroundTruth or PredictedValue")
...
def nest_reference_data(data: List[Dict[str, Any]], attributes: List[ArthurAttribute]) -> \
        List[Dict[str, Any]]:
    ...
    if not isinstance(data, list):
        raise UserValueError(f"Data should be a list of dicts, but has type {type(data)}")
    elif len(data) == 0:
        return []
    ...
    for i in range(len(data)):
        ...
        if not isinstance(row, dict):
            raise UserValueError(f"Data should be a list of dicts, but is a list of type {type(row)}")
        elif "reference_data" in row.keys() and intersection_is_non_empty(attr_names, row.keys()):
            raise UserValueError(f"Reference data should be nested or non-nested, not mixed. Row {i} "
                                 f"contains 'reference_data' field and reference attribute fields.")
        elif "reference_data" in row.keys() and not isinstance(row["reference_data"], dict):
            raise UserTypeError(f"'reference_data' field present but is of type {type(row['reference_data'])}, "
                                f"should be dict")
