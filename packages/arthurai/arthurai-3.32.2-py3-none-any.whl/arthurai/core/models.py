...
from arthurai.common.constants import API_PREFIX, API_PREFIX_V4, AccuracyMetric, Enrichment, IMAGE_FILE_EXTENSION_MAP, \
    ...
    Stage, TextDelimiter, ValueType, PARQUET_INGESTIBLE_INPUT_TYPES, PARQUET_INGESTIBLE_OUTPUT_TYPES, InferenceType, \
...
class ArthurModel(ArthurBaseJsonDataclass):
    ...
    def validate_reference_data(self, reference_dataframe: DataFrame) -> None:
        ...
        if self.input_type in PARQUET_INGESTIBLE_INPUT_TYPES and self.output_type in PARQUET_INGESTIBLE_OUTPUT_TYPES:
            try:
    ...
    def get_attribute_names(self, stage: Optional[Stage] = None, value_type: Optional[ValueType] = None) -> List[str]:
        """
        ...
        if stage is None and value_type is None:
            ...
            return [attr.name for attr in self.attributes if attr.stage == stage and attr.value_type == value_type]
        elif stage is None:
            return [attr.name for attr in self.attributes if attr.value_type == value_type]
        else:
    ...
    def _send_files(self,
        ...
        if data is not None:
            if isinstance(data, DataFrame):
                ...
                directory_path = self.convert_dataframe(df=data, is_ref_data=is_ref_data)
            else:
    ...
    def convert_dataframe(self, df: DataFrame, is_ref_data: bool, max_rows_per_file=DatasetService.MAX_ROWS_PER_FILE) -> str:
        ...
        for chunk in range(num_chunks):
            ...
            if to_parquet:
                ...
                if is_ref_data:
                    ...
                    data_list = self._format_inferences(inferences=df.iloc[start_idx:end_idx], format_timestamps=True)
    ...
    def _ref_df_to_json(self, data: DataFrame, format_timestamps: bool = True) -> str:
        ...
        timestamp_attributes = self.get_attribute_names(value_type=ValueType.Timestamp) if format_timestamps else None
        time_series_attributes = self.get_attribute_names(value_type=ValueType.TimeSeries)
        references = arthur_util.format_timestamps(references, is_reference_data=True,
                                                   timestamp_attributes=timestamp_attributes,
                                                   time_series_attributes=time_series_attributes)

        # do more numpy standardization
        for i in range(len(references)):
            references[i][InferenceType.REFERENCE_DATA] = self._replace_nans_and_infinities_in_dict(references[i][InferenceType.REFERENCE_DATA])
            references[i][InferenceType.REFERENCE_DATA] = self._convert_numpy_to_native(references[i][InferenceType.REFERENCE_DATA])
    ...
    def _format_inferences(self,
        ...
        if inference_timestamps is not None:
            update_column_in_list_of_dicts(inferences, TimestampInferenceType.INFERENCE_TIMESTAMP, inference_timestamps)
        if ground_truth_timestamps is not None:
            update_column_in_list_of_dicts(inferences, TimestampInferenceType.GROUND_TRUTH_TIMESTAMP, ground_truth_timestamps)
        if partner_inference_ids is not None:
        ...
        for i in range(len(inferences)):
            inferences[i][InferenceType.INFERENCE_DATA] = self._replace_nans_and_infinities_in_dict(inferences[i][InferenceType.INFERENCE_DATA])
            inferences[i][InferenceType.INFERENCE_DATA] = self._convert_numpy_to_native(inferences[i][InferenceType.INFERENCE_DATA])
            if InferenceType.GROUND_TRUTH_DATA in inferences[i].keys():
                inferences[i][InferenceType.GROUND_TRUTH_DATA] = self._convert_numpy_to_native(inferences[i][InferenceType.GROUND_TRUTH_DATA])
    ...
    def send_bulk_ground_truths(self, directory_path: Optional[str] = None,
        ...
        if prepped_data is not None or parquet_files or json_files or files:
            return self._send_files(endpoint, InferenceType.GROUND_TRUTH_DATA, directory_path, prepped_data, files,
    ...
    def send_bulk_inferences(self,
        ...
        if prepped_data is not None or parquet_files or json_files or files:
            res = self._send_files(endpoint, InferenceType.INFERENCE_DATA, directory_path, prepped_data, files,
