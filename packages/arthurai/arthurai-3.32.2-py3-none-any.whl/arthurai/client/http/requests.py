...
from arthurai.common.exceptions import UnexpectedTypeError, ResponseServerError, ResponseRedirectError, \
    ArthurUnexpectedError, UserValueError
...
class HTTPClient(AbstractHTTPClient):
    ...
    def send(self,
             ...
             path_prefix: Optional[str] = None) -> Union[Dict, List, bytes, BytesIO, requests.Response]:
        ...
        if multipart:
            ...
            if json is not None:
                if not isinstance(json, dict):
                    raise UnexpectedTypeError(f"Received 'json' parameter with a multipart request but was of type "
                                            f"{type(json)} not dict.")
                for field_name, field_value in json.items():
                    # validate key type
                    if not isinstance(field_name, str):
                        raise UnexpectedTypeError(f"both 'files' and 'json' were supplied but 'json' keys type is "
                    ...
                    if isinstance(field_value, dict) or isinstance(field_value, list):
                        try:
                            ...
                            raise UnexpectedTypeError(f"failed to serialize 'json' input for key '{field_name}'") \
                                from e
                    ...
                    try:
                        ...
                        raise UnexpectedTypeError(f"Received 'data' dict but could not convert field '{field_name}' "
                ...
                if isinstance(files, list):
                    for entry in files:
                        if not (isinstance(entry, tuple) and len(entry) >= 2 and isinstance(entry[0], str)):
                            raise UnexpectedTypeError(f"received list for files argument but did not contain tuples in "
                ...
                # if dict, ensure in tuple format like ("fname", data, [encoding]) or reformat if not
                    ...
                    for fname in files.keys():
                        ...
                        if isinstance(file_obj, tuple):
                            ...
                            raise UnexpectedTypeError(f"files['{fname}'] is of type {type(file_obj)}, not a tuple or "
                                                    f"file-like")
                else:
        ...
        while not success and attempts <= retries:
            ...
            try:
                if multipart:
                    ...
                    if me is None:
                        raise UnexpectedTypeError("Attempted retry on multipart request but MultipartEncoder was None")
                    for fkey in data:
                        try:
                            data[fkey][1].seek(0)
    ...
    def get(self,
        ...
        return self.send(endpoint, method='GET', headers=headers, params=params,
                         ...
                         validation_response_code=validation_response_code, path_prefix=path_prefix)
