...
from arthurai.common.exceptions import MissingParameterError, UnexpectedValueError, UnexpectedTypeError
        ...
        if headers.get('Content-Type') == 'multipart/form-data':
            ...
            if json is not None:
                if not isinstance(json, dict):
                    raise UnexpectedTypeError(f"Received 'json' parameter but was of type {type(json)} not dict")
                for field_name, field_value in json.items():
                    # validate key type
                    if not isinstance(field_name, str):
                        raise UnexpectedTypeError(f"Received 'data' dict but keys were of type {type(field_name)} not "
                                                f"string.")
