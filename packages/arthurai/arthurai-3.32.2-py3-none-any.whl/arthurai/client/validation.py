...
from arthurai.common.exceptions import UnexpectedValueError, ResponseRedirectError, ResponseClientError, \
    ResponseServerError, UnexpectedTypeError, ArthurUserError, ArthurUnexpectedError, PaymentRequiredError
...
def validate_response_status(response_or_code: Union[requests.Response, int],
    ...
    if isinstance(response_or_code, requests.Response):
        ...
        raise UnexpectedTypeError(f"{response_or_code} is of type {type(response_or_code)}, not requests.Response or int")
    ...
    # finally if an expected code is supplied but doesn't match, raise an UnexpectedValueError
        ...
        raise UnexpectedValueError(f"expected response with status code {expected_status_code} but received " +
                                   parsed_response)
...
def validate_multistatus_response_and_get_failures(response: requests.Response, raise_on_failures: bool = False) \
    ...
    if set(body['counts'].keys()) != {'success', 'failure', 'total'}:
        raise UnexpectedValueError(f"response counts keys {set(body['counts'].keys())} are invalid")
    ...
    if len(user_failures) + len(internal_failures) != body['counts']['failure']:
        raise UnexpectedValueError(f"provided response failure count {body['counts']['failure']} does not match "
