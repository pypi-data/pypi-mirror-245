...
from arthurai.common.exceptions import UserValueError, MethodNotApplicableError, ArthurUserError, UnexpectedValueError, \
    UserTypeError
    ...
    def __init__(self,
        ...
        for attr in predicted_attributes:
            if attr.stage != Stage.PredictedValue:
                raise UnexpectedValueError(f"Attribute {attr.name} in predicted_attributes parameter is of stage "
        ...
        if data is not None:
            ...
            if len(data) != len(predictions_df):
                raise UnexpectedValueError(f"Length of input data and predictions do not match")
