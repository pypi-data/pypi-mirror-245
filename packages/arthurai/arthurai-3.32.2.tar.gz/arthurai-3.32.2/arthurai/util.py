...
def format_timestamps(inferences: List[Dict[str, Any]], tz: Optional[Union[timezone, str]] = None,
                      timestamp_attributes: Optional[List[str]] = None,
                      time_series_attributes: Optional[List[str]] = None,
                      is_reference_data: bool = False) -> List[Dict[str, Any]]:
    ...
    """
    inference_key = InferenceType.INFERENCE_DATA if not is_reference_data else InferenceType.REFERENCE_DATA
    if len(inferences) > 0:
        for inference in inferences:
            if TimestampInferenceType.INFERENCE_TIMESTAMP in inference:
                inference_timestamp = inference[TimestampInferenceType.INFERENCE_TIMESTAMP]
                inference[TimestampInferenceType.INFERENCE_TIMESTAMP] = format_timestamp(inference_timestamp, tz)
            if TimestampInferenceType.GROUND_TRUTH_TIMESTAMP in inference:
                ...
                inference[TimestampInferenceType.GROUND_TRUTH_TIMESTAMP] = format_timestamp(gt_timestamp, tz)
            if timestamp_attributes is not None:
                for attr in timestamp_attributes:
                    ...
                    if attr_timestamp is not None:
                        inference[inference_key][attr] = format_timestamp(attr_timestamp, tz)
            if time_series_attributes is not None:
                inference[inference_key] = format_time_series_attr_timestamps(inference, time_series_attributes, tz,
                                                                              is_reference_data)
...
def format_time_series_attr_timestamps(inference: Dict[str, Any], time_series_attributes: List[str],
                                       ...
                                       is_reference_data: bool = False) -> Dict[str, Any]:
    ...
    for attr in time_series_attributes:
        if inference.get(inference_key) is None or not isinstance(inference.get(inference_key), dict):
            raise UserValueError(f"inference does not have the expected {inference_key} field")
        ...
        if time_series_data is not None and core_util.is_list_like(time_series_data):
            for i in range(len(time_series_data)):
                ...
                if time_series_timestamp is not None:
                    inference[inference_key][attr][i]["timestamp"] = format_timestamp(time_series_timestamp, tz,
                                                                                      col_name=attr)
