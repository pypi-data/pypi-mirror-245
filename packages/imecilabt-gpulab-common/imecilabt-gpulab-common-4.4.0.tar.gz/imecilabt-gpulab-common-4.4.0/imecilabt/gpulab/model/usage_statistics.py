...
class WasteReport:
    ...
    def _calc_verdict(self) -> WasteVerdict:
        # This calc was changed.
        # Instead of relying on self.wasted_slot_percent <= 10.0 it now looks at
        # self.waste_since_start.wasted_reason

        # If no waste, always return good.
        if not self.waste_since_start.wasted_reason:
            return WasteVerdict.GOOD

        # Not all jobs can have a meaningful GOOD/BAD verdict yet!
        if self.final and self.total_slot_count <= 4:  # 20 minutes
            return WasteVerdict.SHORT
        if not self.final and self.total_slot_count <= 8:  # 40 minutes
            return WasteVerdict.UNDECIDED

        # new method for verdict:
        if not self.waste_since_start.wasted_reason:
            return WasteVerdict.GOOD
        else:
            return WasteVerdict.WASTE
        # old method for verdict
...
class WasteReview:  # AKA WasteReport2, which replaces the legacy WasteReport
    ...
    verdict_explanation: List[str]  # Why is this job wasting resources? (empty if not wasting)
