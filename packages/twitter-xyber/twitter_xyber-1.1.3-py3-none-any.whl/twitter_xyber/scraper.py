...
class TwitterScraper:
    ...
    def timeline_tweet(self, username: str) -> dict:
        _xhr_calls = []
        tweet_count = 80
        min_count = 20
        max_count = 100
        ...
        with sync_playwright() as pw:
            ...
            if not path.exists("state.json"):
                # If not, call the login method from TwitterBot
                twitter_bot = TwitterBot(headless=False, username='YourUsername', password='YourPassword')
                twitter_bot.login()
    ...
    def scrape_tweet(self) -> dict:
        ...
        with sync_playwright() as pw:
            ...
            page.on("response", intercept_response)
            page.goto("https://twitter.com/i/trends")
            page.wait_for_selector("[data-testid='trend']")
            time.sleep(5)

            for f in _xhr_calls:
                if re.search("guide.json", f.url):
                    trends_calls = [f]
                    break

            trends = []
            for xhr in trends_calls:
                data = xhr.json()
                instruction = data['timeline']['instructions']
                entries = next(ins['addEntries']['entries'] for ins in instruction if ins.get('addEntries') is not None)
                trends_result = next(et['content']['timelineModule']['items'] for et in entries if et['entryId'] == 'trends')

            for tr in trends_result:
                try:
                    trends.append({
                        "name" : tr['item']['content']['trend']['name'],
                        "url" : "https://twitter.com/search?q=" + urllib.parse.quote_plus(tr['item']['content']['trend']['name']),
                        "promoted_content" : None,
                        "query" : urllib.parse.quote_plus(tr['item']['content']['trend']['name']),
                        "rank" : tr['item']['content']['trend']['rank'],
                        "tweet_volume" : int(value_to_float(tr['item']['content']['trend']['trendMetadata']['metaDescription'].split(' ')[0])) if tr['item']['content']['trend']['trendMetadata'].get('metaDescription') is not None else 0,
                        "tweet_volume_string" : tr['item']['content']['trend']['trendMetadata']['metaDescription'] if tr['item']['content']['trend']['trendMetadata'].get('metaDescription') is not None else '',
                    })
                except:
                    pass
