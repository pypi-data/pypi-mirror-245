...
class OpenAIAssistant(LanguageModel):
    ...
    def __init__(self, provider: Literal["k8s"] = "k8s"):
        super().__init__()
        ...
        print("OpenAIAssistant initiated.")
    ...
    def _get_assistant(self):
        ...
        while has_next_page:
            if last_assistant is not None:
                results = self._list_assistants(after=last_assistant.id)
            else:
                results = self._list_assistants()
            ...
            for assistant in results.data:
                ...
                if same_name and same_model:
                    print(
                        f"Assistant {self._assistant_name} found",
                        f"model: {self._model}, id: {assistant.id}.",
                    )
                    return assistant

            has_next_page = results.has_next_page()
            if len(results.data) > 0:
                last_assistant = results.data[-1]
    ...
    async def push_info(self, session_id: str, data: str):
        return await self._append_message(thread_id=session_id, content=data)
    ...
    async def get_recommendation(
        ...
        run = await self._run_thread(thread_id=session_id, instructions=instructions)
        ...
        while run.status not in ["cancelled", "failed", "completed", "expired"]:
            ...
            poll_max_count = poll_max_count - 1

            if poll_max_count < 0:
                return "Thread run exceeded threshold"
