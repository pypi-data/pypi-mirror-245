...
class MoleculePreprocessor:
    ...
    def init_with_smiles(
        cls, listOfSmiles, replaceFilteredSmiles=False, removeSteroInfo=False
    ):
        """
        ...
        for raw_smiles in listOfSmiles:
            ...
            if not mol:
                ...
                log[raw_smiles] = ["No errors detected"]
    ...
    def salt_filter(self):
        ...
        for key in self.dictOfMolecules:
            ...
            sortSmileSplitList = sorted(
                smileSplitList, key=lambda x: x[1], reverse=True
            )
            if (
                ...
                self.log[key].append(
                    "Warning: The salt filter identified a multi-compound SMILES for which the core component could not be determined. A result was generated from the original input, but is probably unreliable."
                )
            else:
                if len(sortSmileSplitList) > 1:
                    self.log[key].append(
                        "Warning: The salt filter has removed at least one component of the input SMILES."
                    )
        ...
        for key in self.dictOfMolecules:
            ...
            if weight < minWeight or weight > maxWeight:
                if self.replaceFilteredSmiles:
                    ...
                    self.log[key].append("!1")
                self.log[key].append(
                    "Warning: The molecular weight is not between 200 and 900 Da. The prediction result may be unreliable."
                )

    def element_filter(
        self, allowedAtomNrs={1, 5, 6, 7, 8, 9, 14, 15, 16, 17, 34, 35, 53}
    ):
        """
        ...
        for key in self.dictOfMolecules:
            ...
            if not len(notAllowedAtomsInMolecule) == 0:
                if self.replaceFilteredSmiles:
                    ...
                    self.log[key].append("!1")
                self.log[key].append(
                    "Warning: Element types other than those present in the training data were detected. A result was generated but is probably unreliable."
                )
    ...
    def neutralise_charges(self):
        ...
        for key in self.dictOfMolecules:
            ...
            if not mol:
                continue
            for i, (reactant, product) in enumerate(neutralization_reactions):
                while mol and mol.HasSubstructMatch(reactant):
                    ...
                    try:
                        ...
                        if not mol:
                            self.log[key].append(
                                "Error: The molecule is broken during neutralization procedure!"
                            )
                            self.log[key].append("!1")
    ...
    def canonalize_tautomer(self, removeStereo=True):
        ...
        for key in self.dictOfMolecules:
            ...
            if not molc:
                self.log[key].append(
                    "Error: The molecule is broken during canonalize procedure!"
                )
                self.log[key].append("!1")
    ...
    def check_molecules_validity(self):
        ...
        for key in self.dictOfMolecules:
            ...
            if not mol:
                self.log[key].append("Error: The invalid molecule!")
                self.log[key].append("!1")
                self.dictOfMolecules[key] = mol
    ...
    def write_log_file(self, filePath=str()):
        ...
        f = open(filePath, "w")
        f.write(
            "log keys !:invalid molecule, 1:removed, 0:changed, S:salt, W:weight, E:element, N:neutralize, C:canonalize, V:validity check.\n"
        )
