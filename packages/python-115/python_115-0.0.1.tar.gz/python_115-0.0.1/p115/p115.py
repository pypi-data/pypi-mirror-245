...
class P115FileSystem:
    ...
    def as_path(
        ...
        id = self.getid(id_or_path, pid)
        path = self.getpath(id_or_path, pid)
    ...
    def chdir(
        ...
        if self.cid == attr["id"]:
            ...
            self.__dict__.update(cid=attr["id"], path=self.getpath(id_or_path, pid))
            return attr["id"]
        else:
    ...
    def getid(self, id_or_path: ID_OR_PATH_TYPE = "", /, pid: Optional[int] = None) -> int:
    ...
    def getpath(self, id_or_path: ID_OR_PATH_TYPE = "", /, pid: Optional[int] = None) -> str:
        if pid is None and (not id_or_path or id_or_path == "."):
            return self.path
        elif isinstance(id_or_path, P115Path):
        ...
        if isinstance(id_or_path, int):
            ...
            ls = []
            attr = self._attr(id)
            ls.extend(p["name"].replace("/", "|") for p in self._files(attr["parent_id"], limit=1)["path"][1:])
            ls.append(attr["name"].replace("/", "|"))
            return "/" + "/".join(ls)
            ...
            if pid is None:
                ...
                return normpath(joinpath(self.getpath(pid), path))
        patht = id_or_path
    ...
    def glob(
        ...
        if pattern == "*":
            ...
            try:
                ...
                return iter((P115Path(self, self.getpath(dirname), **attr),))
        ...
        if isinstance(dirname, int):
            path.__dict__["id"] = self.getid(dir2, dirname)
        elif isinstance(dirname, int):
            path.__dict__["id"] = self.getid(dir2, dirname.id)
        if not path["is_dir"]:
            return iter(())
    ...
    def _iterdir(
        ...
        if not attr["is_dir"]:
            raise NotADirectoryError(errno.ENOTDIR, f"{id_or_path!r} (in {pid!r}) is not a directory")
        if as_path:
            dir_ = self.getpath(id_or_path, pid)
            def wrap(attr):
    ...
    def search(
        ...
        **kwargs, 
        ...
        if as_path:
            def wrap(attr):
                attr = normalize_info(attr)
                return P115Path(self, joinpath(self.getpath(attr["parent_id"]), attr["name"]), **attr)
        else:
            wrap = normalize_info
        search = self._search
        resp = search(payload)
