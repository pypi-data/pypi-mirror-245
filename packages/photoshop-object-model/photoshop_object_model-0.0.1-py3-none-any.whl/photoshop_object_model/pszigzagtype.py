...
class PsSelectionType():
    """
    The selection behavior when a selection already exists: 1: Replace the selected area; 2: Add the selection to an already selected area; 3: Remove the selection from the already selected area; 4: Make the selection only the area where the new selection intersects the already selected area.
    """
...
class PsSmartBlurMode():
    """
    The method to use for smart blurring: 1 : Apply blur to entire image; 2, 3 : Apply blur only to edges of color transitions.
    """
    psSmartBlurNormal = 1
...
class PsSourceSpaceType():
    """
    The color space for source when printing.
    """
    psDocumentSpace = 1
    psProofSpace = 2
...
class PsTextType():
    """
    The type of text: 1: Text that does not wrap; 2: Text that wraps within a bounding box.
    """
    psPointText = 1
    psParagraphText = 2
...
class PsTrimType():
    """
    Type of pixels to trim around an image: 9 = bottom right pixel color; 1 = top left pixel color.
    """
...
class PsUnderlineType():
    """
    The placement of text underlining. Note: 3 and 2 are valid only when PsDirection = 2.
    """
    psUnderlineOff = 1
    psUnderlineRight = 2
    psUnderlineLeft = 3
