...
class SQLQueryBuilder:
    ...
    def select(self, columns: Optional[Union[str, list, tuple, dict]] = None):
        ...
        if isinstance(columns, str):
            ...
            self.query = f"""{self.query} Select {', '.join([f"{col} AS {col_alias}" for col_alias, col in columns.items()])}"""
        return self
    ...
    def set(self, values: dict):
        """更新数据字段"""
        self.query = f"{self.query} SET {', '.join([f'{column} = %s' for column in values.keys()])}"
        self.params.extend(list(values.values()))
        return self
            ...
            if isinstance(conditions, str):
                ...
                for column, condition in conditions.items():
                    if isinstance(condition, dict):
                        ...
                        elif condition_operator in ["LIKE", "NOT LIKE", "=", "<", ">", "<=", ">=", "!=", 'CONTAIN', 'IS']:
            ...
            for cond in where:
                if isinstance(cond, list):
                    if len(cond) == 2:
                        ...
                        value = cond[2]
                        if operator in [ "=", ">", "<", ">=", "<=", "!=", "LIKE", "NOT LIKE", "IN", "NOT IN",]:
