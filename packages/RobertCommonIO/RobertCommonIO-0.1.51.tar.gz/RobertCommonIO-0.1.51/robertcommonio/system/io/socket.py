...
class SocketIPAccssor:
    ...
    def start(self, thread: bool = True):
        if self.config.MODE in [SocketType.TCP_SERVER, SocketType.UDP_SERVER]:
            ...
            if thread is True:
                self.threads['create_server'] = Thread(target=self.create_client, name=f"create_client")
            else:
                self.create_client()

        if self.config.RECV_TIMEOUT is not None and self.config.RECV_TIMEOUT > 0:
            self.threads['clear_invalid'] = Thread(target=self.clear_invalid, name=f"clear_invalid")
