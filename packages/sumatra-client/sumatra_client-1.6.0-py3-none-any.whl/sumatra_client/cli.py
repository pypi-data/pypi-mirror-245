...
def plan(args) -> None:
    ...
    client = Client()
    print(f"Comparing '{scowl_dir}' to LIVE on {CONFIG.instance} ({client.workspace})\n")
...
def workspace_apply_template(args):
    ...
    print(f"Successfully applied template '{args.template}' to workspace '{args.workspace}'")
        ...
        if "subcommands" in cmd:
            ...
            for subcmd in sorted(cmd["subcommands"], key=lambda c: c["name"]):
                ...
                if cmd["name"] == "workspace":
                    if subcmd["name"] in ["delete", "show", "select", "create", "apply-template"]:
