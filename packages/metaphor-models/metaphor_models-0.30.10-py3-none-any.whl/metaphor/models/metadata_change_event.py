...
class ExternalSearchDocument:
    ...
    """Embedding arr, generated from OpenAI embedding model."""
    embedding_string: Optional[str] = None
    """Optional embeddingString; raw text corresponding to the embedding for RAG."""
    ...
    """Unique (but not deterministic) id for node."""
    ...
    def from_dict(obj: Any) -> 'ExternalSearchDocument':
        assert isinstance(obj, dict)
        embedding = from_union([lambda x: from_list(from_float, x), from_none], obj.get("embedding"))
        embedding_string = from_union([from_str, from_none], obj.get("embeddingString"))
        ...
        node_id = from_union([from_str, from_none], obj.get("nodeId"))
        page_id = from_union([from_str, from_none], obj.get("pageId"))
        return ExternalSearchDocument(embedding, embedding_string, last_refreshed, metadata, node_id, page_id)

    def to_dict(self) -> dict:
        result: dict = {}
        if self.embedding is not None:
            result["embedding"] = from_union([lambda x: from_list(to_float, x), from_none], self.embedding)
        if self.embedding_string is not None:
            result["embeddingString"] = from_union([from_str, from_none], self.embedding_string)
        ...
        if self.node_id is not None:
            result["nodeId"] = from_union([from_str, from_none], self.node_id)
