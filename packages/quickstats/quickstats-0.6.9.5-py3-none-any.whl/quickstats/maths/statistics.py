    ...
    for i in range(n_bins):
        ...
        if (obs_aggregate < 2):
            if i != (n_bins - 1):
                ...
                obs_aggregate = np.sum(data_obs[bin_last:])
                exp_aggregate = np.sum(data_exp[bin_last:])
                nll += -1 * calculate_nll(obs_aggregate, exp_aggregate)
                # saturated
                nll_sat += -1 * calculate_nll(obs_aggregate, obs_aggregate)
                if nbin_nll == 0:
                    nbin_nll += 1
        else:
            nll_last = -1 * calculate_nll(obs_aggregate, exp_aggregate)
            nll_sat_last = -1 * calculate_nll(obs_aggregate, obs_aggregate)
            nll += nll_last
            nll_sat += nll_sat_last
            bin_last = i
            nbin_nll += 1
...
def get_clipped_data(x:np.ndarray,
    ...
    if (bin_range is None) or ((clip_lower is False) and (clip_upper is False)):
        return np.array(x)
