...
class HaloInfiniteMatchAPI:
    """A high-level wrapper for `HaloInfiniteClient` for requesting match data.

    This class integrates requests using the `client` to provide a
    ...
    def __init__(
        ...
        cache: BaseCache | Callable[[], BaseCache] | None = Cache.MEMORY,
    ) -> None:
        """Initialize a high-level API for requesting Halo Infinite match data.

        Args:
            client: The HaloInfiniteClient to use for API calls.
            cache: The cache to use for storing requests. Defaults to
                aiocache.SimpleMemoryCache with a NullSerializer. Set to None
                to disable caching.
        """
        self.client = client
        if cache is None:
    ...
    async def get_match(self, match_id: str | UUID) -> Match:
        ...
        Returns:
            The requested match.
        """
        cached = await self._cache.get(match_id)
