...
@splunk_appinspect.tags(Tags.CLOUD, Tags.MANUAL, Tags.AST)
...
def check_for_hidden_python_files(app: "App", reporter: "Reporter") -> None:
    ...
    if platform.system() == "Windows":
        ...
        for filepath in client.get_hidden_python_files():
            ...
            byte_string = open(Path(app.app_temp_dir, filepath), "rb").read()
...
@splunk_appinspect.tags(
    Tags.CLOUD, Tags.AST, Tags.PRIVATE_APP, Tags.PRIVATE_VICTORIA, Tags.MIGRATION_VICTORIA, Tags.PRIVATE_CLASSIC
)
@splunk_appinspect.cert_version(min="1.7.0")
...
class CheckPythonSdkVersion(Check):
    ...
    def check_python_sdk(
        self, app: "App", path_in_app: Path, line_number: int, result: "ApplicationValidationReport"
