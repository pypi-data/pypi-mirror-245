...
def check_that_extracted_splunk_app_does_not_contains_only_app_conf_file(app: "App", reporter: "Reporter") -> None:
    """Check that the extracted Splunk App does not contains only  `app.conf`"""
    for directory, filename, _ in app.iterate_files():
        if str(directory).strip(os.sep) in ("local", "default") and filename != "app.conf":
            return
        elif str(directory).strip(os.sep) not in ("local", "default"):
            return
...
class CheckValidVersionNumber(Check):
    ...
    def _is_valid_version_number(version_number: str) -> bool:
        ...
        if isinstance(parsed_version, version.LegacyVersion):
            # If the version gets parsed as a LegacyVersion, that means it's not PEP-440 compatible
            # Things like "1.3.xy123" and "whatchamacallit" both fall into this category
            # We'll rely on regex and semver to decide if it's valid. This is acceptable because
            # it's still a subset of what Splunkbase supports.
    ...
    def check(self, app: "App") -> Generator[CheckMessage, Any, None]:
        ...
        # Run all of the check methods
        yield from super().check(app) or []

        # Verify that all of the versions match
    ...
    def check_config(self, app: "App", config: "ConfigurationProxy") -> Generator[CheckMessage, Any, None]:
        SETTINGS_TO_CHECK = (
            # (conf_file, section, option, action_if_missing, action_if_invalid)
        ...
        for (
            ...
            if not section.has_option(option_name):
                ...
                yield action_if_missing(
                    f"A `{option_name}` attribute formatted as Major.Minor.Revision is required in "
            ...
            if not self._is_valid_version_number(option.value):
                yield action_if_invalid(
                    ...
                    "must be specified as `Major.Minor.Revision`.",
