...
def parse_version(version_str: Union[ConfigurationSetting, str]) -> Optional[Union[Version, LegacyVersion]]:
...
class CheckForAddonBuilderVersion(Check):
    def __init__(self) -> None:
        super().__init__(
            config=CheckConfig(
                ...
                description=f"Check that the `addon_builder.conf` contains an builder version number between `{self.MINIMUM_BUILDER_VERSION}` and `{self.MAXIMUM_BUILDER_VERSION}` in the `[base]` stanza. "
    ...
    MAXIMUM_BUILDER_VERSION = parse_version("4.1.4")
