...
VALIDATION_EPILOG = """Splunk AppInspect uses Python's default `logging` library. If you are familiar with native Python logging, you can extend its capabilities. 
...
Validate an app using test mode and filters:
    ...
    $ splunk-appinspect inspect filepath/app-package.tgz --mode test --included-tags manual --included-tags appapproval --excluded-tags cloud
...
Validate an app using precert mode and filters:
    ...
    $ splunk-appinspect inspect filepath/app-package.tgz --mode precert --included-tags manual --included-tags appapproval --excluded-tags cloud
...
List only groups and checks with the `appapproval` tag:

    $ splunk-appinspect list groups checks --included-tags appapproval
