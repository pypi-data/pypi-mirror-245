...
class Pipe(Generic[I, O], Iterable[O], ABC):
    upstream: "Optional[Pipe[Any, I]]"

    @staticmethod
    def from_source(source: Callable[[], Iterable[O]]) -> "Pipe[None, O]":
    ...
    def __add__(self, other: "Pipe[Any, O]") -> "Pipe[Any, O]":
    ...
    def chain(self, *others: "Pipe[Any, O]") -> "Pipe[O, O]":
        """
    ...
    def filter(self, predicate: Callable[[O], bool]) -> "Pipe[O, O]":
        """
    ...
    def batch(
        self, size: int = 100, period: float = float("inf")
    ) -> "Pipe[O, List[O]]":
        """
    ...
    def slow(self, freq: float) -> "Pipe[O, O]":
        """
    ...
    def log(self, what: str = "elements", colored: bool = False) -> "Pipe[O, O]":
        """
...
class FilterPipe(Pipe[Y, Y]):
    def __init__(self, upstream: Pipe[X, Y], predicate: Callable[[Y], bool]):
        self.upstream: Pipe[X, Y] = upstream
...
class MapPipe(Pipe[Y, Z]):
    def __init__(self, upstream: Pipe[X, Y], func: Callable[[Y], Z], n_threads: int):
        self.upstream: Pipe[X, Y] = upstream
        self.func = func
...
class DoPipe(Pipe[Y, Y]):
    def __init__(self, upstream: Pipe[X, Y], func: Callable[[Y], Any], n_threads: int):
        self.upstream: Pipe[X, Y] = upstream
        self.func = func
        self.n_threads = n_threads
...
class LogPipe(Pipe[Y, Y]):
    def __init__(self, upstream: Pipe[X, Y], what: str, colored: bool):
        self.upstream: Pipe[X, Y] = upstream
        self.what = what
...
class FlattenPipe(Pipe[Iterable[Y], Y]):
    def __init__(self, upstream: Pipe[X, Iterable[Y]], n_threads: int) -> None:
        self.upstream: Pipe[X, Iterable[Y]] = upstream
        self.n_threads = n_threads
...
class BatchPipe(Pipe[Y, List[Y]]):
    def __init__(self, upstream: Pipe[X, Y], size: int, period: float):
        self.upstream: Pipe[X, Y] = upstream
        self.size = size
        self.period = period
...
class ChainPipe(Pipe[Y, Y]):
    def __init__(self, upstream: Pipe[X, Y], others: List[Pipe]):
        self.upstream: Pipe[X, Y] = upstream
        self.others = others
...
class SlowPipe(Pipe[Y, Y]):
    def __init__(self, upstream: Pipe[X, Y], freq: float):
        self.upstream: Pipe[X, Y] = upstream
        self.freq = freq
