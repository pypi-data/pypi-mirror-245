...
class TestPipe(unittest.TestCase):
    ...
    def test_flatten_typing(self) -> None:
        a: Pipe[Any, str] = Pipe.from_source("abc".__iter__).map(iter).flatten()
        b: Pipe[Iterable[str], str] = (
            Pipe.from_source("abc".__iter__).map(list).flatten()
        )
        c: Pipe[Iterable[str], str] = (
            Pipe.from_source("abc".__iter__).map(set).flatten()
        )
    ...
    def test_flatten(self, n_threads: int):
        ...
        get_pipe: Callable[[], Pipe[Any, int]] = lambda: (
            Pipe.from_source(
                lambda: map(
                    raise_for_4,
                    [
    ...
    def test_accept_typing(self) -> None:
        p: Pipe[Iterable[int], int] = (
            Pipe.from_source(lambda: range(10)).batch().flatten()
        )
        it: Iterator[int] = iter(p)
        from kioss._visit._iter_production import IteratorProducingVisitor

        it2: Iterator[int] = p._accept(IteratorProducingVisitor())
        from kioss._visit._explanation import ExplainingVisitor

        plan: str = p._accept(ExplainingVisitor())
