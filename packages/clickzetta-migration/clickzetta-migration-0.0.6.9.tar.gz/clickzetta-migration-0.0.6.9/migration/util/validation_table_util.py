...
def pk_id_column_validation(source_table: str, destination_table: str, source: Source, destination: Destination, pk_id: str):
    try:
        ...
        min_id = max_min_id[0]
        max_id = max_min_id[1]
        logger.info(f"{source_table}-source_min_id: {min_id}, source_max_id: {max_id}, prepare to check entire table")
        cur_id = min_id
        cur_max_id = 0
        only_in_source = 0
        only_in_source_list = []
        ...
        while cur_id <= max_id:
            ...
            for r in source_result:
                ids_source.add(r[0])
                cur_max_id = r[0]
            logger.debug(f'cur_max_id={cur_max_id}, ')
...
def pk_id_column_validation_with_count(source_table: str, destination_table: str, source: Source, destination: Destination, pk_id: str, expect_count: int):
    try:
        ...
        check_status = 0
        while cur_id <= max_id and current_search_times < max_search_times:
            logger.debug(f'{datetime.now()} cur_id={cur_id}, ')
            ids_source = set()
            source_result = source.execute_sql(f'select {pk_id} from {source_table} where {pk_id} >= {cur_id} order by {pk_id} asc limit 10000;')
            for r in source_result:
