...
class DefinedIndex:
        ...
        for k, v in record.items():
            if v is None:
                ...
                _record[self.original_key_to_key_hash[k]] = sqlite3.Binary(
                    self._compressor.compress(pickle.dumps(v, protocol=pickle.HIGHEST_PROTOCOL))
                    if self._compressor is not False
                    else pickle.dumps(v, protocol=pickle.HIGHEST_PROTOCOL)
                )
    ...
    def _compressor(self):
        if (
            ...
            self.local_storage.compressor = (
                zstandard.ZstdCompressor(level=-1) if zstandard is not None else False
            )
    ...
    def _create_table_and_meta_table(self):
        ...
        for key, value_type in self.schema.items():
            ...
            meta_columns.append((key_hash, pickle.dumps(key, protocol=pickle.HIGHEST_PROTOCOL), value_type))
