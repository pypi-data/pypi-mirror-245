...
from src.pyonfireapi import exception, constants, utils
...
class VM(FireAPI):
    ...
    def list_backups(self, vmid:int):
        """
        List backup of given VM
        :param vmid: ID of the VM
        """
        return self.request(f"vm/{vmid}/backup/list", "GET")

    def create_backup(self, vmid:int ):
        """
        Create a Full Backup of the given VM
        :param vmid: ID of the VM
        """
        return self.request(f"vm/{vmid}/backup/create", "POST")

    def create_backup_status(self, vmid:int, backup_id):
        """
        Get Status of Creating given Backup/VM
        :param vmid: ID of the VM
        ...
        return self.request(f"vm/{vmid}/backup/create/status", "POST", data)

    def restore_backup(self, vmid:int, backup_id):
        """
        Restore Backup of the given VM
        :param vmid: ID of the VM
        :param backup_id: ID of the Backup
        """
        if not backup_id:
            raise exception.ParameterNotGivenException
        data = {"backup_id": backup_id}
        return self.request(f"vm/{vmid}/backup/restore", "POST", data)

    def restore_backup_status(self, vmid:int, backup_id):
        """
        Get Status of Restoring given Backup/VM
        :param vmid: ID of the VM
        :param backup_id: ID of the Backup
        """
        if not backup_id:
            raise exception.ParameterNotGivenException
        data = {"backup_id": backup_id}
        return self.request(f"vm/{vmid}/backup/restore/status", "POST", data)

    def delete_backup(self, vmid:int, backup_id):
        """
        ...
        if not backup_id:
            raise exception.ParameterNotGivenException
        data = {"backup_id": backup_id}
        return self.request(f"vm/{vmid}/backup/delete", "POST", data)
