...
class ResTimeModel:
    ...
    def get_resources_volumes(self, work_name, work_volume, resource_name=None) -> dict:
        ...
        for r in res:
            ...
            min_res_value = math.ceil(precalc[work_name][volume_for_work]['10%'][r])
            max_res_value = math.ceil(precalc[work_name][volume_for_work]['90%'][r])
            volume = math.ceil(precalc[work_name][volume_for_work]['50%'][r])
            if min_res_value == 0:
                min_res_value = 1
            if max_res_value == 0:
                max_res_value = 1
