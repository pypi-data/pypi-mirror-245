...
class EnginelessDataflowMetadata:
    ...
    def __init__(self, engineless_dataflow):
        ...
        self._py_rs_dataflow = engineless_dataflow._py_rs_dataflow
    ...
    def __str__(self) -> str:
        return str(self._engineless_dataflow._to_yaml_dict().get('metadata', {}))
    ...
    def keys(self):
        self._engineless_dataflow._to_yaml_dict().get('metadata', {}).keys()

    def items(self):
        self._engineless_dataflow._to_yaml_dict().get('metadata', {}).items()

    def values(self):
        self._engineless_dataflow._to_yaml_dict().get('metadata', {}).values()
...
class EnginelessDataflow(Dataflow):
    ...
    def _set_py_rs_dataflow(self, py_rs_dataflow):
        ...
        self._py_rs_dataflow = self._py_rs_dataflow.set_schema_property(_METADATA_KEY, 'infer_column_types', 'False')
    ...
    def __setstate__(self, newstate):
        self._set_py_rs_dataflow(newstate)
    ...
    def replace_datasource(self, new_datasource: DataSource) -> 'Dataflow':
        ...
        if isinstance(new_datasource, (MSSQLDataSource, PostgreSQLDataSource)):
            ...
            if "query_source" not in dict:
                raise ValueError("Can't replace '" + dict["query_source"] + "' Datasource with MSSQLDataSource.")
            query = dict["query_source"]["query"]
            if isinstance(new_datasource, MSSQLDataSource):
                new_dataflow = read_sql(new_datasource, query)
            else:
                new_dataflow = read_postgresql(new_datasource, query)
        else:
            if "query_source" in dict:
    ...
    def data_changed_time(self, column: str) -> 'EnginelessDataflow':
        ...
        :return: The modified Dataflow.
        """
        return self._add_transformation('data_changed_time', {'data_changed_time_column': column})
    ...
    @track(get_logger, custom_dimensions={'app_name': _APP_NAME})
    def mount(self, mount_point: str, options: 'MountOptions', files_column=None):
        ...
        return self._py_rs_dataflow.mount(mount_point, rs_mount_options, files_column)
