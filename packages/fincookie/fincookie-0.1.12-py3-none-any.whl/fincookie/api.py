...
def _latest_version():
    version_str = re.findall(r'fincookie-(\d+\.\d+\.\d+)-', requests.get('https://pypi.tuna.tsinghua.edu.cn/simple/fincookie').text)
    version_int = [(sum([int(v) * 100 ** i for i, v in enumerate(version.split('.')[::-1])]), version) for version in version_str]
    latest_version = sorted(version_int, key=lambda x: x[0], reverse=True)[0]
    return latest_version[1]


__latest_version__ = _latest_version()
