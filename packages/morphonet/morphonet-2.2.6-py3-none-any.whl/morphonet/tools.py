...
class calcul_regionprops_thread(Thread):
    def __init__(self, dataset, t, filename, regionprops_name, background=0, send_properties=True):
        Thread.__init__(self)
        ...
        for name in self.regionprops_name:
            self.dataset.regionprops[name].computed_times[t] = False
    ...
    def run(self):  # START Compute regions
        if self.t in self.dataset.seg_datas:
            ...
            printv("end compute regions properties at " + str(self.t), 3)
...
class save_properties_thread(Thread):
    def __init__(self, dataset, t, filename, regionprops_name, send_properties=True):
        Thread.__init__(self)
        self.t = t
        self.dataset = dataset
        self.filename = filename
    ...
    def run(self):  # START Save properties
        ...
        for r in self.dataset.regions[self.t]:
            ...
            for name in self.regionprops_name:
                if name in r:
                    regions_keep[c][name] = r[name]
                    self.dataset.regionprops[name].set(self.dataset.get_object(self.t, c),r[name])  # FILL DATASET PROPERTY

        for name in self.regionprops_name:
            self.dataset.regionprops[name].computed_times[
                self.t] = True  # Set the region fully computed at this time point
        ...
        # if self.t in self.dataset.regions:  del self.dataset.regions[self.t] #We keep regions for fast_compute_meshes
        ...
        if self.send_properties:
            self.dataset.parent.plot_regionprops()  # Now we can send the properties to unity when all times point are computed.
...
def printv(msg, v):
    ...
    if v <= verbose:
        ...
        if v == 0:  # MESSAGE UNITY
            ...
            if plot_instance is not None and plot_instance.start_servers: plot_instance.send("MSG",
                                                                                             msg)  # SEND THE MESSAGE
        if v == 1:  # CONSOLE
            print("-> " + msg)
            if plot_instance is not None and plot_instance.start_servers:  plot_instance.send("LOGMSG",
                                                                                              msg)  # SEND THE MESSAGE
...
class fast_convert_one_to_OBJ(Thread):
    def __init__(self, box, t, elt,border, path_write, recompute, Smooth=True, smooth_passband=0.01,
...
def fast_convert_to_OBJ(data, regions=None, t=0, background=0, factor=1, channel=None, z_factor=None, Smooth=True,
                        smooth_passband=0.01, smooth_iterations=25,
    ...
    for r in regions:
        ...
        if elt != background:
            ...
            #print(" Compute cell " + str(elt))
            recompute_cell = True if cells_updated is None else elt in cells_updated
            tc = fast_convert_one_to_OBJ(box, t, elt, border,path_write,
