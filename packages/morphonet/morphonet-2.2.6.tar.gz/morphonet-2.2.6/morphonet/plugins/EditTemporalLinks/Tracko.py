...
class Tracko(MorphoPlugin):
    ...
    def process(self,t,dataset,objects): #PLUGIN EXECUTION
        ...
        if direction=="forward" :
            while t<self.dataset.end: #From t to t max
                self.compute_links(t,t+1,optical,downsampling) #compute lineage by overlaping
                t+=1
    ...
    def compute_links(self,t, tp,optical,downsampling):
        printv("compute links at " + str(t), 0)
        flow=None
        if optical != "none":
            rawdata0 = self.dataset.get_raw(t)
            rawdata1 = self.dataset.get_raw(tp)
