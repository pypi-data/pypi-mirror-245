...
shared_ptr<DuckDBPyConnection> DuckDBPyConnection::Cursor() {
	...
	res->connection = make_uniq<Connection>(*res->database, role);
...
void CreateNewInstance(DuckDBPyConnection &res, const string &database, const string &role, DBConfig &config) {
...
static shared_ptr<DuckDBPyConnection> FetchOrCreateInstance(const string &database, const string &role, DBConfig &config) {
	auto res = make_shared<DuckDBPyConnection>();
...
shared_ptr<DuckDBPyConnection> DuckDBPyConnection::Connect(const string &database, bool read_only,
                                                           const string &role, const py::dict &config_options) {
...
shared_ptr<DuckDBPyConnection> DuckDBPyConnection::DefaultConnection() {
	if (!default_connection) {
		py::dict config_dict;
		default_connection = DuckDBPyConnection::Connect(":memory:", false, ADMIN_ROLE, config_dict);
	}
