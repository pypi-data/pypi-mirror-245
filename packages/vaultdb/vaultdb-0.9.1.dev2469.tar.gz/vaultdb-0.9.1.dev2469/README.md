...
You would normally install the VaultDB released version using pip as follows:

    pip3 install vaultdb

## Installing locally for development

For development, you may need a VaultDB Python package that is installed from source. Make sure you have the dependencies installed:
...
Note that this will override any existing VaultDB installation you might have. You might also run into conflicts depending on your Python environment. In order to remedy that, it is possible to use virtualenv for installation, e.g. by running the following commands:
...
Alternatively, using virtualenv and pip:
    ...
    # Compile and install VaultDB for Python.
...
`$VAULTDEBUG=1` (or some other non-zero value) to compile in debug mode.
...
There is a test that you can run to check the stubs match the real vaultdb package - this runs in CI (sorry in advance...). If you add a method to the vaultdb py library and forget to add it to the stub, this test will helpfully fail. The test is a run of [mypy.stubtest](https://github.com/python/mypy/issues/5028#issuecomment-740101546).
...
# Install vaultdb python package to
