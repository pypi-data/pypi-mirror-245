...
BindResult ExpressionBinder::BindExpression(ColumnRefExpression &colref_p, idx_t depth) {
	...
	BindResult result;
		...
		if (binder.macro_binding && table_name == binder.macro_binding->alias) {
			...
			//Authorization
			if (colref.column_names.size() == 3)
				context.authorizer->Authorize_Column(colref.column_names[0], table_name, colref.GetColumnName());
			else
				context.authorizer->Authorize_Column("", table_name, colref.GetColumnName());
