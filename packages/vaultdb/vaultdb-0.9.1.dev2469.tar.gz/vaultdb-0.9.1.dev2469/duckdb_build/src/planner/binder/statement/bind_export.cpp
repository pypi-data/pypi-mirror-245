...
BoundStatement Binder::Bind(ExportStatement &stmt) {
	...
	auto remote_data_path = context.GetRemoteDataPath();
	auto remote_merge_path = context.GetRemoteMergePath();
	if (stmt.info->push_to_remote) {
		if (stmt.info->merge_to_remote) {
			if (remote_data_path.empty()) {
				throw PermissionException("Invalid or Non existenent remote data merge path setting.");
			}
			stmt.info->file_path = remote_data_path;
			stmt.merge_remote = true;
		} else {
			if (remote_merge_path.empty()) {
				throw PermissionException("Invalid or Non existenent remote data push path setting.");
			}
			auto current_role = context.authorizer->GetCurrentRole();
			if (stmt.info->file_path.empty()) {
				stmt.info->file_path = remote_merge_path;
				if (!fs.DirectoryExists(stmt.info->file_path))
					fs.CreateDirectory(remote_merge_path);
				stmt.info->file_path = fs.JoinPath(remote_merge_path, config.options.push_folder);
				if (!fs.DirectoryExists(stmt.info->file_path))
					fs.CreateDirectory(stmt.info->file_path);
			}

			stmt.info->file_path = fs.JoinPath(stmt.info->file_path, current_role->name);
			if (!fs.DirectoryExists(stmt.info->file_path))
				fs.CreateDirectory(stmt.info->file_path);

			stmt.info->file_path = fs.JoinPath(stmt.info->file_path, config.options.branch_name);
			if (!fs.DirectoryExists(stmt.info->file_path))
				fs.CreateDirectory(stmt.info->file_path);
			
			string catalog = stmt.database.empty() ? INVALID_CATALOG : stmt.database;			
			stmt.info->file_path = fs.JoinPath(stmt.info->file_path, catalog);
			stmt.merge_remote = false;
		}
	}
	...
	for (auto &t : tables) {
		...
		idx_t id = 0;
		if (stmt.info->push_to_remote && stmt.info->merge_to_remote) {
			info->options["overwrite_or_ignore"].push_back(Value(true));
			// info->options["use_tmp_file"].push_back(Value(true));
			if (table.partition_key_columns.size() > 0) {
				for (auto &col_name : table.partition_key_columns) {
					info->options["PARTITION_BY"].push_back(Value(col_name));
				}
				info->file_path = table.GetRemotePath(context);
			} else
				info->file_path = table.GetRemotePath(context, true);
			id++;
		} else {
			while (true) {
		...
		copy_stmt.merge_remote = stmt.merge_remote;
	...
	//! VaultDB: is remote data merge enabled
	export_node->merge_remote = stmt.merge_remote;
