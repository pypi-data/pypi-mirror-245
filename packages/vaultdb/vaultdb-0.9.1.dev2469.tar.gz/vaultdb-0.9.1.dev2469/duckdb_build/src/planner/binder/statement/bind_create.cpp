...
BoundStatement Binder::Bind(CreateStatement &stmt) {
	...
	case CatalogType::TAG_ENTRY: {
		result.plan = make_uniq<LogicalCreate>(LogicalOperatorType::LOGICAL_CREATE_TAG, std::move(stmt.info));
		break;
	}
	case CatalogType::CONFIG_ENTRY: {
		result.plan = make_uniq<LogicalCreate>(LogicalOperatorType::LOGICAL_CREATE_CONFIG, std::move(stmt.info));
		break;
	}
	case CatalogType::FORTRESS_ENTRY: {
		result.plan = make_uniq<LogicalCreate>(LogicalOperatorType::LOGICAL_CREATE_FORTRESS, std::move(stmt.info));
		break;
	}
