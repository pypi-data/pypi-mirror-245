...
BoundStatement Binder::Bind(DeleteStatement &stmt) {
	...
	del->merge_remote = stmt.merge_remote;
	if (del->merge_remote) {
		auto &merge = get.Cast<LogicalMerge>();
		vector<column_t> merge_key_column_ids = merge.BuildMergeKeyColumnIds();
		del->merge_key_column_ids = std::move(merge_key_column_ids);
	}
