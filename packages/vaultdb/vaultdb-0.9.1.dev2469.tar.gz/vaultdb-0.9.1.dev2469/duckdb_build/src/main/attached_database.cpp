...
bool AttachedDatabase::NeedsInitialization() const {
	return storage->NeedsInitialization();
}
...
void AttachedDatabase::Initialize() {
	...
	if (storage) {
		storage->Initialize();
		catalog->InitializeSecuritySchema(); //!VaultDB: Intialize built in roles
		/*
		if (!storage->InMemory() && storage->NeedsInitialization())
			storage->CreateCheckpoint(false, true); //!VaultDB: save it first time
		*/
	}
}
