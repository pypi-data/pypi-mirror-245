...
ClientContext::ClientContext(shared_ptr<DatabaseInstance> database, string role)
    : db(std::move(database)), interrupted(false), client_data(make_uniq<ClientData>(*this)), transaction(*this),
      authorizer(make_uniq<Authorizer>(*this, role)) {
}
...
ClientContext::CreatePreparedStatement(ClientContextLock &lock, const string &query, unique_ptr<SQLStatement> statement,
	...
	statement->merge_remote = IsMergeEnabled();
...
vector<unique_ptr<SQLStatement>> ClientContext::ParseStatementsInternal(ClientContextLock &lock, const string &query) {
	Parser parser(*this, GetParserOptions());
...
string ClientContext::GetRemoteDataPath() {
	auto &db_config = DBConfig::GetConfig(*this);
	if (db_config.options.data_inheritance_path.empty()){
		auto config = Catalog::GetEntry<ConfigCatalogEntry>(*this, INVALID_CATALOG, SECURITY_SCHEMA, REMOTE_DATA_PATH,
	...
	return db_config.options.data_inheritance_path;
}

string ClientContext::GetRemoteMergePath() {
	auto &db_config = DBConfig::GetConfig(*this);
	if (db_config.options.data_merge_queue_path.empty()){
		auto config = Catalog::GetEntry<ConfigCatalogEntry>(*this, INVALID_CATALOG, SECURITY_SCHEMA, REMOTE_MERGE_PATH,
		                                                  OnEntryNotFound::RETURN_NULL);
		if (!config) {
			return string();
		}
	...
	return db_config.options.data_merge_queue_path;
}

bool ClientContext::IsMergeEnabled() {
	auto &config = ClientConfig::GetConfig(*this);
	return config.enable_data_inheritance && !GetRemoteDataPath().empty();
}
...
void ClientContext::ExecuteAs(const string &role) {
