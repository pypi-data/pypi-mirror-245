...
Connection::Connection(DatabaseInstance &database, string role) : context(make_shared<ClientContext>(database.shared_from_this(), role)) {
...
Connection::Connection(DuckDB &database, string role) : Connection(*database.instance, role) {
}
...
void Connection::Role(const string &role) {
	context->authorizer->Role(role);
}
...
void Connection::ExecuteAs(const string &role) {
	context->ExecuteAs(role);
}
