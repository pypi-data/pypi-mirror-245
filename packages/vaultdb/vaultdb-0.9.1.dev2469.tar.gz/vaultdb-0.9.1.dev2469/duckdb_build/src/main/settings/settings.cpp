...
// Remote Data Path
//===--------------------------------------------------------------------===//
void RemoteDataPathSetting::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	config.options.data_inheritance_path = input.GetValue<string>();
}

void RemoteDataPathSetting::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	config.options.data_inheritance_path = string();
}

Value RemoteDataPathSetting::GetSetting(ClientContext &context) {
	return Value(DBConfig::GetConfig(context).options.data_inheritance_path);
}

//===--------------------------------------------------------------------===//
// Data Branch/Session name
//===--------------------------------------------------------------------===//
void DataBranchNameSetting::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	config.options.branch_name = input.GetValue<string>();
}

void DataBranchNameSetting::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	config.options.branch_name = "master";
}

Value DataBranchNameSetting::GetSetting(ClientContext &context) {
	return Value(DBConfig::GetConfig(context).options.branch_name);
}

//===--------------------------------------------------------------------===//
// Remote Brach Merge Path
//===--------------------------------------------------------------------===//
void RemoteDataMergePathSetting::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	config.options.data_merge_queue_path = input.GetValue<string>();
}

void RemoteDataMergePathSetting::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	config.options.data_merge_queue_path = string();
}

Value RemoteDataMergePathSetting::GetSetting(ClientContext &context) {
	return Value(DBConfig::GetConfig(context).options.data_merge_queue_path);
}
...
// Role
//===--------------------------------------------------------------------===//
void RoleSetting::ResetLocal(ClientContext &context) {
	context.authorizer->Role("");
}

void RoleSetting::SetLocal(ClientContext &context, const Value &input) {
	context.authorizer->Role(input.ToString());
}

Value RoleSetting::GetSetting(ClientContext &context) {
	auto current_role = context.authorizer->GetCurrentRole();
	return current_role ? Value(current_role->name) : Value();
}
