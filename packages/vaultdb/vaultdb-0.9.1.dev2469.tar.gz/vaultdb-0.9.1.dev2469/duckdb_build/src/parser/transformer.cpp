...
Transformer::Transformer(ParserOptions &options)
    : context(nullptr), parent(nullptr), options(options), stack_depth(DConstants::INVALID_INDEX) {
}

Transformer::Transformer(Transformer &parent)
    : context(nullptr), parent(&parent), options(parent.options), stack_depth(DConstants::INVALID_INDEX) {
}

Transformer::~Transformer() {
}

Transformer::Transformer(ClientContext &context, ParserOptions &options)
    : context(&context), parent(nullptr), options(options), stack_depth(DConstants::INVALID_INDEX) {
}

Transformer::Transformer(ClientContext &context, Transformer &parent)
    : context(&context), parent(&parent), options(parent.options), stack_depth(DConstants::INVALID_INDEX) {
	if (this->parent)
		this->current_statement_type = parent.current_statement_type;
}
...
unique_ptr<SQLStatement> Transformer::TransformStatementInternal(duckdb_libpgquery::PGNode &stmt) {
	//!vaultdb: Just make sure that we do not do auth on wrong table
	current_statement_type = stmt.type;
	...
	case duckdb_libpgquery::T_PGCreateRoleStmt:
		return TransformCreateRole(PGCast<duckdb_libpgquery::PGCreateRoleStmt>(stmt));
	case duckdb_libpgquery::T_PGAlterRoleStmt:
		return TransformAlterRole(PGCast<duckdb_libpgquery::PGAlterRoleStmt>(stmt));
	case duckdb_libpgquery::T_PGCreateTagStmt:
		return TransformCreateTag(PGCast<duckdb_libpgquery::PGCreateTagStmt>(stmt));
	case duckdb_libpgquery::T_PGCreateConfigStmt:
		return TransformCreateConfig(PGCast<duckdb_libpgquery::PGCreateConfigStmt>(stmt));
	case duckdb_libpgquery::T_PGCreateFortressStmt:
		return TransformCreateFortress(PGCast<duckdb_libpgquery::PGCreateFortressStmt>(stmt));
	case duckdb_libpgquery::T_PGAlterFortressStmt:
		return TransformAlterFortress(PGCast<duckdb_libpgquery::PGAlterFortressStmt>(stmt));
	case duckdb_libpgquery::T_PGLockFortressStmt:
		return TransformLockFortress(PGCast<duckdb_libpgquery::PGLockFortressStmt>(stmt));
	case duckdb_libpgquery::T_PGUnLockFortressStmt:
		return TransformUnlockFortress(PGCast<duckdb_libpgquery::PGUnLockFortressStmt>(stmt));
	case duckdb_libpgquery::T_PGGrantStmt:
		return TransformPrivilege(AlterRoleType::GRANT_PRIVILEGES, PGCast<duckdb_libpgquery::PGGrantStmt>(stmt));
	case duckdb_libpgquery::T_PGRevokeStmt:
		return TransformPrivilege(AlterRoleType::REVOKE_PRIVILEGES, PGCast<duckdb_libpgquery::PGGrantStmt>(stmt));
