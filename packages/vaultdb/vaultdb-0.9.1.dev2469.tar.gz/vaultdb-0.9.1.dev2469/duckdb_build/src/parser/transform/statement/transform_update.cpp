...
unique_ptr<UpdateStatement> Transformer::TransformUpdate(duckdb_libpgquery::PGUpdateStmt &stmt) {
	...
	if (context){
		if (result->table->type==TableReferenceType::BASE_TABLE) {
			auto unauthorized_columns  = context->authorizer->GetUnauthorizedColumns((BaseTableRef &)*result->table);
			for (auto &col:result->set_info->columns) {
				if (std::find(unauthorized_columns.begin(), unauthorized_columns.end(), col) != unauthorized_columns.end()){
					throw PermissionException("Access Denied on column %s !", col);
				}
			}
			context->authorizer->AuthorizeWhereClause(stmt.whereClause, unauthorized_columns);
		} else if (!context->authorizer->IsSuperuser())
			throw PermissionException("TODO: this Update statement is not authorised!");

		result->set_info->condition = context->authorizer->AddFortressExpression((TableRef &)*result->table, std::move(result->set_info->condition), duckdb_libpgquery::PG_PRIVILEGE_UPDATE);
	}
