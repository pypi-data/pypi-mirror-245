...
unique_ptr<AlterStatement> Transformer::TransformAlter(duckdb_libpgquery::PGAlterTableStmt &stmt) {
	...
	for (auto c = stmt.cmds->head; c != nullptr; c = c->next) {
		...
		case duckdb_libpgquery::PG_AT_PartitionBy: {
			vector<string> partition_cols;
			auto cdef = reinterpret_cast<duckdb_libpgquery::PGPartitionByOption *>(command->def);
			for (auto c = cdef->colnames->head; c != nullptr; c = lnext(c)) {
				auto str_val = reinterpret_cast<duckdb_libpgquery::PGValue *>(c->data.ptr_value)->val.str;
				partition_cols.push_back(str_val);
			}
			result->info = make_uniq<AlterPartitionKeyInfo>(std::move(data), partition_cols);
			break;
		}
		...
		case duckdb_libpgquery::PG_AT_AlterColumnAddTAG: {
			result->info = make_uniq<AddColumnTagInfo>(std::move(data), command->name, command->tagname);
			break;
		}
		case duckdb_libpgquery::PG_AT_AlterColumnDeleteTAG: {
			result->info = make_uniq<RemoveColumnTagInfo>(std::move(data), command->name, command->tagname);
			break;
		}
