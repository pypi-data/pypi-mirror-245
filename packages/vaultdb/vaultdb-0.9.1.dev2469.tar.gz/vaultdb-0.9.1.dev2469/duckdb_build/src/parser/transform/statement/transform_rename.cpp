...
unique_ptr<AlterStatement> Transformer::TransformRename(duckdb_libpgquery::PGRenameStmt &stmt) {
	...
	case duckdb_libpgquery::PG_OBJECT_COLUMN: {
		...
		if (context){
			context->authorizer->Authorize_table(data.schema, data.name, duckdb_libpgquery::PG_PRIVILEGE_ALTER);
			//Column Authorization	
			context->authorizer->Authorize_Column(data.schema, data.name, old_name);
		}
	...
	case duckdb_libpgquery::PG_OBJECT_VIEW: {
		...
		string new_name = stmt.newname;
		if (context)
			context->authorizer->Authorize_view(data.schema, data.name, duckdb_libpgquery::PG_PRIVILEGE_ALTER);
