...
unique_ptr<CreateStatement> Transformer::TransformCreateTable(duckdb_libpgquery::PGCreateStmt &stmt) {
	...
	// Authorize role
	if (context)
		context->authorizer->Authorize_schema(info->schema, duckdb_libpgquery::PG_PRIVILEGE_CREATE);
	...
	if (stmt.partitionClause) {
		auto partition_stmt = reinterpret_cast<duckdb_libpgquery::PGPartitionByOption *>(stmt.partitionClause);
		for (auto c = partition_stmt->colnames->head; c != nullptr; c = lnext(c)) {
	...
	for (auto c = stmt.tableElts->head; c != nullptr; c = lnext(c)) {
		...
		case duckdb_libpgquery::T_PGColumnDef: {
			...
			if (std::find(partition_cols.begin(), partition_cols.end(), centry.Name()) != partition_cols.end() )
				info->partition_key_columns.push_back(centry.Name());
