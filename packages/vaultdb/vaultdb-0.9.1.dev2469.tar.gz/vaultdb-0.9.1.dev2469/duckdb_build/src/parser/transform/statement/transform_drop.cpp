...
unique_ptr<SQLStatement> Transformer::TransformDrop(duckdb_libpgquery::PGDropStmt &stmt) {
	...
	case duckdb_libpgquery::PG_OBJECT_VIEW:
		...
		break;
	case duckdb_libpgquery::PG_OBJECT_ROLE:
	...
	case duckdb_libpgquery::PG_OBJECT_CONFIG:
		info.type = CatalogType::CONFIG_ENTRY;
		break;
	...
	case duckdb_libpgquery::PG_OBJECT_SCHEMA: {
		// Authorize role
		if (context && !context->authorizer->IsSuperuser())
	...
	default: {
		...
		// Authorize role
		if (context)
			context->authorizer->Authorize_schema(info.schema, duckdb_libpgquery::PG_PRIVILEGE_DROP);
		break;
	}
	}
