...
unique_ptr<TableRef> Transformer::TransformRangeVar(duckdb_libpgquery::PGRangeVar &root) {
	...
	if (context)
		context->authorizer->Authorize_table((TableRef &)*result, current_statement_type);
...
QualifiedName Transformer::TransformQualifiedName(duckdb_libpgquery::PGRangeVar &root) {
	...
	if (context)
		context->authorizer->Authorize_table(qname.schema, qname.name, current_statement_type);
