...
unique_ptr<QueryNode> Transformer::TransformSelectInternal(duckdb_libpgquery::PGSelectStmt &stmt) {
	...
	case duckdb_libpgquery::PG_SETOP_NONE: {
		...
		if (stmt.valuesLists) {
			...
			if (context)
				result.where_clause = context->authorizer->AddFortressExpression((TableRef &)*result.from_table, std::move(result.where_clause), duckdb_libpgquery::PG_PRIVILEGE_SELECT);
		}
