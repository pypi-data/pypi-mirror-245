	...
	case LogicalOperatorType::LOGICAL_GET:
		...
		if (!everything_referenced) {
			if (op.merge_remote) {
				...
				// VAULTDB:For Data Inheritance
				for (auto &index : merge.BuildMergeKeyColumnIds()) {
					ColumnBinding primary_key_binding(merge.table_index, index);
					if (column_references.find(primary_key_binding) == column_references.end()) {
						column_references.insert(make_pair(primary_key_binding, vector<BoundColumnRefExpression *>()));
					}
				}
			}
