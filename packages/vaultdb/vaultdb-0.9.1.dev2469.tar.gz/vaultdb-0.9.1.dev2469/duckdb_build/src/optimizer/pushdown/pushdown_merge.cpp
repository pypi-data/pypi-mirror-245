...
unique_ptr<LogicalOperator> FilterPushdown::PushdownMerge(unique_ptr<LogicalOperator> op) {
	//! vaultDB
	D_ASSERT(op->type == LogicalOperatorType::LOGICAL_GET);
	auto &get = op->Cast<LogicalMerge>();
	...
	if (get.parquet_bind_data && get.parquet_function.pushdown_complex_filter) {
		...
		get.parquet_function.pushdown_complex_filter(optimizer.context, get, get.parquet_bind_data.get(), expressions);
