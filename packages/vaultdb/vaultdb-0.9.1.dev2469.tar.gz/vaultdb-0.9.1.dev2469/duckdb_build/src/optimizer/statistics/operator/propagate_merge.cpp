...
unique_ptr<NodeStatistics> StatisticsPropagator::PropagateStatistics(LogicalMerge &get,
                                                                     unique_ptr<LogicalOperator> *node_ptr) {
	if (get.function.cardinality) {
		node_stats = get.function.cardinality(context, get.bind_data.get());
	}

	if (get.parquet_bind_data) {
		//! VaultDB: use parquet statistics
		if (node_stats) {
			auto parquet_stats = get.parquet_function.cardinality(context, get.parquet_bind_data.get());
			AddCardinalities(node_stats, *parquet_stats);
			if (!node_stats && (!parquet_stats->has_estimated_cardinality || !parquet_stats->has_max_cardinality))
				node_stats = std::move(parquet_stats);
		} else
			node_stats = get.parquet_function.cardinality(context, get.parquet_bind_data.get());
	}
	...
	for (idx_t i = 0; i < get.column_ids.size(); i++) {
		...
		if (get.parquet_bind_data) {
			//! VaultDB: use parquet statistics
			auto parquet_stats =
			    get.parquet_function.statistics(context, get.parquet_bind_data.get(), get.column_ids[i]);
			if (stats && parquet_stats)
				stats->Merge(*parquet_stats);
		}
		if (stats) {
			ColumnBinding binding(get.table_index, i);
			auto entry =
			    statistics_map.find(binding); // # We need to do something to keep remote and local stats separate
			if (entry != statistics_map.end())
				entry->second->Merge(*stats);
			else
