...
static void PragmaExecuteAsRole(ClientContext &context, const FunctionParameters &parameters) {
	if (!parameters.values.empty())
		context.ExecuteAs(parameters.values[0].ToString());
}

static void PragmaEnableDataInheritance(ClientContext &context, const FunctionParameters &parameters) {
	ClientConfig::GetConfig(context).enable_data_inheritance = true;
}

static void PragmaDisableDataInheritance(ClientContext &context, const FunctionParameters &parameters) {
	ClientConfig::GetConfig(context).enable_data_inheritance = false;
}
...
void PragmaFunctions::RegisterFunction(BuiltinFunctions &set) {
	...
	set.AddFunction(PragmaFunction::PragmaCall("execute_as", PragmaExecuteAsRole, {LogicalType::VARCHAR}));

	set.AddFunction(PragmaFunction::PragmaStatement("enable_data_inheritance", PragmaEnableDataInheritance));
	set.AddFunction(PragmaFunction::PragmaStatement("disable_data_inheritance", PragmaDisableDataInheritance));
