...
static unique_ptr<FunctionData> DuckDBColumnsBind(ClientContext &context, TableFunctionBindInput &input,
	...
	names.emplace_back("is_partition_key");
	return_types.emplace_back(LogicalType::BOOLEAN);
...
public:
	...
	virtual bool IsPartitionKey(idx_t col) = 0;
...
public:
	...
	bool IsPartitionKey(idx_t col) override {
		auto result = std::find(entry.partition_key_columns.begin(), entry.partition_key_columns.end(),
		                        ColumnName(col)) != entry.partition_key_columns.end();
		return result;
	}
...
void ColumnHelper::WriteColumns(idx_t start_index, idx_t start_col, idx_t end_col, DataChunk &output) {
	for (idx_t i = start_col; i < end_col; i++) {
		...
		output.SetValue(col++, index, Value::BOOLEAN(IsPartitionKey(i)));
	}
}
