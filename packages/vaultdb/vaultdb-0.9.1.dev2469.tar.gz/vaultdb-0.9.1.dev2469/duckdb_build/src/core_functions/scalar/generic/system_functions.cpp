...
// current_role
static void CurrentRoleFunction(DataChunk &input, ExpressionState &state, Vector &result) {
	auto currentrole = state.GetContext().authorizer->GetCurrentRole();
	Value val(currentrole->name);
	result.Reference(val);
}
...
ScalarFunction CurrentRoleFun::GetFunction() {
	return ScalarFunction({}, LogicalType::VARCHAR, CurrentRoleFunction);
}
