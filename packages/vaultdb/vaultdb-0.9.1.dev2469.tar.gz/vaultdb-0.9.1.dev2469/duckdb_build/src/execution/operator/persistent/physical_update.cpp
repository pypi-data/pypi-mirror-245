...
SinkResultType PhysicalUpdate::Sink(ExecutionContext &context, DataChunk &chunk, OperatorSinkInput &input) const {
	...
	if (merge_remote) {
		...
		DataChunk insert_data;
		insert_data.Initialize(Allocator::DefaultAllocator(), update_chunk.GetTypes());
		auto row_id_data = FlatVector::GetData<row_t>(row_ids);
		SelectionVector sel(STANDARD_VECTOR_SIZE);
		SelectionVector remote_data_sel(STANDARD_VECTOR_SIZE);
		idx_t update_count = 0;
		idx_t remote_insert_count = 0;
		for (idx_t i = 0; i < update_chunk.size(); i++) {
			...
			if (gstate.updated_columns.find(row_id) == gstate.updated_columns.end()) {
				...
				if (row_id == REMOTE_ROW_ID)
					remote_data_sel.set_index(remote_insert_count++, i);
				else
					sel.set_index(update_count++, i);
			}
		}

		if (remote_insert_count > 0)
			insert_data.Slice(update_chunk, remote_data_sel, remote_insert_count);
		if (update_count != update_chunk.size()) {
			// we need to slice here
			update_chunk.Slice(sel, update_count);
			chunk.Slice(sel, update_count);
		}
		if (insert_data.size() > 0) {
			mock_chunk.Reset();
			// for the append we need to arrange the columns in a specific manner (namely the "standard table order")
			mock_chunk.SetCardinality(insert_data);
			for (idx_t i = 0; i < columns.size(); i++) {
				mock_chunk.data[columns[i].index].Reference(insert_data.data[i]);
			}
			table.LocalAppend(tableref, context.client, mock_chunk);

			gstate.updated_count += insert_data.size();
		}
	}

	if (update_chunk.size() > 0) {
		mock_chunk.Reset();
		auto &row_ids_tobe_updated = chunk.data[chunk.ColumnCount() - 1];
		if (update_is_del_and_insert) {
			auto row_id_data = FlatVector::GetData<row_t>(row_ids_tobe_updated);
			SelectionVector sel(STANDARD_VECTOR_SIZE);
			idx_t update_count = 0;
			for (idx_t i = 0; i < update_chunk.size(); i++) {
				auto row_id = row_id_data[i];
			...
			if (update_only_columns.size() > 0)
				table.Update(tableref, context.client, row_ids_tobe_updated, update_only_columns, update_chunk);
			else
				table.Update(tableref, context.client, row_ids_tobe_updated, columns, update_chunk);
		}
		gstate.updated_count += update_chunk.size();
	}
