...
void PhysicalExport::ExtractEntries(ClientContext &context, vector<reference<SchemaCatalogEntry>> &schema_list,
	...
	for (auto &schema_p : schema_list) {
		...
		schema.Scan(context, CatalogType::ROLE_ENTRY, [&](CatalogEntry &entry) {
			if (!entry.internal && entry.type == CatalogType::ROLE_ENTRY)
				result.roles.push_back(entry);
		});

		schema.Scan(context, CatalogType::TAG_ENTRY, [&](CatalogEntry &entry) {
			if (!entry.internal && entry.type == CatalogType::TAG_ENTRY)
				result.tags.push_back(entry);
		});

		schema.Scan(context, CatalogType::CONFIG_ENTRY, [&](CatalogEntry &entry) {
			if (!entry.internal && entry.type == CatalogType::CONFIG_ENTRY)
				result.configs.push_back(entry);
		});

		schema.Scan(context, CatalogType::FORTRESS_ENTRY,
		            [&](CatalogEntry &entry) { result.fortresses.push_back(entry); });
	}
}
...
SourceResultType PhysicalExport::GetData(ExecutionContext &context, DataChunk &chunk,
	...
	WriteCatalogEntries(ss, entries.roles);
	WriteCatalogEntries(ss, entries.tags);
	//WriteCatalogEntries(ss, entries.configs); //!Vaultdb! No need to export configs for now
	WriteCatalogEntries(ss, entries.fortresses);
