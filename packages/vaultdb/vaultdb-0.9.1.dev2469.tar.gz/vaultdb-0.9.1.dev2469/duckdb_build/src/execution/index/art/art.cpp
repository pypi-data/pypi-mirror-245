...
bool ART::Search(Value value, idx_t max_count, vector<row_t> &result_ids) {
	ArenaAllocator arena_allocator(Allocator::Get(db));
	auto key = CreateKey(arena_allocator, value.type().InternalType(), value);
	return SearchEqual(key, max_count, result_ids);
}
...
//===--------------------------------------------------------------------===//
idx_t ART::Count() {
	// VAULTDB: it is used to check if there are rows so that merge doesn't loop through records unnecessorily.
	idx_t result = 0;
	for (auto &allocator : *allocators) {
		result += allocator->GetTotalSegmentCount();
	}
	return result;
}
