...
static catalog_entry_vector_t GetCatalogEntries(vector<reference<SchemaCatalogEntry>> &schemas) {
	...
	for (auto &schema_p : schemas) {
		...
		schema.Scan(CatalogType::ROLE_ENTRY, [&](CatalogEntry &entry) {
			if (entry.internal) {
				return;
			}
			entries.push_back(entry);
		...
		schema.Scan(CatalogType::TAG_ENTRY, [&](CatalogEntry &entry) {
			if (entry.internal) {
				return;
			}
			entries.push_back(entry);
		...
		schema.Scan(CatalogType::CONFIG_ENTRY, [&](CatalogEntry &entry) {
			if (entry.internal) {
				return;
			}
			entries.push_back(entry);
		...
		schema.Scan(CatalogType::FORTRESS_ENTRY, [&](CatalogEntry &entry) {
			if (entry.internal) {
				return;
			}
			entries.push_back(entry);
...
void SingleFileCheckpointReader::LoadFromStorage() {
	...
	Connection con(storage.GetDatabase(), INTERNAL_ROLE);
...
void CheckpointWriter::WriteEntry(CatalogEntry &entry, Serializer &serializer) {
	...
	case CatalogType::CONFIG_ENTRY: {
		...
		WriteConfig(config, serializer);
		break;
	}
	case CatalogType::FORTRESS_ENTRY: {
		auto &fort = entry.Cast<FortressCatalogEntry>();
		WriteFortress(fort, serializer);
		break;
	}
	default:
...
void CheckpointReader::ReadEntry(ClientContext &context, Deserializer &deserializer) {
	...
	case CatalogType::ROLE_ENTRY: {
		ReadRole(context, deserializer);
		break;
		}
	case CatalogType::TAG_ENTRY: {
		ReadTag(context, deserializer);
		break;
		}
	case CatalogType::CONFIG_ENTRY: {
		ReadConfig(context, deserializer);
		break;
		}
	case CatalogType::FORTRESS_ENTRY: {
		ReadFortress(context, deserializer);
		break;
		}
	default:
		throw InternalException("Unrecognized catalog type in CheckpointWriter::WriteEntry");
...
// Roles
//===--------------------------------------------------------------------===//
void CheckpointWriter::WriteRole(RoleCatalogEntry &role, Serializer &serializer) {
	if (role.internal)
		return;
	serializer.WriteProperty(100, "role", &role);
}

void CheckpointReader::ReadRole(ClientContext &context, Deserializer &deserializer) {
	auto info = deserializer.ReadProperty<unique_ptr<CreateInfo>>(100, "role");
	auto &changed_info = info->Cast<CreateRoleInfo>();
	catalog.CreateRole(context, changed_info);
}

//===--------------------------------------------------------------------===//
// Tags
//===--------------------------------------------------------------------===//
void CheckpointWriter::WriteTag(TagCatalogEntry &tag, Serializer &serializer) {
	if (tag.internal)
		return;
	serializer.WriteProperty(100, "tag", &tag);
}

void CheckpointReader::ReadTag(ClientContext &context, Deserializer &deserializer) {
	auto info = deserializer.ReadProperty<unique_ptr<CreateInfo>>(100, "tag");
	auto &changed_info = info->Cast<CreateTagInfo>();
	catalog.CreateTag(context, changed_info);
}

//===--------------------------------------------------------------------===//
// Config
//===--------------------------------------------------------------------===//
void CheckpointWriter::WriteConfig(ConfigCatalogEntry &config, Serializer &serializer) {
	if (config.internal)
		return;
	serializer.WriteProperty(100, "config", &config);
}

void CheckpointReader::ReadConfig(ClientContext &context, Deserializer &deserializer) {
	auto info = deserializer.ReadProperty<unique_ptr<CreateInfo>>(100, "config");
	auto &changed_info = info->Cast<CreateConfigInfo>();
	catalog.CreateConfig(context, changed_info);
}

//===--------------------------------------------------------------------===//
// Fortress
//===--------------------------------------------------------------------===//
void CheckpointWriter::WriteFortress(FortressCatalogEntry &fort, Serializer &serializer) {
	if (fort.internal)
		return;
	serializer.WriteProperty(100, "fort", &fort);
}

void CheckpointReader::ReadFortress(ClientContext &context, Deserializer &deserializer) {
	auto info = deserializer.ReadProperty<unique_ptr<CreateInfo>>(100, "fort");
	auto &changed_info = info->Cast<CreateFortressInfo>();
	catalog.CreateFortress(context, changed_info);
}
