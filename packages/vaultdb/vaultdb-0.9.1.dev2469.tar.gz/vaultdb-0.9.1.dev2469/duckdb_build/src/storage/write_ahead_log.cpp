...
// ROLE
//===--------------------------------------------------------------------===//
void WriteAheadLog::WriteCreateRole(const RoleCatalogEntry &entry) {
	if (skip_writing) {
		return;
	}
	BinarySerializer serializer(*writer);
	serializer.Begin();
	serializer.WriteProperty(100, "wal_type", WALType::CREATE_ROLE);
	serializer.WriteProperty(101, "role", &entry);
	serializer.End();
}

void WriteAheadLog::WriteDropRole(const RoleCatalogEntry &entry) {
	if (skip_writing) {
		return;
	}
	BinarySerializer serializer(*writer);
	serializer.Begin();
	serializer.WriteProperty(100, "wal_type", WALType::DROP_ROLE);
	serializer.WriteProperty(101, "schema", entry.schema.name);
	serializer.WriteProperty(102, "name", entry.name);
	serializer.End();
}
...
// TAG
//===--------------------------------------------------------------------===//
void WriteAheadLog::WriteCreateTag(const TagCatalogEntry &entry) {
	if (skip_writing) {
		return;
	}
	BinarySerializer serializer(*writer);
	serializer.Begin();
	serializer.WriteProperty(100, "wal_type", WALType::CREATE_TAG);
	serializer.WriteProperty(101, "tag", &entry);
	serializer.End();
}

void WriteAheadLog::WriteDropTag(const TagCatalogEntry &entry) {
	if (skip_writing) {
		return;
	}
	BinarySerializer serializer(*writer);
	serializer.Begin();
	serializer.WriteProperty(100, "wal_type", WALType::DROP_TAG);
	serializer.WriteProperty(101, "schema", entry.schema.name);
	serializer.WriteProperty(102, "name", entry.name);
	serializer.End();
}
...
// CONFIG
//===--------------------------------------------------------------------===//
void WriteAheadLog::WriteCreateConfig(const ConfigCatalogEntry &entry) {
	if (skip_writing) {
		return;
	}
	BinarySerializer serializer(*writer);
	serializer.Begin();
	serializer.WriteProperty(100, "wal_type", WALType::CREATE_CONFIG);
	serializer.WriteProperty(101, "config", &entry);
	serializer.End();
}
...
// FORTRESS
//===--------------------------------------------------------------------===//
void WriteAheadLog::WriteCreateFortress(const FortressCatalogEntry &entry) {
	if (skip_writing) {
		return;
	}
	BinarySerializer serializer(*writer);
	serializer.Begin();
	serializer.WriteProperty(100, "wal_type", WALType::CREATE_FORTRESS);
	serializer.WriteProperty(101, "fortress", &entry);
	serializer.End();
}

void WriteAheadLog::WriteDropFortress(const FortressCatalogEntry &entry) {
	if (skip_writing) {
		return;
	}
	BinarySerializer serializer(*writer);
	serializer.Begin();
	serializer.WriteProperty(100, "wal_type", WALType::DROP_FORTRESS);
	serializer.WriteProperty(101, "schema", entry.schema.name);
	serializer.WriteProperty(102, "name", entry.name);
	serializer.End();
}
