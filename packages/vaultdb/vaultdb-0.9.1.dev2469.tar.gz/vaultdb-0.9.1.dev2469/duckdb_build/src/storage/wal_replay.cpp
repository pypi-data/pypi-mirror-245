...
bool WriteAheadLog::Replay(AttachedDatabase &database, string &path) {
	Connection con(database.GetDatabase(), INTERNAL_ROLE);
...
void ReplayState::ReplayEntry(WALType entry_type, BinaryDeserializer &deserializer) {
	...
	case WALType::CREATE_ROLE:
		ReplayCreateRole(deserializer);
		break;
	case WALType::DROP_ROLE:
		ReplayDropRole(deserializer);
		break;
	case WALType::CREATE_TAG:
		ReplayCreateTag(deserializer);
		break;
	case WALType::DROP_TAG:
		ReplayDropTag(deserializer);
		break;
	case WALType::CREATE_CONFIG:
		ReplayCreateConfig(deserializer);
		break;
	case WALType::CREATE_FORTRESS:
		ReplayCreateFortress(deserializer);
		break;
	case WALType::DROP_FORTRESS:
		ReplayDropFortress(deserializer);
		break;
	default:
...
// Replay Role
//===--------------------------------------------------------------------===//
void ReplayState::ReplayCreateRole(BinaryDeserializer &deserializer) {
	auto info = deserializer.ReadProperty<unique_ptr<CreateInfo>>(101, "role");
	info->on_conflict = OnCreateConflict::IGNORE_ON_CONFLICT;
	catalog.CreateRole(context, info->Cast<CreateRoleInfo>());
}

void ReplayState::ReplayDropRole(BinaryDeserializer &deserializer) {
	DropInfo info;
...
// Replay Tag
//===--------------------------------------------------------------------===//
void ReplayState::ReplayCreateTag(BinaryDeserializer &deserializer) {
	auto info = deserializer.ReadProperty<unique_ptr<CreateInfo>>(101, "tag");
	info->on_conflict = OnCreateConflict::IGNORE_ON_CONFLICT;
	catalog.CreateTag(context, info->Cast<CreateTagInfo>());
}

void ReplayState::ReplayDropTag(BinaryDeserializer &deserializer) {
	DropInfo info;

	info.type = CatalogType::TAG_ENTRY;
	info.schema = deserializer.ReadProperty<string>(101, "schema");
	info.name = deserializer.ReadProperty<string>(102, "name");
	if (deserialize_only) {
		return;
...
// Replay Config
//===--------------------------------------------------------------------===//
void ReplayState::ReplayCreateConfig(BinaryDeserializer &deserializer) {
	auto info = deserializer.ReadProperty<unique_ptr<CreateInfo>>(101, "config");
	info->on_conflict = OnCreateConflict::IGNORE_ON_CONFLICT;
	catalog.CreateConfig(context, info->Cast<CreateConfigInfo>());
}

//===--------------------------------------------------------------------===//
// Replay Fortress
//===--------------------------------------------------------------------===//
void ReplayState::ReplayCreateFortress(BinaryDeserializer &deserializer) {
	auto info = deserializer.ReadProperty<unique_ptr<CreateInfo>>(101, "fortress");
	info->on_conflict = OnCreateConflict::IGNORE_ON_CONFLICT;
	catalog.CreateFortress(context, info->Cast<CreateFortressInfo>());
}

void ReplayState::ReplayDropFortress(BinaryDeserializer &deserializer) {
	DropInfo info;

	info.type = CatalogType::FORTRESS_ENTRY;
	info.schema = deserializer.ReadProperty<string>(101, "schema");
	info.name = deserializer.ReadProperty<string>(102, "name");
	if (deserialize_only) {
		return;
