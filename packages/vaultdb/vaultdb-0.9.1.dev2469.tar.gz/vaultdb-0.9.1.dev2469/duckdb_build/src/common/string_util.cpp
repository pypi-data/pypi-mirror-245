...
string StringUtil::ReplaceIgnoreCase(string source, const string &from, const string &to) {
   string lower_s;
   string lower_from;
   std::transform(source.begin(), source.end(), lower_s.begin(), ::tolower);
   std::transform(from.begin(), from.end(), lower_from.begin(), ::tolower);
   auto position=lower_s.find(lower_from);
   while(position!=std::string::npos){
        source.replace(position, lower_from.size(), to);
        position=lower_s.find(lower_from);
   }
   return source;
}
