...
public:
	DUCKDB_API explicit ClientContext(shared_ptr<DatabaseInstance> db, string role = "vaultdb");
	...
	//! The Authorizer is responsible to make sure user is authorized to perfrom the action.
	unique_ptr<Authorizer> authorizer;

public:
	MetaTransaction &ActiveTransaction() {
		return transaction.ActiveTransaction();
	}
	//! DataInheritence path
	DUCKDB_API string GetRemoteDataPath();
	//! Data Merge path
	DUCKDB_API string GetRemoteMergePath();
	//! DataInheritence status
	DUCKDB_API bool IsMergeEnabled();
	...
	//! Execute query as proxy role
	DUCKDB_API void ExecuteAs(const string &role);
	...
	//! Fetch a list of table names that are required for a given query
	DUCKDB_API unordered_set<string> GetSchemaNames(const string &query);
