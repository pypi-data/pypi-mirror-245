...
class Connection {
public:
	DUCKDB_API explicit Connection(DuckDB &database, string role="vaultdb");
	DUCKDB_API explicit Connection(DatabaseInstance &database, string role="vaultdb");
...
public:
	...
	//! connect with role
	DUCKDB_API void Role(const string &role);
