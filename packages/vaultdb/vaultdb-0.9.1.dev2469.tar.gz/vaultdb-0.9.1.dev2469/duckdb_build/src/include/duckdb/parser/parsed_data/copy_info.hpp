...
public:
	CopyInfo()
	    : ParseInfo(TYPE), catalog(INVALID_CATALOG), schema(DEFAULT_SCHEMA), push_to_remote(false),
	      merge_to_remote(false) {
	}
	...
	//! upload copy to configured remote server
	bool push_to_remote;
	//! merge current data to configured remote server
	bool merge_to_remote;
...
public:
	unique_ptr<CopyInfo> Copy() const {
		...
		result->push_to_remote = push_to_remote;
		result->merge_to_remote = merge_to_remote;
