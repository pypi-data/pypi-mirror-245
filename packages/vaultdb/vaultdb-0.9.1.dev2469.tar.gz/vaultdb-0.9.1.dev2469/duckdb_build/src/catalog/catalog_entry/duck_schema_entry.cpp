...
DuckSchemaEntry::DuckSchemaEntry(Catalog &catalog, string name_p, bool is_internal)
    : SchemaCatalogEntry(catalog, std::move(name_p), is_internal),
      ...
      roles(catalog), tags(catalog), configs(catalog), fortresses(catalog),
...
optional_ptr<CatalogEntry> DuckSchemaEntry::CreateRole(CatalogTransaction transaction, CreateRoleInfo &info) {
	auto role = make_uniq<RoleCatalogEntry>(catalog, *this, info);
	if (info.internal)
		role->internal = true;
	return AddEntry(transaction, std::move(role), info.on_conflict);
}

optional_ptr<CatalogEntry> DuckSchemaEntry::CreateTag(CatalogTransaction transaction, CreateTagInfo &info) {
	auto tag = make_uniq<TagCatalogEntry>(catalog, *this, info);
	if (info.internal)
		tag->internal = true;
	return AddEntry(transaction, std::move(tag), info.on_conflict);
}

optional_ptr<CatalogEntry> DuckSchemaEntry::CreateConfig(CatalogTransaction transaction, CreateConfigInfo &info) {
	auto config = make_uniq<ConfigCatalogEntry>(catalog, *this, info);
	if (info.internal)
		config->internal = true;
	
	auto &db_config = DBConfig::GetConfig(transaction.GetContext());
	if (info.name==REMOTE_DATA_PATH)
		db_config.options.data_inheritance_path = info.value;
	else
		db_config.options.data_merge_queue_path = info.value;

	return AddEntry(transaction, std::move(config), info.on_conflict);
}

optional_ptr<CatalogEntry> DuckSchemaEntry::CreateFortress(CatalogTransaction transaction, CreateFortressInfo &info) {
	auto fortress = make_uniq<FortressCatalogEntry>(catalog, *this, info);
	fortress->AddFortressToTableEntry(transaction.GetContext());
	if (info.internal)
		fortress->internal = true;
	return AddEntry(transaction, std::move(fortress), info.on_conflict);
}
...
void DuckSchemaEntry::Verify(Catalog &catalog) {
	...
	roles.Verify(catalog);
	tags.Verify(catalog);
	configs.Verify(catalog);
	fortresses.Verify(catalog);
}
