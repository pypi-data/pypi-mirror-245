...
TableCatalogEntry::TableCatalogEntry(Catalog &catalog, SchemaCatalogEntry &schema, CreateTableInfo &info)
    : StandardEntry(CatalogType::TABLE_ENTRY, schema, catalog, info.table), fortress(std::move(info.fortress)), partition_key_columns(std::move(info.partition_key_columns)), columns(std::move(info.columns)),
...
string TableCatalogEntry::GetRemotePath(ClientContext &context, bool filename) {
	auto &fs = FileSystem::GetFileSystem(context);

	auto data_inheritance_path = context.GetRemoteDataPath();
	if (!fs.DirectoryExists(data_inheritance_path))
		throw InternalException("remote Path Doesn't exists!");

	auto file_path = fs.JoinPath(data_inheritance_path, "vaultdb");
	if (!fs.DirectoryExists(file_path))
		fs.CreateDirectory(file_path);

	file_path = fs.JoinPath(file_path, catalog.GetName());
	if (!fs.DirectoryExists(file_path))
		fs.CreateDirectory(file_path);

	file_path = fs.JoinPath(file_path, schema.name);
	if (!fs.DirectoryExists(file_path))
		fs.CreateDirectory(file_path);

	file_path = fs.JoinPath(file_path, name);
	if (!fs.DirectoryExists(file_path))
		fs.CreateDirectory(file_path);

	if (filename)
		file_path = fs.JoinPath(file_path, "data.parquet");
	return file_path;
}

vector<column_t> TableCatalogEntry::GetPrimaryKeyColumnIndex() {
	vector<column_t> primary_key_column_index;
	for (auto &cond : constraints) {
		if (cond->type != ConstraintType::UNIQUE)
			continue;
		auto &pk = (UniqueConstraint &)*cond;
		if (!pk.is_primary_key)
			continue;
		for (auto &column_name : pk.columns) {
			auto idx = GetColumnIndex(column_name);
			primary_key_column_index.push_back(idx.index);
		}
	}
...
static void BindExtraColumns(TableCatalogEntry &table, LogicalGet &get, LogicalProjection &proj, LogicalUpdate &update,
	...
	if (found_column_count > 0 && found_column_count != bound_columns.size()) {
		...
		for (auto &check_column_id : bound_columns) {
			...
			//! vaultdb: add only missing column
			auto it = std::find(get.column_ids.begin(), get.column_ids.end(), check_column_id.index);
			if (it == get.column_ids.end()) {
				proj.expressions.push_back(make_uniq<BoundColumnRefExpression>(
				    column.Type(), ColumnBinding(get.table_index, get.column_ids.size())));
				get.column_ids.push_back(check_column_id.index);
			} else {
				auto indx = it - get.column_ids.begin();
				proj.expressions.push_back(
				    make_uniq<BoundColumnRefExpression>(column.Type(), ColumnBinding(get.table_index, indx)));
			}
...
void TableCatalogEntry::BindUpdateConstraints(LogicalGet &get, LogicalProjection &proj, LogicalUpdate &update,
	...
	for (auto &constraint : fortress_constraints) {
	...
	if (update.merge_remote) {
		//! vaultdb: add all columns when merge_remote is enabled
		update.update_is_del_and_insert = true;
	}
