...
DuckTableEntry::DuckTableEntry(Catalog &catalog, SchemaCatalogEntry &schema, BoundCreateTableInfo &info,
	...
	if (!storage) {
		...
		for (idx_t i = 0; i < bound_constraints.size(); i++) {
			...
			if (constraint->type == ConstraintType::UNIQUE) {
				...
				if (unique.is_primary_key) {
					...
					for (auto &pk : unique.keys)
						primary_keys.push_back(columns.LogicalToPhysical(pk));
				}
		...
		if (primary_keys.size() > 0) {
			vector<column_t> column_ids;
			vector<unique_ptr<Expression>> unbound_expressions;
			string tobmstone_index_name = "Primary_Key_Hash";
			unbound_expressions.push_back(make_uniq<BoundColumnRefExpression>("Primary_Key_Hash", LogicalType::HASH,
			                                                                  ColumnBinding(0, column_ids.size())));
			column_ids.push_back(0);
			storage->info->tombstone =
			    make_uniq<ART>(tobmstone_index_name, IndexConstraintType::UNIQUE, column_ids, TableIOManager::Get(*storage), 
						std::move(unbound_expressions), storage->db, nullptr);
		}
	}
...
unique_ptr<CatalogEntry> DuckTableEntry::AlterEntry(ClientContext &context, AlterInfo &info) {
	...
	case AlterTableType::ADD_TAG: {
		auto add_info = table_info.Cast<AddColumnTagInfo>();
		return AddTag(context, add_info);
	}
	case AlterTableType::REMOVE_TAG: {
		auto remove_info = table_info.Cast<RemoveColumnTagInfo>();
		return RemoveTag(context, remove_info);
	}
	case AlterTableType::ALTER_PARTITION_KEY: {
		auto alter_part_info = table_info.Cast<AlterPartitionKeyInfo>();
		return ModifyPartionKeys(context, alter_part_info);
	}
...
unique_ptr<CatalogEntry> DuckTableEntry::RenameColumn(ClientContext &context, RenameColumnInfo &info) {
	...
	for (auto &i : fortress) {
		create_info->fortress.insert(i);
	}
	for (auto &i : partition_key_columns) {
		create_info->partition_key_columns.push_back(i);
	}
...
unique_ptr<CatalogEntry> DuckTableEntry::AddTag(ClientContext &context, AddColumnTagInfo &info) {
	auto create_info = make_uniq<CreateTableInfo>(schema, name);
	create_info->temporary = temporary;
	for (auto &i : fortress) {
		create_info->fortress.insert(i);
	...
	for (auto &col : columns.Logical()) {
		...
		if (default_idx == col.Logical()) {
			// set the TAG of this column
			copy.tag_names.push_back(std::move(info.tag_name));
		}
		create_info->columns.AddColumn(std::move(copy));
	}
...
unique_ptr<CatalogEntry> DuckTableEntry::RemoveTag(ClientContext &context, RemoveColumnTagInfo &info) {
	auto create_info = make_uniq<CreateTableInfo>(schema, name);
	create_info->temporary = temporary;
	for (auto &i : fortress) {
		create_info->fortress.insert(i);
	...
	for (auto &col : columns.Logical()) {
		auto copy = col.Copy();
		if (default_idx == col.Logical()) {
			// Remove the TAG from this column
			auto idx = std::find(copy.tag_names.begin(), copy.tag_names.end(), info.tag_name);
			if (idx != copy.tag_names.end())
				copy.tag_names.erase(idx);
		}
		create_info->columns.AddColumn(std::move(copy));
	}
...
unique_ptr<CatalogEntry> DuckTableEntry::ModifyPartionKeys(ClientContext &context, AlterPartitionKeyInfo &info) {
	auto create_info = make_uniq<CreateTableInfo>(schema, name);
	create_info->temporary = temporary;
	...
	create_info->partition_key_columns = std::move(info.partition_key_columns);
