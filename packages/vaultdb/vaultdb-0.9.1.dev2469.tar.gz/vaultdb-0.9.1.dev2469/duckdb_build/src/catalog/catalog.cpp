...
optional_ptr<CatalogEntry> Catalog::CreateRole(CatalogTransaction transaction, CreateRoleInfo &info) {
	auto &schema = GetSchema(transaction, SECURITY_SCHEMA);
	return CreateRole(transaction, schema, info);
}

optional_ptr<CatalogEntry> Catalog::CreateRole(CatalogTransaction transaction, SchemaCatalogEntry &schema,
                                               CreateRoleInfo &info) {
	return schema.CreateRole(transaction, info);
}

optional_ptr<CatalogEntry> Catalog::CreateRole(ClientContext &context, CreateRoleInfo &info) {
	return CreateRole(GetCatalogTransaction(context), info);
}

//===--------------------------------------------------------------------===//
// TAG
//===--------------------------------------------------------------------===//
optional_ptr<CatalogEntry> Catalog::CreateTag(CatalogTransaction transaction, CreateTagInfo &info) {
	auto &schema = GetSchema(transaction, SECURITY_SCHEMA);
	return CreateTag(transaction, schema, info);
}

optional_ptr<CatalogEntry> Catalog::CreateTag(CatalogTransaction transaction, SchemaCatalogEntry &schema,
                                              CreateTagInfo &info) {
	return schema.CreateTag(transaction, info);
}

optional_ptr<CatalogEntry> Catalog::CreateTag(ClientContext &context, CreateTagInfo &info) {
	return CreateTag(GetCatalogTransaction(context), info);
}

//===--------------------------------------------------------------------===//
// CONFIG
//===--------------------------------------------------------------------===//
optional_ptr<CatalogEntry> Catalog::CreateConfig(CatalogTransaction transaction, CreateConfigInfo &info) {
	auto &schema = GetSchema(transaction, SECURITY_SCHEMA);
	return CreateConfig(transaction, schema, info);
}

optional_ptr<CatalogEntry> Catalog::CreateConfig(CatalogTransaction transaction, SchemaCatalogEntry &schema,
                                              CreateConfigInfo &info) {
	return schema.CreateConfig(transaction, info);
}

optional_ptr<CatalogEntry> Catalog::CreateConfig(ClientContext &context, CreateConfigInfo &info) {
	return CreateConfig(GetCatalogTransaction(context), info);
}

//===--------------------------------------------------------------------===//
// Fortress
//===--------------------------------------------------------------------===//
optional_ptr<CatalogEntry> Catalog::CreateFortress(CatalogTransaction transaction, CreateFortressInfo &info) {
	auto &schema = GetSchema(transaction, info.schema);
	return CreateFortress(transaction, schema, info);
}

optional_ptr<CatalogEntry> Catalog::CreateFortress(CatalogTransaction transaction, SchemaCatalogEntry &schema,
                                                   CreateFortressInfo &info) {
	return schema.CreateFortress(transaction, info);
}

optional_ptr<CatalogEntry> Catalog::CreateFortress(ClientContext &context, CreateFortressInfo &info) {
	return CreateFortress(GetCatalogTransaction(context), info);
}
...
void Catalog::InitializeSecuritySchema() {
	if (db.IsSystem() || db.IsTemporary())
		return;
		
	CatalogTransaction data(GetDatabase(), 1, 1);
	auto vaultdbrole_result = TryLookupEntryInternal(data, CatalogType::ROLE_ENTRY, SECURITY_SCHEMA, ADMIN_ROLE);
	if (!vaultdbrole_result.Found()) {
		CreateRoleInfo vaultdbrole;
		vaultdbrole.name = ADMIN_ROLE;
		vaultdbrole.schema = SECURITY_SCHEMA;
		vaultdbrole.login = true;
		vaultdbrole.superuser = true;
		vaultdbrole.internal = true;
		vaultdbrole.on_conflict = OnCreateConflict::IGNORE_ON_CONFLICT;
		CreateRole(data, vaultdbrole);
	}

	auto pitag_result = TryLookupEntryInternal(data, CatalogType::TAG_ENTRY, SECURITY_SCHEMA, PI_TAG);
	if (!pitag_result.Found()) {
		CreateTagInfo personal_info_tag;
		personal_info_tag.schema = SECURITY_SCHEMA;
		personal_info_tag.name = PI_TAG;
		personal_info_tag.internal = true;
		personal_info_tag.comment = "Personal Information (PI) must be protected";
		personal_info_tag.function = make_uniq<ConstantExpression>(Value(99999999));
		personal_info_tag.on_conflict = OnCreateConflict::IGNORE_ON_CONFLICT;
		CreateTag(data, personal_info_tag);
	}

	auto piitag_result = TryLookupEntryInternal(data, CatalogType::TAG_ENTRY, SECURITY_SCHEMA, PII_TAG);
	if (!piitag_result.Found()) {
		CreateTagInfo personal_indetifiable_info_tag;
		personal_indetifiable_info_tag.schema = SECURITY_SCHEMA;
		personal_indetifiable_info_tag.name = PII_TAG;
		personal_indetifiable_info_tag.internal = true;
		personal_indetifiable_info_tag.comment = "Personal Identifiable Information (PII) must be protected";
		personal_indetifiable_info_tag.function = make_uniq<ConstantExpression>(Value(99999999));
		personal_indetifiable_info_tag.on_conflict = OnCreateConflict::IGNORE_ON_CONFLICT;
		CreateTag(data, personal_indetifiable_info_tag);
	}

	auto fortresss_schema_entry = GetSchema(data, SECURITY_SCHEMA, OnEntryNotFound::RETURN_NULL);
	if (!fortresss_schema_entry)
		return;
	
	vector<reference<FortressCatalogEntry>> forts;
	fortresss_schema_entry->Scan(CatalogType::FORTRESS_ENTRY, [&](CatalogEntry &fort) { forts.push_back(fort.Cast<FortressCatalogEntry>()); });
	for (auto &fort_p : forts) {
		auto &fort = fort_p.get();
		auto basetable = unique_ptr_cast<TableRef, BaseTableRef>(fort.table->Copy());
		auto table_entry_result = TryLookupEntryInternal(data, CatalogType::TABLE_ENTRY, basetable->schema_name, basetable->table_name);
		if (table_entry_result.Found()) {
			auto &table_entry = table_entry_result.entry->Cast<TableCatalogEntry>();
			table_entry.fortress.insert(fort.name);
		}
	}
}
