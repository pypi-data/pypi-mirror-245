...
typedef enum PGAlterTableType {
	...
	PG_AT_PartitionBy,           	 /* Partition Table by columns */
	...
	PG_AT_AlterColumnAddTAG,         /* alter column Add TAG */
	PG_AT_AlterColumnDeleteTAG,      /* alter column Remove TAG */
...
typedef struct PGCreateStmt {
	...
	PGNode *partitionClause; 			  /* PARTITION BY clause option */
 ...
 *		Config Statements
 *
 *  Note that PG_CONFIGOPT_CONFIG must be set in options.
 * ----------------------
 */
typedef enum PGConfigOption {
	PG_CONFIGOPT_CONFIG = 1 << 0,			/* do SET BOTH PATH to SAME VALUE */
	PG_CONFIGOPT_DATA_PATH = 1 << 1,		/* do SET DATA PATH */
	PG_CONFIGOPT_MERGE_PATH = 1 << 2       /* do SET MERGE PATH */
} PGConfigOption;

typedef struct PGCreateConfigStmt {
	PGNodeTag type;
	PGRangeVar *config; 				/* the config to create */
	int options;          				/* OR of PGConfigOption flags */
	PGOid ownerId; 						/* ID of owner, or InvalidOid for default */
	PGOnCreateConflict onconflict;      /* what to do on create conflict */
} PGCreateConfigStmt;
 ...
 *		Role Statements
 *
 *  Note that PG_ROLEOPT_ROLE must be set in options.
 * ----------------------
 */
typedef enum PGRoleOption {
	PG_ROLEOPT_ROLE = 1 << 0,               /* do CREATE ROLE */
	PG_ROLEOPT_SUPERUSER = 1 << 1,          /* do SUPERUSER */
	PG_ROLEOPT_NOSUPERUSER = 1 << 2,        /* NONSUPERUSER */
	PG_ROLEOPT_LOGIN = 1 << 3,              /* LOGIN OPTION */
	PG_ROLEOPT_NOLOGIN = 1 << 4             /* NOLOGIN */
} PGRoleOption;

typedef struct PGCreateRoleStmt {
	PGNodeTag type;
	PGRangeVar *role; 					/* the role to create */
	int options;          				/* OR of PGRoleOption flags */
	PGOid ownerId; 						/* ID of owner, or InvalidOid for default */
	PGOnCreateConflict onconflict;      /* what to do on create conflict */
} PGCreateRoleStmt;

typedef struct PGAlterRoleStmt {
	PGNodeTag type;
	PGRangeVar *role; 					/* the role to create */
	int options;          				/* OR of PGRoleOption flags */
	PGOid ownerId; 						/* ID of owner, or InvalidOid for default */
...
typedef struct PGCreateTagStmt {
	PGNodeTag type;
	PGRangeVar *tag; 					/* the TAG to create */
	PGNode *comment; 					/* the TAG to create */
	PGNode *function;					/* Expression to use when permission is not there */
	PGOid ownerId; 						/* ID of owner, or InvalidOid for default */
	PGOnCreateConflict onconflict;      /* what to do on create conflict */
} PGCreateTagStmt;

typedef struct PGCreateFortressStmt {
	PGNodeTag type;
	PGRangeVar *fortress;				/* the Fortress to create */
	PGRangeVar *tablename;				/* the tablename to create */
	PGNode *whereClause;       			/* qualifications */
	PGOid ownerId; 						/* ID of owner, or InvalidOid for default */
	PGOnCreateConflict onconflict;      /* what to do on create conflict */
} PGCreateFortressStmt;

typedef struct PGAlterFortressStmt: PGCreateFortressStmt {
} PGAlterFortressStmt;

typedef struct PGLockFortressStmt {
	PGNodeTag type;
	PGRangeVar *fortress;				/* the Fortress to create */
	PGNode *start_date;              /* window's own name */
	PGNode *end_date;              /* window's own name */
} PGLockFortressStmt;

typedef struct PGUnLockFortressStmt {
	PGNodeTag type;
	PGRangeVar *fortress;				/* the Fortress to create */
	PGRangeVar *role;				/* the Fortress to create */
} PGUnLockFortressStmt;
 ...
 * ----------------------
 */
typedef enum PGGrantOption {
	PG_GRANTOPT_NOGRANT = 1 << 0,        /* do WITHOUT GRANT OPTION */
	PG_GRANTOPT_GRANT = 1 << 1,          /* do WITH GRANT OPTION */
} PGGrantOption;

typedef enum PGPrivilegeOption {
	PG_PRIVILEGE_OPT 	  = 1 << 0,        /* PRIVILEGE */
	PG_PRIVILEGE_SELECT   = 1 << 1,        /* SELECT PRIVILEGE */
	PG_PRIVILEGE_EXECUTE  = 1 << 1,       /* EXECUTE PRIVILEGE */
	PG_PRIVILEGE_INSERT   = 1 << 2,        /* INSERT PRIVILEGE */
	PG_PRIVILEGE_UPDATE   = 1 << 3,        /* UPDATE PRIVILEGE */
	PG_PRIVILEGE_DELETE   = 1 << 4,        /* DELETE PRIVILEGE */
	PG_PRIVILEGE_TRUNCATE = 1 << 5,        /* TRUNCATE PRIVILEGE */
	PG_PRIVILEGE_CREATE   = 1 << 6,        /* CREATE PRIVILEGE */
	PG_PRIVILEGE_ALTER    = 1 << 6,        	/* CREATE PRIVILEGE */
	PG_PRIVILEGE_DROP     = 1 << 7,        /* DROP PRIVILEGE */
	PG_PRIVILEGE_COPY     = 1 << 8,        /* COPY PRIVILEGE */
	PG_PRIVILEGE_LOAD     = 1 << 9,        /* LOAD PRIVILEGE */
	PG_PRIVILEGE_ALL      = 1 << 10,       /* ALL PRIVILEGE */
} PGPrivilegeOption;

typedef struct PGGrantStmt {
	PGNodeTag type;
	PGRangeVar *role; 					/* the role */	
	PGObjectType resourcetype; 			/* the grant on type such as table */
	PGList *aliases;               		/* target column or tag names */
	PGRangeVar *resourcename; 			/* the resource name */
	int privilegeOptions; 				/* the privileges */
	int grantoption;          			/* OR with grant option */
	PGOid ownerId; 						/* ID of owner, or InvalidOid for default */
	PGOnCreateConflict onconflict;      /* what to do on create conflict */
} PGGrantStmt;

typedef struct PGRevokeStmt : PGGrantStmt {
	int revokeoption;          			/* revoke option */
} PGRevokeStmt;
...
typedef struct PGExportStmt {
	...
	PGList *options;      /* PGList of PGDefElem nodes */
	bool push_to_remote;   /* push to remote */
	bool merge_to_remote;   /* merge with remote */
} PGExportStmt;
