...
class KeycloakMongo(Mongo):
    ...
    def __init__(self, config: configparser.ConfigParser | None = None,
        ...
        properties:
            ...
            ClientId = the client id to use. The default is admin-cli.

        :param client_id: the the client id to use. The default is admin-cli.
    ...
    def client_id(self) -> str:
        """The Keycloak client id. The default is admin-cli."""
    ...
    async def get_current_user_roles(self, request: Request) -> list[Role]:
        ...
        """
        return deepcopy(await self.__get_my_roles(request))
    ...
    async def has_role_current_user(self, request: Request, role_name: str) -> bool:
        ...
        :raises ValueError: if something went wrong getting role information.
        """
        return any(role.role == role_name for role in await self.__get_my_roles(request))
    ...
    async def __get_my_roles(self, request: Request) -> list[Role]:
        ...
        if cached_val is not None:
            ...
            roles = {}
            def new_role(role_name, role_dict):
                role = Role()
                role.role = role_name
            ...
            async def one():
                async with session_get(role_base_url / 'users' / sub / 'role-mappings' / 'realm' / 'composite') as response_:
                    for role_dict in await response_.json():
                        new_role(role_dict['name'], role_dict)
            async def two():
                async with session_get(role_base_url / 'ui-ext' / 'effective-roles' / 'users' / sub) as response_:
                    for role_dict in await response_.json():
                        new_role(role_dict['role'], role_dict)
            async def three():
                async with session_get(role_base_url / 'users' / sub / 'role-mappings') as response_:
                    for role_dict in (await response_.json())['realmMappings']:
                        new_role(role_dict['name'], role_dict)
            await gather(one(), two(), three())
