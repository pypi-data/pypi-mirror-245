...
def run_ansible_playbook(playbook_path):
    try:
        result = subprocess.run(['ansible-playbook', playbook_path], capture_output=True, text=True, check=True)
        ...
        for task_name, duration in task_durations.items():
            if duration > 10:
                print(
                    f"Task {task_name} has a long execution duration ({duration} seconds). Please check for performance issues and consider optimization.")
            elif duration > 5:
                print(
                    f"Task {task_name} has a relatively long execution duration ({duration} seconds). It is recommended to optimize for better efficiency.")
            else:
                print(f"Task {task_name} has a good execution duration ({duration} seconds).")
