...
class GenericDownlinkStage(GenericStage):
    ...
    def send_adv_payloads(self, tx_max_durations=TX_MAX_DURATIONS, retries=RETRIES):
        ...
        for tx_max_duration in tx_max_durations:
            ...
            for retry in retries:
                ...
                sent_payload = self.edge.send_packet_through_gw(gateway_id=self.gw_id, raw_packet=brg_hb, debug=True,
                                                                return_payload=True, tx_max_duration=tx_max_duration, repetitions=tx_max_retries)
                ...
                time.sleep(max(MAX_RX_TX_PERIOD_SECS, tx_max_duration/1000))
        time.sleep(10)
        return sent_pkts
    ...
    def generate_stage_report(self):
        ...
        x_value = ('tx_max_duration', 'TX Max Duration') if not self.legacy else ('tx_max_retries', 'TX Max Retries')
        ...
        for channel, channel_df in trendline_info.groupby('BLE Adv. Channel'):
            ...
            if self.legacy:
                if not ((rsquared > 0.81) and (0.9 < slope < 1.1)):
                    channel_pass = False
            else:
                if not ((rsquared > 0.8) and (slope > 0)):
                    channel_pass = False
