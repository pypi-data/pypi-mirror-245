...
class MessageLocal(Item, ABC):
    ...
    def __init__(self, original_body: str, is_http_api: bool, api_type_id: int, endpoint: str,
                 importance: MessageImportance = MessageImportance.MEDIUM, original_subject: str = None,
    ...
    def get_message_provider_id(self, message_channel_id: int, recipient: Recipient) -> int:
        ...
        if message_channel_id == SMS_MESSAGE_CHANNEL_ID and recipient.get_canonical_telephone().startswith("972"):
            ...
            raise Exception("Can determine the Message Provider")
        """
    ...
    def _set_recipients(self, to_recipients: List[Recipient], cc_recipients: List[Recipient]) -> None:
        ...
        for recipient in to_recipients:
            self.__body_after_text_template[recipient.get_person_id()]["body_after_text_template"] = template.get_variable_values_and_chosen_option()
        logger.end()

    def _get_body_after_text_template(self) -> dict:
    ...
    def _get_subject_after_html_template(self) -> str:
        return self.__subject_after_html_template  # Unresolved attribute reference '__subject_after_html_template' for class 'MessageLocal'
