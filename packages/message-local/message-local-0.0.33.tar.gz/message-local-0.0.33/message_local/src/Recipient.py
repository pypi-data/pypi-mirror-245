...
class Recipient:
    ...
    def get_canonical_telephone(self):
        ...
        cleaned_number = ''.join(char for char in self.__telephone_number if char.isdigit())

        # If a country code is provided, prepend it to the cleaned number
        # country_id = user.get_country_id()
        country_id = 972
        if country_id:
            cleaned_number = f"{country_id}{cleaned_number}"
