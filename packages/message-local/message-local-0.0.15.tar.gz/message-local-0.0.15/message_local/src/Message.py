...
class Message(Item, ABC):
    ...
    def __init__(self, body: str, importance: MessageImportance, is_direct_api: bool, api_type_id:int, endpoint:str, subject: str = None, headers: dict = DEFAULT_HEADERS) -> None:
    ...
    @abstractmethod
    def _can_send(self, external_user_id: int = None, api_data: dict = None, outgoing_body: dict = None ) -> bool:
        if self._is_direct_api:
            self._can_send_direct(external_user_id=external_user_id, api_data=api_data)
        else:
            self._can_send_indirect(external_user_id=external_user_id, outgoing_body=outgoing_body)

    def _can_send_direct(self, external_user_id: int = None, api_data: dict = None) -> bool:
        try:
            try_to_call_api_result = self.__direct.try_to_call_api(external_user_id=external_user_id,
    ...
    def _can_send_indirect(self, external_user_id:int, outgoing_body: dict ):
        try:
            api_check, self.__api_call_id, arr = self.__indirect.before_call_api(external_user_id=external_user_id, api_type_id=self._api_type_id,
                                                                        endpoint=self._endpoint,
                                                                        outgoing_header=self._headers,
    ...
    @abstractmethod
    def _after_send_attempt(self, external_user_id:int = None, outgoing_body:dict = None, incoming_message:str = None, http_status_code:int = None, response_body:str = None) -> None:
        if self._is_direct_api:
            self.after_direct_send()
        else:
            self.after_indirect_send(external_user_id=external_user_id,
                                            outgoing_body=outgoing_body,
    ...
    def after_indirect_send(self, external_user_id:int, outgoing_body:dict, incoming_message:str, http_status_code:int, response_body:str):
        self.__indirect.after_call_api(external_user_id=external_user_id,
                                            api_type_id=self._api_type_id,
                                            endpoint=self._endpoint,
                                            outgoing_header=self._headers,
                                            outgoing_body=outgoing_body,
