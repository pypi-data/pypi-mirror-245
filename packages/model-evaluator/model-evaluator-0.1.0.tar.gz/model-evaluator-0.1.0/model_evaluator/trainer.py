...
class Plotter:
    ...
    def replot(self) -> None:
        if self._plot_options in [PlottingOptions.PLOT_ONLY_TRAIN, PlottingOptions.PLOT_BOTH] \
            ...
            plt.plot(
                np.linspace(1, 1 + len(self._train_history) / self._batch_size, len(self._train_history)),
                self._train_history,
        ...
        if self._plot_options in [PlottingOptions.PLOT_ONLY_TEST, PlottingOptions.PLOT_BOTH] \
            ...
            plt.plot(
                np.linspace(1, 1 + len(self._test_history)),
                self._test_history,
                self._test_line,
                label='Test Loss')
...
class Trainer:
    def __init__(self,
        ...
        self._plotter = Plotter(plotting_options, batch_size)
    ...
    def train(self, epochs: int,
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                predictions, labels = self._evaluate(test_loader)
    ...
    def _evaluate_metrics(self, data_loader: DataLoader,
        ...
        self._model.eval()
        with torch.no_grad():
            predictions, labels = self._evaluate(data_loader)
            if predictions.shape[1] == 1:
                predictions = predictions.view(-1)
                classes = predictions >= 0.5
            else:
    ...
    def _evaluate(self, data_loader: DataLoader) -> tuple[Tensor, Tensor]:
        result_preds = []
        result_labels = []
