    ...
    for path in [__file__]+[_.filename for _ in inspect.stack()]:
        for splitter in ['\\',]:
            while splitter in path:
                ...
                if os.path.exists(path+splitter+'cache') and os.path.exists(path+splitter+'json') and os.path.exists(path+splitter+'executable'):
                    _root=path
except IndexError:
    pass
Logcount = 0
...
def manage_args(**merged_keys):
    def decorator(func):
        ...
        def wrapper(*a, **b):
            ...
            # 调用处没有传参的而定义处具有默认值的，应当加入 all_args 进行参数传递
...
def get_lock(name=None, interval=2, max_wait=30):
    ...
    while True:
        f=createfile(path=lockpath(f'{name}.txt'), silent=False,encoding='utf-8',content=dicttojson(traceback()[1:]))
        if f:
            f.close()
            break
        sleep(interval)
...
def jsonpath(s=''):
    return _mix_path(projectpath('json'), s)
...
class txt():
    ...
    @manage()
    def get(self, silent=None, not_null=None, interval=2, refresh=True,
            save=True,
            b=None,**leak):
        """
        第一行放最后一行
    ...
    @listed()
    def add(self, i=None, silent=None):
        """
        线程安全
        @return:
        """
...
class Json(txt):
    ...
    def delete(self, d=None,s=None):
        if used(d):
            txt.delete(self, s=dicttojson(d))
        if used(s):
            txt.delete(self, s=s)
...
class cache(txt):
    ...
    def get(self, silent=None, interval=2):
        ...
        if self.is_json:
            Json.delete(self,d=ret)
        else:
            txt.delete(self,s=ret)
        releaselock(name='Json'+self.path)
...
def text(a=None,page=None,strict=None):
    if used_type(a,WebElement)and used(page):
        ...
        for e in page.elements(root=a,xpath='.//*',strict=False,not_deep_text=True):
            t=Str(e.get_attribute('alt'))
            if not originals==t:
                s+=t
        for e in page.elements(root=a,xpath='.//*[text() != ""]/text()',strict=False,not_deep_text=True):
            if not originals==e:
                s+=e
    return s
...
if (element) {
    ...
    def fullscreen(self, path=None, scale=100, autodown=True, pause=1, clip=True, clipinterval=0.6,top=0, bottom=0, left=0, right=0, maxheight=40000, _look=False,                   overwrite=None,b=None,**leak):
        """
    ...
    @manage_args(xpath=['s'])
    def elements(self, xpath=None, depth=2,silent=True, strict=True, root=None,refresh=False, complete=None,interval=1,not_deep_text=None,
                 b=None, **leak
                 ) -> list:
        """
...
if useDisk(d='d') == 'HerMAJESTY':
    Debug()
consoletxt = Json(projectpath('/console.txt'))
