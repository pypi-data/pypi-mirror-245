...
from typing import (AbstractSet, Any, ClassVar, Dict, Iterator, Mapping,
...
class Multihash:
    ...
    def __new__(cls, codec: Union[str, int, Multicodec]) -> "Multihash":
        ...
        if codec.tag != "multihash":
            raise MultihashValueError(
                f"Multicodec named {repr(codec.name)} exists, but is not a multihash."
            )
        if not raw.exists(codec.name):
            raise MultihashKeyError(
                f"No implementation for multihash multicodec {repr(codec.name)}."
            )
        ...
        if codec.name in _cache:
            ...
            if instance.codec == codec:
                ...
                if instance._implementation is None:
                    ...
                    return instance
                if (
                    codec.name in raw._hashfun
                    and instance._implementation == raw._hashfun[codec.name]
                ):
    ...
    def wrap(self, raw_digest: BytesLike) -> bytes:
        ...
        if max_digest_size is not None and size > max_digest_size:
            raise MultihashValueError(
                f"Digest size {max_digest_size} is listed for {self.name}, "
    ...
    def digest(self, data: BytesLike, *, size: Optional[int] = None) -> bytes:
        ...
        if size is None:
            ...
            assert size == len(
                raw_digest
            ), f"Expected {size}B digest, found {len(raw_digest)}B digest."
        return self.codec.wrap(varint.encode(size) + raw_digest)
    ...
    def unwrap(self, digest: Union[BytesLike, BufferedIOBase]) -> bytes:
        ...
        if code != self.code:
            raise MultihashValueError(
                f"Decoded code {code} differs from multihash code {self.code}."
            )
...
def digest(
    data: BytesLike, hashfun: Union[str, int, Multihash], *, size: Optional[int] = None
) -> bytes:
    """
    Computes the raw digest of the given data and wraps it into a multihash digest.
...
def _validate_raw_digest_size(
    name: str, raw_digest: bytes, max_digest_size: Optional[int]
) -> None:
    if max_digest_size is not None and len(raw_digest) > max_digest_size:
        raise MultihashValueError(
            f"Multihash {name} has max digest size {max_digest_size}, "
            f"but a digest of larger size {len(raw_digest)} was unwrapped instead."
        )


def unwrap(
    digest: Union[BytesLike, BufferedIOBase],
    hashfun: Union[None, str, int, Multihash] = None,
) -> bytes:
    """
...
def unwrap_raw(
    ...
    if not exists(code=code):
        ...
        raise MultihashKeyError(
            f"Multicodec {_hexcode(code)} is not a multihash{n_bytes_read}."
        )
    ...
    if digest_size != len(hash_digest):
        raise MultihashValueError(
            f"Multihash digest lists size {digest_size}, but the hash digest has size {len(hash_digest)} instead."
        )
