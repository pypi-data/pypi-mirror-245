...
def _jit_register_sha23ke(m, register) -> bool:  # type: ignore
    ...
    if register is not None:
        register(
            f"sha{sha_version}-{digest_bits}",
            _sha(sha_version, digest_bits),
            digest_bits // 8,
        )
    return True
...
def _sha2_512(digest_bits: int) -> Hashfun:
    ...
    try:
        from Cryptodome.Hash import \
            SHA512  # pylint: disable = import-outside-toplevel
    except ImportError as e:
        raise ImportError(
            "Module 'Cryptodome' must be installed to use the 'sha2-256' hash functions. "
            "Consider running 'pip install pycryptodomex'."
        ) from e
...
def _jit_register_dbl_sha23(m, register) -> bool:  # type: ignore
    ...
    if register is not None:
        register(
            f"sha{sha_version}-{digest_bits}",
            _dbl_sha23(sha_version, digest_bits),
            digest_bits // 8,
        )
    return True
