...
from ._hashfuns import Hashfun, repeat_hashfun, validate_hashfun_args
    ...
    if name not in _hashfun:
        if not _jit_register_hashfun(name):
            raise MultihashKeyError(
                f"No implementation for multihash multicodec {repr(name)}."
            )
...
def register(
    name: str, hashfun: Hashfun, digest_size: Optional[int], *, overwrite: bool = False
) -> None:
    """
    ...
    if not overwrite and name in _hashfun:
        raise MultihashValueError(
            f"An implementation for the multihash multicodec named {repr(name)} already exists."
        )
    if name not in _hashfun:
        multihash = multicodec.get(name)
        if multihash.tag != "multihash":
            raise MultihashValueError(
                f"Multicodec '{multihash.name}' exists, but it is not a multihash multicodec."
            )
...
def unregister(name: str) -> None:
    ...
    if name not in _hashfun:
        raise MultihashKeyError(
            f"There is no implementation for multihash multicodec with name {repr(name)}."
        )
    del _hashfun[name]
...
def _identity(data: BytesLike, size: Optional[int] = None) -> bytes:
    ...
    if len(d) < size:
        raise MultihashValueError(
            "With 'identity' hash, size must be at most data lenght in bytes."
        )
    return d[:size]
...
def _jit_register_hashfun_sha(name: str, check_only: bool = False) -> bool:
    ...
    if m is not None:
        from ._hashfuns.sha import \
            _jit_register_sha1  # pylint: disable = import-outside-toplevel
    ...
    if m is not None:
        from ._hashfuns.sha import \
            _jit_register_sha23ke  # pylint: disable = import-outside-toplevel

        return _jit_register_sha23ke(m, None if check_only else register)
    m = re.fullmatch(_sha2_512_regex, name)
    if m is not None:
        from ._hashfuns.sha import \
            _jit_register_sha2_512  # pylint: disable = import-outside-toplevel

        return _jit_register_sha2_512(m, None if check_only else register)
    m = re.fullmatch(_sha2_256_trunc254_padded_regex, name)
    if m is not None:
        from ._hashfuns.filecoin import \
            _jit_register_sha_256_trunc254_padded  # pylint: disable = import-outside-toplevel

        return _jit_register_sha_256_trunc254_padded(
            m, None if check_only else register
        )
    return False
...
def _jit_register_hashfun_bla(name: str, check_only: bool = False) -> bool:
    ...
    if m is not None:
        from ._hashfuns.blake import \
            _jit_register_blake2  # pylint: disable = import-outside-toplevel

        return _jit_register_blake2(m, None if check_only else register)
    m = re.fullmatch(_blake3_regex, name)
    if m is not None:
        from ._hashfuns.blake import \
            _jit_register_blake3  # pylint: disable = import-outside-toplevel
...
def _jit_register_hashfun_kec(name: str, check_only: bool = False) -> bool:
    ...
    if m is not None:
        from ._hashfuns.keccak import \
            _jit_register_keccak  # pylint: disable = import-outside-toplevel

        return _jit_register_keccak(m, None if check_only else register)
    return False


_skein_regex = re.compile(
    r"skein(256|512|1024)-([89]|[1-9][0-9]|[1-9][0-9][0-9]|10[0-2][0-9])"
)
...
def _jit_register_hashfun_ske(name: str, check_only: bool = False) -> bool:
    ...
    if m is not None:
        from ._hashfuns.skein import \
            _jit_register_skein  # pylint: disable = import-outside-toplevel
...
def _jit_register_hashfun_mur(name: str, check_only: bool = False) -> bool:
    ...
    if m is not None:
        from ._hashfuns.murmur3 import \
            _jit_register_murmur3  # pylint: disable = import-outside-toplevel
...
def _jit_register_hashfun_md5(name: str, check_only: bool = False) -> bool:
    ...
    if m is not None:
        from ._hashfuns.md import \
            _jit_register_md5  # pylint: disable = import-outside-toplevel
...
def _jit_register_hashfun_rip(name: str, check_only: bool = False) -> bool:
    ...
    if m is not None:
        from ._hashfuns.md import \
            _jit_register_ripemd  # pylint: disable = import-outside-toplevel
...
def _jit_register_hashfun_kan(name: str, check_only: bool = False) -> bool:
    ...
    if m is not None:
        from ._hashfuns.kangarootwelve import \
            _jit_register_kangarootwelve  # pylint: disable = import-outside-toplevel
