...
from typing import (BinaryIO, List, Optional, Tuple, TypeVar, Union, cast,
                    overload)
...
def decode(b: Union[BytesLike, BufferedIOBase, BinaryIO]) -> int:
    ...
    if isinstance(b, byteslike) and len(b) > num_bytes_read:
        raise ValueError(
            "A bytes-like object was passed, but not all bytes were used by the encoding."
        )
    return x
...
def _no_next_byte_error(num_bytes_read: int) -> ValueError:
    ...
    return ValueError(
        f"Byte #{num_bytes_read-1} was a continuation byte, but byte #{num_bytes_read} not available."
    )
...
def decode_raw(
    b: Union[BytesLike, BufferedIOBase, BinaryIO]
    ...
    if num_bytes_read > 1 and x < 2 ** (7 * (num_bytes_read - 1)):
        raise ValueError(
            f"Number {x} was not minimally encoded (as a {num_bytes_read} bytes varint)."
        )
    if stream_mode is not None:
