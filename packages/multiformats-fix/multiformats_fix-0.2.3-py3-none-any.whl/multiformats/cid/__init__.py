...
from typing import Any, FrozenSet, Tuple, Type, TypeVar, Union, cast

from bases import base58btc
from typing_extensions import Final, Literal
from typing_validation import validate

from multiformats import multibase, multicodec, multihash, varint
...
def _binary_cid_from_str(cid: str) -> Tuple[bytes, Multibase]:
    ...
    if b[0] == 0x12:
        ...
        raise ValueError(
            "CIDv0 may not be multibase encoded (found multibase encoded bytes starting with 0x12)."
        )
    return b, mb
...
def _CID_validate_raw_digest(
    raw_digest: Union[str, BytesLike], hashfun: Multihash
) -> bytes:
...
def _CID_validate_multihash_digest(
    digest: Union[str, BytesLike]
) -> Tuple[Multihash, bytes]:
...
def _CID_validate_version(
    version: int, base: Multibase, codec: Multicodec, hashfun: Multihash
) -> int:
    if version in (2, 3):
    ...
    if version == 0:
        if base.name != "base58btc":
            raise ValueError(
                f"CIDv0 multibase must be 'base58btc', found {repr(base.name)} instead."
            )
        if codec.name != "dag-pb":
            raise ValueError(
                f"CIDv0 multicodec must be 'dag-pb', found {repr(codec.name)} instead."
            )
        if hashfun.name != "sha2-256":
            raise ValueError(
                f"CIDv0 multihash must be 'sha2-256', found {repr(hashfun.name)} instead."
            )
    return version
...
class CID:
    ...
    def __new__(
        ...
        return CID._new_instance(
            cls, base, version, codec, hashfun, (hashfun, raw_digest)
        )

    @staticmethod
    def _new_instance(
        CID_subclass: Type[_CIDSubclass],
        base: Multibase,
        version: int,
        codec: Multicodec,
        hashfun: Multihash,
        ...
        if isinstance(digest, bytes):
            ...
            assert (
                _hashfun == hashfun
            ), "You passed different multihashes to a _new_instance call with digest as a pair."
    ...
    def human_readable(self) -> str:
        ...
        hashfun_str = (
            f"({self.hashfun.name} : {len(raw_digest)*8} : {raw_digest.hex().upper()})"
        )
        return (
            f"{self.base.name} - cidv{self.version} - {self.codec.name} - {hashfun_str}"
        )
    ...
    def encode(self, base: Union[None, str, Multibase] = None) -> str:
        ...
        if self.version == 0:
            if base is not None:
                raise ValueError(
                    "CIDv0 cannot be multibase-encoded, please set multibase=None."
                )
    ...
    def set(
        ...
        *,
        ...
        if base is not None and base not in (self.base, self.base.name):
            ...
            base = self.base
        if codec is not None and codec not in (
            self.codec,
            self.codec.name,
            self.codec.code,
        ):
    ...
    def __bytes__(self) -> bytes:
        if self.version == 0:
            return self.digest
        return (
            varint.encode(self.version) + varint.encode(self.codec.code) + self.digest
        )
