...
def register(
    name: str,
    raw_encoder: Optional[RawEncoder],
    raw_decoder: Optional[RawDecoder],
    ...
    *,
    ...
    if addr_size == 0 and (raw_encoder is not None or raw_decoder is not None):
        raise MultiaddrValueError(
            "Protocol admits no address (addr_size=0), set raw encoder and decoder to None."
        )
    if not overwrite and name in _proto_impl:
        raise MultiaddrValueError(
            f"Implementation for protocol {repr(name)} already exists."
        )
...
def unregister(name: str) -> None:
    ...
    validate(name, str)
    if name not in _proto_impl:
        raise MultiaddrKeyError(
            f"Implementation for protocol {repr(name)} does not exist."
        )
...
def _validate_size(name: str, b: BytesLike, size: int) -> None:
    if len(b) != size:
        raise MultiaddrValueError(
            f"Incorrect length for {repr(name)} bytes: found {len(b)}, expected {size}."
        )
