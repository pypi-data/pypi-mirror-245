...
from .raw import ProtoImpl, RawDecoder, RawEncoder, _validate_size
...
class Proto:
    ...
    def __new__(cls, codec: Union[str, int, Multicodec]) -> "Proto":
        ...
        if isinstance(codec, str):
            ...
            if existing_codec != codec:
                raise MultiaddrValueError(
                    f"Multicodec named {repr(codec.name)} exists, but is not the one given."
                )
            codec = existing_codec
        # check that the codec is a multiaddr multicodec:
        if codec.tag != "multiaddr":
            raise MultiaddrValueError(
                f"Multicodec named {repr(codec.name)} exists, but is not a multiaddr."
            )
    ...
    def validate(self, addr_value: Union[str, BytesLike]) -> Tuple[str, bytes]:
        ...
        if addr_size == 0:
            raise MultiaddrValueError(
                f"Protocol admits no address value, but {repr(addr_value)} was passed."
            )
        if isinstance(addr_value, byteslike):
            assert raw_decoder is not None
            addr_value_str = raw_decoder(
                addr_value
            )  # raises MultiaddrValueError if addr_value is invalid
    ...
    def __truediv__(
        self, value: Union[int, str, BytesLike, "Proto", "Addr", "Multiaddr"]
    ) -> Union["Addr", "Multiaddr"]:
    ...
    def __bytes__(self) -> bytes:
        if self.addr_size != 0:
            raise MultiaddrValueError(
                "Missing address value for protocol, cannot compute bytes."
            )
...
class Addr:
    ...
    def __new__(
        cls, proto: Union[str, int, Multicodec, Proto], value: Union[str, BytesLike]
    ) -> "Addr":
...
class Multiaddr(Sequence[Union[Addr, Proto]]):
    ...
    def __new__(cls, *addrs: Union[Addr, Proto]) -> "Multiaddr":
        ...
        for idx, addr in enumerate(addrs):
            if isinstance(addr, Proto):
                ...
                if proto.addr_size != 0:
                    if idx == l - 1:
                        ...
                        raise MultiaddrValueError(
                            f"Protocol {repr(proto.name)} expects an address, but is followed by another protocol instead."
                        )
            else:
                validate(addr, Addr)
                proto = addr.proto
            if proto in proto_map:
                raise MultiaddrValueError(
                    f"Protocol {repr(proto.name)} appears twice in multiaddr."
                )
            proto_map[proto] = idx
    ...
    def index(
        self, value: Union[Addr, Proto], start: int = 0, stop: Optional[int] = None
    ) -> int:
        """
        ...
        if proto not in self._proto_map:
            raise MultiaddrValueError(
                f"Protocol {repr(proto.name)} does not appear in multiaddr {str(self)}"
            )
        idx = self._proto_map[proto]
        if isinstance(value, Addr):
            if self[idx] != value:
                raise MultiaddrValueError(
                    f"Address {repr(value)} does not appear in multiaddr {str(self)}"
                )
            if not start <= idx < stop:
                raise MultiaddrValueError(
                    f"Address {repr(value)} does not appear in sub-multiaddr {str(self[start:stop])} "
    ...
    def __truediv__(
        self, other: Union[int, str, BytesLike, Addr, Proto, "Multiaddr"]
    ) -> "Multiaddr":
        if isinstance(
            other,
            (
            ...
            if not self.is_incomplete:
                raise MultiaddrValueError(
                    "Unexpected address value. Expected Proto, Addr or Multiaddr."
                )
            addrs = list(self)
            tail_proto = addrs[-1]
    ...
    def __bytes__(self) -> bytes:
        if self.is_incomplete:
            raise MultiaddrValueError(
                "Missing address value for last protocol, cannot compute bytes."
            )
