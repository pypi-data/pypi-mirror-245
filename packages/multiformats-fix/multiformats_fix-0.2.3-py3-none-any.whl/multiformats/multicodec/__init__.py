...
from typing import (AbstractSet, Any, Dict, Iterable, Iterator, Mapping,
...
class Multicodec:
    ...
    def unwrap(self, multicodec_data: BytesLike) -> bytes:
        ...
        if code != self.code:
            ...
            raise MulticodecValueError(
                f"Found code {hexcode} when unwrapping data, expected code {self.hexcode}."
            )
    ...
    def _as_tuple(
        self,
    ) -> Tuple[Type["Multicodec"], str, str, int, Literal["draft", "permanent"]]:
...
def get(name: Optional[str] = None, *, code: Optional[int] = None) -> Multicodec:
    ...
    if (name is None) == (code is None):
        raise MulticodecValueError(
            "Must specify exactly one between 'name' and 'code'."
        )
    if name is not None:
...
def multicodec(name: str, *, tag: Optional[str] = None) -> Multicodec:
    ...
    if tag is not None and codec.tag != tag:
        raise MulticodecKeyError(
            f"Multicodec {repr(name)} exists, but its tag is not {repr(tag)}."
        )
    return codec


def exists(
    name: Union[None, str, Multicodec] = None, *, code: Optional[int] = None
) -> bool:
    """
...
def unwrap_raw(
    multicodec_data: Union[BytesLike, BufferedIOBase]
    ...
    if not exists(code=code):
        raise MulticodecKeyError(
            f"No multicodec is known with unwrapped code {_hexcode(code)}."
        )
...
def validate_multicodec(codec: Multicodec) -> None:
    ...
    if mc != codec:
        raise MulticodecValueError(
            f"Multicodec named {codec.name} exists, but is not the one given."
        )
...
def register(codec: Multicodec, *, overwrite: bool = False) -> None:
    ...
    if not overwrite and codec.code in _code_table:
        raise MulticodecValueError(
            f"Multicodec with code {repr(codec.code)} already exists: {_code_table[codec.code]}"
        )
    if codec.name in _name_table and _name_table[codec.name].code != codec.code:
        raise MulticodecValueError(
            f"Multicodec with name {repr(codec.name)} already exists: {_name_table[codec.name]}"
        )
...
def table(
    *,
    ...
    status: Union[None, str, AbstractSet[str], Sequence[str]] = None,
) -> Iterator[Multicodec]:
    """
