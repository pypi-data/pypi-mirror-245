...
from bases import (base2, base8, base10, base16, base32, base32hex, base32z,
                   base36, base58btc, base58flickr, base64, base64url)
...
def register(name: str, enc: RawEncoding, *, overwrite: bool = False) -> None:
    ...
    if not overwrite and name in _raw_encodings:
        raise MultibaseValueError(
            f"Raw encoding with name {repr(name)} already exists: {_raw_encodings[name]}"
        )
...
def _jit_register_proquint_encoding() -> None:
    ...
    _proquint_consonants_revdir = MappingProxyType(
        {char: idx for idx, char in enumerate(_proquint_consonants)}
    )
    _proquint_vowels_revdir = MappingProxyType(
        {char: idx for idx, char in enumerate(_proquint_vowels)}
    )
    ...
    def proquint_raw_encoder(b: BytesLike) -> str:
        ...
        for idx in range(0, len(b), 2):
            ...
            if len(byte_block) == 2:  # ordinary byte pair
                ...
                char_blocks.append(char_block)
        prefix = (
            "ro-"  # follows multibase code "p" to make "pro-", e.g. "pro-lusab-babad"
        )
    ...
    def proquint_raw_decoder(s: str) -> bytes:
        ...
        if not s.startswith("ro-"):
            raise binascii.Error(
                "Multibase proquint encoded strings must start with 'ro-'."
            )
        ...
        if len(s) % 6 not in (3, 5):
            raise binascii.Error(
                "Proquint encoded string length must give remainder of 3 or 5 when divided by 6."
            )
        ...
        for idx, char in enumerate(s):
            if idx % 6 == 5:  # separator
                if char != "-":
                    raise binascii.Error(
                        f"Incorrect char at position {idx}: expected '-', found {repr(char)}."
                    )
            elif idx % 2 == 0:  # consonant
                if char not in consonants_set:
                    raise binascii.Error(
                        f"Incorrect char at position {idx}: expected consonant in {repr(consonants)}, "
                        f"found {repr(char)}."
                    )
                ...
                if char not in vowels_set:
                    raise binascii.Error(
                        f"Incorrect char at position {idx}: expected vowel in {repr(vowels)}, "
                        f"found {repr(char)}."
                    )
        ...
        if len(s) % 6 == 3:
            ...
            if pad_bits != 0:
                raise binascii.Error(
                    f"Expected pad bits to be 00, found {bin(pad_bits)[2:]} instead."
                )
            # add an extra byte
            nbytes += 1
...
def _jit_register_base_encoding(
    b: Literal[16, 32, 36, 64],
    _hex: bool = False,
    _pad: bool = False,
    _upper: bool = False,
    _url: bool = False,
...
_jit_registered_encodings: Dict[str, Tuple[Callable[..., Any], Any]] = {
    ...
    **{f"base{b}": (_jit_register_base_encoding, (b,)) for b in (16, 36)},
    **{
