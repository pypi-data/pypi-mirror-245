...
from bases import (base2, base8, base10, base16, base32, base32hex, base32z,
                   base36, base45, base58btc, base58flickr, base58ripple,
                   base64, base64url)
...
from .raw import RawDecoder, RawEncoder
...
class Multibase:
    ...
    def __new__(
        cls, name: str, code: str, status: str = "draft", description: str = ""
    ) -> "Multibase":
        ...
        if re.match(r"^0x([0-9a-zA-Z][0-9a-zA-Z])+$", code):
            ...
            if ord_code in range(0x20, 0x7F):
                raise MultibaseValueError(
                    "Multibase codes in hex format cannot be printable ASCII characters."
                )
            code = chr(ord_code)
        elif len(code) != 1:
            raise MultibaseValueError(
                "Multibase codes must be single-character strings or the hex digits '0x...' of a non-empty bytestring."
            )
        return code
    ...
    def _validate_status(status: str) -> Literal["draft", "candidate", "default"]:
        if status not in ("draft", "candidate", "default"):
            raise MultibaseValueError(
                f"Invalid multibase encoding status {repr(status)}."
            )
    ...
    def raw_encoder(self) -> RawEncoder:
        ...
        if enc is None:
            raise NotImplementedError(
                f"Multibase/decoding for {repr(self.name)} is not yet implemented."
            )
        return enc.encode
    ...
    def decode(self, s: str) -> bytes:
        ...
        if encoding != self:
            raise MultibaseValueError(
                f"Expected {repr(self.name)} encoding, "
    ...
    def _as_tuple(
        self,
    ) -> Tuple[Type["Multibase"], str, str, Literal["draft", "candidate", "default"]]:
...
def get(name: Optional[str] = None, *, code: Optional[str] = None) -> Multibase:
    ...
    if (name is None) == (code is None):
        raise MultibaseValueError(
            "Must specify exactly one between encoding name and code."
        )
    if code is not None:
...
def register(base: Multibase, *, overwrite: bool = False) -> None:
    ...
    if not overwrite and base.code in _code_table:
        raise MultibaseValueError(
            f"Multibase encoding with code {repr(base.code)} already exists: {_code_table[base.code]}"
        )
    if base.name in _name_table and _name_table[base.name].code != base.code:
        raise MultibaseValueError(
            f"Multibase encoding with name {repr(base.name)} already exists: {_name_table[base.name]}"
        )
...
def validate_multibase(multibase: Multibase) -> None:
    ...
    if mc != multibase:
        raise MultibaseValueError(
            f"Multibase named {multibase.name} exists, but is not the one given."
        )
