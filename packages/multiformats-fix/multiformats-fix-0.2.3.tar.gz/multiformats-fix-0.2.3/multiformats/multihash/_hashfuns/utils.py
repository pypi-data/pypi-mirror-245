    ...
    *,
    ...
    if size is not None and size < 0:
        raise MultihashValueError(
            "If specified, digest size must be non-negative integer."
        )
    if size is not None and max_digest_size is not None and size > max_digest_size:
        raise MultihashValueError(
            "If specified, digest size must not exceed maximum digest size for hash function."
        )
    if size_required and size is None:
        raise MultihashValueError(
            f"Digest size is mandatory for hash function{' '+name if name else ''}."
        )


def repeat_hashfun(
    hashfun: Hashfun, repeat: int = 1, truncate: Literal["end", "always"] = "end"
) -> Hashfun:
    """
