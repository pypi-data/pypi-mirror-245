...
def _kangarootwelve() -> Hashfun:
    try:
        from Cryptodome.Hash import \
            KangarooTwelve  # pylint: disable = import-outside-toplevel
    except ImportError as e:
        raise ImportError(
            "Module 'Cryptodome' must be installed to use the 'kangarootwelve' hash function. "
            "Consider running 'pip install pycryptodomex'."
        ) from e

    def hashfun(data: BytesLike, size: Optional[int] = None) -> bytes:
        validate_hashfun_args(
            data, size, None, size_required=True, name="kangarootwelve"
        )
