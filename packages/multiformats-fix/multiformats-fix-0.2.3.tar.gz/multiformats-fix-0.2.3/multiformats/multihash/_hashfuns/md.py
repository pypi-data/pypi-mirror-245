...
def _ripemd(digest_bits: int) -> Hashfun:
    ...
    try:
        from Cryptodome.Hash import \
            RIPEMD160  # pylint: disable = import-outside-toplevel
    except ImportError as e:
        raise ImportError(
            "Module 'Cryptodome' must be installed to use the 'ripemd-160' hash function. Consider running 'pip install pycryptodomex'."
        ) from e
