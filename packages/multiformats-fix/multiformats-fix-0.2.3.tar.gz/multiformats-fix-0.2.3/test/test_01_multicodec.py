...
def test_multicodec_contructor(m_json: Dict[str, str]) -> None:
    ...
    assert (
        str(m)
        == f"Multicodec({', '.join(f'{k}={repr(v)}' for k, v in m_json.items())})"
    )
...
def test_table(
    tag: Union[None, str, List[str]],
    status: Union[None, str, List[str]],
    name: str,
    code: int,
) -> None:
...
table_failure_cases = [
    ...
    [
        ...
        "Repeated permanent codes not allowed.",
        True,
    ],
    [
        [
        ...
        "Repeated codes allowed if exactly one is permanent.",
        False,
    ],
    [
        [
...
def test_table_failure_modes(
    multicodecs: List[Multicodec], reason: str, fails: bool
) -> None:
