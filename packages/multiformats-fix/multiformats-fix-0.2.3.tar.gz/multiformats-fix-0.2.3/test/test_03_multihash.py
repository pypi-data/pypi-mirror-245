...
from Cryptodome.Hash import RIPEMD160, SHA512, KangarooTwelve

from multiformats import multihash
from multiformats.multihash import digest, unwrap, wrap
...
def blake2_digest(
    data: bytes, variant: str, digest_bits: int, size: Optional[int]
) -> bytes:
    assert variant in ("b", "s")
    m: hashlib._Hash = getattr(hashlib, "blake2{}".format(variant))(
...
def skein_digest(
    data: bytes, variant: int, digest_bits: int, size: Optional[int]
) -> bytes:
    assert variant in (256, 512, 1024)
    m: hashlib._Hash = getattr(skein, "skein{}".format(variant))(
...
def _test(
    hash_fn: str, data: bytes, hash_digest: bytes, size: Optional[int] = None
) -> None:
    if size is not None:
...
with importlib_resources.open_text(
    "test", "multihash-test-str-vectors.csv"
) as csv_table:
...
with importlib_resources.open_text(
    "test", "multihash-test-hex-vectors.csv"
) as csv_table:
