...
def fpk_server(command):
    ...
    if command == "start":
        ...
        global_server_instance = httpd  # Store the server instance
        ...
        if global_server_instance:
            global_server_instance.shutdown()  # Stop the server
...
def main():
    try:
        with SessionManager() as session:
            ...
            parser.add_argument('subcommand', nargs='?', default=None, help='Subcommand for server (start/stop)')
            ...
            if command == "callback":
                ...
                if args.subcommand in ["start", "stop"]:
                    fpk_server(args.subcommand)
                else:
                    print("Invalid server subcommand. Use 'start' or 'stop'.")
