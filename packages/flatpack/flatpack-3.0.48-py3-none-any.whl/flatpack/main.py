...
def fpk_run_server(operation):
    ...
    try:
        ...
        # Remove or comment out these lines to keep the server running
        # httpd.shutdown()
        # httpd.server_close()
        # ngrok.disconnect(ngrok_url)
        print("Server will continue to run...")
...
def fpk_server_operation():
    # Code that needs the server to be running goes here
    pass
    ...
    try:
        with SessionManager() as session:
            ...
            if command == "callback":
                ...
                fpk_run_server(fpk_server_operation)
