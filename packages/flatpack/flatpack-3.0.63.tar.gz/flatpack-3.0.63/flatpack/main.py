...
class FPKServer:
    ...
    def start(self, port, handler):
        # Check if NGROK_AUTHTOKEN is set and not empty
        ...
        # Run ngrok in asyncio event loop
        asyncio.get_event_loop().run_until_complete(self.setup_ngrok(port))
        self.running = True

    async def setup_ngrok(self, port):
        self.listener = await ngrok.forward(port, authtoken_from_env=True)
    ...
    def stop(self):
        ...
        asyncio.get_event_loop().run_until_complete(ngrok.disconnect(self.listener.url()))
        self.httpd.shutdown()
    ...
    try:
        with SessionManager() as session:
            ...
            if command == "callback":
                ...
                nest_asyncio.apply()
                server = FPKServer()
                server.start(fpk_find_free_port(), FPKCustomHTTPHandler)
