...
class ProjectFutureEndpoints(ProjectEndpoints):
    ...
    def joined_cohort_metric(
        self, configuration: JoinableMetric, data_cohorts: List[str], filter_cohorts: Optional[List[str]] = None
    ) -> MetricResponse:
        """
        ...
        return self.session.project.aggregate_cohort_metric(
            all_cohort_uids, configuration
        )
