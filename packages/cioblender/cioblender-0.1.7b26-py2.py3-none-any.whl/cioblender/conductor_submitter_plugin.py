...
class SaveBeforeSubmitOperator(bpy.types.Operator):
    bl_idname = "render.save_before_submit"
    bl_label = "Save Before Submitting to Render Farm"
    bl_options = {'REGISTER', 'INTERNAL'}

    action: bpy.props.EnumProperty(
        items=[
            ('SAVE', "Save", "Save the scene before submitting to the render farm"),
            ('CONTINUE', "Continue", "Continue submission without saving the scene")
        ],
        default='CONTINUE'
    )
    ...
    def execute(self, context):
        if self.action == 'SAVE':
            ...
            self.report({'INFO'}, "Continuing without saving.")
        return {'FINISHED'}
    ...
    def draw(self, context):
        layout = self.layout
        layout.label(text="Do you want to save changes before submitting to the render farm?")
        layout.label(text="Note: Saving the scene may increase upload time if it's already on the render farm.")
