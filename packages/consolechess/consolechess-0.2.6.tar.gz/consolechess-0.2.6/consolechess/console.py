...
def main() -> None:
    ...
    while result is None:
        ...
        if move == "resign":
            ...
            console.print(
                f"{winner.upper()} won the game ({description})."
            ...
            if can_claim_draw or response == "yes":
                board.print()
                console.print(
                    "The game ended in a [bold]DRAW[/bold] "
                    f"({board.status.description})"
                    if board.status.description is not None
                    else "The game ended in a [bold]DRAW[/bold]."
                )
                board.draw()
                print(f"Moves: {board.export_moves()}\n")
                result = board.status
            ...
            if board._game_over:
                ...
                if (winner := result.winner) is not None:
                    console.print(
                        f"{winner.upper()} won the game ({result.description})."
