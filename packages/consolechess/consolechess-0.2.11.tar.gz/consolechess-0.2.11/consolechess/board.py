...
class ChessBoard:
            ...
            if self.double_forward_last_move in adjacent_squares:
                if piece.color == "white":
                    if FILES.index(self.double_forward_last_move[0]) < FILES.index(
                        ...
                        yield utils.step_diagonal_up_left(initial_square, 1)
                    if FILES.index(self.double_forward_last_move[0]) > FILES.index(
                        initial_square[0]
                    ):
                        yield utils.step_diagonal_up_right(initial_square, 1)
                if piece.color == "black":
                    if FILES.index(self.double_forward_last_move[0]) < FILES.index(
                        initial_square[0]
                    ):
                        yield utils.step_diagonal_down_left(initial_square, 1)
                    if FILES.index(self.double_forward_last_move[0]) > FILES.index(
                        initial_square[0]
                    ):
                        yield utils.step_diagonal_down_right(initial_square, 1)
    ...
    def move(self: "ChessBoard", notation: str) -> None:
        ...
        if "O-O-O" in notation:
            ...
            pawn_promotion = piece_abbr[grp] if (grp := match.group(4)) != "" else None
            candidates = [
                sq
                for sq in self
    ...
    def king_is_in_check(self: "ChessBoard", color: Color) -> bool | None:
        ...
        iterators = (
            utils.iter_to_top(king_sq),
            utils.iter_to_left(king_sq),
            utils.iter_to_right(king_sq),
            utils.iter_to_bottom(king_sq),
        )
        ...
        iterators = (
            utils.iter_bottom_left_diagonal(king_sq),
            utils.iter_bottom_right_diagonal(king_sq),
            utils.iter_top_left_diagonal(king_sq),
            utils.iter_top_right_diagonal(king_sq),
        )
        for iterator in iterators:
            for sq in iterator:
                if (
    ...
    def resign(self: "ChessBoard", color: Color | None = None) -> GameStatus:
        ...
        return GameStatus(self._game_over, self._winner, "opponent_resignation")
