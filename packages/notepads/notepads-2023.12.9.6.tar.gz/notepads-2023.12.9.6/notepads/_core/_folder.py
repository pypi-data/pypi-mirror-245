...
class NotepadsFolder(object):
    ...
    def __repr__(self):
        return f'Folder["{self.path}"](name={self.name}, version={self.version}, description={self.description}, author={self.author}, len_files={len(self.children)})'
    ...
    def update_file(self, name, *, version=None, description=None, author=None):
        try:
            for child in self.children:
                if child == name:
                    self.children[child].update(name=name, version=version, description=description, author=author)
                    return child
    ...
    def read_file(self, name):
        try:
            for child in self.children:
                if child == name:
                    return self.children[child].read()
    ...
    def write_file(self, name, content):
        try:
            for child in self.children:
                if child == name:
                    self.children[child].write(content)
