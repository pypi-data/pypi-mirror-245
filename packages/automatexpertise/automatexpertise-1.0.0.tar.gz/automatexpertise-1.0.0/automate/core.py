...
def run_container_host(enterprise):
    ...
    if container_name not in [container.name for container in containers]:
        ...
        """)
        os.system("sshpass -p 'kali' ssh -o StrictHostKeyChecking=no root@localhost /usr/local/bin/init-services.sh")
    else:
...
def run_container_macvlan(ip, enterprise):
    ...
    if container_name not in [container.name for container in containers]:
        ...
        skillseries/automatexpertise
        """)
        # Lancement du service apache2
        os.system(f"ssh-keygen -f '~/.ssh/known_hosts' -R '{ip}' 2>/dev/null")
        os.system(f"sshpass -p 'kali' ssh -o StrictHostKeyChecking=no root@{ip} /usr/local/bin/init-services.sh")
    else:
        print(f"Le conteneur {container_name} existe déjà.")
...
""")
        ...
        if mode == "1":
            host_mode(name)
            print(f"Le conteneur automatexpertise-{name} a été créé avec succès.")
            sys.exit(0)
        elif mode == "2":
            macvlan_mode(name)
            print(f"Le conteneur automatexpertise-{name} a été créé avec succès.")
...
def start_container(container_name):
    try:
        ...
        if network_mode == 'host':
            os.system("ssh-keygen -f '~/.ssh/known_hosts' -R localhost 2>/dev/null")
            os.system("sshpass -p 'kali' ssh -o StrictHostKeyChecking=no root@localhost /usr/local/bin/start-services.sh")
            ...
            os.system(f"ssh-keygen -f '~/.ssh/known_hosts' -R '{container_ip}' 2>/dev/null")
            os.system(f"sshpass -p 'kali' ssh -o StrictHostKeyChecking=no root@'{container_ip}' /usr/local/bin/start-services.sh")
        ...
        print(f"Le conteneur {container_name} a été démarré avec succès.")
        client.close()
