...
def algorithm_computation_download(
    ...
    if not dry_run:
        asyncio.run(wf.download_algorithm_computation_result(algorithm_computation_id, output_zip_file))

    click.echo(f"File {output_zip_file}  (dry_run={dry_run})", nl=False)
