    ...
    def check(self):
        ...
        if self.subcmd == "install":
            if os.path.isfile(self.config) != True and os.path.dirname(self.config):
                errmes = "Error: Not found the config file. [{0}]".format(self.config)
                print_err(errmes)
                exit(1)
            try:
...
class ConfParser_snakeland:
        ...
        for rawrow in conflst:
            ...
            if var[0] == "'":
                if var[len(var) - 1] == "'":
                    ...
                    errmes = "Error: Quote mark is not in place. [{0}]".format(row)
                    print_err(errmes)
                    exit(1)
    ...
    def _load_src(self):
        ...
        secname: str = ""
        secnames: list = list()
        secnames = [self._get_sectionname(row) for row in conflst if forif(row) == True]
    ...
    def analyze(self):
        ...
        for key in dfdictkeys_es:
            if dfdict.get(key, None) == None:
                errmes = (
                    "Error: Not found {0} value on default section of config.".format(
                        key
                    )
                )
                print_err(errmes)
        ...
        for key in dfdict.keys():
            ...
            if key not in dfdictkeys_opt:
                errmes = (
                    "Error: Invalid {0} value on default section of config.".format(key)
                )
                print_err(errmes)
                exit(1)
        for srcfpath, v in srcdict.items():
    ...
    def check(self):
        ...
        if os.path.isabs(value) != True:
            errmes = "Error: DSTBASEDIR is NOT absolute path. [{0}]".format(value)
            print_err(errmes)
            exit(1)
        varname = "OSCHECK"
        ...
        if var != "":
            if os.path.isdir(var) != True:
                errmes = "Error: Not found INSTALLCMD directory. [{0}]".format(var)
                print_err(errmes)
                exit(1)
        varname = "TARGETPY3"
        var = dfdict.get(varname, "")
        if var != "":
            if os.path.isabs(var) != True:
                errmes = "Error: TARGETPY3 path is NOT absolute path. [{0}]".format(var)
                print_err(errmes)
                exit(1)
            if var.find(dstbasedir) != 0:
                errmes = (
                    "Error: TARGETPY3 path is NOT based on DSTBASEDIR. [{0}]".format(
                        var
                    )
                )
                print_err(errmes)
        ...
        if var != "":
            if os.path.isabs(var) != True:
                errmes = "Error: TARGETCMD path is NOT absolute path. [{0}]".format(var)
                print_err(errmes)
                exit(1)
            if var.find(dstbasedir) != 0:
                errmes = (
                    "Error: TARGETCMD path is NOT based on DSTBASEDIR. [{0}]".format(
                        var
                    )
                )
                print_err(errmes)
        ...
        for srcfpath, vdict in srcdict.items():
            ...
            if os.path.isabs(os.path.expanduser(value)) == True:
                ...
                if os.path.isabs(value) == True:
                    errmes = "Error: DSTDIR is Not Absolute path. [{0}]".format(value)
                    print_err(errmes)
                    exit(1)
    ...
    def _normalize_PY3VERSION(self, value: str) -> str:
        ...
        if re.match(ptn, value) != None:
            ...
            if py3minor_s > py3minor_e:
                ...
                py3minor_e = i
            templist = [
                "3.{0}".format(i) for i in range(py3minor_e, py3minor_s - 1, -1)
            ]
            return " ".join(templist)
        ptn = r"3\.[1-9]?[0-9]"
        splitted: list = []
...
class Main_snakeland:
    ...
    def py3xhunt(py3xstr: str) -> str:
        def iter_py3xcmds_on_pathdir():
            ...
            for d in osenv_path:
                ...
                for s in os.listdir(d):
                    fpath = os.path.abspath(d + "/" + s)
                    if os.path.isfile(fpath):
                        fname = os.path.basename(fpath)
                        if fname.startswith("python3"):
                            yield fpath
        ...
        for cmd in py3cmdlist:
            if sys.version_info.minor >= 5:
                retproc = subprocess.run([cmd, "--version"], capture_output=True)
    ...
    def findpy3opt2str(args) -> str:
        ...
        if args.range != "":
            ...
            if py3_minor1 >= py3_minor2:
                ...
                py3_minor1 = n
            tmplist = ["3.{0}".format(i) for i in range(py3_minor2, py3_minor1 - 1, -1)]
            return " ".join(tmplist)
        if args.order != "":
    ...
    def copy(srcdict: dict, dstbasedir: str, config: str, dryrun: bool = False) -> list:
        ...
        for f, vdict in srcdict.items():
            ...
            if dryrun:
                print("Dryrun: copy, {0} to {1}".format(srcfpath, vdict["DSTDIR"]))
            else:
                if os.path.isfile(srcfpath) != True:
                    errmes = "Error: Not found the file. [{0}]".format(srcfpath)
                    print_err(errmes)
                    exit(1)
                try:
    ...
    def _install_confpy(confpy3: str) -> str:
        ...
        if reobj != None:
            ...
            if len(tmplist) != 1:
                errmes = "Error: Invalid PY3VERSION later format. [{0}]".format(confpy3)
                print_err(errmes)
                exit(1)
        ...
        if reobj != None:
            ...
            tmplist = re.findall(ptn, confpy3)
            if len(tmplist) != 1:
                errmes = "Error: Invalid PY3VERSION older format. [{0}]".format(confpy3)
                print_err(errmes)
                exit(1)
        ...
        if os.path.isdir(pkgdir) != True:
            if dryrun:
                ...
                try:
                    ...
                    errmes = "Error: Can not make the directory. [{0}]".format(pkgdir)
                    print_err(errmes)
                    exit(1)
        ...
        if os.path.isdir(appdir) != True:
            if dryrun:
                ...
                try:
                    ...
                    errmes = "Error: Can not make the directory. [{0}]".format(appdir)
                    print_err(errmes)
                    exit(1)
        return
    ...
    def install(dfdict: dict, py3ver: str, dryrun: bool = False) -> list:
        ...
        if [py3mode, cmdmode].count(True) == 0:
            ...
            exit(1)
        if cmdmode:
            meses = ["#!/bin/sh", 'exec {0} "$@"'.format(dfdict["TARGETCMD"])]
            installcmd_dir = dfdict["INSTALLCMD"]
            installcmd = os.path.abspath(installcmd_dir + "/" + dfdict["CMDNAME"])
            if dryrun:
