...
def experiment_controller(func,demo_args,demo_kwargs,demo_globals,api_key='cp-XXX_FAKE_KEY_FOR_TESTING_XXXX',event_ingress=_A,event_egress=_A):
	...
	app_registration=AppRegistration(api_key=api_key,runner_type='python',runner_version=__version__,parameters=[x[B]for x in all_vars if not x[C]],types=[x[J]for x in all_vars if not x[C]],demo_values=[x[D]for x in all_vars if not x[C]],descriptions=[x[G]for x in all_vars if not x[C]],constraints=[x[H]for x in all_vars if not x[C]])
