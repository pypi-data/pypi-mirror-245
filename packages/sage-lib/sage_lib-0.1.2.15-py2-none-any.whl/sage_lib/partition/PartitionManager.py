...
class PartitionManager(FileManager): 
    ...
    def read_files(self, file_location:str=None, source:str='VASP', subfolders:bool=False, verbose:bool=False):
        ...
        if subfolders:
            ...
            if source.upper() == 'VASP':
                ...
                self.read_configXYZ(file_location=file_location, add_container=True, verbose=verbose)
    ...
    def readSubFolder(self, file_location:str=None, source:str='VASP', verbose=False):
        ...
        for root, dirs, files in os.walk(file_location):
            ...
            if source == 'OUTCAR': file_location_edited = f'{root}/{file_location}/OUTCAR'
            else: file_location_edited = f'{root}/{file_location}' 
            print(file_location_edited)
            try:
                SR = self.read_files(file_location=file_location_edited, source=source, subfolders=False, verbose=verbose)
            except: pass
            #DFT_SR = self.readVASPFolder(file_location=root, add_container=True, v=verbose)
    ...
    def read_configXYZ(self, file_location:str=None, add_container:bool=True, verbose:bool=False):
        '''
        '''
        file_location = file_location if type(file_location) == str else self.file_location
        ...
        for i, line in enumerate(lines):
            ...
            if line.strip().isdigit():
                ...
                if num_atoms > 0:
                    SR = SingleRun(file_location)
                    SR.AtomPositionManager = PeriodicSystem()
                    SR.AtomPositionManager.read_configXYZ(lines=lines[i:i+num_atoms+2])
