...
class PokeAPI:
    def __init__(self):
        ...
        self.csv_subpath = pkg_resources.resource_filename(__name__, 'ItemID.csv')
        print(f'\n{self.csv_subpath}\n')
        self.item_data = pd.read_csv(self.csv_subpath)
    ...
    def GET(self, call_type_str, item_name=None, item_id=None, time_span=None, save_csv=False, date_as_unix=False):
        # Convert input string to enum using getattr
        call_type = getattr(self, call_type_str.upper(), None)
        '''
        For calling the specific type of GET to the PokeMMO Hub API
        Call types to the api can be any of the following types:
        
        call_type_str can be something like: 'help' 'Max' 'MIN'
