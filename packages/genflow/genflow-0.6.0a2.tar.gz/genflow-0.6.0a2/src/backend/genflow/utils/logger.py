...
def configure(log_level: Optional[str] = None, log_file: Optional[Path] = None):
    if os.getenv("GENFLOW_LOG_LEVEL") in VALID_LOG_LEVELS and log_level is None:
        log_level = os.getenv("GENFLOW_LOG_LEVEL")
    ...
    if not log_file:
        cache_dir = Path(appdirs.user_cache_dir("genflow"))
        log_file = cache_dir / "genflow.log"
