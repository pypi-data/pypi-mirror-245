...
from genflow.main import setup_app
from genflow.services.database.utils import session_getter
from genflow.services.getters import get_db_service, get_settings_service
from genflow.services.utils import initialize_services, initialize_settings_service
from genflow.utils.logger import configure, logger
...
def run(
    host: str = typer.Option(
        "127.0.0.1", help="Host to bind the server to.", envvar="GENFLOW_HOST"
    ),
    workers: int = typer.Option(
        1, help="Number of worker processes.", envvar="GENFLOW_WORKERS"
    ),
    timeout: int = typer.Option(300, help="Worker timeout in seconds."),
    port: int = typer.Option(7860, help="Port to listen on.", envvar="GENFLOW_PORT"),
    components_path: Optional[Path] = typer.Option(
        Path(__file__).parent / "components",
        help="Path to the directory containing custom components.",
        envvar="GENFLOW_COMPONENTS_PATH",
    ),
    ...
    log_level: str = typer.Option(
        "critical", help="Logging level.", envvar="GENFLOW_LOG_LEVEL"
    ),
    log_file: Path = typer.Option(
        "logs/genflow.log", help="Path to the log file.", envvar="GENFLOW_LOG_FILE"
    ),
    cache: Optional[str] = typer.Option(
        envvar="GENFLOW_LANGCHAIN_CACHE",
...
def run_on_mac_or_linux(host, port, log_level, options, app, open_browser=True):
    webapp_process = Process(
        target=run_genflow, args=(host, port, log_level, options, app)
    )
...
def run_on_windows(host, port, log_level, options, app):
    """
    Run the Genflow server on Windows.
    """
    print_banner(host, port)
    run_genflow(host, port, log_level, options, app)
...
def print_banner(host, port):
    ...
    info_text = (
        ...
        "[bold][link=https://github.com/aiplanethub/genflow]GitHub Repo[/link][/bold] :rocket:"
    )
...
def run_genflow(host, port, log_level, options, app):
    """
    Run Genflow server on localhost
    """
    try:
        if platform.system() in ["Windows"]:
            # Run using uvicorn on MacOS and Windows
            ...
            from genflow.server import GenflowApplication

            GenflowApplication(app, options).run()
...
def superuser(
    ...
    with session_getter(db_service) as session:
        from genflow.services.auth.utils import create_super_user
