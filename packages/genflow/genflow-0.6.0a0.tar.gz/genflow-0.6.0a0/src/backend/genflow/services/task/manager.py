...
from genflow.services.task.backends.anyio import AnyIOBackend
from genflow.services.task.backends.base import TaskBackend
from genflow.services.task.utils import get_celery_worker_status


def check_celery_availability():
    try:
        from genflow.worker import celery_app
...
class TaskService(Service):
    ...
    def get_backend(self) -> TaskBackend:
        if USE_CELERY:
            from genflow.services.task.backends.celery import CeleryBackend
