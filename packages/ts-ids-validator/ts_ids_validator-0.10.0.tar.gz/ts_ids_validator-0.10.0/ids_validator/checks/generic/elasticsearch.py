...
def _find_nested_field(
    ...
    if isinstance(value, dict):
        if ESKeys.TYPE in value and value.get(ESKeys.TYPE) == ESKeys.NESTED:
            yield path
            value.pop(ESKeys.TYPE)
            if value:
...
def get_nested_fields(
    ...
    es_document = nested_get(
        es_document, f"{ESKeys.ES_MAPPING_KEY}.{ESKeys.PROPERTIES_KEY}"
    )
    ...
    if es_document:
        for path in _find_nested_field((ESKeys.PROPERTIES_KEY,), es_document):
...
def validate_path_exists_as_array_of_objects(
    ...
    if schema_object is None:
        return CheckResult.critical(
            ...
            " not found in the IDS. Each defined nested field defined"
    ...
    if not isinstance(schema_object, dict) or schema_object.get("type") != "array":
        return CheckResult.critical(
            ...
            f" must be a JSON primitive of type '{JSONTypes.ARRAY}' in the IDS."
            f" Found {schema_object} of type {type(schema_object).__name__}"
        )

    if nested_get(schema_object, "items.type") != JSONTypes.OBJECT:
        return CheckResult.critical(
            message=f"elasticsearch.json nested field, {field_path}.items,"
            f" items must be of type '{JSONTypes.OBJECT}'."
        )
...
def _get_properties(schema_object: dict, field: str, path: str) -> Dict[str, Any]:
    ...
    """
    if ESKeys.PROPERTIES_KEY not in schema_object:
        raise IDSPathError(
            field,
            path,
    ...
    properties = schema_object.get(ESKeys.PROPERTIES_KEY)
...
def validate_instance_path_exists_in_schema(path: str, ids: Dict[str, Any]) -> bool:
    ...
    while path_parts:
        ...
        if isinstance(schema_obj, dict):
            obj_type = schema_obj.get(ESKeys.TYPE)
            ...
            if obj_type == JSONTypes.OBJECT:
                schema_obj = _get_properties(schema_obj, field, path)
                continue

            if obj_type == JSONTypes.ARRAY:
                items = schema_obj.get(ESKeys.ITEMS)
                ...
                items_type = items.get(ESKeys.TYPE)

                if items_type == JSONTypes.OBJECT:
...
def validate_nonsearchable_fields(
    ...
    """
    if ESKeys.NON_SEARCHABLE not in es_document:
        return []

    non_searchable_fields = es_document.get(ESKeys.NON_SEARCHABLE)
    ...
    if not has_datacubes and validate_instance_path_exists_in_schema(
        ESKeys.DATACUBES_KEY, ids
    ):
        errors.append(
            CheckResult.critical(
                message=f"'{ESKeys.DATACUBES_KEY}' exists in the IDS but is not denoted as a nonSearchableField"
                f" in elasticsearch.json. '{ESKeys.DATACUBES_KEY}' must be a nonSearchableField"
...
def validate_elasticsearch_json(
    ...
    """
    es_document = validator_parameters.artifact.elasticsearch
    if not es_document:
        return [
            CheckResult.critical(
