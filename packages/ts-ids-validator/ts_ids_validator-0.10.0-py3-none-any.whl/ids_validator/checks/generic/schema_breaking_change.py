...
def check_version_bump_is_valid(previous: Version, current: Version) -> CheckResults:
    ...
    return [
        CheckResult.critical(
            f"The current version ({current}) is not valid compared to the previous version ({previous}). "
...
def format_failures(
    ...
    """
    failure_header = (
        f"The previous ({previous_ids.version}) and current ({current_ids.version}) schemas share the same major version number yet the following difference(s)"
        " require a major version change. Implementing the change(s) without bumping the schema's major version "
        "would result in data inconsistency while importing data to Athena."
    )
