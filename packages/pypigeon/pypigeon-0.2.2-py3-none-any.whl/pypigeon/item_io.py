...
from .pigeon_core.api.items import collections_items_create_item
from .pigeon_core.api.items import collections_items_get_item_data
from .pigeon_core.api.items import collections_items_put_item_data
from .pigeon_core.models import CollectionsItemsCreateItemFormStyleUpload
from .pigeon_core.models import CollectionsItemsCreateItemFormStyleUploadMetadata
from .pigeon_core.models import CollectionsItemsCreateItemNewItemRequest
from .pigeon_core.models import ItemColumnsItem
from .pigeon_core.models import ItemParser
from .pigeon_core.models import ItemStorage
from .pigeon_core.models import ItemType
from .pigeon_core.types import File
...
# straightforward, since the httpx client used by pigeon_core returns
    ...
    def _call_api(self, stream: BinaryIO) -> None:
        try:
            collections_items_put_item_data.sync(
                ...
                self.item_id,
                client=self._client,
                binary_body=File(payload=stream),
            )
...
class PigeonItemWriterBinary(PigeonItemWriterCore, BinaryIO):
...
class PigeonItemWriterText(PigeonItemWriterCore, TextIO):
...
class PigeonItemCreatorCore(PigeonItemWriterCore):
    def __init__(
        self,
        collection_id: str,
        version_id: str,
        ...
        columns: Optional[list[ItemColumnsItem]] = None,
    ...
    def _call_api(self, stream: BinaryIO) -> None:
        try:
            request = CollectionsItemsCreateItemFormStyleUpload(
                content=File(payload=stream, file_name=self.item_name),
                folder_id=self.item_folder_id,
                type=ItemType[self.item_type.upper()],
                columns=self.item_columns if self.item_columns else UNSET,
                metadata=(
                    CollectionsItemsCreateItemFormStyleUploadMetadata.from_dict(
                        self.item_metadata
                    )
            ...
            item = collections_items_create_item.sync(
                self.collection_id,
                self.version_id,
                multipart_data=request,
                json_body=cast(CollectionsItemsCreateItemNewItemRequest, Mock()),
                client=self._client,
            )
...
class PigeonItemCreatorBinary(PigeonItemCreatorCore, PigeonItemWriterBinary):
...
class PigeonItemCreatorText(PigeonItemCreatorCore, PigeonItemWriterText):
