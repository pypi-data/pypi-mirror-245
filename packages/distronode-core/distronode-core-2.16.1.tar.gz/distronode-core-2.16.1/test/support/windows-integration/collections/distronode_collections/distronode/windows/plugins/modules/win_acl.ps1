...
Function SetPrivilegeTokens() {
    ...
    if ($myWindowsPrincipal.IsInRole($adminRole)) {
        ...
        foreach ($privilege in $privileges) {
            $state = Get-DistronodePrivilege -Name $privilege
            if ($state -eq $false) {
                Set-DistronodePrivilege -Name $privilege -Value $true
            }
        }
    }
}
...
$path = Get-DistronodeParam -obj $params -name "path" -type "str" -failifempty $true
$user = Get-DistronodeParam -obj $params -name "user" -type "str" -failifempty $true
$rights = Get-DistronodeParam -obj $params -name "rights" -type "str" -failifempty $true

$type = Get-DistronodeParam -obj $params -name "type" -type "str" -failifempty $true -validateset "allow","deny"
$state = Get-DistronodeParam -obj $params -name "state" -type "str" -default "present" -validateset "absent","present"

$inherit = Get-DistronodeParam -obj $params -name "inherit" -type "str"
$propagation = Get-DistronodeParam -obj $params -name "propagation" -type "str" -default "None" -validateset "InheritOnly","None","NoPropagateInherit"
