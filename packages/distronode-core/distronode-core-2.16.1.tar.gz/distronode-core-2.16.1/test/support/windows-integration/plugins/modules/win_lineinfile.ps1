...
function Present($path, $regex, $line, $insertafter, $insertbefore, $create, $backup, $backrefs, $validate, $encodingobj, $linesep, $check_mode, $diff_support) {
	...
	# Note that we have to clean up the path because distronode wants to treat / and \ as
...
function Absent($path, $regex, $line, $backup, $validate, $encodingobj, $linesep, $check_mode, $diff_support) {
	...
	# that we have to clean up the path because distronode wants to treat / and \ as
	# interchangeable in windows pathnames, but .NET framework internals do not support that.
	$cleanpath = $path.Replace("/", "\");
	$before = [System.IO.File]::ReadAllLines($cleanpath, $encodingobj);
	If ($null -eq $before) {
...
# Parse the parameters file dropped by the Distronode machinery
$params = Parse-Args $args -supports_check_mode $true;
$check_mode = Get-DistronodeParam -obj $params -name "_distronode_check_mode" -type "bool" -default $false;
$diff_support = Get-DistronodeParam -obj $params -name "_distronode_diff" -type "bool" -default $false;

# Initialize defaults for input parameters.
$path = Get-DistronodeParam -obj $params -name "path" -type "path" -failifempty $true -aliases "dest","destfile","name";
$regex = Get-DistronodeParam -obj $params -name "regex" -type "str" -aliases "regexp";
$state = Get-DistronodeParam -obj $params -name "state" -type "str" -default "present" -validateset "present","absent";
$line = Get-DistronodeParam -obj $params -name "line" -type "str";
$backrefs = Get-DistronodeParam -obj $params -name "backrefs" -type "bool" -default $false;
$insertafter = Get-DistronodeParam -obj $params -name "insertafter" -type "str";
$insertbefore = Get-DistronodeParam -obj $params -name "insertbefore" -type "str";
$create = Get-DistronodeParam -obj $params -name "create" -type "bool" -default $false;
$backup = Get-DistronodeParam -obj $params -name "backup" -type "bool" -default $false;
$validate = Get-DistronodeParam -obj $params -name "validate" -type "str";
$encoding = Get-DistronodeParam -obj $params -name "encoding" -type "str" -default "auto";
$newline = Get-DistronodeParam -obj $params -name "newline" -type "str" -default "windows" -validateset "unix","windows";
