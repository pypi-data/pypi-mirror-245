...
def collect_sdist_files(complete_file_list: list[str]) -> list[str]:
    ...
    sdist_files.extend(f'lib/distronode_core.egg-info/{name}' for name in egg_info)
...
def collect_wheel_files(complete_file_list: list[str]) -> list[str]:
    ...
    wheel_files.append(f'distronode_core-{__version__}.data/scripts/distronode-test')
