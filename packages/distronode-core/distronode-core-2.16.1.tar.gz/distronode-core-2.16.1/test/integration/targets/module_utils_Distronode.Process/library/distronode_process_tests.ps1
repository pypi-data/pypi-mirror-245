...
#DistronodeRequires -CSharpUtil Distronode.Process
...
$tests = @{
    "ParseCommandLine empty string" = {
        ...
        $actual = [Distronode.Process.ProcessUtil]::ParseCommandLine("")
        Assert-Equal -Actual $actual -Expected $expected
    }

    "ParseCommandLine single argument" = {
        $expected = @("powershell.exe")
        $actual = [Distronode.Process.ProcessUtil]::ParseCommandLine("powershell.exe")
        Assert-Equal -Actual $actual -Expected $expected
    }

    "ParseCommandLine multiple arguments" = {
        $expected = @("powershell.exe", "-File", "C:\temp\script.ps1")
        $actual = [Distronode.Process.ProcessUtil]::ParseCommandLine("powershell.exe -File C:\temp\script.ps1")
        Assert-Equal -Actual $actual -Expected $expected
    }

    "ParseCommandLine comples arguments" = {
        $expected = @('abc', 'd', 'ef gh', 'i\j', 'k"l', 'm\n op', 'ADDLOCAL=qr, s', 'tuv\', 'w''x', 'yz')
        $actual = [Distronode.Process.ProcessUtil]::ParseCommandLine('abc d "ef gh" i\j k\"l m\\"n op" ADDLOCAL="qr, s" tuv\ w''x yz')
        Assert-Equal -Actual $actual -Expected $expected
    }

    "SearchPath normal" = {
        $expected = "$($env:SystemRoot)\System32\WindowsPowerShell\v1.0\powershell.exe"
        $actual = [Distronode.Process.ProcessUtil]::SearchPath("powershell.exe")
    ...
    "SearchPath missing" = {
        $failed = $false
        try {
            [Distronode.Process.ProcessUtil]::SearchPath("fake.exe")
        }
        catch {
            $failed = $true
    ...
    "CreateProcess basic" = {
        $actual = [Distronode.Process.ProcessUtil]::CreateProcess("whoami.exe")
        $actual.GetType().FullName | Assert-Equal -Expected "Distronode.Process.Result"
    ...
    "CreateProcess stderr" = {
        $actual = [Distronode.Process.ProcessUtil]::CreateProcess("powershell.exe [System.Console]::Error.WriteLine('hi')")
    ...
    "CreateProcess exit code" = {
        $actual = [Distronode.Process.ProcessUtil]::CreateProcess("powershell.exe exit 10")
        $actual.StandardOut | Assert-Equal -Expected ""
        $actual.StandardError | Assert-Equal -Expected ""
    ...
    "CreateProcess bad executable" = {
        $failed = $false
        try {
            [Distronode.Process.ProcessUtil]::CreateProcess("fake.exe")
        }
        catch {
            $failed = $true
    ...
    "CreateProcess with unicode" = {
        $actual = [Distronode.Process.ProcessUtil]::CreateProcess("cmd.exe /c echo 💩 café")
        $actual.StandardOut | Assert-Equal -Expected "💩 café`r`n"
        $actual.StandardError | Assert-Equal -Expected ""
        $actual.ExitCode | Assert-Equal -Expected 0

        $actual = [Distronode.Process.ProcessUtil]::CreateProcess($null, "cmd.exe /c echo 💩 café", $null, $null)
        $actual.StandardOut | Assert-Equal -Expected "💩 café`r`n"
        $actual.StandardError | Assert-Equal -Expected ""
        $actual.ExitCode | Assert-Equal -Expected 0
    }
    ...
    "CreateProcess without working dir" = {
        ...
        $actual = [Distronode.Process.ProcessUtil]::CreateProcess($null, 'powershell.exe $pwd.Path', $null, $null)
    ...
    "CreateProcess with working dir" = {
        ...
        $actual = [Distronode.Process.ProcessUtil]::CreateProcess($null, 'powershell.exe $pwd.Path', "C:\Windows", $null)
        $actual.StandardOut | Assert-Equal -Expected $expected
        $actual.StandardError | Assert-Equal -Expected ""
        $actual.ExitCode | Assert-Equal -Expected 0
    }
    ...
    "CreateProcess without environment" = {
        ...
        $actual = [Distronode.Process.ProcessUtil]::CreateProcess($null, 'powershell.exe $env:TEST; $env:USERNAME', $null, $null)
        $actual.StandardOut | Assert-Equal -Expected $expected
        $actual.StandardError | Assert-Equal -Expected ""
        $actual.ExitCode | Assert-Equal -Expected 0
    }
    ...
    "CreateProcess with environment" = {
        ...
        $actual = [Distronode.Process.ProcessUtil]::CreateProcess($null, 'cmd.exe /c set', $null, $env_vars)
    ...
    "CreateProcess with string stdin" = {
        ...
        $actual = [Distronode.Process.ProcessUtil]::CreateProcess($null, 'powershell.exe [System.Console]::In.ReadToEnd()',
    ...
    "CreateProcess with lpApplicationName" = {
        ...
        $actual = [Distronode.Process.ProcessUtil]::CreateProcess($full_path, "Write-Output 'abc'", $null, $null)
        $actual.StandardOut | Assert-Equal -Expected $expected
        $actual.StandardError | Assert-Equal -Expected ""
        $actual.ExitCode | Assert-Equal -Expected 0

        $actual = [Distronode.Process.ProcessUtil]::CreateProcess($full_path, "powershell.exe Write-Output 'abc'", $null, $null)
        $actual.StandardOut | Assert-Equal -Expected $expected
        $actual.StandardError | Assert-Equal -Expected ""
        $actual.ExitCode | Assert-Equal -Expected 0
    }
    ...
    "CreateProcess with unicode and us-ascii encoding" = {
        ...
        $actual = [Distronode.Process.ProcessUtil]::CreateProcess($null, "cmd.exe /c echo $poop café", $null, $null, '', 'us-ascii')
