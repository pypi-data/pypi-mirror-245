#!/usr/bin/env bash
# Make sure that distronode-test is able to parse collection config when using a venv.
...
distronode-test sanity --test import --color --venv -v
distronode-test units --color --venv -v
