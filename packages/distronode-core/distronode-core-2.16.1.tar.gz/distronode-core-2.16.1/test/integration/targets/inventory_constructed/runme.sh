...
distronode-inventory -i static_inventory.yml -i constructed.yml --graph | tee out.txt
...
distronode-inventory -i static_inventory.yml -i no_leading_separator_constructed.yml --graph | tee out.txt
...
distronode-inventory -i tag_inventory.yml -i keyed_group_default_value.yml --graph | tee out.txt
...
distronode-inventory -i tag_inventory.yml -i keyed_group_list_default_value.yml --graph | tee out.txt
...
distronode-inventory -i tag_inventory.yml -i keyed_group_str_default_value.yml --graph | tee out.txt

grep '@host_fedora' out.txt


# keyed group with 'trailing_separator' set to 'False' for key's value empty
distronode-inventory -i tag_inventory.yml -i keyed_group_trailing_separator.yml --graph | tee out.txt
...
distronode-inventory -i invs/1/one.yml -i invs/2/constructed.yml --graph | tee out.txt
