...
#DistronodeRequires -CSharpUtil Distronode.Service
...
$tests = [Ordered]@{
    "Props on service created by New-Service" = {
        $actual = New-Object -TypeName Distronode.Service.Service -ArgumentList $serviceName

        $actual.ServiceName | Assert-Equal -Expected $serviceName
        $actual.ServiceType | Assert-Equal -Expected ([Distronode.Service.ServiceType]::Win32OwnProcess)
        $actual.StartType | Assert-Equal -Expected ([Distronode.Service.ServiceStartType]::DemandStart)
        $actual.ErrorControl | Assert-Equal -Expected ([Distronode.Service.ErrorControl]::Normal)
        ...
        $actual.ServiceSidInfo | Assert-Equal -Expected ([Distronode.Service.ServiceSidInfo]::None)
        ...
        if ([Environment]::OSVersion.Version -ge [Version]'6.3') {
            $actual.LaunchProtection | Assert-Equal -Expected ([Distronode.Service.LaunchProtection]::None)
        }
        else {
            $actual.LaunchProtection | Assert-Equal -Expected $null
        }
        $actual.State | Assert-Equal -Expected ([Distronode.Service.ServiceStatus]::Stopped)
        ...
        $actual.ServiceFlags | Assert-Equal -Expected ([Distronode.Service.ServiceFlags]::None)
        $actual.DependedBy.Count | Assert-Equal 0
    }

    "Service creation through util" = {
        $testName = "$($serviceName)_2"
        $actual = [Distronode.Service.Service]::Create($testName, '"{0}"' -f $path)
    ...
    "Fail to open non-existing service" = {
        $failed = $false
        try {
            $null = New-Object -TypeName Distronode.Service.Service -ArgumentList 'fake_service'
        }
        catch [Distronode.Service.ServiceManagerException] {
    ...
    "Open with specific access rights" = {
        $service = New-Object -TypeName Distronode.Service.Service -ArgumentList @(
            $serviceName, [Distronode.Service.ServiceRights]'QueryConfig, QueryStatus'
        )

        # QueryStatus can get the status
        $service.State | Assert-Equal -Expected ([Distronode.Service.ServiceStatus]::Stopped)
    ...
    "Modfiy ServiceType" = {
        $service = New-Object -TypeName Distronode.Service.Service -ArgumentList $serviceName
        $service.ServiceType = [Distronode.Service.ServiceType]::Win32ShareProcess

        $actual = Invoke-Sc -Action qc -Name $serviceName
        $service.ServiceType | Assert-Equal -Expected ([Distronode.Service.ServiceType]::Win32ShareProcess)
        $actual.TYPE | Assert-Equal -Expected "20  WIN32_SHARE_PROCESS"

        $null = Invoke-Sc -Action config -Name $serviceName -Arguments @{type = "own" }
        $service.Refresh()
        $service.ServiceType | Assert-Equal -Expected ([Distronode.Service.ServiceType]::Win32OwnProcess)
    }

    "Create desktop interactive service" = {
        $service = New-Object -Typename Distronode.Service.Service -ArgumentList $serviceName
        $service.ServiceType = [Distronode.Service.ServiceType]'Win32OwnProcess, InteractiveProcess'

        $actual = Invoke-Sc -Action qc -Name $serviceName
        $actual.TYPE | Assert-Equal -Expected "110  WIN32_OWN_PROCESS (interactive)"
        $service.ServiceType | Assert-Equal -Expected ([Distronode.Service.ServiceType]'Win32OwnProcess, InteractiveProcess')

        # Change back from interactive process
        $service.ServiceType = [Distronode.Service.ServiceType]::Win32OwnProcess
    ...
    "Modify StartType" = {
        $service = New-Object -TypeName Distronode.Service.Service -ArgumentList $serviceName
        $service.StartType = [Distronode.Service.ServiceStartType]::Disabled

        $actual = Invoke-Sc -Action qc -Name $serviceName
        $service.StartType | Assert-Equal -Expected ([Distronode.Service.ServiceStartType]::Disabled)
        $actual.START_TYPE | Assert-Equal -Expected "4   DISABLED"

        $null = Invoke-Sc -Action config -Name $serviceName -Arguments @{start = "demand" }
        $service.Refresh()
        $service.StartType | Assert-Equal -Expected ([Distronode.Service.ServiceStartType]::DemandStart)
    }
    ...
    "Modify StartType auto delayed" = {
        ...
        $service = New-Object -TypeName Distronode.Service.Service -ArgumentList $serviceName
        $service.StartType = [Distronode.Service.ServiceStartType]::Disabled  # Start from Disabled

        # Disabled -> Auto Start Delayed
        $service.StartType = [Distronode.Service.ServiceStartType]::AutoStartDelayed

        $actual = Invoke-Sc -Action qc -Name $serviceName
        $service.StartType | Assert-Equal -Expected ([Distronode.Service.ServiceStartType]::AutoStartDelayed)
        $actual.START_TYPE | Assert-Equal -Expected "2   AUTO_START  (DELAYED)"

        # Auto Start Delayed -> Auto Start
        $service.StartType = [Distronode.Service.ServiceStartType]::AutoStart

        $actual = Invoke-Sc -Action qc -Name $serviceName
        $service.StartType | Assert-Equal -Expected ([Distronode.Service.ServiceStartType]::AutoStart)
        ...
        $service.StartType = [Distronode.Service.ServiceStartType]::DemandStart
    ...
    "Modify ErrorControl" = {
        $service = New-Object -TypeName Distronode.Service.Service -ArgumentList $serviceName
        $service.ErrorControl = [Distronode.Service.ErrorControl]::Severe

        $actual = Invoke-Sc -Action qc -Name $serviceName
        $service.ErrorControl | Assert-Equal -Expected ([Distronode.Service.ErrorControl]::Severe)
        $actual.ERROR_CONTROL | Assert-Equal -Expected "2   SEVERE"

        $null = Invoke-Sc -Action config -Name $serviceName -Arguments @{error = "ignore" }
        $service.Refresh()
        $service.ErrorControl | Assert-Equal -Expected ([Distronode.Service.ErrorControl]::Ignore)
    }
    ...
    "Modify FailureActions" = {
        $newAction = [Distronode.Service.FailureActions]@{
            ResetPeriod = 86400
            RebootMsg = 'Reboot msg'
            Command = 'Command line'
            Actions = @(
                [Distronode.Service.Action]@{Type = [Distronode.Service.FailureAction]::RunCommand; Delay = 1000 },
                [Distronode.Service.Action]@{Type = [Distronode.Service.FailureAction]::RunCommand; Delay = 2000 },
                [Distronode.Service.Action]@{Type = [Distronode.Service.FailureAction]::Restart; Delay = 1000 },
                [Distronode.Service.Action]@{Type = [Distronode.Service.FailureAction]::Reboot; Delay = 1000 }
            )
        }
        $service = New-Object -TypeName Distronode.Service.Service -ArgumentList $serviceName
        ...
        $service.FailureActions = [Distronode.Service.FailureActions]@{ResetPeriod = 172800 }
        ...
        $service.FailureActions = [Distronode.Service.FailureActions]@{RebootMsg = "New reboot msg" }
        ...
        $service.FailureActions = [Distronode.Service.FailureActions]@{Command = "New command line" }
        ...
        $service.FailureActions = [Distronode.Service.FailureActions]@{
            ResetPeriod = 86400
            Actions = @(
                [Distronode.Service.Action]@{Type = [Distronode.Service.FailureAction]::RunCommand; Delay = 5000 },
                [Distronode.Service.Action]@{Type = [Distronode.Service.FailureAction]::None; Delay = 0 }
            )
        }
        ...
        $service.FailureActions.Actions[1].Type | Assert-Equal -Expected ([Distronode.Service.FailureAction]::None)

        # Test setting just Actions without ResetPeriod
        $service.FailureActions = [Distronode.Service.FailureActions]@{
            Actions = [Distronode.Service.Action]@{Type = [Distronode.Service.FailureAction]::RunCommand; Delay = 10000 }
        }
        $actual = Invoke-Sc -Action qfailure -Name $serviceName
        $actual.'RESET_PERIOD (in seconds)' | Assert-Equal -Expected 86400
        $actual.REBOOT_MESSAGE | Assert-Equal -Expected 'New reboot msg'
        ...
        $actual.Actions[0].Type | Assert-Equal -Expected ([Distronode.Service.FailureAction]::RunCommand)
        $actual.Actions[0].Delay | Assert-Equal -Expected 5000
        $actual.Actions[1].Type | Assert-Equal -Expected ([Distronode.Service.FailureAction]::Reboot)
    ...
    "Modify ServiceSidInfo" = {
        $service = New-Object -TypeName Distronode.Service.Service -ArgumentList $serviceName
        $service.ServiceSidInfo = [Distronode.Service.ServiceSidInfo]::None

        $actual = Invoke-Sc -Action qsidtype -Name $serviceName
        $service.ServiceSidInfo | Assert-Equal -Expected ([Distronode.Service.ServiceSidInfo]::None)
        $actual.SERVICE_SID_TYPE | Assert-Equal -Expected 'NONE'

        $null = Invoke-Sc -Action sidtype -Name $serviceName -Arguments @(, 'unrestricted')
        $service.ServiceSidInfo | Assert-Equal -Expected ([Distronode.Service.ServiceSidInfo]::Unrestricted)

        $service.ServiceSidInfo = [Distronode.Service.ServiceSidInfo]::Restricted

        $actual = Invoke-Sc -Action qsidtype -Name $serviceName
        $service.ServiceSidInfo | Assert-Equal -Expected ([Distronode.Service.ServiceSidInfo]::Restricted)
    ...
    "Modify Triggers" = {
        $service = [Distronode.Service.Service]$serviceName
        $service.Triggers = @(
            [Distronode.Service.Trigger]@{
                Type = [Distronode.Service.TriggerType]::DomainJoin
                Action = [Distronode.Service.TriggerAction]::ServiceStop
                SubType = [Guid][Distronode.Service.Trigger]::DOMAIN_JOIN_GUID
            },
            [Distronode.Service.Trigger]@{
                Type = [Distronode.Service.TriggerType]::NetworkEndpoint
                Action = [Distronode.Service.TriggerAction]::ServiceStart
                SubType = [Guid][Distronode.Service.Trigger]::NAMED_PIPE_EVENT_GUID
                DataItems = [Distronode.Service.TriggerItem]@{
                    Type = [Distronode.Service.TriggerDataType]::String
                    Data = 'my named pipe'
                }
            },
            [Distronode.Service.Trigger]@{
            ...
            [Distronode.Service.Trigger]@{
                Type = [Distronode.Service.TriggerType]::Custom
                Action = [Distronode.Service.TriggerAction]::ServiceStart
                SubType = [Guid]'9bf04e57-05dc-4914-9ed9-84bf992db88c'
                DataItems = @(
                    [Distronode.Service.TriggerItem]@{
                        Type = [Distronode.Service.TriggerDataType]::Binary
                        Data = [byte[]]@(1, 2, 3, 4)
                    },
                    [Distronode.Service.TriggerItem]@{
                        Type = [Distronode.Service.TriggerDataType]::Binary
            ...
            [Distronode.Service.Trigger]@{
                Type = [Distronode.Service.TriggerType]::FirewallPortEvent
                Action = [Distronode.Service.TriggerAction]::ServiceStop
                SubType = [Guid][Distronode.Service.Trigger]::FIREWALL_PORT_CLOSE_GUID
                DataItems = [Distronode.Service.TriggerItem]@{
                    Type = [Distronode.Service.TriggerDataType]::String
        ...
        $actual.Triggers[0].SubType | Assert-Equal -Expected "$([Distronode.Service.Trigger]::DOMAIN_JOIN_GUID) [DOMAIN JOINED]"
        ...
        $actual.Triggers[1].SubType | Assert-Equal -Expected "$([Distronode.Service.Trigger]::NAMED_PIPE_EVENT_GUID) [NAMED PIPE EVENT]"
        ...
        $actual.Triggers[2].SubType | Assert-Equal -Expected "$([Distronode.Service.Trigger]::NAMED_PIPE_EVENT_GUID) [NAMED PIPE EVENT]"
        ...
        $actual.Triggers[5].SubType | Assert-Equal -Expected "$([Distronode.Service.Trigger]::FIREWALL_PORT_CLOSE_GUID) [PORT CLOSE]"
        ...
        $service.Triggers = [Distronode.Service.Trigger]@{
            Type = [Distronode.Service.TriggerType]::GroupPolicy
            Action = [Distronode.Service.TriggerAction]::ServiceStart
            SubType = [Guid][Distronode.Service.Trigger]::MACHINE_POLICY_PRESENT_GUID
        }
        ...
        $actual.Triggers[0].SubType | Assert-Equal -Expected "$([Distronode.Service.Trigger]::MACHINE_POLICY_PRESENT_GUID) [MACHINE POLICY PRESENT]"
        $actual.Triggers[0].Data.Count | Assert-Equal -Expected 0
        ...
        $actual[0].Type | Assert-Equal -Expected ([Distronode.Service.TriggerType]::NetworkEndpoint)
        $actual[0].Action | Assert-Equal -Expected ([Distronode.Service.TriggerAction]::ServiceStart)
        $actual[0].SubType = [Guid][Distronode.Service.Trigger]::NAMED_PIPE_EVENT_GUID
        $actual[0].DataItems.Count | Assert-Equal -Expected 1
        $actual[0].DataItems[0].Type | Assert-Equal -Expected ([Distronode.Service.TriggerDataType]::String)
        $actual[0].DataItems[0].Data | Assert-Equal -Expected 'abc'

        $actual[1].Type | Assert-Equal -Expected ([Distronode.Service.TriggerType]::NetworkEndpoint)
        $actual[1].Action | Assert-Equal -Expected ([Distronode.Service.TriggerAction]::ServiceStart)
        $actual[1].SubType = [Guid][Distronode.Service.Trigger]::NAMED_PIPE_EVENT_GUID
        $actual[1].DataItems.Count | Assert-Equal -Expected 1
        $actual[1].DataItems[0].Type | Assert-Equal -Expected ([Distronode.Service.TriggerDataType]::String)
        $actual[1].DataItems[0].Data | Assert-Equal -Expected 'def'

        $actual[2].Type | Assert-Equal -Expected ([Distronode.Service.TriggerType]::Custom)
        $actual[2].Action | Assert-Equal -Expected ([Distronode.Service.TriggerAction]::ServiceStart)
        $actual[2].SubType = [Guid]'d4497e12-ac36-4823-af61-92db0dbd4a76'
        $actual[2].DataItems.Count | Assert-Equal -Expected 2
        $actual[2].DataItems[0].Type | Assert-Equal -Expected ([Distronode.Service.TriggerDataType]::Binary)
        , $actual[2].DataItems[0].Data | Assert-Equal -Expected ([byte[]]@(17, 34, 51, 68))
        $actual[2].DataItems[1].Type | Assert-Equal -Expected ([Distronode.Service.TriggerDataType]::Binary)
        , $actual[2].DataItems[1].Data | Assert-Equal -Expected ([byte[]]@(170, 187, 204, 221))

        $actual[3].Type | Assert-Equal -Expected ([Distronode.Service.TriggerType]::Custom)
        $actual[3].Action | Assert-Equal -Expected ([Distronode.Service.TriggerAction]::ServiceStart)
        $actual[3].SubType = [Guid]'435a1742-22c5-4234-9db3-e32dafde695c'
        $actual[3].DataItems.Count | Assert-Equal -Expected 2
        $actual[3].DataItems[0].Type | Assert-Equal -Expected ([Distronode.Service.TriggerDataType]::String)
        $actual[3].DataItems[0].Data | Assert-Equal -Expected '11223344'
        $actual[3].DataItems[1].Type | Assert-Equal -Expected ([Distronode.Service.TriggerDataType]::String)
        $actual[3].DataItems[1].Data | Assert-Equal -Expected 'aabbccdd'

        $actual[4].Type | Assert-Equal -Expected ([Distronode.Service.TriggerType]::FirewallPortEvent)
        $actual[4].Action | Assert-Equal -Expected ([Distronode.Service.TriggerAction]::ServiceStop)
        $actual[4].SubType = [Guid][Distronode.Service.Trigger]::FIREWALL_PORT_CLOSE_GUID
        $actual[4].DataItems.Count | Assert-Equal -Expected 1
        $actual[4].DataItems[0].Type | Assert-Equal -Expected ([Distronode.Service.TriggerDataType]::String)
        , $actual[4].DataItems[0].Data | Assert-Equal -Expected @('1234', 'tcp', 'imagepath', 'servicename')

        $actual[5].Type | Assert-Equal -Expected ([Distronode.Service.TriggerType]::IpAddressAvailability)
        $actual[5].Action | Assert-Equal -Expected ([Distronode.Service.TriggerAction]::ServiceStop)
        $actual[5].SubType = [Guid][Distronode.Service.Trigger]::NETWORK_MANAGER_LAST_IP_ADDRESS_REMOVAL_GUID
...
foreach ($testImpl in $tests.GetEnumerator()) {
    $serviceName = "distronode_$([System.IO.Path]::GetRandomFileName())"
