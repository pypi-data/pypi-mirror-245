...
mkdir -p distronode_collections/ns/col/plugins/lookup

(
  cd distronode_collections/ns/col/plugins/lookup
...
distronode-test sanity --test import --color --failure-ok --lint --python "${DISTRONODE_TEST_PYTHON_VERSION}" "${@}" 1> actual-stdout.txt 2> actual-stderr.txt
...
distronode-test sanity --test import --color --truncate 0 plugins/lookup/vendor1.py plugins/lookup/vendor2.py "${@}"
