...
# distronode.module_utils.common.sys_info imports distro, and itself gets imported
# in DataLoader, so all we have to do to test the fallback is run `distronode`.
ansirun="$(distronode -i ../../inventory -a "echo \$PYTHONPATH" localhost)"
