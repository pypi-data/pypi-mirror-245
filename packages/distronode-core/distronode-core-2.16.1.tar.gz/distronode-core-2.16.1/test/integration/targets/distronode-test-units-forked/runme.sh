...
options=$("${TEST_DIR}"/../distronode-test/venv-pythons.py --only-versions)
IFS=', ' read -r -a pythons <<< "${options}"

for python in "${pythons[@]}"; do
  echo "*** Checking Python ${python} ***"

  if distronode-test units --truncate 0 --target-python "venv/${python}" "${@}" > output.log 2>&1 ; then
    cat output.log
  ...
  grep 'PASSED tests/unit/plugins/modules/test_distronode_forked.py::test_passed' output.log
  grep 'PASSED tests/unit/plugins/modules/test_distronode_forked.py::test_warning' output.log
  grep 'XFAIL tests/unit/plugins/modules/test_distronode_forked.py::test_kill_xfail' output.log
  grep 'FAILED tests/unit/plugins/modules/test_distronode_forked.py::test_kill' output.log
  grep 'FAILED tests/unit/plugins/modules/test_distronode_forked.py::test_exception' output.log
  grep 'XFAIL tests/unit/plugins/modules/test_distronode_forked.py::test_exception_xfail' output.log
