...
# run type tests
distronode-playbook -i ../../inventory types.yml -v "$@"

# test timeout
distronode-playbook -i ../../inventory timeout.yml -v "$@"

# our Play class allows for 'user' or 'remote_user', but not both.
# first test that both user and remote_user work individually
set +e
result="$(distronode-playbook -i ../../inventory user.yml -v "$@" 2>&1)"
set -e
...
set +e
result="$(distronode-playbook -i ../../inventory remote_user_and_user.yml -v "$@" 2>&1)"
set -e
...
set +e
result="$(distronode-playbook -i ../../inventory empty.yml -v "$@" 2>&1)"
set -e
...
set +e
result="$(distronode-playbook -i ../../inventory empty_hosts.yml -v "$@" 2>&1)"
set -e
...
set +e
result="$(distronode-playbook -i ../../inventory malformed_tasks.yml -v "$@" 2>&1)"
set -e
...
set +e
result="$(distronode-playbook -i ../../inventory malformed_pre_tasks.yml -v "$@" 2>&1)"
set -e
...
set +e
result="$(distronode-playbook -i ../../inventory malformed_post_tasks.yml -v "$@" 2>&1)"
set -e
...
distronode-playbook -i ../../inventory roles_null.yml -v "$@"

# test roles: 123 -- errors
echo "EXPECTED ERROR: Ensure we fail properly if roles is malformed."
set +e
result="$(distronode-playbook -i ../../inventory malformed_roles.yml -v "$@" 2>&1)"
set -e
...
set +e
result="$(distronode-playbook -i ../../inventory old_style_role.yml -v "$@" 2>&1)"
set -e
...
set +e
result="$(distronode-playbook -i ../../inventory malformed_vars_prompt.yml -v "$@" 2>&1)"
set -e
grep -q "ERROR! Invalid vars_prompt data structure, missing 'name' key" <<< "$result"

# test vars_prompt: null
distronode-playbook -i ../../inventory vars_prompt_null.yml -v "$@"

# test vars_files: null
distronode-playbook -i ../../inventory vars_files_null.yml -v "$@"

# test vars_files: filename.yml
distronode-playbook -i ../../inventory vars_files_string.yml -v "$@"
