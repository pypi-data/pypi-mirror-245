...
COMMAND=(distronode-playbook -i ../../inventory test_tags.yml -v --list-tasks)
...
# Test distronode_run_tags
distronode-playbook -i ../../inventory distronode_run_tags.yml -e expect=all "$@"
distronode-playbook -i ../../inventory distronode_run_tags.yml -e expect=all --tags all "$@"
distronode-playbook -i ../../inventory distronode_run_tags.yml -e expect=list --tags tag1,tag3 "$@"
distronode-playbook -i ../../inventory distronode_run_tags.yml -e expect=list --tags tag1 --tags tag3 "$@"
distronode-playbook -i ../../inventory distronode_run_tags.yml -e expect=untagged --tags untagged "$@"
distronode-playbook -i ../../inventory distronode_run_tags.yml -e expect=untagged_list --tags untagged,tag3 "$@"
distronode-playbook -i ../../inventory distronode_run_tags.yml -e expect=tagged --tags tagged "$@"

distronode-playbook test_template_parent_tags.yml "$@" 2>&1 | tee out.txt
[ "$(grep out.txt -ce 'Tagged_task')" = "1" ]; rm out.txt

distronode-playbook test_template_parent_tags.yml --tags tag1 "$@" 2>&1 | tee out.txt
[ "$(grep out.txt -ce 'Tagged_task')" = "1" ]; rm out.txt

distronode-playbook test_template_parent_tags.yml --skip-tags tag1 "$@" 2>&1 | tee out.txt
