...
def delegate(args: CommonConfig, host_state: HostState, exclude: list[str], require: list[str]) -> None:
    """Delegate execution of distronode-test to another environment."""
...
def delegate_command(args: EnvironmentConfig, host_state: HostState, exclude: list[str], require: list[str]) -> None:
    ...
    if host_delegation:
        if data_context().content.collection:
            ...
            content_root = os.path.join(working_directory, 'distronode')

        distronode_bin_path = os.path.join(working_directory, 'distronode', 'bin')

        with tempfile.NamedTemporaryFile(prefix='distronode-source-', suffix='.tgz') as payload_file:
    ...
    command = generate_command(args, host_state.controller_profile.python, distronode_bin_path, content_root, exclude, require)
...
def download_results(args: EnvironmentConfig, con: Connection, content_root: str, success: bool) -> None:
    ...
    with tempfile.NamedTemporaryFile(prefix='distronode-test-result-', suffix='.tgz') as result_file:
        try:
...
def generate_command(
    ...
    python: PythonConfig,
    distronode_bin_path: str,
    content_root: str,
    exclude: list[str],
    require: list[str],
) -> list[str]:
    """Generate the command necessary to delegate distronode-test."""
    cmd = [os.path.join(distronode_bin_path, 'distronode-test')]
    cmd = [python.path] + cmd

    env_vars = dict(
        DISTRONODE_TEST_CONTENT_ROOT=content_root,
    )

    if isinstance(args.controller, OriginConfig):
        # Expose the distronode and distronode_test library directories to the Python environment.
        # This is only required when delegation is used on the origin host.
        library_path = process_scoped_temporary_directory(args)

        os.symlink(DISTRONODE_LIB_ROOT, os.path.join(library_path, 'distronode'))
        os.symlink(DISTRONODE_TEST_ROOT, os.path.join(library_path, 'distronode_test'))
