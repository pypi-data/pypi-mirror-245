"""Distronode integration test infrastructure."""
...
def check_inventory(args: IntegrationConfig, inventory_path: str) -> None:
    ...
    if not isinstance(args.controller, OriginConfig):
        if os.path.exists(inventory_path):
            ...
            if 'distronode_ssh_private_key_file' in inventory:
                display.warning('Use of "distronode_ssh_private_key_file" in inventory with the --docker or --remote option is unsupported and will likely fail.')
...
def integration_test_environment(
    ...
    distronode_config_src = args.get_distronode_config()
    distronode_config_relative = os.path.join(data_context().content.integration_path, '%s.cfg' % args.command)
    ...
    if args.no_temp_workdir or 'no/temp_workdir/' in target.aliases:
        ...
        distronode_config = distronode_config_src
        vars_file = os.path.join(data_context().content.root, data_context().content.integration_vars_path)

        yield IntegrationEnvironment(data_context().content.root, integration_dir, targets_dir, inventory_path, distronode_config, vars_file)
        return
    ...
    try:
        ...
        distronode_config = os.path.join(temp_dir, distronode_config_relative)

        vars_file_src = os.path.join(data_context().content.root, data_context().content.integration_vars_path)
        vars_file = os.path.join(temp_dir, data_context().content.integration_vars_path)

        file_copies = [
            (distronode_config_src, distronode_config),
        ...
        yield IntegrationEnvironment(temp_dir, integration_dir, targets_dir, inventory_path, distronode_config, vars_file)
    finally:
        if not args.explain:
...
def integration_test_config_file(
    ...
    config_vars = (env_config.distronode_vars or {}).copy()

    config_vars.update(
        distronode_test=dict(
...
def command_integration_script(
    ...
    if env_config:
        display.info('>>> Environment Config\n%s' % json.dumps(dict(
            ...
            distronode_vars=env_config.distronode_vars,
    ...
    with integration_test_environment(args, target, inventory_path) as test_env:  # type: IntegrationEnvironment
        ...
        env = integration_environment(args, target, test_dir, test_env.inventory_path, test_env.distronode_config, env_config, test_env)
        cwd = os.path.join(test_env.targets_dir, target.relative_path)

        env.update(
            # support use of adhoc distronode commands in collections without specifying the fully qualified collection name
            DISTRONODE_PLAYBOOK_DIR=cwd,
        )
...
def command_integration_role(
    ...
    if isinstance(args, WindowsIntegrationConfig):
        ...
        gather_facts = False
        variables.update(
            win_output_dir=r'C:\distronode_testing',
        )
    elif isinstance(args, NetworkIntegrationConfig):
    ...
    with integration_test_environment(args, target, inventory_path) as test_env:  # type: IntegrationEnvironment
        ...
        if env_config:
            if env_config.distronode_vars:
                variables.update(env_config.distronode_vars)
        ...
        with named_temporary_file(args=args, directory=test_env.integration_dir, prefix='%s-' % target.name, suffix='.yml', content=playbook) as playbook_path:
            ...
            cmd = ['distronode-playbook', filename, '-i', os.path.relpath(test_env.inventory_path, test_env.integration_dir)]
            ...
            env['DISTRONODE_ROLES_PATH'] = test_env.targets_dir
...
def integration_environment(
    ...
    test_dir: str,
    inventory_path: str,
    distronode_config: t.Optional[str],
    ...
    env = distronode_environment(args, distronode_config=distronode_config)
    ...
    integration = dict(
        ...
        DISTRONODE_CALLBACKS_ENABLED=','.join(sorted(set(callback_plugins))),
        DISTRONODE_TEST_CI=args.metadata.ci_provider or get_ci_provider().code,
        DISTRONODE_TEST_COVERAGE='check' if args.coverage_check else ('yes' if args.coverage else ''),
        OUTPUT_DIR=test_dir,
        INVENTORY_PATH=os.path.abspath(inventory_path),
    )

    if args.debug_strategy:
        env.update(DISTRONODE_STRATEGY='debug')

    if 'non_local/' in target.aliases:
        if args.coverage:
            display.warning('Skipping coverage reporting on Distronode modules for non-local test: %s' % target.name)

        env.update(DISTRONODE_TEST_REMOTE_INTERPRETER='')
...
class IntegrationEnvironment:
    ...
    def __init__(self, test_dir: str, integration_dir: str, targets_dir: str, inventory_path: str, distronode_config: str, vars_file: str) -> None:
        self.test_dir = test_dir
        self.integration_dir = integration_dir
        self.targets_dir = targets_dir
        self.inventory_path = inventory_path
        self.distronode_config = distronode_config
...
def requirements(host_profile: HostProfile) -> None:
    ...
    if isinstance(host_profile, ControllerHostProfile) and host_profile.controller:
        ...
        install_requirements(host_profile.args, host_profile.python, distronode=True, command=True)  # integration, windows-integration, network-integration
