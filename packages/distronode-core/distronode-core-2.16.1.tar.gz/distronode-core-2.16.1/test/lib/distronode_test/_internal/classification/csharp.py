...
def extract_csharp_module_utils_imports(path: str, module_utils: set[str], is_pure_csharp: bool) -> set[str]:
    ...
    imports = set()
    if is_pure_csharp:
        pattern = re.compile(r'(?i)^using\s((?:Distronode|DistronodeCollections)\..+);$')
    else:
        pattern = re.compile(r'(?i)^#\s*distronoderequires\s+-csharputil\s+((?:Distronode|distronode.collections|\.)\..+)')
