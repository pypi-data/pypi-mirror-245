...
class BinSymlinksTest(SanityVersionNeutral):
    ...
    def test(self, args: SanityConfig, targets: SanityTargets) -> TestResult:
        bin_root = os.path.join(DISTRONODE_SOURCE_ROOT, 'bin')
        ...
        for bin_path in bin_paths:
            ...
            map_dest = DISTRONODE_BIN_SYMLINK_MAP.get(os.path.basename(bin_path))

            if not map_dest:
                errors.append((bin_path, 'missing from DISTRONODE_BIN_SYMLINK_MAP in file "%s"' % symlink_map_path))
                continue

            if dest != map_dest:
                errors.append((bin_path, 'points to "%s" instead of "%s" from DISTRONODE_BIN_SYMLINK_MAP in file "%s"' % (dest, map_dest, symlink_map_path)))
                continue
        ...
        for bin_name, dest in DISTRONODE_BIN_SYMLINK_MAP.items():
            if bin_name not in bin_names:
                bin_path = os.path.join(bin_root, bin_name)
                errors.append((bin_path, 'missing symlink to "%s" defined in DISTRONODE_BIN_SYMLINK_MAP in file "%s"' % (dest, symlink_map_path)))
