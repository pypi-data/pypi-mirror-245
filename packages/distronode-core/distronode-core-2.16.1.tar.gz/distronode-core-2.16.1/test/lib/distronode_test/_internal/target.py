...
def walk_integration_targets() -> c.Iterable[IntegrationTarget]:
    ...
    # also detect targets which are distronode roles, looking for standard entry points
...
def categorize_integration_test(name: str, aliases: list[str], force_target: bool) -> tuple[IntegrationTargetType, IntegrationTargetType]:
    ...
    actual_type = None
    strict_mode = data_context().content.is_distronode
...
class IntegrationTarget(CompletionTarget):
    ...
    def __init__(self, path: str, modules: frozenset[str], prefixes: dict[str, str]) -> None:
        ...
        if runme_path in file_paths:
            ...
            self.type = 'role'  # distronode will consider these empty roles, so distronode-test should as well
        ...
        if data_context().content.is_distronode and (self.name == 'distronode-test' or self.name.startswith('distronode-test-')):
            groups.append('distronode-test')
