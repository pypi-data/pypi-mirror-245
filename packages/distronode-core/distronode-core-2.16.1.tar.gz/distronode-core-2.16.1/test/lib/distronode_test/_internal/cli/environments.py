...
def add_environments(
    ...
    """Add arguments for the environments used to run distronode-test and commands it invokes."""
    ...
    if not get_ci_provider().supports_core_ci_auth():
        sections.append('Remote provisioning options have been hidden since no Distronode Core CI API key was found.')
...
def add_environment_venv(
    ...
    """Add environment arguments for running in distronode-test managed virtual environments."""
    exclusive_parser.add_argument(
        '--venv',
        action='store_true',
...
def add_global_docker(
    ...
    # Docker support isn't related to distronode-core-ci.
    # However, distronode-core-ci support is a reasonable indicator that the user may need the `--dev-*` options.
    suppress = None if get_ci_provider().supports_core_ci_auth() else argparse.SUPPRESS
...
def add_environment_remote(
    ...
    """Add environment arguments for running in distronode-core-ci provisioned remote virtual machines."""
