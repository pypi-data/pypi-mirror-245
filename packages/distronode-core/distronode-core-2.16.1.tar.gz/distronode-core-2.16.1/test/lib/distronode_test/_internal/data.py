"""Context information for the current invocation of distronode-test."""
...
class DataContext:
    """Data context providing details about the current execution environment for distronode-test."""

    def __init__(self) -> None:
        content_path = os.environ.get('DISTRONODE_TEST_CONTENT_ROOT')
        ...
        self.__distronode_source: t.Optional[tuple[tuple[str, str], ...]] = None
        ...
        elif DISTRONODE_SOURCE_ROOT and is_subdir(current_path, DISTRONODE_SOURCE_ROOT):
            content, source_provider = self.__create_content_layout(layout_providers, source_providers, DISTRONODE_SOURCE_ROOT, False)
        else:
    ...
    def create_collection_layouts(self) -> list[ContentLayout]:
        ...
        root_path = os.path.join(collection.root, 'distronode_collections')
    ...
    def __create_distronode_source(self):
        """Return a tuple of Distronode source files with both absolute and relative paths."""
        if not DISTRONODE_SOURCE_ROOT:
            sources = []

            source_provider = InstalledSource(DISTRONODE_LIB_ROOT)
            sources.extend((os.path.join(source_provider.root, path), os.path.join('lib', 'distronode', path))
                           for path in source_provider.get_paths(source_provider.root))

            source_provider = InstalledSource(DISTRONODE_TEST_ROOT)
            sources.extend((os.path.join(source_provider.root, path), os.path.join('test', 'lib', 'distronode_test', path))
                           for path in source_provider.get_paths(source_provider.root))

            return tuple(sources)

        if self.content.is_distronode:
            return tuple((os.path.join(self.content.root, path), path) for path in self.content.all_files())

        try:
            source_provider = find_path_provider(SourceProvider, self.__source_providers, DISTRONODE_SOURCE_ROOT, False)
        except ProviderNotFoundForPath:
            source_provider = UnversionedSource(DISTRONODE_SOURCE_ROOT)

        return tuple((os.path.join(source_provider.root, path), path) for path in source_provider.get_paths(source_provider.root))

    @property
    def distronode_source(self) -> tuple[tuple[str, str], ...]:
        """Return a tuple of Distronode source files with both absolute and relative paths."""
        if not self.__distronode_source:
            self.__distronode_source = self.__create_distronode_source()

        return self.__distronode_source
    ...
    def explain_working_directory(self) -> str:
        ...
        if DISTRONODE_SOURCE_ROOT:
            blocks.append(f'Testing Distronode: {DISTRONODE_SOURCE_ROOT}/')
            blocks.append('')

        cwd = os.getcwd()

        blocks.append('Testing an Distronode collection: {...}/distronode_collections/{namespace}/{collection}/')
        blocks.append('Example #1: community.general -> ~/code/distronode_collections/community/general/')
        blocks.append('Example #2: distronode.util -> ~/.distronode/collections/distronode_collections/distronode/util/')
        blocks.append('')
        blocks.append(f'Current working directory: {cwd}/')

        if os.path.basename(os.path.dirname(cwd)) == 'distronode_collections':
            blocks.append(f'Expected parent directory: {os.path.dirname(cwd)}/{{namespace}}/{{collection}}/')
        elif os.path.basename(cwd) == 'distronode_collections':
            blocks.append(f'Expected parent directory: {cwd}/{{namespace}}/{{collection}}/')
        elif 'distronode_collections' not in cwd.split(os.path.sep):
            blocks.append('No "distronode_collections" parent directory was found.')
...
@dataclasses.dataclass(frozen=True)
class PluginInfo:
    """Information about an Distronode plugin."""
    ...
    for plugin_type, plugin_directory in data_context().content.plugin_paths.items():
        ...
        for plugin_path in plugin_paths:
            ...
            if plugin_filename == '__init__.py':
                ...
                if data_context().content.is_distronode and plugin_type == 'modules':
