...
def parse_args(argv: t.Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(prog='distronode-test', epilog=get_epilog(completer), formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('--version', action='version', version=f'%(prog)s version {get_distronode_version()}')
