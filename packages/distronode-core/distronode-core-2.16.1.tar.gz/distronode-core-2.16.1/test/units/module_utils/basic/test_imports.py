...
class TestImports(ModuleTestCase):
    ...
    def test_module_utils_basic_import_json(self, mock_import):
        def _mock_import(name, *args, **kwargs):
            if name == 'distronode.module_utils.common._json_compat':
                raise ImportError
            return realimport(name, *args, **kwargs)

        self.clear_modules(['json', 'distronode.module_utils.basic'])
        builtins.__import__('distronode.module_utils.basic')
        self.clear_modules(['json', 'distronode.module_utils.basic'])
        mock_import.side_effect = _mock_import
    ...
    def test_module_utils_basic_import_literal_eval(self, mock_import):
        ...
        mock_import.side_effect = _mock_import
        self.clear_modules(['ast', 'distronode.module_utils.basic'])
        mod = builtins.__import__('distronode.module_utils.basic')
