...
<#
.SYNOPSIS
Invokes an Distronode module in a new Runspace. This cmdlet will output the
...
Write-DistronodeLog "INFO - creating new PowerShell pipeline for $ModuleName" "module_wrapper"
...
if ($host.Name -eq "ConsoleHost") {
    Write-DistronodeLog "INFO - setting console input encoding to UTF8 for $ModuleName" "module_wrapper"
    $ps.AddScript('[Console]::InputEncoding = New-Object Text.UTF8Encoding $false').AddStatement() > $null
}

# set the variables
foreach ($variable in $Variables) {
    Write-DistronodeLog "INFO - setting variable '$($variable.Name)' for $ModuleName" "module_wrapper"
...
if ($Environment) {
    ...
    Write-DistronodeLog "INFO - setting environment vars for $ModuleName" "module_wrapper"
    $ps.Runspace.SessionStateProxy.SetVariable("_DistronodeEnvironment", $Environment)
    $ps.AddScript(@'
foreach ($env_kv in $_DistronodeEnvironment.GetEnumerator()) {
...
if ($Modules) {
    foreach ($module in $Modules.GetEnumerator()) {
        Write-DistronodeLog "INFO - create module util '$($module.Key)' for $ModuleName" "module_wrapper"
...
if ($Breakpoints.Count -gt 0) {
    Write-DistronodeLog "INFO - adding breakpoint to runspace that will run the modules" "module_wrapper"
...
Write-DistronodeLog "INFO - start module exec with Invoke() - $ModuleName" "module_wrapper"
...
try {
    ...
    Write-DistronodeError @error_params
...
Write-DistronodeLog "INFO - module exec ended $ModuleName" "module_wrapper"
$stdout = $sb.ToString()
if ($stdout) {
...
if ($module_output.Count -gt 0) {
    ...
    Write-DistronodeLog "INFO - using the output stream for module output - $ModuleName" "module_wrapper"
...
if ($null -ne $rc) {
    Write-DistronodeLog "INFO - got an rc of $rc from $ModuleName exec" "module_wrapper"
...
if (
    ...
    Write-DistronodeLog "WARN - module had errors, outputting error info $ModuleName" "module_wrapper"
    ...
    foreach ($err in $ps.Streams.Error) {
        $error_msg = Format-DistronodeException -ErrorRecord $err

        # need to use the current hosts's UI class as we may not have
        # a console to write the stderr to, e.g. psrp
        Write-DistronodeLog "WARN - error msg for for $($ModuleName):`r`n$error_msg" "module_wrapper"
