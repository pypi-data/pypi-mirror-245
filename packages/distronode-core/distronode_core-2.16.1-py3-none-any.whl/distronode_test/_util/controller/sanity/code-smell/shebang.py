    ...
    for path in sys.argv[1:] or sys.stdin.read().splitlines():
        with open(path, 'rb') as path_fd:
            ...
            if path.startswith('lib/distronode/modules/'):
                ...
                is_module = True
            elif re.search('^test/support/[^/]+/collections/distronode_collections/[^/]+/[^/]+/plugins/modules/', path):
                is_module = True
            elif path == 'test/lib/distronode_test/_util/target/cli/distronode_test_cli_stub.py':
                pass  # distronode-test entry point must be executable and have a shebang
            elif re.search(r'^lib/distronode/cli/[^/]+\.py', path):
