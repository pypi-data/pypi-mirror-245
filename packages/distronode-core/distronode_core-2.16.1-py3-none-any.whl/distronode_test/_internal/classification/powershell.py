...
def extract_powershell_module_utils_imports(path: str, module_utils: set[str]) -> set[str]:
    ...
    if data_context().content.is_distronode and '# POWERSHELL_COMMON' in code:
        imports.add('Distronode.ModuleUtils.Legacy')
    ...
    for line in lines:
        line_number += 1
        match = re.search(r'(?i)^#\s*(?:requires\s+-modules?|distronoderequires\s+-powershell)\s*((?:Distronode|distronode_collections|\.)\..+)', line)
        ...
        elif data_context().content.is_distronode or \
                import_name.startswith('distronode_collections.%s' % data_context().content.prefix):
