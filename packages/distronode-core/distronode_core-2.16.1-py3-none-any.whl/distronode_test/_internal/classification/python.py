...
def get_python_module_utils_name(path: str) -> str:
    ...
    if data_context().content.collection:
        prefix = 'distronode_collections.' + data_context().content.collection.prefix + 'plugins.module_utils'
    else:
        prefix = 'distronode.module_utils'
...
def get_import_path(name: str, package: bool = False) -> str:
    ...
    if name.startswith('distronode.module_utils.') or name == 'distronode.module_utils':
        path = os.path.join('lib', filename)
    elif data_context().content.collection and (
            name.startswith('distronode_collections.%s.plugins.module_utils.' % data_context().content.collection.full_name) or
            name == 'distronode_collections.%s.plugins.module_utils' % data_context().content.collection.full_name):
...
class ModuleUtilFinder(ast.NodeVisitor):
    ...
    def __init__(self, path: str, module_utils: set[str]) -> None:
        ...
        if path.startswith('lib/distronode/module_utils/'):
            package = os.path.split(path)[0].replace('/', '.')[4:]

            if package != 'distronode.module_utils' and package not in VIRTUAL_PACKAGES:
                self.add_import(package, 0)

        self.module = None

        if data_context().content.is_distronode:
            # Various parts of the Distronode source tree execute within different modules.
            ...
            path_map = (
                ('^lib/distronode/', 'distronode/'),
                ('^test/lib/distronode_test/_util/controller/sanity/validate-modules/', 'validate_modules/'),
                ('^test/units/', 'test/units/'),
                ('^test/lib/distronode_test/_internal/', 'distronode_test/_internal/'),
                ('^test/integration/targets/.*/distronode_collections/(?P<ns>[^/]*)/(?P<col>[^/]*)/', r'distronode_collections/\g<ns>/\g<col>/'),
                ('^test/integration/targets/.*/library/', 'distronode/modules/'),
            )
            ...
            # This assumes that all files within the collection are executed by Distronode as part of the collection.
    ...
    def visit_Import(self, node: ast.Import) -> None:
        ...
        # import distronode.module_utils.MODULE[.MODULE]
        # import distronode_collections.{ns}.{col}.plugins.module_utils.module_utils.MODULE[.MODULE]
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        ...
        if not module.startswith('distronode'):
            return

        # from distronode.module_utils import MODULE[, MODULE]
        # from distronode.module_utils.MODULE[.MODULE] import MODULE[, MODULE]
        # from distronode_collections.{ns}.{col}.plugins.module_utils import MODULE[, MODULE]
        # from distronode_collections.{ns}.{col}.plugins.module_utils.MODULE[.MODULE] import MODULE[, MODULE]
    ...
    def is_module_util_name(name: str) -> bool:
        ...
        if data_context().content.is_distronode and name.startswith('distronode.module_utils.'):
            return True

        if data_context().content.collection and name.startswith('distronode_collections.%s.plugins.module_utils.' % data_context().content.collection.full_name):
            return True
