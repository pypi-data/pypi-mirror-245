"""Initialize locale settings. This must be imported very early in distronode-test startup."""
...
"""
The standard locale used by distronode-test and its subprocesses and delegated instances.
"""

FALLBACK_LOCALE = 'C.UTF-8'
"""
The fallback locale to use when the standard locale is not available.
This was added in distronode-core 2.14 to allow testing in environments without the standard locale.
It was not needed in previous distronode-core releases since they do not verify the locale during startup.
"""
...
def configure_locale() -> tuple[str, t.Optional[str]]:
    ...
    if (fs_encoding := sys.getfilesystemencoding()).lower() != 'utf-8':
        raise LocaleError(f'distronode-test requires the filesystem encoding to be UTF-8, but "{fs_encoding}" was detected.')
    ...
    if not configured_locale:
        raise LocaleError('distronode-test could not initialize a supported locale:\n' +
