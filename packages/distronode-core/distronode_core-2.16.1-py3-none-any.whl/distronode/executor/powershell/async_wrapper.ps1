...
Write-DistronodeLog "INFO - starting async_wrapper" "async_wrapper"

if (-not $Payload.environment.ContainsKey("DISTRONODE_ASYNC_DIR")) {
    Write-DistronodeError -Message "internal error: the environment variable DISTRONODE_ASYNC_DIR is not set and is required for an async task"
    $host.SetShouldExit(1)
    return
}
$async_dir = [System.Environment]::ExpandEnvironmentVariables($Payload.environment.DISTRONODE_ASYNC_DIR)
...
Write-DistronodeLog "INFO - creating async results path at '$results_path'" "async_wrapper"
...
$pipe_name = "distronode-async-$jid-$([guid]::NewGuid())"
...
$bootstrap_wrapper = {
    ...
    # from here on
    trap {
        $wrapper_path = "$($env:TEMP)\distronode-async-wrapper-error-$(Get-Date -Format "yyyy-MM-ddTHH-mm-ss.ffffZ").txt"
...
Write-DistronodeLog "INFO - creating named pipe '$pipe_name'" "async_wrapper"
...
try {
    Write-DistronodeLog "INFO - creating async process '$exec_args'" "async_wrapper"
    $process = Invoke-CimMethod -ClassName Win32_Process -Name Create -Arguments @{CommandLine = $exec_args }
    $rc = $process.ReturnValue

    Write-DistronodeLog "INFO - return value from async process exec: $rc" "async_wrapper"
    if ($rc -ne 0) {
        $error_msg = switch ($rc) {
            2 { "Access denied" }
    ...
    Write-DistronodeLog "INFO - created async process PID: $watchdog_pid" "async_wrapper"
    ...
    $result = @{
        ...
        distronode_job_id = $local_jid
        _distronode_suppress_tmpdir_delete = $true
        distronode_async_watchdog_pid = $watchdog_pid
    }

    Write-DistronodeLog "INFO - writing initial async results to '$results_path'" "async_wrapper"
    $result_json = ConvertTo-Json -InputObject $result -Depth 99 -Compress
    Set-Content $results_path -Value $result_json

    $np_timeout = $Payload.async_startup_timeout * 1000
    Write-DistronodeLog "INFO - waiting for async process to connect to named pipe for $np_timeout milliseconds" "async_wrapper"
    ...
    if (-not $wait_async.IsCompleted) {
        $msg = "Distronode encountered a timeout while waiting for the async task to start and connect to the named"
    ...
    Write-DistronodeLog "INFO - writing exec_wrapper and payload to async process" "async_wrapper"
...
Write-DistronodeLog "INFO - outputting initial async result: $result_json" "async_wrapper"
Write-Output -InputObject $result_json
Write-DistronodeLog "INFO - ending async_wrapper" "async_wrapper"
