...
class TableHandler:
    ...
    def _load_settings(self):
        ...
        try:
            ...
            logger.error("Error loading settings: %s", str(exc))
...
class CsvHashComparator(BaseOperator):
    ...
    def give_up_diffing(self, level, diff_instance):
        ...
        if base_filename is None and other_filename is None:
            ...
            logger.warning("Giving up diffing because both filenames are None in %s", level.path())
            return True
    ...
    def hash_file(filename, buffer_size=8192, hash_algorithm=hashlib.sha256):
        logger.info("Hashing file: %s", filename)
        try:
            ...
            logger.info("File hashed successfully")
            return file_hash.hexdigest()
        except (IOError, FileNotFoundError) as exc:
            logger.error("Error hashing file: %s", str(exc))
...
class TableDiff:
    ...
    def _compare_settings_files(
        ...
        logger.info("Using comparer: %s and compare_csv_files: %s", self.comparer, compare_csv_files)
        try:
            diffs = self.comparer(
                base_table.settings,
            ...
            logger.info("Settings files compared successfully")
            ...
            logger.error("Error comparing settings.files: %s", str(exc))
    ...
    def compare(self, table_1, table_2, compare_csv_files=True, interpret_diff=True):
        ...
        try:
            ...
            logger.info("Tables compared successfully")
            ...
            logger.info("Diff interpreted successfully")
            return interpreted_diff
        except RuntimeError as exc:
            logger.error("Error comparing tables: %s", str(exc))
            return None
