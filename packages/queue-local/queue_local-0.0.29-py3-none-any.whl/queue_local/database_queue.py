...
from .utils import get_thread_id, get_ipv6, get_ipv4
...
class DatabaseQueue(OurQueue, GenericCRUD):
    ...
    def get(self, action_ids: tuple = ()) -> dict:
        ...
        try:
            ...
            update_where = f"process_id IS NULL AND queue_status_id = {StatusEnum.NEW.value}" + \
            ...
            if queue_item:
                ...
                query = f"UPDATE {self.default_table_name} " \
                        ...
                        f"server_ip_v4=%s, server_ip_v6=%s, start_timestamp = NOW() " \
                        f"WHERE {self.id_column_name} = %s"
                params = (StatusEnum.RETRIEVED.value, updated_user_id, self.user.get_user_JWT(),
                          get_thread_id(), get_ipv4(), get_ipv6(), queue_item[self.id_column_name])
